  ...
  )
{
  ...
  //+Byo Porting >> add PcdMediaDetectCount
  EFI_STATUS                  FlushStatus;
  UINTN                       MediaDetectCount;
  ...
  //+Byo Porting >> add PcdMediaDetectCount
  MediaDetectCount = PcdGet8(PcdMediaDetectCount);
  if(!MediaDetectCount)MediaDetectCount+=1; //just in case MediaDetectCount is zero.
  //
  ...
  do {
    ...
    MediaDetectCount--;
  }while(MediaDetectCount);
  
  //Flush Key
  do{
    FlushStatus = gST->ConIn->ReadKeyStroke(gST->ConIn,&FlushKey);
  }while(!EFI_ERROR (FlushStatus));
  ...
  if(!EFI_ERROR (Status) && MediaPresent){
    AsciiPrint ("\n>>Media Presence.");
  }
  //+Byo Porting <<
