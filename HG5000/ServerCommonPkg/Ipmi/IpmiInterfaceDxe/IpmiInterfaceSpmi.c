...
#include <AcpiPlatformDxe/AcpiPlatform.h>
  ...
  )
{
  ...
  EFI_STATUS                    Status;
  UINT8                         Checksum;
  IPMI_PRIVATE_DATA             *IpmiData = NULL;
  ...
  IpmiData = (IPMI_PRIVATE_DATA *) Context;

  if ((IpmiData->IpmiVersion >> 4) * 10 + (IpmiData->IpmiVersion & 0x0F) < 15) {
    return;
  }

  Checksum = CalculateCheckSum8 ((UINT8*) (&mSpmi), mSpmi.Header.Length);
  mSpmi.Header.Checksum = Checksum;
  mSpmi.Reversion       = IpmiData->IpmiVersion << 4;

  Status = gBS->LocateProtocol (&gEfiAcpiTableProtocolGuid, NULL,  (VOID**) &AcpiTableProtocol);
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "[%a]:[%dL] LocateProtocol Fail!\n", __FUNCTION__, __LINE__));
    return;
  }

  AcpiTableHandle = 0;
  Status = AcpiTableProtocol->InstallAcpiTable (AcpiTableProtocol, &mSpmi, mSpmi.Header.Length, &AcpiTableHandle);
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "[%a]:[%dL] InstallAcpiTable Fail!\n", __FUNCTION__, __LINE__));
    return;
  }
}
