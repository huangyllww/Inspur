  ...
  )
{
  ...
  //
// wz190912 - >>  
  Pls = (UINT8)((State & XHC_PORTSC_PLS) >> 5);
  if (Pls == 3) {
    PortStatus->PortStatus |= USB_PORT_STAT_SUSPEND;
  } else if(Pls == 7){
    PortStatus->PortStatus |= USB_PORT_STAT_POLLING;
  }
// wz190912 - <<  
  ...
  Status = XhcPeiPollPortStatusChange (Xhc, ParentRouteChart, PortNumber, PortStatus);  // wz190823 -
