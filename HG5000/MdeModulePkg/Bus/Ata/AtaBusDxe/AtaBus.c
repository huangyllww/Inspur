  ...
  )
{
  ...
  AtaDevice = ATA_DEVICE_FROM_BYO_DISKINFO(This);
  Size = AsciiStrSize(AtaDevice->Mn);
  ...
  *ModelNameSize = Size;
  AsciiStrCpy(ModelName, AtaDevice->Mn);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  AtaDevice = ATA_DEVICE_FROM_BYO_DISKINFO(This);
  Size = AsciiStrSize(AtaDevice->Sn);
  ...
  *SerialNumberSize = Size;
  AsciiStrCpy(SerialNumber, AtaDevice->Sn);  
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  *Size = MultU64x32(AtaDevice->BlockMedia.LastBlock, AtaDevice->BlockMedia.BlockSize);
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  AtaDevice = ATA_DEVICE_FROM_BYO_DISKINFO(This);
  Size      = GetDevicePathSize(AtaDevice->DevicePath);
  ...
  CopyMem(Dp, AtaDevice->DevicePath, Size);
  *DevicePath = Dp;
  ...
  )
{
  ...
  if(CompareGuid(&AtaDevice->DiskInfo.Interface, &gEfiDiskInfoAhciInterfaceGuid)){
    *PortIndex = AtaDevice->Port;
  } else {
    *PortIndex = AtaDevice->Port * 2 + AtaDevice->PortMultiplierPort;
  }
  ...
  )
{
  ...
  CopyMem(AtaDevice->Mn, AtaDevice->IdentifyData->ModelName, sizeof(AtaDevice->IdentifyData->ModelName));
  SwapWordArray((UINT8*)AtaDevice->Mn, sizeof(AtaDevice->Mn));  
  AtaDevice->Mn[40] = 0;
  TrimStr8(AtaDevice->Mn);
  
  CopyMem(AtaDevice->Sn, AtaDevice->IdentifyData->SerialNo, sizeof(AtaDevice->IdentifyData->SerialNo));
  SwapWordArray((UINT8*)AtaDevice->Sn, sizeof(AtaDevice->Sn));
  AtaDevice->Sn[20] = 0;
  TrimStr8(AtaDevice->Sn);
  ...
  )
{
  ...
  if ((AtaPassThru->Mode->Attributes & EFI_ATA_PASS_THRU_ATTRIBUTES_LOGICAL) == 0) {
    DEBUG((EFI_D_ERROR, "AtaPT no Logical attrib\n"));
    //
