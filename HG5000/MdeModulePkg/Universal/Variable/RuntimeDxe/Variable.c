...
#include "Variable.h"
#include <Library/CacheMaintenanceLib.h>    // wz190218 +
  ...
  )
{
  ...
  if ((FvHeader->Signature != EFI_FVH_SIGNATURE) || (!CompareGuid (&gEfiSystemNvDataFvGuid, &FvHeader->FileSystemGuid))) {
    ...
    DEBUG ((EFI_D_ERROR, "Firmware Volume for Variable Store is corrupted(%X %g)\n", \
      FvHeader->Signature, &FvHeader->FileSystemGuid));
  ...
  )
{
  ...
  if(mNvVariableCache == NULL || mNvFvHeaderCache == NULL){
  ...
  WriteBackInvalidateDataCacheRange((VOID*)(UINTN)NvStorageBase, NvStorageSize);
  CopyMem(FvHeader, (UINT8*)(UINTN)NvStorageBase, NvStorageSize);
  ...
  DEBUG((EFI_D_INFO, "NonVolatileLastVariableOffset:%X\n", mVariableModuleGlobal->NonVolatileLastVariableOffset));
  DEBUG((EFI_D_INFO, "HwErrVariableTotalSize       :%X\n", mVariableModuleGlobal->HwErrVariableTotalSize));
  DEBUG((EFI_D_INFO, "CommonVariableTotalSize      :%X\n", mVariableModuleGlobal->CommonVariableTotalSize));
  ...
  )
{
  ...
  if(PcdGetBool(PcdSyncHobVariableToFlash)){
