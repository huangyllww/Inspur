...
**/
  ...
  /*<BEGIN>: Addedd by John 13482, 20200330 PN:NA LABEL:BSC_HYGON_001 */
  /*<DESC> : Hardware porting. */
  #define BSC_DEBUG(level, fmt, ...) \
    do {                           \
      if (DebugPrintEnabled() && DebugPrintLevelEnabled(level)) {  \
         DebugPrint((level), fmt, ##__VA_ARGS__);      \
      }                            \
    } while (FALSE)
  /*<END>: Addedd by John 13482, 20200330 */
#else
  #define DEBUG(Expression)
  /*<BEGIN>: Addedd by John 13482, 20200330 PN:NA LABEL:BSC_HYGON_001 */
  /*<DESC> : Hardware porting. */
