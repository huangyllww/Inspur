...
STATIC BMM_CALLBACK_DATA   gBmmCallbackInfo = {0, };
  ...
  if(Item == ITEM_SECUREBOOT && NewStatus == STATUS_ENABLE){
    DEBUG((EFI_D_INFO, "secure enable, try to disable csm\n"));
     HiiGetBrowserData(
      ...
      sizeof(gSetupDataTemp), 
      (UINT8*)&gSetupDataTemp
      );
    gSetupDataTemp.Csm = 0;
    HiiSetBrowserData(
      &gPlatformSetupVariableGuid,
      PLATFORM_SETUP_VARIABLE_NAME,
      sizeof(gSetupDataTemp), 
...
VOID BmValueInit()
{
  ZeroMem(&gBmmCallbackInfo, sizeof(gBmmCallbackInfo));
}
  ...
  )
{
  ...
  if (BmmCallbackInfo->BmmSaveNotifyHandle != NULL) {
