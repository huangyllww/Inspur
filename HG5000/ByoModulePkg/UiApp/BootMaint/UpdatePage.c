...
{
  EFI_STRING       String;  
  EFI_STRING_ID    NewStrId = 0;
  CHAR8            *LanguageString = NULL;
  CHAR8            *LangCode;
  CHAR8            *Lang = NULL;
  CHAR16           Buffer[128];


  if (HiiHandle == NULL || StrId == 0) {
    DEBUG((EFI_D_ERROR, "%a(%X,%X,%X)\n", __FUNCTION__, HiiHandle, StrId));
    goto ProcExit;
  }
  ...
  while (*LangCode != 0) {
    ...
    String = HiiGetString (HiiHandle, StrId, Lang);
    if (String != NULL) {
      UnicodeSPrint(Buffer, sizeof(Buffer), String, Index);
      NewStrId = HiiSetString(HiiHandle, NewStrId, Buffer, Lang);
      FreePool (String);
    }
  }
  ...
  )
{
  ...
  UINT16                      Index;
  UINT16                      Key;
  EFI_STRING_ID               TypeStr;
  EFI_STRING_ID               TypeStrHelp;
  UINT16                      VarDevOrder;
  UINT8                       *VarData;
  UINT8                       *LegacyOrder;
  UINT8                       *OldData;
  ...
  case FORM_SET_FD_ORDER_ID:
    ...
    TypeStr     = STRING_TOKEN(STR_FLOPPY);
    TypeStrHelp = STRING_TOKEN(STR_FLOPPY_HELP);
    BbsType     = BBS_FLOPPY;
  ...
  case FORM_SET_HD_ORDER_ID:
    ...
    TypeStr     = STRING_TOKEN(STR_HARDDISK);
    TypeStrHelp = STRING_TOKEN(STR_HARDDISK_HELP);
  ...
  case FORM_SET_CD_ORDER_ID:
    ...
    TypeStr     = STRING_TOKEN(STR_CDROM);
    TypeStrHelp = STRING_TOKEN(STR_CDROM_HELP);
    BbsType     = BBS_CDROM;
  ...
  case FORM_SET_NET_ORDER_ID:
    ...
    TypeStr     = STRING_TOKEN(STR_NET);
    TypeStrHelp = STRING_TOKEN(STR_NET_HELP);
  ...
  case FORM_SET_BEV_ORDER_ID:
    ...
    TypeStr     = STRING_TOKEN(STR_BEV);
    TypeStrHelp = STRING_TOKEN(STR_BEV_HELP);
  ...
  if (NULL != VarData) {
    ...
    for (Index = 0; Index < OptionMenu->MenuNumber; Index++) {
      ...
      StrRef = UpdateLegacyItemName(CallbackData->BmmHiiHandle, TypeStr, Index);
      StrRefHelp = UpdateLegacyItemName(CallbackData->BmmHiiHandle, TypeStrHelp, Index);      

      if(StrRef == 0 || StrRefHelp == 0){
        continue;
      }
