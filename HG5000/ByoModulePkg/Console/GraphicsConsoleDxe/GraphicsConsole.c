...
EFI_GRAPHICS_OUTPUT_BLT_PIXEL mDefGraphicsEfiColors[16] = {
  //
  // B    G    R   reserved
  //
...
EFI_GRAPHICS_OUTPUT_BLT_PIXEL   *mGraphicsEfiColors = mDefGraphicsEfiColors;
  ...
  )
{
  ...
  NewModeBuffer = AllocateZeroPool (sizeof (GRAPHICS_CONSOLE_MODE_DATA) * TotalCount);
  ...
  if(DeltaXPersent){
    NewModeBuffer[ValidCount].Columns       = MaxColumns * (100 - DeltaXPersent) / 100;
    NewModeBuffer[ValidCount].Rows          = MaxRows;
    NewModeBuffer[ValidCount].GopWidth      = HorizontalResolution;
    NewModeBuffer[ValidCount].GopHeight     = VerticalResolution;
    NewModeBuffer[ValidCount].GopModeNumber = GopModeNumber;
    NewModeBuffer[ValidCount].DeltaX        = HorizontalResolution - NewModeBuffer[ValidCount].Columns * EFI_GLYPH_WIDTH;
    NewModeBuffer[ValidCount].DeltaY        = (VerticalResolution - (NewModeBuffer[ValidCount].Rows * EFI_GLYPH_HEIGHT)) >> 1;
    ValidCount++;
  }
