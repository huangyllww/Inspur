  ...
  )
{
  ...
  while (!IsUnicodeFiFoEmpty (TerminalDevice) && !IsEfiKeyFiFoFull (TerminalDevice)) {
    ...
    // robin debug.
    // DEBUG ((EFI_D_INFO, "robin, TermType:%x, InputState:%x, unicodeChar:%x\n", TerminalDevice->TerminalType, TerminalDevice->InputState, UnicodeChar));
    switch (TerminalDevice->InputState) {
    case INPUT_STATE_DEFAULT:
      break;
      
    // robin + >> 
    // for console redirection function key support.
    case INPUT_STATE_F1_F8:
      switch (UnicodeChar) {
        case '1':
      ...
      if (Key.ScanCode != SCAN_NULL) {
        // DEBUG ((EFI_D_INFO, "robin, send f1-f8.\n"));
        Key.UnicodeChar = 0;
        EfiKeyFiFoInsertOneKey (TerminalDevice, &Key);
        //
        // Cancel the timer.
    ...
    case INPUT_STATE_F9_F12:
      ...
      if (Key.ScanCode != SCAN_NULL) {
        // DEBUG ((EFI_D_INFO, "robin,send f9-f12.\n"));
        Key.UnicodeChar = 0;
        EfiKeyFiFoInsertOneKey (TerminalDevice, &Key);
        //
        // Cancel the timer.
    ...
    case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET:
      ...
      if (TerminalDevice->TerminalType == TerminalTypePcAnsi    ||
        ...
        case '1':
          if (TerminalDevice->TerminalType == TerminalTypeVt100 ||
            TerminalDevice->TerminalType == TerminalTypeVtUtf8) {
            // DEBUG ((EFI_D_INFO, "robin, continue for function F1-f8.\n"));
            TerminalDevice->InputState = INPUT_STATE_F1_F8;
            continue;
          }
          break;
        ...
        case '2':
          if (TerminalDevice->TerminalType == TerminalTypeVt100 ||
            TerminalDevice->TerminalType == TerminalTypeVtUtf8) {
            // DEBUG ((EFI_D_INFO, "robin, continue for function f9-f12.\n"));
            TerminalDevice->InputState = INPUT_STATE_F9_F12;
            continue;
          }
          break;
        // robin + <<
