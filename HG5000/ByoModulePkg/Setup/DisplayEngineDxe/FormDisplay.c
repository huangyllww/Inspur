  ...
  )
{
  ...
  if (NewString = AdjustWideStr (String)) {
    FreePool (String);
    String = NewString;
  }
  ...
  )
{
  ...
  gConfirmSuccess       = GetToken (STRING_TOKEN (CONFIRM_SUCCESS), gHiiHandle);
  ...
  )
{
  ...
  PcdSetPtr (PcdCurrentFormSetGuid, &Temp, (VOID*)&(FormData->FormSetGuid));
  ...
  while (TRUE) {
    ...
    case CfReadKey:
      ...
      case '-':
        ...
        if (Statement->OpCode->OpCode == EFI_IFR_ACTION_OP || Statement->OpCode->OpCode == EFI_IFR_ORDERED_LIST_OP) {
          if (Key.UnicodeChar == '+') {
...
// wz200707 + >>		
        if(ScreenOperation == UiHelp && (gSetupHotKeyDisable & BIT0)){
          ScreenOperation = UiNoOperation;
        }
// wz200707 + <<        
...
// wz200707 + >>
          if(HotKey != NULL && (gSetupHotKeyDisable & BIT0)){
            if(HotKey->KeyData->ScanCode != SCAN_ESC){
              HotKey = NULL;
            }
          }
// wz200707 + <<          
    ...
    case CfUiReset:
      ...
      if ((FirstFormId == FormData->FormId && NULL != FormData->ByoCurrentFormSetLink) && IsByoFormset(FormData->ByoFormSetList, FormData->HiiHandle)) {
        ...
        if (!EFI_ERROR(Status)) {
// Always Exit, not confirm changes.
    ...
    case CfUiUp:
      ...
      DMSG ((EFI_D_ERROR, "jawen_d, CfUiUp, Difference :0x%x.\n", Difference));
      if (Difference == -1) {
        //
        // Not Repaint again.	  	
        //
    ...
    case CfUiDown:
      ...
      DMSG ((EFI_D_ERROR, "jawen_d, CfUiDown, Difference :%d.\n", Difference));
      if (Difference == -1) {
        //
        // Not Repaint again.	  	
        //
