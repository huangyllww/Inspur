...
UINTN PdAdminErrorTimes = 0;
UINTN PdPopErrorTimes = 0;
  ...
  )
{
  ...
  //
  if (PcdGet8 (PcdPdCurrentUser) == 0xF1) {
    PdAdminErrorTimes++;
    if (PdAdminErrorTimes == 3) {
      PdAdminErrorTimes = 0;
    } else {
      UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gPassowordInvalid, NULL);
    }
  } else if (PcdGet8 (PcdPdCurrentUser) == 0xF2) {
    PdPopErrorTimes++;
    if (PdPopErrorTimes == 3) {
      PdPopErrorTimes = 0;
    } else {
      UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gPassowordInvalid, NULL);
    }
  ...
  )
{
  ...
  *StringPtr = 0;
  ...
  if ((StrLen(StringPtr) == 0) && PcdGet8(PcdConfirmDeletePwd)) {
    SELECTION_TYPE              Choice;

    TempString = HiiGetString(gHiiHandle, STRING_TOKEN(CONFIRM_DELETE_PASSWORD), NULL);
    Choice = UiConfirmDialog(DIALOG_YESNO, NULL, NULL, TEXT_ALIGIN_CENTER, TempString, NULL);
    FreePool (TempString);
...
CHAR16 *StrGetPopPromptString(EFI_IFR_STRING *IfrString)
{
  ...
  StrCpy(PromptStr, gPromptEnter);
  StrCat(PromptStr, TmpStr);
  ...
  )
{
  ...
  *OptionString = NULL;
  ...
  case EFI_IFR_STRING_OP:
    if (Selected) {
      EFI_IFR_QUESTION_HEADER   *Q = &(((EFI_IFR_STRING*)Question->OpCode)->Question);
      DEBUG((EFI_D_INFO, "EFI_IFR_STRING_OP, Q.Flags:%X\n", Q->Flags));
      if((Q->Flags & EFI_IFR_FLAG_CALLBACK) && (Q->Flags & (EFI_IFR_FLAG_OPTIONS_ONLY | EFI_IFR_FLAG_RECONNECT_REQUIRED))){
        gUserInput->InputValue.Buffer = NULL;
      } else {
        StringPtr = AllocateZeroPool (Question->CurrentValue.BufferLen + sizeof (CHAR16));
        ASSERT (StringPtr);
        if(!(PcdGet8(PcdSkipInputStringFlag) && !StrCmp((UINT16*)Question->CurrentValue.Buffer, (UINT16*)PcdGetPtr(PcdSkipInputString)))){
