  ...
  )
{
  ...
  HII_STRING_PACKAGE_INSTANCE         *StringPackage;
  UINTN                               ResultSize;
  STATIC EFI_HII_HANDLE               *DyHandles = NULL;
  EFI_STATUS                          Status;
  UINTN                               Index;
  BOOLEAN                             IsDymamic;
  ...
  IsDymamic = FALSE;
  if(DyHandles == NULL){
    Status = gBS->LocateProtocol(&gByoDymamicFormsetHiiListProtocolGuid, NULL, (VOID**)&DyHandles);
    if(EFI_ERROR(Status)){
      DyHandles = NULL;
    }
  }
  if(DyHandles != NULL){
    for(Index=0;DyHandles[Index]!=NULL;Index++){
      if(DyHandles[Index] == PackageList){
        IsDymamic = TRUE;
        break;
      }
    }
  }
//DEBUG((EFI_D_INFO, "IsDy:%d %X\n", IsDymamic, PackageList));
  ...
  for (Link = PackageListNode->StringPkgHdr.ForwardLink;
    ...
    StringPackage = CR (Link, HII_STRING_PACKAGE_INSTANCE, StringEntry, HII_STRING_PACKAGE_SIGNATURE);
    if(IsDymamic && AsciiStrCmp(StringPackage->StringPkgHdr->Language, "en-US") != 0){
      continue;
    }
