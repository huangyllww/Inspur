  ...
  )
{
  ...
  while (!IsNull (&FormSet->FormListHead, Link)) {
    ...
    if (RetVal) {
     DMSG ((EFI_D_ERROR, "IsNvUpdateRequiredForFormSet, gMoveActionStage :%d.\n", gMoveActionStage));
      if (gMoveActionStage == 1) {
        gMoveActionStage = 2;
      }
  ...
  )
{
  ...
  do {
    ...
    if (mHiiPackageListUpdated) {
     ...
     DMSG ((EFI_D_ERROR, "Package is Updated, gMoveActionStage :%d.\n", gMoveActionStage));
      if (gMoveActionStage == 2) {
        gMoveActionStage = 3;
      }
