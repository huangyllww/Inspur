...
UINTN    gMoveActionStage = 0;

EFI_GUID  gLsiCfgToolFormsetGuid = 
  {0x6D7C060B, 0x3D98, 0x406C, { 0xAE, 0x9D, 0x9D, 0x24, 0x77, 0xCA, 0x6F, 0xA3 }};

EFI_GUID  gWangXunLanFormsetGuid = 
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    do {
      ...
      if (!IsNvUpdateRequiredForFormSet (FormSet)) {
        DMSG ((EFI_D_ERROR, "End SendForm, gMoveActionStage :%d.\n", gMoveActionStage));
        if (gMoveActionStage != 3) {
  ...
  if (RetrieveData) {
    ...
    *ResultsDataSize = BufferSize;
    ...
    if(BrowserStorage == NULL){
      DEBUG((EFI_D_ERROR, "ProcessStorage(T:%d N:%s)\n", Storage->Type, Storage->Name));
    }
  ...
  if (Storage == NULL) {
    ...
    if (Question->Operand == EFI_IFR_DATE_OP || Question->Operand == EFI_IFR_TIME_OP) {
      ...
      case QF_DATE_STORAGE_TIME:
        ...
        // wz201105 + >> set default cmos time when it runs out of range.		
			if(EFI_ERROR(Status)){
				ZeroMem(&EfiTime, sizeof(EfiTime));
				EfiTime.Year	= PcdGet16(PcdMinimalValidYear);
				EfiTime.Month = 1;
				EfiTime.Day	= 1;
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "SubmitForFormSet, gMoveActionStage :%d.\n", gMoveActionStage));
  if ((!IsNvUpdateRequiredForFormSet (FormSet)) && (gMoveActionStage != 3)) {
    return EFI_SUCCESS;
  }

  if(CompareGuid(&FormSet->Guid, &gLsiCfgToolFormsetGuid)){
    DMSG ((EFI_D_ERROR, "skip SubmitForFormSet GUID :%g.\n", &FormSet->Guid));
    return EFI_SUCCESS;;
  }

  if(CompareGuid(&FormSet->Guid, &gWangXunLanFormsetGuid)){
    DMSG ((EFI_D_ERROR, "skip SubmitForFormSet GUID :%g.\n", &FormSet->Guid));
    return EFI_SUCCESS;;
  }
  ...
  )
{
  ...
  DEBUG((EFI_D_ERROR, __FUNCTION__"(L %d), SettingScope :%d. \n", __LINE__, SettingScope));
  DisResetPtr = (UINT8 *)(UINTN)PcdGet64(PcdDisableKBCtrlAltDelResetDataPtr);
  if(DisResetPtr == NULL){
    ...
    PcdSet64(PcdDisableKBCtrlAltDelResetDataPtr, (UINTN)DisResetPtr);
  }
  *DisResetPtr = 1;
  DEBUG((EFI_D_ERROR, __FUNCTION__"(L %d), DisResetPtr :0x%x-%d.\n", __LINE__, DisResetPtr, *DisResetPtr));

  DMSG ((EFI_D_ERROR, "\n SubmitForm, SettingScope :%d. \n", SettingScope));
  if (SettingScope == SystemLevel && gExitRequired) {
    DEBUG((EFI_D_ERROR, __FUNCTION__"(), CleanScreen, \n\n"));
    gST->ConOut->ClearScreen(gST->ConOut);
  
  	CmosWrite(0x50,0);//
  	CmosWrite(0x51,0);// stop ipmi boot set permanent enter setup ;
  	CmosWrite(0x52,0);//
  }
  switch (SettingScope) {
  case FormLevel:
  ...
  )
{
  ...
  while (!IsNull (&FormSet->StorageListHead, Link)) {
    ...
    DMSG ((EFI_D_ERROR, "CleanBrowserStorage, gMoveActionStage :%d.\n", gMoveActionStage));
    if (Storage->BrowserStorage->Type == EFI_HII_VARSTORE_EFI_VARIABLE_BUFFER) {
      if (Storage->ConfigRequest == NULL || Storage->BrowserStorage->ConfigRequest == NULL || gMoveActionStage == 3) {
        continue;
      }
  ...
  )
{
  ...
  switch (gBrowserSettingScope) {
    ...
    case SystemLevel:
      ...
      while (!IsNull (&gBrowserFormSetList, Link)) {
        FormSet = FORM_BROWSER_FORMSET_FROM_LINK (Link);
		Title = HiiGetString(FormSet->HiiHandle, FormSet->FormSetTitle, "en-US");
        if (!ValidateFormSet(FormSet)) {
          continue;
        }
		if(StrStr(Title,L"AVAGO")!=NULL){
	      Link = GetNextNode (&gBrowserFormSetList, Link);
          continue;
		}
