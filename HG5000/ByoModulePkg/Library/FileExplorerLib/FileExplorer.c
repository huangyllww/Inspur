  ...
  )
{
  ...
  if (LibFileSystemRemoved (QuestionId)) {
    ...
    if (strTitle && strFileSystemMissing) {
      UiConfirmDialog(DIALOG_INFO, strTitle, NULL, TEXT_ALIGIN_CENTER, strFileSystemMissing, NULL);
      FreePool (strTitle);
      FreePool (strFileSystemMissing);
    } else {
      UiConfirmDialog(DIALOG_INFO, L"File Explorer", NULL, TEXT_ALIGIN_CENTER, L"File System Missing !", NULL);
    }
  ...
  )
{
  ...
  Status = gBS->HandleProtocol(DiskHandle, &gEfiDiskInfoProtocolGuid, &DiskInfo);
  ...
  if(CompareGuid(&DiskInfo->Interface, &gEfiDiskInfoUsbInterfaceGuid)){
    ...
    Buffer[8] = ' ';
    CopyMem (&Buffer[9], UsbInquiryData.ProductID, sizeof(UsbInquiryData.ProductID));
    TrimStr8(Buffer);
    ...
    SwapWordArray(Buffer, sizeof(IdentifyData.ModelName));
    TrimStr8(Buffer); 
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    for (Index = 0; Index < NoSimpleFsHandles; Index++) {
      ...
      UpdatePartitionName(MenuEntry->DisplayString, MAX_CHAR, FileContext->DeviceHandle, VolumeLabel);
...
// 0 1 2 3  4  5
//             0
VOID UpdateStringWithWidth(CHAR16 *DisplayString, UINTN BufferSize, CHAR16 *FileName)
{
  UINTN           i, j, n, f, k;
  ...
  for(i=0;i<j;i++){
    if(FileName[i] >= 0x4E00 && FileName[i] < 0x9FFF){
      if(f == 1){
        if(n >= k){break;}
        DisplayString[n++] = WIDE_CHAR;
        f = 2;
      }
    } else {
      if(f == 2){
        if(n >= k){break;}
        DisplayString[n++] = NARROW_CHAR;
        f = 1;
      }
    }
    
    if(n >= k){break;}
    DisplayString[n++] = FileName[i];
  }
  ...
  )
{
  ...
  if(IsRoot){
    AddRootParentFolder(DeviceHandle);
    OptionNumber++;
  }
...
// wz200708 - >>
      if (NewFileContext->IsDir) {
        BufferSize = (StrLen(DirInfo->FileName) * 2 + 16) * 2;
        NewMenuEntry->DisplayString = AllocateZeroPool (BufferSize);
        for (Index = 0;Index< StrLen(DirInfo->FileName); Index++ ) {
           if(DirInfo->FileName[Index] >= 0x4E00 && DirInfo->FileName[Index] < 0x9FFF){
             Flag = TRUE;
           }
        }

        NewMenuEntry->DisplayString[0] = NARROW_CHAR;
        NewMenuEntry->DisplayString[1] = '<';        
        UpdateStringWithWidth(NewMenuEntry->DisplayString+2, BufferSize-10, DirInfo->FileName);
        Len = StrLen(NewMenuEntry->DisplayString);
        NewMenuEntry->DisplayString[Len] = NARROW_CHAR;
        NewMenuEntry->DisplayString[Len+1] = '>';
        NewMenuEntry->DisplayString[Len+2] = 0;
...
//-     NewMenuEntry->DisplayString = LibStrDuplicate (DirInfo->FileName);
        ...
        BufferSize = (StrLen(DirInfo->FileName) * 2 + 16) * 2;
        NewMenuEntry->DisplayString = AllocateZeroPool (BufferSize);
        UpdateStringWithWidth(NewMenuEntry->DisplayString, BufferSize, DirInfo->FileName);	
        }
// wz200708 - <<
  ...
  )
{
  ...
  if (RootDirectory == NULL) {
    ...
    Status = LibFindFiles (FileHandle, FileName, DeviceHandle, FALSE);
  }
  if (EFI_ERROR (Status)) {
    goto Done;
  }
