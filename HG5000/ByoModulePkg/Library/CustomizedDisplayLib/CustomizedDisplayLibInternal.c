  ...
  )
{
  ...
  // Draw all menu title.
  //
  if (LastIndex == (UINTN)-1 || LangChange) { 
...
PatchForTerminal ()
{
  ...
  ASSERT (Buffer != NULL);
  LibSetUnicodeMem (Buffer, gScreenDimensions.RightColumn, L' ');
  ...
  for(Index = 0; Index < HandleCount; Index ++){
    ...
    while (!IsDevicePathEndType (Next)) {
      if ((Next->Type == MESSAGING_DEVICE_PATH) && (Next->SubType == MSG_UART_DP)) {
        ...
        SimpleTextOut->SetAttribute (SimpleTextOut, EFI_WHITE |EFI_BACKGROUND_BLUE);
        for (Row = (gScreenDimensions.BottomRow - 2 - PcdGet8(PcdOemCopyRightLineNumber)); Row < gScreenDimensions.BottomRow-1; Row ++) {
          SimpleTextOut->SetAttribute (SimpleTextOut, EFI_WHITE | EFI_BACKGROUND_BLUE);
          SimpleTextOut->SetCursorPosition (SimpleTextOut, 0, Row);
          SimpleTextOut->OutputString (SimpleTextOut, Buffer);
        }
      }
...
//static BOOLEAN    bLastChar = TRUE;
  ...
  if (bFirstIn ) {
    ...
    ClearWithBlt (
        ...
        gScreenDimensions.BottomRow - 2 - PcdGet8(PcdOemCopyRightLineNumber),
        gScreenDimensions.BottomRow - 1 - PcdGet8(PcdOemCopyRightLineNumber),
...
//    }
  ...
  if (bFirstIn) {
    ...
    if (PcdGet8 (PcdUCREnable)) {
      Buffer1 = AllocateZeroPool (gScreenDimensions.RightColumn * 2 + 2);
      for (Index = 0; Index + 2 < (gScreenDimensions.RightColumn - gScreenDimensions.LeftColumn); Index++) {
        Buffer1[Index] = 0x20;
      }
    }
    ...
    for (Row = gScreenDimensions.TopRow + 3; Row <= gScreenDimensions.BottomRow - 4 - PcdGet8(PcdOemCopyRightLineNumber); Row++) {
      ...
      if (PcdGet8 (PcdUCREnable)) {// can not SetCursorPosition above 100 for terminal, but can use print to fill it
        PrintStringAt ((UINTN) -1, (UINTN) -1, Buffer1);
        PrintCharAt (-1, -1, Character);
      } else {
