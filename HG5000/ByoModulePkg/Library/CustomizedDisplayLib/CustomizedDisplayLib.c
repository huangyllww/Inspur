...
/**
  Adjust String with WIDE-NARROW format.
  ...
  )  
{
  ...
  //
  // Check WIDE-NARROW format for Chinese.
  //
  n = 0;
  f = 0;
  for (i=0; i<Len; i++) {
    if(DisplayStr[i] >= 0x4E00 && DisplayStr[i] < 0x9FFF){
      n = 1;
    }	
    if(DisplayStr[i] == WIDE_CHAR){
      f = 1;
    }
  }
  if ((n == 0) || ((n == 1) && (f == 1))) {
  ...
  // Create New.
  //
  NewStr = AllocateZeroPool (2 * Len * sizeof(CHAR16));
  if (NewStr == NULL) {
    return NULL;
  }

  DEBUG((EFI_D_ERROR, __FUNCTION__"(), NewStr :"));  
  n = 0;
  f = 1;	
  for (i=0; i<Len; i++) {	
    if (DisplayStr[i] >= 0x4E00 && DisplayStr[i] < 0x9FFF){
      if (f == 1) {
        NewStr[n++] = WIDE_CHAR;
        DEBUG((EFI_D_ERROR, "0x%x,", WIDE_CHAR));	
        f = 2;
      }
    } else {
      if(f == 2){
        NewStr[n++] = NARROW_CHAR;
        DEBUG((EFI_D_ERROR, "0x%x,", NARROW_CHAR));
        f = 1;
      }
    }
    
    NewStr[n++] = DisplayStr[i];
    DEBUG((EFI_D_ERROR, "0x%x,", DisplayStr[i]));	
  }
