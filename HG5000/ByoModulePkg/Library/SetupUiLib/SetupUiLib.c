  ...
  )
{
  ...
  } else if (Type != DIALOG_NO_KEY && Type != DIALOG_WAIT_KEY && Type != DIALOG_NO_REPAINT) {
...
//      continue;
//    }
    StringNew = AllocatePool(StrSize(StringTemp));
    if (StringNew == NULL) {
      continue;
    }
    i = 0;
    j = 0;
    while(1){
      if(StringTemp[i] != NARROW_CHAR && StringTemp[i] != WIDE_CHAR){
        StringNew[j] = StringTemp[i];
        if(StringNew[j] == 0){
          break;
        }
        j++;
...
//-   DEBUG((EFI_D_INFO, "IT:%d\n", InputType));
  ...
  if(Type != DIALOG_NO_KEY && !NotRepaint && Type != DIALOG_BOX && Type != DIALOG_NO_REPAINT){
  ...
  )
{
  ...
  if (IsSetupMode) {
    ...
    if (!PcdGet8 (PcdUCREnable)) {
      ...
      NewHorizontalResolution = PcdGet32 (PcdTerminalVideoHorizontalResolution);
      NewVerticalResolution   = PcdGet32 (PcdTerminalVideoVerticalResolution);
      NewColumns              = PcdGet32 (PcdTerminalConOutColumn);
      NewRows                 = PcdGet32 (PcdTerminalConOutRow);
    }
