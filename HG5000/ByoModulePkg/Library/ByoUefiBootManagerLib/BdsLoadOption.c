...
EFI_BOOT_MANAGER_LOAD_OPTION *LibDupBootOption(EFI_BOOT_MANAGER_LOAD_OPTION *BootOption)
{
  EFI_BOOT_MANAGER_LOAD_OPTION  *r = NULL;
  UINTN                         Size;
  

  r = AllocatePool(sizeof(EFI_BOOT_MANAGER_LOAD_OPTION));
  ASSERT(r != NULL);

  CopyMem(r, BootOption, sizeof(EFI_BOOT_MANAGER_LOAD_OPTION));

  Size = StrSize(BootOption->Description);
  r->Description = AllocatePool(Size);
  ASSERT(r->Description != NULL);
  StrCpy(r->Description, BootOption->Description);

  Size = GetDevicePathSize(BootOption->FilePath);
  r->FilePath = AllocatePool(Size);
  ASSERT(r->FilePath != NULL); 
  CopyMem(r->FilePath, BootOption->FilePath, Size);

  if(BootOption->OptionalDataSize != 0){
    r->OptionalData = AllocatePool(BootOption->OptionalDataSize);
    ASSERT(r->OptionalData != NULL);
    CopyMem(r->OptionalData, BootOption->OptionalData, BootOption->OptionalDataSize);
  }
...
VOID 
CopyBootOption(
  EFI_BOOT_MANAGER_LOAD_OPTION   *Dst,
  EFI_BOOT_MANAGER_LOAD_OPTION   *Src
  )
{
  UINTN                         Size;
...
EFI_BOOT_MANAGER_LOAD_OPTION *LibDupBootOptions(EFI_BOOT_MANAGER_LOAD_OPTION *BootOption, UINTN Count)
{
  ...
  Options = AllocatePool(Count * sizeof(EFI_BOOT_MANAGER_LOAD_OPTION));
  if(Options == NULL){
    return NULL;
  }

  for(Index=0;Index<Count;Index++){
    CopyBootOption(&Options[Index], &BootOption[Index]);
  }
  ...
  )
{
  ...
  CopyBootOption(NewOption + Count, Option);
  ...
  )
{
  ...
  FreePool (OptionOrder);
  ByoEfiBootManagerFreeLoadOptions (LoadOption, LoadOptionCount);
}
  ...
  )
{
  ...
  if(Option->Legacy){
    Option->GroupType = (UINT8)GetLegacyBootGroupType(FilePath, OptionalData);  
  } else {
    Option->GroupType = (UINT8)GetEfiBootGroupType(FilePath);
  }

  Option->DupDp = NULL;
  Option->ItemOrder = 0x10000;
  ...
  )
{
  ...
  if(LoadOption->DupDp != NULL){
    FreePool(LoadOption->DupDp);
  }
