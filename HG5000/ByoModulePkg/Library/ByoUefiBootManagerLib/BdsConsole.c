  ...
  *HostCount = 0;
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    if(IgdHandle == NULL && IgdPciDp != NULL && CompareMem(PciDp, IgdPciDp, IgdPciDpSize) == 0){
      ...
      PcdSet32(PcdSystemMiscConfig, SystemMiscCfg | SYS_MISC_CFG_IGD_PRESENT);
    } else {
      if(PegHandle == NULL){
        PegHandle = HandleBuffer[Index];
        if(ptHostInfo->AddOnGfxId == 0xFFFFFFFF){
          Status = PciIo->Pci.Read (
                            PciIo,
                            EfiPciIoWidthUint32,
                            0,
  ...
  )
{
  ...
  PcdSet64(PcdFirstVideoHostHandle, (UINT64)(UINTN)DisplayHost[0]);
  ...
  )
{
  ...
  do {
    ...
    if(ConsoleType == ConOut && !IsDevPathSerialPort(Instance)){
      continue;
    }
...
BOOLEAN CheckKeyBoardPresent()
{
  ...
  BOOLEAN                       UsbKbPresent = FALSE;
  UINT32                        MiscCfg;
  EFI_USB_DEVICE_DESCRIPTOR     DevDesc;
  CHAR16                        *StrProduct = NULL;
  ...
  if(PcdGetBool(PcdKbcPresent)){
    ...
    DEBUG((EFI_D_INFO, "Ps2Kb:%d, Ps2Ms:%d\n", Ps2Kb, Ps2Ms));
    MiscCfg = PcdGet32(PcdSystemMiscConfig);
    if(Ps2Kb){
      MiscCfg |= SYS_MISC_CFG_STS_PS2KB;
    }
    if(Ps2Ms){
      MiscCfg |= SYS_MISC_CFG_STS_PS2MS;
    }
    PcdSet32(PcdSystemMiscConfig, MiscCfg);
  ...
  if(!EFI_ERROR(Status)){
    for(Index=0; Index<HandleCount; Index++){
      ...
      DEBUG((EFI_D_INFO, "%r %X %X %X\n", Status, IfDesc.InterfaceClass, IfDesc.InterfaceSubClass, IfDesc.InterfaceProtocol));
      if (!EFI_ERROR (Status) && 
          IfDesc.InterfaceClass == CLASS_HID &&
          IfDesc.InterfaceSubClass == SUBCLASS_BOOT &&
          IfDesc.InterfaceProtocol == PROTOCOL_KEYBOARD) {
        UsbKbPresent = TRUE; 
        ...
        if (!EFI_ERROR (Status)) {
          ...
          if (!EFI_ERROR(Status)) {
            DEBUG((EFI_D_INFO, "StrProduct:%s\n", StrProduct));
            if(StrStr(StrProduct, L"Virtual") != NULL){
              DEBUG((EFI_D_INFO, "IsVirtual\n"));
              IsVirtual = TRUE;
            }
            FreePool(StrProduct);
          }
  ...
  if(UsbKbPresent){
    if(IsVirtual){
      MiscCfg = PcdGet32(PcdSystemMiscConfig);
      MiscCfg |= SYS_MISC_CFG_STS_ONLY_VIRTUAL_USB_KB;
      PcdSet32(PcdSystemMiscConfig, MiscCfg);
    }
    return TRUE;
  } else {
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "No Console\n"));
    REPORT_STATUS_CODE (EFI_ERROR_CODE, (EFI_PERIPHERAL_LOCAL_CONSOLE | EFI_P_EC_NOT_DETECTED));  
  } else {
    InvokeHookProtocol(gBS, &gByoConsoleOutDetectedProtocolGuid);
    REPORT_STATUS_CODE (EFI_PROGRESS_CODE, (EFI_PERIPHERAL_LOCAL_CONSOLE | EFI_P_PC_DETECTED));
...
/*  if (!CheckKeyBoardPresent()) {
    DEBUG((EFI_D_ERROR, "No KB\n"));
    PcdSet32(PcdSystemMiscConfig, PcdGet32(PcdSystemMiscConfig) | SYS_MISC_CFG_STS_NO_KB);
