  ...
  )
{
  ...
  if (ResultsData == NULL) {
    DEBUG((EFI_D_ERROR, "ResultsData -> NULL\n"));
    return FALSE;
  }
  ...
  if (ConfigResp == NULL) {
    DEBUG((EFI_D_ERROR, "ConfigResp -> NULL\n"));
    return FALSE;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "ConfigToBlock:%r\n", Status));
    return FALSE;
  }
  ...
  )
{
  ...
  if (StringLength == 0) {
    DEBUG ((EFI_D_INFO, "%a line %d UnicodeSPrint failed.\n", __FUNCTION__, __LINE__));
  }
  ...
  if (RequestString != NULL) {
    Status = StrCatS (NewString, StringLength + 1, RequestString);
    if (EFI_ERROR(Status)) {
  ...
  Status = StrCatS (NewString, StringLength + 1, RequestElement);
  if (EFI_ERROR(Status)) {
    DEBUG ((EFI_D_INFO, "%a line %d StrCatS failed.\n", __FUNCTION__, __LINE__));
  }
  DEBUG((EFI_D_ERROR, __FUNCTION__"(), NewString :%s.\n", NewString));
  return NewString;
}
