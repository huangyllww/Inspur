  ...
  switch (*ScanCode) {
    ...
    case 0x52:
      ...
      if (lpBda->ShiftStatus & BIT5) {
        InsertKey (lpBda, 0x5230); //keypad, '0'
      } else{
    ...
    case 0xe053:
      ...
      if((lpBda->ShiftStatus & (BIT2 | BIT3)) == (BIT2 | BIT3)) {
        if(*mDisableKBCtrlAltDelReset == 1){
          DEBUG((EFI_D_INFO, "USB KB reset disabled\n"));
        } else {
...
//  }
  ...
  if (ScanCodeCheck[0] == 1) {
    ...
    if (lpBda->ShiftStatus & BIT5) {
      ...
      //
      ExtendScanCode = ExtendScanCode * 0x100;
      if (lpBda->ShiftStatus & BIT3 ||ScanCodeCheck[1] == 0x52) {
        //
        // With Alt key press
        //
        ExtendScanCode = 0xE000;
    ...
    /*
      if(ExtendScanCode == 0x53){
        ExtendScanCode = 0xE053;
      } else {
        //ExtendScanCode = 0xFF;
        ExtendScanCode = ExtendScanCode * 0x100;
...
TranslateKey (UINT8 ScanCode)
{
  ...
  // DEBUG((EFI_D_INFO, "robin, USB KB  scancode: %lx.\n", ExtendScanCode));
  ...
  */
  ...
  // robin + >>
  // Clear E0 extended flag
  if((ExtendScanCode & 0xFF00) == 0xE000) {
    lpBda->ExtKbdSts1 &= ~BIT1;
  } 
  // robin + <<
}
