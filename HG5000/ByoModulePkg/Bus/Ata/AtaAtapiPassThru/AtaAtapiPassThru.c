  ...
  )
{
  ...
  ErrorInfo.Host = Host;
  ErrorInfo.Port = Port;
  ErrorInfo.PortMp = PortMp;
  ErrorInfo.IsIdeMode = IsIdeMode;
  REPORT_STATUS_CODE_WITH_EXTENDED_DATA (
    EFI_ERROR_CODE,
    DXE_ATA_DEVICE_FAILURE,
    (VOID*)&ErrorInfo,
    sizeof(ErrorInfo)
    );
}
  ...
  )
{
  ...
  if(Status == EFI_DEVICE_ERROR && Packet->Acb->AtaCommand != ATA_CMD_SECURITY_UNLOCK){
    DEBUG((EFI_D_ERROR, "ATA(%d,%d) CMD:%X ERROR\n", Port, PortMultiplierPort, Packet->Acb->AtaCommand));
    ReportHddError(Instance->ControllerHandle, Port, PortMultiplierPort, (Mode == EfiAtaIdeMode));
  }
  return Status;
}
  ...
  )
{
  ...
  ATA_PASSTHRU_HOOK_PARAMETER       Param;
  ...
  Param.Sign = ATA_PASSTHRU_HOOK_PARAMETER_SIGN;
  Param.Controller = Controller;
  Param.AtaPassThru = &(Instance->AtaPassThru);
  Param.PciIo = PciIo;
  switch(Instance->Mode){
    case EfiAtaIdeMode:
      Param.SataMode = ATA_PT_SATA_MODE_IDE;
      break;
    case EfiAtaAhciMode:
      Param.SataMode = ATA_PT_SATA_MODE_AHCI;
      break;
    case EfiAtaRaidMode:
      Param.SataMode = ATA_PT_SATA_MODE_RAID;
      break;       
    default:
      Param.SataMode = ATA_PT_SATA_MODE_UNKNOWN;
      break;      
  }

  InvokeHookProtocol2(gBS, &gAtaPassThruHookProtocolGuid, &Param);
  return Status;
  ...
  )
{
  ...
  *AhciReg = &(Instance->AhciRegisters);
  return EFI_SUCCESS;
}
