  ...
  )
{
  ...
  // DEBUG ((EFI_D_INFO,"%a(),line:%d,Status:%r\n",__FUNCTION__,__LINE__,Status));
  ...
  )
{
  ...
  UINTN                   FisBaseAddr;
  UINT32                  Value;
  ATA_DEVICE_ERROR_INFO   ErrorInfo;
...
//           );
      ErrorInfo.Signature = ATA_DEVICE_ERROR_INFO_TAG;
      ErrorInfo.Host   = ControllerHandle;
      ErrorInfo.Port   = Port;
      ErrorInfo.PortMp = PortMultiplier;
      ErrorInfo.IsIdeMode = FALSE;
...
AHCI_INIT_HOOK_POD_AFTER gAhciInitHookPodAfter = NULL;
...
// winddy + <<
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "AhciReset error:%r\n", Status));
    return EFI_DEVICE_ERROR;
  }
  ...
  for (Port = 0; Port < EFI_AHCI_MAX_PORTS; Port ++) {
    if ((PortImplementBitMap & (((UINT32)BIT0) << Port)) != 0) {
      ...
      if(gAhciInitHookPodAfter != NULL){
        gAhciInitHookPodAfter(PciIo, Port);
      }
      ...
      if (PhyDetectDelay == 0) {
        ...
        DEBUG((EFI_D_ERROR, "Port[%d] Phy not detected\n", Port));
        continue;
      }
      ...
      if (EFI_ERROR (Status)) {
        DEBUG((EFI_D_ERROR, "Port[%d] Sig Not Received\n", Port));
        continue;
      }
      ...
      if ((Data & EFI_AHCI_ATAPI_SIG_MASK) == EFI_AHCI_ATAPI_DEVICE_SIG) {
        ...
        if (EFI_ERROR (Status)) {
          DEBUG((EFI_D_ERROR, "Port[%d] AhciIdentifyPacket:%r\n", Port, Status));
          continue;
        }
        ...
        if (EFI_ERROR (Status)) {
          DEBUG((EFI_D_ERROR, "Port[%d] AhciIdentify:%r\n", Port, Status));
      ...
      for(RetryIndex=0; RetryIndex<3; RetryIndex++){    
        ...
        ReportHddError(Instance->ControllerHandle, Port, 0, FALSE);
      }
      if(RetryIndex >= 3){
        continue;
      }
