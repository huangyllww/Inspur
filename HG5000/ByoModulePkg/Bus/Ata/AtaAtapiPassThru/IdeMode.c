  ...
  )
{
  ...
  for(RetryIndex=0;RetryIndex<3;RetryIndex++){
    ...
    ReportHddError(Instance->ControllerHandle, Channel, Device, TRUE);
  }
...
//         );
    ...
    ErrorInfo.Host   = Instance->ControllerHandle;
    ErrorInfo.Port   = Channel;
    ErrorInfo.PortMp = Device;
    ErrorInfo.IsIdeMode = TRUE;
...
// wz181023 + >>
// Sync ZX solution for winxp cdrom working in PIO mode
