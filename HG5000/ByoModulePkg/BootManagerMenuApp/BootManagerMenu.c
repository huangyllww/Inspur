...
EFI_BDS_BOOT_MANAGER_PROTOCOL *gBdsBootMgr      = NULL;
...
{
  gBdsBootMgr->PrintPostString(Row, Column, String, 0);
}
...
//DEBUG((EFI_D_INFO, __FUNCTION__"(%d)\n", BootOptionCount));
  ...
  DEBUG((EFI_D_INFO, "Columns = %d ,Rows =%d\n", Columns, Rows));
...
//- DEBUG((EFI_D_INFO, "[%d]:%s\n", Index, BootOption[Index].Description));
    ...
    p = gBdsBootMgr->GetExtBmBootName (
  ...
  )
{
  ...
  Status = gBS->LocateProtocol(&gEfiBootManagerProtocolGuid, NULL, &gBdsBootMgr);
  if(EFI_ERROR(Status)){
    goto ProcExit;
  }  

  gBdsBootMgr->ConnectAll();
  gBdsBootMgr->RefreshOptions();
  gBdsBootMgr->GetOptions(&BootOption, &BootOptionCount);  
  ...
  Status = gBdsBootMgr->CreateSetupOption(&BootOption[BootOptionCount]);	
  if(!EFI_ERROR(Status)){
    BootOptionCount++;
  }  
  ...
  while (!ExitApplication) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      case CHAR_CARRIAGE_RETURN:
        gST->ConOut->ClearScreen(gST->ConOut);
        BootFromSelectOption (BootOption, BootOptionCount, BootMenuData.SelectItem, gBdsBootMgr);
        DrawBootPopupMenu (&BootMenuData);
        break;
  ...
  gBdsBootMgr->FreeOptions(BootOption, BootOptionCount);
