  ...
  )
{
  ...
  DEBUG((EFI_D_ERROR, "LegacyGetPhysicPortIndex SetupData.ScreenPrintNum = 0x%x\n", SetupData.ScreenPrintNum));
  DEBUG((EFI_D_ERROR, "LegacyGetPhysicPortIndex DevIndex = 0x%x\n", LogicIndex));
  
  HostInfo = PlatHostInfo->HostList;
  for(Index=0;Index<PlatHostInfo->HostCount;Index++){
    ...
    if(Ctx->HostIndex == PlatSataHostIndex){
      DEBUG((EFI_D_ERROR, "LegacyGetPhysicPortIndex Ctx->PortCount = 0x%x\n", Ctx->PortCount));
      for (PortIndex = 0; PortIndex < Ctx->PortCount; PortIndex++) {
        if (Ctx->PortLogicToPhysic[SetupData.ScreenPrintNum][PortIndex].LogicPort == LogicIndex) {
          DEBUG((EFI_D_ERROR, "LegacyGetPhysicPortIndex return PhysicsPort = 0x%x\n", Ctx->PortLogicToPhysic[SetupData.ScreenPrintNum][PortIndex].PhysicsPort));
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if(!EFI_ERROR(Status)){
      PciHandle = GetPciHandleFromBDF(gBS, Bus, Device, Function);
...
// [X][XXXX] -> X000:XXXX 
    ...
    if(DescString != NULL && AsciiStrnCmp(DescString, " SATA ", 6) == 0 && DescString[6] >= '1' && DescString[6] <= '9'){
      ...
      DescString[6] = '0' + (UINT8)LegacyGetPhysicPortIndex(PlatHostInfo, PlatSataHostIndex, DescString[6] - '1');
      
      if(PlatSataHostIndex != 0xFFFF){
        CopyMem(DescString, "SATA", 4);
        DescString[4] = '0' + (UINT8)PlatSataHostIndex;
        DescString[5] = '-';
      } else {
        DescString++;
