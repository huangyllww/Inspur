...
// wz190114 + >>
VOID CopyBbsTableToShadowRam(LEGACY_BIOS_INSTANCE *Private)
{
  UINT16                 BbsCount;
  UINTN                  CopySize;
  EFI_IA32_REGISTER_SET  Regs;
  ...
  BbsCount = 0x30 + 8 + 5;                             // 0x30:SATA 8:USB, 5:SD
  ...
  if (Regs.X.AX == 0) {
    LocalBbsTable = (BBS_TABLE*)(UINTN)(Regs.X.DS * 16 + Regs.X.BX);
    DEBUG((EFI_D_INFO, "%X -> %X L:%X\n", Private->BbsTablePtr, LocalBbsTable, CopySize));
    CopyMem((void*)(UINTN)LocalBbsTable, Private->BbsTablePtr, CopySize);
    Private->BbsTablePtr = (VOID *)LocalBbsTable;
    EfiToLegacy16BootTable = &Private->IntThunk->EfiToLegacy16BootTable;
    EfiToLegacy16BootTable->BbsTable = (UINT32)(UINTN)LocalBbsTable;
    Private->IntThunk->BbsTableCount = BbsCount;
  }
...
//- PrintHddInfo (LocalHddInfo);
  ...
  if (!Private->LegacyBootEntered) {
    ...
    CopyBbsTableToShadowRam(Private);         // wz190114 +  for dos int19h reboot.
  }
