...
void CapsuleInit(PEI_FAT_PRIVATE_DATA *PrivateData)
{
  ...
  for (Index = 0; Index < PrivateData->VolumeCount; Index++) {
    ...
    if (!EFI_ERROR(Status)) {
      for (i = 0; i < NoFile; i++) {
        if(PrivateData->CapsuleCount < PEI_FAT_MAX_CAPSULE_NUMBEER){
          CopyMem (
  ...
  )
{
  ...
  CHAR16                 *Next;
  BOOLEAN                IsFile;
  CHAR16                 *TagetPathFile = NULL;
  

  DEBUG((EFI_D_INFO, "%a\n", __FUNCTION__));

  TagetPathFile = BuildTargetPathStr(BiosIdInfo->RecoveryFilePath, BiosIdInfo->RecoveryFileName);
  DEBUG((EFI_D_INFO, "TagetPathFile:%s\n", TagetPathFile));  
...
ProcExit:
  if(TagetPathFile != NULL){FreePool(TagetPathFile);}
  if (EFI_ERROR (Status) && *NoFile == 0) {
    return EFI_NOT_FOUND;
  } else {
    *FileBuffer = Buffer;
  ...
  )
{
  ...
  *NoFile = 0;
  ...
  if(EFI_ERROR(Status) || BiosIdInfo->RecoveryFilePath[0] == 0){
    IsOemFilePath = FALSE;
  } else {
    IsOemFilePath = TRUE;
  }
  ...
  if(!IsOemFilePath){
    return FindRecoveryFileOld(PrivateData, VolumeIndex, NoFile, FileBuffer);
  } else {
