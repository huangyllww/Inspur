  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    Data = CpmTablePpiPtr->CommonFunction.IoRead8 (0x70);
    DEBUG((EFI_D_ERROR, __FUNCTION__"(), 0x70 Port :0x%x.\n", Data));
    Data |= BIT7;
    CpmTablePpiPtr->CommonFunction.IoWrite8(0x70, Data); //Enable LPC_PD_L/AGPIO21 for NMI.
  }
