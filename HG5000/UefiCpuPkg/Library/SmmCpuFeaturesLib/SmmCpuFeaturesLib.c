  ...
  )
{
  ...
  mSmrrCacheType = PcdGet8(PcdCpuSmrrCacheType);
  DEBUG ((DEBUG_INFO, "mSmrrCacheType:0x%X\n", mSmrrCacheType));

  if(mSmrrCacheType == 0xFF){
    mSmrrSupported = FALSE;
  ...
  )
{
  ...
  if (mSmrrSupported) {
    ...
    if ((CpuHotPlugData->SmrrSize < SIZE_4KB) ||
      ...
      AsmWriteMsr64 (mSmrrPhysBaseMsr, CpuHotPlugData->SmrrBase | mSmrrCacheType);
...
VOID
EFIAPI
SmmCpuBspFeaturesRendezvousEntry (
  VOID
  )
{
  mPort70 = IoRead8(0x70);

  if(FeaturePcdGet(PcdCpuSmmProtectPort74)){
    mPort74 = IoRead8(0x74);
  }
}
...
VOID
EFIAPI
SmmCpuBspFeaturesRendezvousExit (
  VOID
  )
{
  IoWrite8(0x70, mPort70);

  if(FeaturePcdGet(PcdCpuSmmProtectPort74)){
    IoWrite8(0x74, mPort74);
  }
}
