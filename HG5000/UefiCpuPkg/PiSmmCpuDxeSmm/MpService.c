...
// if SMRR is diabled, smm core will update cache when enter smi
// if shadow ram range is still WP, this area cannot be write.
// so here set this area as WT for usb info writing in smi.
// hard code is dirty, is it any graceful way ...
  for(Index=3;Index<=10;Index++){                       // C0000 - FFFFF
    gSmiMtrrs.Fixed.Mtrr[Index] = 0x0404040404040404;   // WT
  }
