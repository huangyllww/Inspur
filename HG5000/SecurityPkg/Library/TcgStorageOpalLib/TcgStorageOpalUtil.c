  ...
  )
{
  ...
  if (Ret != TcgResultSuccess || MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS) {
    DEBUG ((DEBUG_INFO, "start session with admin SP as SID authority failed: Ret=0x%X MethodStatus=0x%X\n", Ret, MethodStatus));
    goto done;
  }
  ...
  )
{
  ...
  if (Ret != TcgResultSuccess || MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS) {
    DEBUG ((DEBUG_INFO, "start session with admin SP using old Password failed 0x%X 0x%X\n", Ret, MethodStatus));
    goto done;
  }
  ...
  )
{
  ...
  if (Ret != TcgResultSuccess || MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS) {
    DEBUG ((DEBUG_INFO, "OpalStartSession (L%d) %d %X\n", __LINE__, Ret, MethodStatus));
    goto done;
  }
