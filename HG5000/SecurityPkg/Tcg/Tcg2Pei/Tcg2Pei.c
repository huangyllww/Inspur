  ...
  )
{
  ...
  if(LogicalId.Family == HYGON_FAMILY_18_DN && LogicalId.Revision >= HYGON_REV_F18_DN_B0){
    if (PcdGetBool(PcdHygonFtpmEnable)) {
      return EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  if(EFI_SUCCESS==CheckHgfTpmSupport()){
    PcdSetBool(PcdFtpmSupport,TRUE);
  }
  if(0xFFFF!=MmioRead16(0xFED40000)){
    PcdSetBool(PcdDtpmSupport,TRUE);
  }
