...
#if !defined(TKN_LOAD_DEFAULT_RTC_IF_CMOS_BAD)
  ...
  )
{
  ...
  while(1){
	if((RtcRead( RTC_ADDRESS_REGISTER_C) )==0){
		break;
		}
	}
  ...
  DEBUG((EFI_D_INFO, "T:%02X%02X%02X %02X:%02X:%02X\n", Time.Year, Time.Month, Time.Day, Time.Hour, Time.Minute, Time.Second));
...
#if TKN_LOAD_DEFAULT_RTC_IF_CMOS_BAD  
  IsCmosBad = PcdGet8(PcdIsPlatformCmosBad);
#else
  IsCmosBad = FALSE;
#endif
DEBUG((EFI_D_ERROR, __FUNCTION__"(), IsCmosBad :0x%x.\n", IsCmosBad));
  ...
  )
{
  ...
  if (RegisterD.Bits.Vrt == 0) {
    DEBUG((EFI_D_ERROR, "%a() L%d vrt:%d\n", __FUNCTION__, __LINE__, RegisterD.Bits.Vrt));  
//- return EFI_DEVICE_ERROR;
  }
  //
  ...
  )
{
  ...
  if ((CenturyRtcAddress != 0) && (mModuleGlobal.CenturyRtcAddress != CenturyRtcAddress)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if(Century != RtcRead(mModuleGlobal.CenturyRtcAddress)) {
