...
//
UINTN RootBridgeNumber[1] = { 8 };
UINT64 RootBridgeAttribute[1][8] = { 
  {
...
EFI_PCI_ROOT_BRIDGE_DEVICE_PATH mEfiPciRootBridgeDevicePath[1][8] = {
  {
    {
      ACPI_DEVICE_PATH,
      ACPI_DP,
...
// PCIE base address length is set as 128M
// so max bus is 0x7F.
PCI_ROOT_BRIDGE_RESOURCE_APPETURE  mResAppeture[1][8] = {
  {
    {0x00, 0x0f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0x10, 0x1f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},   
    {0x20, 0x2f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},  
    {0x30, 0x3f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0x40, 0x4f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0x50, 0x5f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},   
    {0x60, 0x6f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},  
    {0x70, 0x7f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},     
  }
};
  ...
  )
{
  ...
  for(Loop2 = 0; Loop2 < Info->PciHostCount; Loop2++){
    mResAppeture[0][Loop2].BusBase  = Info->BusBase[Loop2];
    mResAppeture[0][Loop2].BusLimit = Info->BusLimit[Loop2];   
  }
  ...
  for (Loop1 = 0; Loop1 < HOST_BRIDGE_NUMBER; Loop1++) {
    ...
    HostBridge->RootBridgeNumber = Info->PciHostCount;
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "%a [%d] %lX\n", __FUNCTION__, DieNum, Address));
  
  Value = (UINT32)(Address >> 32);
  NbSmnWrite (DieNum, 0x13f00048, &Value);
  Value = (UINT32)(Address & 0xFFFFFF00);
  NbSmnWrite (DieNum, 0x13f00044, &Value);
}

void SetNbApicRangeAndId (
  UINT32                    DieNum,
  UINT64                    Address,
  UINT32                    IoApicId
  )
{
  ...
  Value = (UINT32)(Address >> 32);
  NbSmnWrite (DieNum, 0x13b102f4, &Value);
  Value = (UINT32)(Address & 0xFFFFFF00);
  NbSmnWrite (DieNum, 0x13b102f0, &Value);
  Value |= 1;
  NbSmnWrite (DieNum, 0x13b102f0, &Value);

  Value = (IoApicId & 0xFF) << 24;
  NbSmnWrite (DieNum, 0x2801000, &Value);
}
...
STATIC UINT8 gFabricId[] = { 0x04, 0x24, 0x44, 0x64, 0x84, 0xA4, 0xC4, 0xE4 };
  ...
  ) 
{
    ...
    MMIOBaseReg  = (0x200|((RF_RBnum) << 4));
    MMIOLimitReg = (0x204 |((RF_RBnum) << 4));
    MMIOCtrlReg = (0x208 |((RF_RBnum) << 4));

    DEBUG((EFI_D_INFO, "Program DF Base=0x%x Limit=0x%x RB#%x\n", DF_Base, DF_Limit, RF_RBnum));

    BaseValue = DF_Base >> 16;
    if(RF_RBnum == 0)  {
      ...
      if ((LogicalId.Revision & (AMD_REV_F17_ZP_Ax | AMD_REV_F17_ZP_B0 | AMD_REV_F17_ZP_B1)) != 0) 
      {
//PLAT-13614 Set MMIO range FEC0_0000 to FEC2_FFFF to be Non-Posted
        LimitValue = 0xFEC0; 
      } else {
        //PLAT-19403 [SP3] DF MMIO region needs to include the HPET space
        LimitValue = 0xFED0; 
      }
    } else {
        LimitValue = DF_Limit >> 16;
    }
    TempRF_RBnum = 0;
    for (Node = 0; Node < 8; Node++) {
      Dev = 0x18+Node;
      Base = PCI_LIB_ADDRESS(0, Dev, 0, 0);
      if(PciRead32(Base) != 0xFFFFFFFF){
        if (TempRF_RBnum == RF_RBnum) {
          break;
        }
        TempRF_RBnum++;
      }
    }
    CtrlValue = (gFabricId[Node] << 4)|3;

    DEBUG((EFI_D_INFO, "DF Program Base = %x, Limit =%x, Ctrl = %x\n", BaseValue, LimitValue, CtrlValue));
    ...
    for(Node=0;Node<8;Node++){
      ...
      PciWrite32(Base + MMIOBaseReg, BaseValue);
      PciWrite32(Base + MMIOLimitReg, LimitValue);          
      PciWrite32(Base + MMIOCtrlReg, CtrlValue);  

//-   DEBUG((EFI_D_INFO, "[%X] <- %X\n", Base + MMIOBaseReg, BaseValue));
//-   DEBUG((EFI_D_INFO, "[%X] <- %X\n", Base + MMIOLimitReg, LimitValue));      
//-   DEBUG((EFI_D_INFO, "[%X] <- %X\n", Base + MMIOCtrlReg, CtrlValue));
    }
  ...
  )
{
  ...
  EFI_PHYSICAL_ADDRESS                  OldBaseAddr;
  UINT64                                AddrLen;
      ...
      while (List != &HostBridgeInstance->Head) {
        ...
        for (Index = TypeIo; Index < TypeBus; Index++) {
          if (RootBridgeInstance->ResAllocNode[Index].Status != ResNone) {
            ...
            DevicePath = (EFI_PCI_ROOT_BRIDGE_DEVICE_PATH*)RootBridgeInstance->DevicePath;
            Instance = DevicePath->AcpiDevicePath.UID;
            Info = GetPlatformCommInfo();
            ASSERT(Instance <= PCI_HOST_MAX_COUNT);
            ...
            switch (Index) {
              ...
              case TypeIo:  
                ...
                BaseAddress = Info->PlatPciIoLimit;
                DEBUG((EFI_D_INFO, "Io[%d] As:%d L:%X\n", Instance, BitsOfAlignment, AddrLen)); 
                ...
                DEBUG((EFI_D_INFO, "AllocateIoSpace:%r\n", Status));

                if (!EFI_ERROR (Status)) {
                  RootBridgeInstance->ResAllocNode[Index].Base   = (UINTN)BaseAddress;
                  RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;
                  
                  Info->IoBase[Instance] = (UINT16)BaseAddress & 0xF000;
                  Info->PlatPciIoCur     = (UINT16)(BaseAddress + AddrLen);
                  Info->PlatPciIoCur     = (UINT16)ALIGN_VALUE(Info->PlatPciIoCur, 0x1000);
                  Info->IoSize[Instance] = Info->PlatPciIoCur - Info->IoBase[Instance];

                  DEBUG((EFI_D_INFO, "Io[%d] (%X,%X)\n", Instance, Info->IoBase[Instance], Info->IoSize[Instance]));
              ...
              case TypeMem32:
                ...
                if(BitsOfAlignment < 20){
                  BitsOfAlignment = 20;
                }
                if (BitsOfAlignment >= 32) {
                  BitsOfAlignment = 0;
                }
                DEBUG((EFI_D_ERROR, "[%d] PciM32 Abits:%d L:0x%X\n", Instance, BitsOfAlignment, AddrLen));
                
                BaseAddress = PcdGet32(PcdPci32AllocLimitAddr);
                OldBaseAddr = BaseAddress;

// NBIO_IOMMU_LENGTH(0x80000) and 1M align
                ...
                ASSERT(!EFI_ERROR(Status));
                SetNbMmuRange(Instance, BaseAddress);

// NBIO_IOAPIC_LENGTH(0x1000), 64K align
                Status = gDS->AllocateMemorySpace (
                                EfiGcdAllocateMaxAddressSearchTopDown, 
                                EfiGcdMemoryTypeMemoryMappedIo, 
                                16,
                ...
                ASSERT(!EFI_ERROR(Status));
                Info->NbApicBase[Instance] = (UINT32)BaseAddress;
                Info->NbApicId[Instance]   = Instance + 0x81;
                SetNbApicRangeAndId(Instance, Info->NbApicBase[Instance], Info->NbApicId[Instance]);  
                
                Status = gDS->AllocateMemorySpace (
                                EfiGcdAllocateMaxAddressSearchTopDown, 
                                EfiGcdMemoryTypeMemoryMappedIo, 
                                BitsOfAlignment,
                                AddrLen + SIZE_32MB,                    // add hotplug
                                &BaseAddress,
                                mDriverImageHandle,
                                NULL
                                );
                ...
                if (!EFI_ERROR (Status)) {
                  RootBridgeInstance->ResAllocNode[Index].Base   = (UINTN)BaseAddress;
                  RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;
                  PcdSet32(PcdPci32AllocLimitAddr, (UINT32)BaseAddress);

                  if(Instance!=0){
                    OldBaseAddr--;
                  }
                  ProgramDF(Instance, (UINT32)BaseAddress, (UINT32)OldBaseAddr);
                  Info->MmioBase[Instance] = (UINT32)BaseAddress;
                  Info->MmioSize[Instance] = (UINT32)(OldBaseAddr - BaseAddress + 1);                   
                } else {
              ...
              case TypeMem64: 
                ...
                DEBUG((EFI_D_ERROR, "[%d] Pci64 Abits:%d L:0x%lX\n", Instance, BitsOfAlignment, AddrLen));
                if(AddrLen == 0){
                  break;
                } 
                
                BaseAddress = Info->Mmio64Base[Instance] + Info->Mmio64Size[Instance] - 1;
                Status = gDS->AllocateMemorySpace (
                                EfiGcdAllocateMaxAddressSearchTopDown, 
                                EfiGcdMemoryTypeMemoryMappedIo, 
                                BitsOfAlignment,
              ...
              case TypePMem32:                  
              case TypePMem64:
                  DEBUG((EFI_D_ERROR, "MemType %d not support\n", Index)); 
    ...
    if (RootBridgeHandle == RootBridgeInstance->Handle) {
      while ( *Temp == 0x8A) {
        ...
        if (Ptr->ResType == ACPI_ADDRESS_SPACE_TYPE_IO && 
            GetPowerOfTwo64 (Ptr->AddrRangeMax + 1) != (Ptr->AddrRangeMax + 1)) {
          DEBUG((EFI_D_ERROR, "Ptr->AddrRangeMax:%lX\n", Ptr->AddrRangeMax));
          return EFI_INVALID_PARAMETER;
        }
        ...
        case ACPI_ADDRESS_SPACE_TYPE_MEM:
          ...
          if (RootBridgeInstance->RootBridgeAttrib == EFI_PCI_HOST_BRIDGE_COMBINE_MEM_PMEM &&
            ...
            DEBUG((EFI_D_ERROR, "RootBridgeAttrib:%lX, SpecificFlag:%X\n", RootBridgeInstance->RootBridgeAttrib, Ptr->SpecificFlag)); 
            return EFI_INVALID_PARAMETER;
          }
  ...
  )
{
  ...
  LIST_ENTRY                            *List; 
  PCI_PREPROCESS_HOOK_PARAMETER         Param;
  ...
  Param.Sign = PCI_PREPROCESS_HOOK_PARAMETER_SIGN;
  Param.Io         = &RootBridgeInstance->Io;
  Param.PciAddress = PciAddress;
  Param.Phase      = Phase;
  InvokeHookProtocol2(gBS, &gPciPreprocessControllerHookProtocolGuid, &Param);
