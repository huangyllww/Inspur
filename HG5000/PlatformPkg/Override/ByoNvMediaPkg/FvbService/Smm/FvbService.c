...
)
{
    ...
    DEBUG((EFI_D_INFO, "FvbProtocolGetBlockSize(%lX)\n", Lba));
...
)
{
  ...
  //
  DEBUG ((EFI_D_INFO, "%a (L%d) %X %X %X\n", __FUNCTION__, __LINE__, FwhInstance->FvBase, \
                      FwVolHeader->ExtHeaderOffset, InstanceNum));
...
)
{
    ...
    for (Idx = 0; Idx < sizeof (mPlatformFvBaseAddress)/sizeof (mPlatformFvBaseAddress[0]); Idx++) {
        ...
        DEBUG((EFI_D_INFO, "Blocks:%d\n", FwhInstance->NumOfBlocks));
