  ...
  )
{
  ...
  if(!IS_PCI_BRIDGE (&PciDev->Pci)) {
    if(PciDev->RomBar.BarType == PciBarTypeMem32) {
      Node = CreateResourceNode (
              PciDev,
              PciDev->RomBar.Length,
              PciDev->RomBar.Alignment,
              PCI_MAX_BAR,
              PciBarTypeMem32,
              PciResUsageTypical
              );
  ...
  )
{
  ...
  DEBUG((EFI_D_ERROR, "%a (%X,%X,%X) Dec:%X Csm:%d\n", __FUNCTION__, \
    Bridge->BusNumber, Bridge->DeviceNumber, Bridge->FunctionNumber, Bridge->Decodes, gCsmOn));  
  ...
  )
{
  ...
  UINT32              Address32;
  PCI_BAR             *Bar;	
  //DEBUG((EFI_D_ERROR,"%a, Bar %d\n", __FUNCTION__,Node->Bar));
  ...
  Bar     = (Node->Bar == PCI_MAX_BAR ? &Node->PciDev->RomBar : &(Node->PciDev->PciBar[Node->Bar]));
  ...
  )
{
  ...
  while (Parent != NULL) {
    ...
    Node.PciDev     = Parent;
    Node.Length     = PciDevice->RomBar.Length;
    Node.Alignment  = 0;
