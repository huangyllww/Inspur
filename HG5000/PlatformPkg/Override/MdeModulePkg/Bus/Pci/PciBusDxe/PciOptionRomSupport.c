  ...
  )
{
  ...
  if ((EfiOpRomImageNode == NULL) ||
      ...
      (EfiOpRomImageNode->EndingOffset >= PciIoDevice->RomBar.Length) ||
      (BufferSize == NULL)
      ) {
  ...
  )
{
  ...
  PciIoDevice->RomBar.Offset = RomBarIndex;
  if (EFI_ERROR (Status)) {
    return EFI_NOT_FOUND;
  }
  ...
  // Rombar
  //
  PciIoDevice->RomBar.BaseAddress = OriginalValue & 0xFFFFF800;
  PciIoDevice->RomBar.BarType     = PciBarTypeMem32;
  PciIoDevice->RomBar.Length      = (~Value) + 1;
  PciIoDevice->RomBar.Alignment   = PciIoDevice->RomBar.Length - 1;
  
  if(PciIoDevice->RomBar.Length == 0) {
    PciIoDevice->RomBar.BarType      = PciBarTypeUnknown;
	PciIoDevice->RomBar.BaseAddress  = 0;
	PciIoDevice->RomBar.Alignment    = 0;
  }
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "%a() (%X,%X,%X) %X\n", __FUNCTION__, 
		       ...
		       PciDevice->RomBar.Length));

  RomSize       = PciDevice->RomBar.Length;
...
NextImage:
  ...
  } while (((Indicator & 0x80) == 0x00) && ((UINTN) (RomBarOffset - (UINT8 *) RomBar) < PciDevice->RomBar.Length));
