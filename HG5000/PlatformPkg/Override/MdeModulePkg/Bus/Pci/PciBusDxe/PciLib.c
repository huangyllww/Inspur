  ...
  )
{
  ...
  while (TRUE) {
    ...
    while (PciResAlloc->GetNextRootBridge (PciResAlloc, &RootBridgeHandle) == EFI_SUCCESS) {
      ...
      //
      RootBridgeDev->RomBar.Length= Mem32Bridge->Length;
      ...
      if (!ReAllocate) {
        ...
        if (MaxOptionRomSize > Mem32Bridge->Length) {
          ...
          RootBridgeDev->RomBar.Length  = MaxOptionRomSize;
        }
...
// Mem32 needs 0x300000, alignment is 2^20
  ...
  while (PciResAlloc->GetNextRootBridge (PciResAlloc, &RootBridgeHandle) == EFI_SUCCESS) {
    ...
    //
    ProcessOptionRom (RootBridgeDev, Mem32Base, RootBridgeDev->RomBar.Length);//RomSize);
