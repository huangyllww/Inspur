  ...
  if (!PciIoDevice->AllOpRomProcessed) {
    ...
    if (gPciPlatformProtocol != NULL) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        PciIoDevice->RomBar.Length  = PlatformOpRomSize;
...
// no     : 2
  ...
  if(HasEfiImage){
    if(PciIoDevice->Pci.Hdr.ClassCode[2] == 1 &&  // ignore NVME option rom
       PciIoDevice->Pci.Hdr.ClassCode[1] == 8 && 
       PciIoDevice->Pci.Hdr.ClassCode[0] == 2){
      HasEfiImage = 0;
    } else if(PciIoDevice->Pci.Hdr.VendorId == 0x1B21 && PciIoDevice->Pci.Hdr.DeviceId == 0x0622){
      HasEfiImage = 0;
    }
  }
