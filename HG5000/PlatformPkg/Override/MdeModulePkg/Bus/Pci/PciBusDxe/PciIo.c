...
{
  PCI_BAR       *Bar;
  Bar = (BarIndex == PCI_MAX_BAR ? &PciIoDevice->RomBar : &PciIoDevice->PciBar[BarIndex]);
  ...
  if (BarIndex >= PCI_MAX_BAR) {
    DEBUG((EFI_D_ERROR, "%a L%d %d\n", __FUNCTION__, __LINE__, BarIndex));
    return EFI_INVALID_PARAMETER;
  }

  if (!CheckBarType (PciIoDevice, BarIndex, Type)) {
    DEBUG((EFI_D_ERROR, "%a L%d %d\n", __FUNCTION__, __LINE__, PciIoDevice->PciBar[BarIndex].BarType));
    return EFI_INVALID_PARAMETER;
  }
  ...
  if ((*Offset + Count * (UINTN)(1 << Width)) - 1 >= Bar->Length) {
    DEBUG((EFI_D_ERROR, "%a L%d %d\n", __FUNCTION__, __LINE__, PciIoDevice->PciBar[BarIndex].Length));
    return EFI_INVALID_PARAMETER;
  }

  *Offset = *Offset + Bar->BaseAddress;
  ...
  )
{
  ...
  case PciBarTypeMem:
    ...
    if (Bar->BarType != PciBarTypeMem32  &&
        Bar->BarType != PciBarTypePMem32 &&
        Bar->BarType != PciBarTypePMem64 &&
        Bar->BarType != PciBarTypeMem64    ) {
      DEBUG((EFI_D_ERROR, "%a L%d BarIndex:%d, BarType:%d %d\n", __FUNCTION__, __LINE__, BarIndex, BarType, Bar->BarType));
      return FALSE;
    }
  ...
  case PciBarTypeIo:
    if (Bar->BarType != PciBarTypeIo32 &&
        Bar->BarType != PciBarTypeIo16){
      DEBUG((EFI_D_ERROR, "%a L%d BarIndex:%d, BarType:%d %d\n", __FUNCTION__, __LINE__, BarIndex, BarType, Bar->BarType));
      return FALSE;
    }
  ...
  )
{
  ...
  PciIoDevice = PCI_IO_DEVICE_FROM_PCI_IO_THIS (This);
  Bar = (BarIndex == PCI_MAX_BAR ? &PciIoDevice->RomBar : &PciIoDevice->PciBar[BarIndex]);
  DEBUG((EFI_D_ERROR,"%a ,BarIndex %d\n",__FUNCTION__,BarIndex));
  ...
  if (Resources != NULL) {
    ...
    *Resources   = Descriptor;
    ...
    Descriptor->AddrRangeMin = Bar->BaseAddress;
    Descriptor->AddrLen      = Bar->Length;
    Descriptor->AddrRangeMax = Bar->Alignment;
  ...
  )
{
  ...
  PciIoDevice = PCI_IO_DEVICE_FROM_PCI_IO_THIS (This);
  Bar = (BarIndex == PCI_MAX_BAR ? &PciIoDevice->PciBar[BarIndex] : &PciIoDevice->RomBar);
  ...
  if (Bar->BarType == PciBarTypeUnknown) {
    return EFI_UNSUPPORTED;
  }
  //
