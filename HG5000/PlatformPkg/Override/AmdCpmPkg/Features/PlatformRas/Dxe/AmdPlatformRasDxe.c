...
//BYO Modify for RAS >>
#include <Library/RasElogEventLib.h>
...
BOOLEAN gAerEn;
BOOLEAN gDpcEn;
UINT32                        Dhyana_PcieUncorrErrStatus; // Only for the Dhyana platfrom recording PCIE_UNCORR_ERR_STATUS values
/*----------------------------------------------------------------------------------------
...
//BYO Modify for RAS >>
EFI_STATUS
MemTestErrorDetection ( VOID );

EFI_STATUS
MemTrainingErrorDetection ( VOID );
//BYO Modify for RAS <<
...
//  ApeiErstInstall ();
  ...
  if(PcdGetBool(PcdAmdCcxCfgPFEHEnable)){
    ...
    NbioErrorDetection ();
  //BYO Modify for RAS >>
    MemTestErrorDetection ();
    MemTrainingErrorDetection ();
  //BYO Modify for RAS <<
  //
 ...
 )
{
  ...
  //
  //  Allocate memory and Initialize for NBIO Error Data block and NBIO Error Data block pointer
  //
...
PcieAerHestUpdate ( VOID )
{
  ...
  AerReportPcd=TRUE; //Always enable PcieRpAerHestTable->Flags Bit0, if disable bit wangxun card will reporter error
...
)
{
  ...
  //BYO Modify for RAS >>
  DEBUG((EFI_D_INFO,"/n###############LogProcessorErrorDXE: XCL Send Processerror\n"));
...
)
{
	...
	Status = gBS->LocateProtocol (&gAmdMemSmbiosServicesProtocolGuid, NULL, (VOID **)&AmdMemSmbiosServices);
  	if (EFI_ERROR (Status)) {
    	DEBUG ((EFI_D_INFO, "%a  AmdMemSmbiosServices not found.\n",__FUNCTION__));
    	return EFI_PROTOCOL_ERROR;
  	}
  	Status = AmdMemSmbiosServices->GetMemDmiInfo (AmdMemSmbiosServices, &MemDmiInfo);
  	if (EFI_ERROR (Status)) {
    	DEBUG((EFI_D_INFO, "%a  Error invoking AmdMemSmbiosServices.\n",__FUNCTION__));
    return EFI_PROTOCOL_ERROR;
  	}
    ...
    if (Status == EFI_NOT_FOUND) {
      AsciiStrCpy (GenericMemErrEntry->GenErrorDataEntry.FruText, "DIMM# Not Sourced");  // Error Fru Text String
    } else {
      AsciiStrCpy (GenericMemErrEntry->GenErrorDataEntry.FruText, "DIMM# Sourced"); // New Fru Text String
    }
	...
	DEBUG ((EFI_D_INFO, "ErrorType %x\n",(UINT8)RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaStatusMsr.Field.ErrorCodeExt));
    MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].ErrorType = (UINT8)RasMcaErrorInfo->McaBankErrorInfo[BankIndex].McaStatusMsr.Field.ErrorCodeExt;	
    MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].ErrorGranularity = 0x04;
    MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].ErrorOperation = UmcExtErrorCode.Field.DramEccErr;
	MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].VendorSyndrome = MCA_SYND_TYPE_DRAM_ERR;
	MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].MemoryArrayErrorAddress = (UINT32)SystemMemoryAddress;
	MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].DeviceErrorAddress = (UINT32)SystemMemoryAddress;
	MemDmiInfo.T18[NormalizedAddress.normalizedSocketId][NormalizedAddress.normalizedChannelId][DimmInfo.DimmId].ErrorResolution = 0x40;
...
)
{
  ...
  if (UmcExtErrorCode.Field.AddrCmdParityErr || UmcExtErrorCode.Field.WriteDataCrcErr) {
    ...
    AsciiStrCpy (MemParityErrEntry->GenErrorDataEntry.FruText, "DIMM# Sourced"); // New Fru Text String
  } else {
    AsciiStrCpy (MemParityErrEntry->GenErrorDataEntry.FruText, "DIMM# Not Sourced");  // Error Fru Text String
  }
  ...
  //BYO Modify for RAS >>
  DEBUG((EFI_D_INFO,"/n###############LogMemParityErrorDXE: XCL Send MemParityerror\n"));
  SendElogEventMca((UINT8*)RasMcaErrorInfo, BankIndex, NULL);
  //BYO Modify for RAS <<
  ...
  )
{
  ...
  //BYO Modify for RAS >>
  SendElogEventNbio((UINT8*)RasNbioErrorInfo);
  //BYO Modify for RAS <<
...
NbioErrorDetection ( VOID )
{
  ...
  //Get PCIE_UNCORR_ERR_STATUS address
  PcieUncorrErrStatusAddr = (UINT32 *) (UINTN)PcdGet64(PcdPcieUceStsMapBaseAddr);
  ...
  for (Die = 0; Die < MAX_DIE_SUPPORT; Die++) {
    ...
    //	if(x<8)
    //	{
    //    	PcieUncorrErrStatusAddr=0x11100154+(x*0x1000);
    //	} else {
    //    	PcieUncorrErrStatusAddr=0x11200154+((x-8)*0x1000);
    //	}
    	PcieUncorrErrStatus = *PcieUncorrErrStatusAddr;
    //	RasSmnRead ((UINT32)DieBusNum, PcieUncorrErrStatusAddr, &PcieUncorrErrStatus);
    	if(PcieUncorrErrStatus!=0)
    	{    		
    		NbioGlobalStatusHi=(NbioGlobalStatusHi | (1<<x));
    		break;
    	}
		PcieUncorrErrStatusAddr++;
    }
    Dhyana_PcieUncorrErrStatus = PcieUncorrErrStatus;
  ...
  )
{
  ...
  if (!IsRootPortError && RootErrStatusSet) {
    ...
    Status = AmdRasApeiProtocol->AddBootErrorRecordEntry ((UINT8*)GenPcieAerErrEntry,          // IN UINT8* pErrRecord
      ...
      //BYO Modify for RAS >>
        DEBUG((EFI_D_INFO,"/n###############PcieErrorLog: XCL Send Pcieerror\n"));
        SendElogEventPcie((UINT8*)GenPcieAerErrEntry, PcieUncorrStatus, 0);
        //BYO Modify for RAS <<   
...
MemTestErrorDetection ( VOID )
{
  ...
  DEBUG ((EFI_D_INFO, "MemTestErrorDetection, Enter\n"));

  MbistHob = GetFirstGuidHob (&gAmdMemoryMbistHobGuid);
  if (MbistHob == NULL) {
    DEBUG ((EFI_D_ERROR, "MBIST HOB Not located, Exiting\n"));
    return Status;
  }
  MbistHobData = GET_GUID_HOB_DATA (MbistHob);
  DEBUG ((EFI_D_INFO, "Data presented in MBIST table:\n"));
  DEBUG ((EFI_D_INFO, "Number of sockets = %d\n", MbistHobData->NumSockets));
  DEBUG ((EFI_D_INFO, "Number of Dies  = %d\n", MbistHobData->NumDiePerSocket * MbistHobData->NumSockets));
  DEBUG ((EFI_D_INFO, "Number of SubTests = %d\n", MbistHobData->NumSubTests));
  DEBUG ((EFI_D_INFO, "Number of channels per socket = %d\n", MbistHobData->NumDiePerSocket * MbistHobData->NumChannelPerDie));
  DEBUG ((EFI_D_INFO, "\n"));

  DEBUG ((EFI_D_INFO, "MBIST Test Results:\n"));
  DEBUG ((EFI_D_INFO, "Socket #,  Die #,  SubTest #,   Channel #,  ChipSel#,  Valid,  Status\n"));
      ...
      for (SubTest = 0; SubTest < MbistHobData->NumSubTests; SubTest++) { //Subtest Loop
        for (Channel = 0; Channel < MbistHobData->NumChannelPerDie; Channel++) {
          ...
          for (ChipSelect = 0; ChipSelect < 4; ChipSelect++) {
            ...
            DEBUG ((EFI_D_INFO, "%d          ", Socket));
            DEBUG ((EFI_D_INFO, "%d          ", Die));
            DEBUG ((EFI_D_INFO, "%d          ", SubTest));
            DEBUG ((EFI_D_INFO, "%d          ", Channel));
            DEBUG ((EFI_D_INFO, "%d          ", ChipSelect));
            // Valid
            DEBUG ((EFI_D_INFO, "%d       ", IsValid));
            // MBIST Results
            if (IsValid == 1) {
              if(((MbistHobData->MbistTestStatus[Socket][Die][SubTest][Channel].ErrorStatus >> ChipSelect) & 1) ^ 1) {
                DEBUG ((EFI_D_INFO, "Pass\n"));
              } else {
                DEBUG ((EFI_D_INFO, "Fail\n"));
                Status = SendElogEventMemTest((UINT8)Socket, (UINT8)Channel, (UINT8)ChipSelect);
              }
            } else {
              DEBUG ((EFI_D_INFO, "NA\n"));
            }
...
IsMemTrained (
  IN      UINT8                     CurSocket,
  IN      UINT8                     Channel,
  IN      UINT32                    DieNumberOfPerSocket
  )

  {
    ...
    while (ApcbToBoardChanXLatTab[Index].TranslatedChannelId != 0xFF) {
        if (Channel == ApcbToBoardChanXLatTab[Index].TranslatedChannelId) {
			...
			DEBUG((EFI_D_ERROR, "IsMemTrained: %x \n",ValidChannel));
            break;
        }
        Index++;
        ValidChannel = FALSE;
...
MemTrainingErrorDetection ( VOID )
{
  ...
  DEBUG((EFI_D_ERROR, "MemTrainingErrorDetection: == ENTER ==\n"));
  ...
  DEBUG((EFI_D_ERROR, "NumberOfInstalledProcessors: %x\n",NumberOfInstalledProcessors));

  for (Socket = 0; Socket < NumberOfInstalledProcessors; Socket++) {
    for (Channel = 0; Channel < 4; Channel++) {
		
		DEBUG((EFI_D_ERROR, "Channel: %x\n",Channel));
      if(!IsMemTrained(Socket, Channel, DieNumberOfPerSocket)) {
        Status = SendElogEventMemTraining(Socket, Channel, 0xFF);
      }
    }
  }
  DEBUG((EFI_D_ERROR, "NumberOfInstalledProcessors: %r\n",Status));
