...
typedef struct {
  ...
  OUT CHAR8                     DeviceLocator[12];       ///< The string number of the string that identifies the physically labeled socket or board position where the memory device is located.
  ...
  OUT CHAR8                     SerialNumber[13];        ///< Serial Number.
  OUT CHAR8                     PartNumber[25];         ///< Part Number.
  OUT CHAR8                     AssetTag[8];           ///< Asset Tag.
  OUT UINT8                     Attributes;             ///< Bits 7-4: Reserved, Bits 3-0: rank.
  OUT UINT8                     DevWidth;               /// wz200505 +
...
/// DMI Type 18 - 32-bit Memory Error Information

typedef struct {
  OUT	UINT8                     ErrorType;                  ///< The enumeration value from AMD_MEMORY_ERROR_TYPE.
  OUT	UINT8                     ErrorGranularity;           ///< The enumeration value from AMD_MEMORY_ERROR_GRANULARITY.
  OUT	UINT8                     ErrorOperation;             ///< The enumeration value from AMD_MEMORY_ERROR_OPERATION.
  OUT	UINT32                    VendorSyndrome;
  OUT	UINT32                    MemoryArrayErrorAddress;
  OUT	UINT32                    DeviceErrorAddress;
  OUT	UINT32                    ErrorResolution;
} TYPE18_DMI_INFO;
...
typedef struct {
  ...
  OUT TYPE18_DMI_INFO           T18[MAX_SOCKETS_SUPPORTED][MAX_CHANNELS_PER_SOCKET][MAX_DIMMS_PER_CHANNEL]; ///< Type 18 struc
