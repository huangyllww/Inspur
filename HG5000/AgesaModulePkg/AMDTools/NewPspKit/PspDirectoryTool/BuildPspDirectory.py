...
def ParseArg ():
  ...
  DPParser.add_argument ("-p", "--program", help="Specify the program name, Valid choices: DN, DP", required=True)
  ...
  if (args.subcommand == 'dp'):
    if args.program.upper() not in ["ZP", "RV", "DN", "DP"]:
      log.info ("[Error] Not a valid program string, only DN & DP are supported")
      FatalErrorExit ()
