...
#include "Filecode.h"
#include <Library/ReportStatusCodeLib.h>         // winddy +
    ...
    while (CurrentFchSmmSxNodePtr->FchSxNodePtr != NULL) {
      if (CurrentFchSmmSxNodePtr->Context.Type == SleepTypeValue &&
        ...
        if (CurrentFchSmmSxNodePtr->CallBack2Function != NULL) {
          ...
          DEBUG((DEBUG_INFO, "S%d dispatched(%X) %r\n", SleepTypeValue, \
            CurrentFchSmmSxNodePtr->CallBack2Function, Status));   // winddy -
        }
