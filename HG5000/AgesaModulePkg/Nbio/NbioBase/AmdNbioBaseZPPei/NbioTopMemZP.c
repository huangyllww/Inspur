  ...
  )
{
  ...
  //        } else if (Range->Base > 0xFFFFFFFF) {
    ...
    if ((MsrData & BIT21) != 0) {
      ...
      IDS_HDT_CONSOLE (MAIN_FLOW, "MSR TOP_MEM2[63:32] is 0x%08x\n", RShiftU64 (MsrData, 32));
      IDS_HDT_CONSOLE (MAIN_FLOW, "MSR TOP_MEM2[31:0] is 0x%08x\n", MsrData);
      if ((MsrData & (UINT64)0x0000FFFFFF800000) > ((UINT64)0x0000010000000000)) {
        ...
        // TOM3 is INCLUSIVE, so set it to TOM - 1 using bits 47:23
    ...
    IDS_HDT_CONSOLE (MAIN_FLOW, "GnbSetTomZP setting GnbTom2 to 0x%lx\n", GnbTom2);
    IDS_HDT_CONSOLE (MAIN_FLOW, "GnbSetTomZP setting GnbTom3 to 0x%lx\n", GnbTom3);
    if (GnbTom2 != 0) {
