  ...
  if (GetPatchEquivalentId (&ProcessorEquivalentId, MicrocodeEquivalenceTable, EquivalencyEntries, StdHeader)) {
    // parse the patch table to see if we have one for the current cpu
      ...
      if (ValidateMicrocode ((MICROCODE_PATCH *) (UINTN) (*UcodePatchAddr), ProcessorEquivalentId, StdHeader)) {
        IDS_HDT_CONSOLE (CPU_TRACE, "  Validate Microcode Success \n");  
        ...
        // MSRC001_100C should only be non-zero for ZP revs < B1. If this changes to include B1, the workaround for
        // PLAT-16684 and the corresponding CBS option need to be implemented in CcxZenZpS3RestoreSmi as well.
        if ((LogicalId.Family == AMD_FAMILY_17_ZP) && ((LogicalId.Revision & AMD_REV_F17_ZP_LT_B1) != 0)) {
