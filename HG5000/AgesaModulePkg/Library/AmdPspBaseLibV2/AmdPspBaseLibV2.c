  ...
  )
{
  ...
  //Waiting till PSP fill the final result  
  while (PspMbox->Cmd.Field.Recovery == 0) {
    ;
  }
  
  return ((PspMbox->Cmd.Field.Recovery == 0x2) ? TRUE : FALSE);
}
  ...
  )
{
  ...
  UINT32 SocketNum;
  UINT32 DieNumPerSocket;
  UINT16 SocketId;
  UINT16 DieId;
  UINT32 HostBridgeBusBase;
  
  if ((CcxIsBsp (NULL) == FALSE)) {
    return;
  }
  
  SocketNum = (UINT32)FabricTopologyGetNumberOfProcessorsPresent();
  ...
  if (GetPspMmioBase (&Value32) == FALSE) {
    ...
    for (SocketId = 0; SocketId < SocketNum; SocketId++) {
        DieNumPerSocket = (UINT32)FabricTopologyGetNumberOfDiesOnSocket(SocketId);
        for (DieId = 0; DieId < DieNumPerSocket; DieId++) {
            MmioTarget.TgtType = TARGET_DIE;
            MmioTarget.SocketNum = SocketId;
            MmioTarget.DieNum = DieId;
            PspMmioBase  = 0;
            
            Status = FabricAllocateMmio (&PspMmioBase, &Length, ALIGN_1M, MmioTarget, &Attributes);
            ASSERT (Status == EFI_SUCCESS );
            if (EFI_ERROR (Status)) {
              IDS_HDT_CONSOLE_PSP_TRACE ("PspBarInitEarlyV2 Allocate MMIO Fail For Socket %x Die %x\n", SocketId, DieId);
            } else {
              IDS_HDT_CONSOLE_PSP_TRACE ("PspBarInitEarlyV2 Allocate MMIO For Socket %x Die %x Address @0x%lx\n", SocketId, DieId, PspMmioBase);
            }

            HostBridgeBusBase = (UINT32)FabricTopologyGetHostBridgeBusBase (SocketId, DieId, 0);
                      
            //Read PSP BASE Address in NBMISC
            Value32 = (UINT32) RShiftU64 (PspMmioBase, 32);
            SmnRegisterWrite(HostBridgeBusBase, NBMSIC_SMN_BASE + NBMSIC_PSP_BASE_ADDR_HI_OFFSET, &Value32, 0);
            Value32 = (UINT32) (PspMmioBase | (BIT0 + BIT8));
            SmnRegisterWrite(HostBridgeBusBase, NBMSIC_SMN_BASE + NBMSIC_PSP_BASE_ADDR_LO_OFFSET, &Value32, 0);
        }
    }
  }
}
