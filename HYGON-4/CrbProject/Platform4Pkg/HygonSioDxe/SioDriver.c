  ...
  )
{
  ...
  if (RemainingDevicePath != NULL) {
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      AcpiNode = (ACPI_HID_DEVICE_PATH *) RemainingDevicePath; ///[SIO]-DP:Acpi(PNP0303,0x0)

      
      for (Index = 0; Index < Count; Index++) {        
//      DEBUG((EFI_D_INFO, "[SIO] AcpiNode->HID=0x%x UID=0x%x\n",AcpiNode->HID,AcpiNode->UID));        
//      DEBUG((EFI_D_INFO, "[SIO] HID[%d]=0x%x UID[%d]=0x%x\n",Index,Devices[Index].HID,Index,Devices[Index].UID));
  ...
  ///DEBUG((EFI_D_INFO, "[MTN-DBG] Support In \n"));

  //
  // See if the parent device path can be opened BY_DRIVER
  //
  Status = gBS->OpenProtocol (
                  Controller,                                             ///D17F0 Busc ISA Bridge Handler
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if ((Pci.Hdr.Command & (EFI_PCI_COMMAND_IO_SPACE | EFI_PCI_COMMAND_MEMORY_SPACE))
      ...
      if (Pci.Hdr.ClassCode[2] == PCI_CLASS_BRIDGE) {   ///D17F0 Busc ISA Bridge Handler
        //
 ...
 //-    DEBUG((EFI_D_INFO, "[SIO] ClassCode[2][1][0]=0x%02x%02x%02x Controller=0x%x\n",Pci.Hdr.ClassCode[2],Pci.Hdr.ClassCode[1],Pci.Hdr.ClassCode[0],Controller));///0x060100 0x060400 0x060401...... Controller Is D17F0 Busc ISA Bridge
...
//-         DEBUG((EFI_D_INFO, "[SIO] BDF=0x%02x%02x%02x\n",SioDevice.Bus,SioDevice.Device,SioDevice.Funtion ));            
            if(!SioDeviceEnabled (&SioDevice)) {  ///D17F0 Busc ISA Bridge Handler
              Status = EFI_UNSUPPORTED;
            }
          }
        }
...
//DEBUG((EFI_D_INFO, "[SIO] Sio Driver Start In Controller=0x%x\n",Controller));
