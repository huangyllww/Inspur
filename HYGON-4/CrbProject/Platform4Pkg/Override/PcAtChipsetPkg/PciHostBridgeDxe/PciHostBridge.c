...
UINTN RootBridgeNumber[1] = { PCI_HOST_MAX_COUNT };
UINT64 RootBridgeAttribute[1][PCI_HOST_MAX_COUNT] = {
  {
...
EFI_PCI_ROOT_BRIDGE_DEVICE_PATH mEfiPciRootBridgeDevicePath[1][PCI_HOST_MAX_COUNT] = {
  {
    {
      ACPI_DEVICE_PATH,
      ACPI_DP,
...
PCI_ROOT_BRIDGE_RESOURCE_APPETURE  mResAppeture[1][PCI_HOST_MAX_COUNT] = {
  {
    ...
    {0x80, 0x8f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0x90, 0x9f, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xA0, 0xAf, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xB0, 0xBf, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xC0, 0xCf, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xD0, 0xDf, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xE0, 0xEf, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},
    {0xF0, 0xFF, 0x00000000, 0xFFFFFFFFFFFFFFFFULL, 0, 0xffff},    
  }
};
  ...
  )
{
  ...
  case EfiPciHostBridgeAllocateResources:
    DEBUG((EFI_D_INFO, "EfiPciHostBridgeAllocateResources\n"));
          ...
          if (RootBridgeInstance->ResAllocNode[Index].Status != ResNone) {
            ...
            switch (Index) {
              ...
              case TypeIo:
                ...
                if (!EFI_ERROR (Status)) {
                  RootBridgeInstance->ResAllocNode[Index].Base   = BaseAddress;
                  RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;

                  Info->IoBase[Instance] = (UINT16)BaseAddress;
                  Info->IoSize[Instance] = (UINT16)AddrLen;
              ...
              case TypeMem32:
                ...
                DEBUG((EFI_D_INFO, "[%d] PciM32 Abits:%d L:0x%X\n", Instance, BitsOfAlignment, AddrLen));

                if(Instance == 0){
                  Mmio32Limit = (UINT32)PcdGet64(PcdPciExpressBaseAddress);
                }

                BaseAddress = Mmio32Limit;
                ...
                Info->NbioMmuBase[Instance] = (UINT32)BaseAddress;
                DEBUG((EFI_D_INFO, "NbioMmuBase[%d]:%x\n", Instance, Info->NbioMmuBase[Instance]));
                ...
                DEBUG((EFI_D_INFO, "NbApicBase[%d]:%x\n", Instance, Info->NbApicBase[Instance]));
                ...
                DEBUG((EFI_D_INFO, "malloc MMIO32(%r):%lX\n", Status, BaseAddress));
                if (!EFI_ERROR (Status)) {
                  RootBridgeInstance->ResAllocNode[Index].Base   = BaseAddress;
                  RootBridgeInstance->ResAllocNode[Index].Status = ResAllocated;
                  Info->MmioBase[Instance] = (UINT32)BaseAddress;
                  Info->MmioSize[Instance] = (UINT32)(EndAddress - BaseAddress);
                  Mmio32Limit = (UINT32)BaseAddress;
                } else {
                  ReturnStatus = Status;
                  RootBridgeInstance->ResAllocNode[Index].Length = 0;
                }
              ...
              case TypeMem64:
                ...
                DEBUG((EFI_D_INFO, "[%d] Pci64 Abits:%d L:0x%lX\n", Instance, BitsOfAlignment, AddrLen));
                if(AddrLen == 0){
                  break;
                }

// HYGON_GX_CPU 
//   MMIO64  256M, 256M align   vIOMMU
//             4M, 4M align     vIOMMU Ctrl

                BaseAddress = Info->Mmio64Base[Instance] + Info->Mmio64Size[Instance];
                EndAddress  = BaseAddress;

                if(Info->CpuModel == CPU_MODEL_748X){
                  Status = gDS->AllocateMemorySpace (
                                  EfiGcdAllocateMaxAddressSearchTopDown,
                                  EfiGcdMemoryTypeMemoryMappedIo,
                                  28,
                  ...
                  DEBUG((EFI_D_INFO, "malloc vIOMMU(%r):%lX\n", Status, BaseAddress));
                  Info->vIOMMUBase[Instance] = BaseAddress;
                  ...
                  DEBUG((EFI_D_INFO, "malloc vIOMMUCtrl(%r):%lX\n", Status, BaseAddress));
                  Info->vIOMMUCtrlBase[Instance] = BaseAddress;   
                }
                ...
                DEBUG((EFI_D_INFO, "malloc MMIO64(%r):%lX\n", Status, BaseAddress));
                if (!EFI_ERROR (Status)) {
  ...
  case EfiPciHostBridgeSetResources:
    DEBUG((EFI_D_INFO, "EfiPciHostBridgeSetResources\n"));
    Info = (PLATFORM_COMM_INFO*)GetPlatformCommInfo();
    LibPciHostResourceSet(Info);
    LibSetHygonMiscBase(Info);
    break;
