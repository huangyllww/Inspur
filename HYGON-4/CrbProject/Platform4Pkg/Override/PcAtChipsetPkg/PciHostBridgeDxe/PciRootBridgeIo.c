  ...
  )
{
  ...
  if (OperationType == IoOperation) {
    ...
    if (PcdGetBool(PcdPciRootBridgeIoBusRangeCheck) && (PciRbAddr->Bus < PrivateData->BusBase || PciRbAddr->Bus > PrivateData->BusLimit)) {
      return EFI_INVALID_PARAMETER;
    }
...
//- Limit = MAX_PCI_REG_ADDRESS;
  ...
  if (Count == 0) {
    if (Address > Limit) {
      DEBUG((EFI_D_ERROR, "%a L%d %lX %lX\n", __FUNCTION__, __LINE__, Address, Limit));
      return EFI_UNSUPPORTED;
    }
  } else {
    MaxCount = RShiftU64 (Limit, Width);
    if (MaxCount < (Count - 1)) {
      DEBUG((EFI_D_ERROR, "%a L%d %lX %lX\n", __FUNCTION__, __LINE__, Limit, MaxCount));
      return EFI_UNSUPPORTED;
    }
    if (Address > LShiftU64 (MaxCount - Count + 1, Width)) {
      DEBUG((EFI_D_ERROR, "%a L%d %lX %lX %X %X\n", __FUNCTION__, __LINE__, Address, MaxCount, Count, Width));
      return EFI_UNSUPPORTED;
    }
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_ERROR, "%a L%d %X %X %X %X\n", __FUNCTION__, __LINE__, Width, Address, Count, Buffer));
    return Status;
  }
