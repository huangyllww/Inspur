...
/**
  ...
  return a few other attribute of the FV.
...
)
{
    ...
    Status = FvbGetLbaAddress (Instance, Lba, &LbaAddress, &LbaLength, NULL);
    if (EFI_ERROR(Status)) {
        DEBUG((EFI_D_ERROR, "FvbGetLbaAddress:%r\n", Status));
        return Status;
    }
    ...
    if ( (Attributes & EFI_FVB2_WRITE_STATUS) == 0)  {
        DEBUG((EFI_D_ERROR, "Vol no write attrib\n"));
        return (EFI_ACCESS_DENIED);
    }
    ...
    //
    if (BlockOffset > LbaLength) {
        DEBUG((EFI_D_ERROR, "BlockOffset out of range\n"));
        return (EFI_INVALID_PARAMETER);
    }
...
)
{
    if (FvBase == PcdGet32(PcdFlashNvStorageVariableBase)) {
        if (CompareMem (&FwVolHeader->FileSystemGuid, &gEfiSystemNvDataFvGuid, sizeof(EFI_GUID)) != 0 ) {
            DEBUG ((EFI_D_ERROR, "FileSystemGuid:%g\n", &FwVolHeader->FileSystemGuid));
            return FALSE;
        }
    } else {
...
)
{
    ...
    Status = gSmst->SmmLocateProtocol (&gEfiSmmNvMediaAccessProtocolGuid, NULL, &gMediaAcc);
    ASSERT_EFI_ERROR (Status);
    ...
    if (GuidHob != NULL) {
      ...
      DEBUG((EFI_D_INFO, "Restore NVRAM(%lX <- %lX, %lX)\n", FtwLwd->TargetAddress, FtwLwd->SpareAddress, FtwLwd->Length));
      while (BlockCount > 0) {
...
//-   DumpMem8((VOID*)(UINTN)FtwLwd->TargetAddress, 256);
      ...
      CopyMem(p, (VOID*)(UINTN)FtwLwd->SpareAddress, (UINTN)FtwLwd->Length);
...
//-   DumpMem8((VOID*)(UINTN)FtwLwd->TargetAddress, 256);
    ...
    for (Idx = 0; Idx < sizeof (mPlatformFvBaseAddress)/sizeof (mPlatformFvBaseAddress[0]); Idx++) {
        ...
        if (!IsFvHeaderValid (BaseAddress, FwVolHeader)) {
            ...
            //
            DEBUG ((EFI_D_ERROR, "FvbService.c: Writing back healthy FV header\n"));
