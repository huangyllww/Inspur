...
//     |  |    |    |    +-------------- Alternate Shift
//     |  |    |    |    |
STATIC UINT8 ScanCodeConvertTBLForUefi[0x5B][5]={
    ...
    0,0x45,0xC5,0xFF,0xFF,      // NumLk 
    0,0x46,0xFF,0xFF,0xFF,      // ScrLk 
    3,0x47, '7',0x77,0x07,      //  7 Home
    3,0x48, '8',0x8D,0x08,      //  8  ^
    3,0x49, '9',0x84,0x09,      //  9 PgUp
    4,'-', '-',0x8E,0xF0,      //  -
    3,0x4B, '4',0x73,0x04,      //  4  <
    3,0x4C, '5',0x8F,0x05,      //  5
    3,0x4D, '6',0x74,0x06,      //  6  >
    4,'+', '+',0x90,0xF0,      //  +
    3,0x4F, '1',0x75,0x01,      //  1 End
    3,0x50, '2',0x91,0x02,      //  2  v
    3,0x51, '3',0x76,0x03,      //  3 PgDn
    3,0x52, '0',0x92,0x00,      //  0 Ins
    3,0x53, '.',0x93,0xFF,      //  Del
...
/**
  Find Key Descriptor in Key Conversion Table given its USB keycode.

  @param  UsbKeyboardDevice   The USB_KB_DEV instance.
  @param  KeyCode             USB Keycode.

  @return The Key Descriptor in Key Conversion Table.
  ...
  )
{
  ...
  //
  // Calculate the index of Key Descriptor in Key Conversion Table
  //
  if (KeyCode <= 0x65) {
  ...
  )
{
  ...
  //
  // Re-allocate resource for KeyConversionTable
  //
  ...
  for (Index = 0; Index < KeyboardLayout->DescriptorCount; Index++) {
    ...
    //
    // Fill the key into KeyConversionTable, whose index is calculated from USB keycode.
    //
...
/**
  ...
  This function initializes Key Conversion Table for the USB keyboard device.
  ...
  )
{
  EFI_STATUS                    Status;
  EFI_USB_IO_PROTOCOL           *UsbIo;
  static USB_KB_DEV             *UsbKeyboardDevice = NULL;
  BOOLEAN                       IsUsbKeyboardInit = TRUE;
//EFI_LEGACY_BIOS_PROTOCOL      *LegacyBios;
  ...
  if (UsbKeyboardDevice == NULL) {
    ...
    gLastKbFlag2  = *((UINT8*)(UINTN)0x418);
    DEBUG((EFI_D_INFO, "LastLedState:%X, LastKbFlag2:%X\n", UsbKeyboardDevice->LastLedState, gLastKbFlag2));
  }
...
//
ErrorExit:
  if (UsbKeyboardDevice != NULL && IsUsbKeyboardInit == FALSE) {
...
--*/
{
  ...
  HasKey = GetKeyFromBda(&ScanCode, &KeyChar);
...
//DEBUG((EFI_D_INFO, "Scan:%X, Key:%X, F1:%X, F2:%X\n", ScanCode, KeyChar, KbFlag1, KbFlag2));
  ...
  if(gLastKbFlag2 != KbFlag2){
    if((gLastKbFlag2 & BIT3) != (KbFlag2 & BIT3)){      // pause status changed.
      if(!HasKey){
        ScanCode = 0x8000;
        KeyChar  = CHAR_NULL;
        HasKey   = TRUE;
  ...
  if((KbFlag1 & (KB_ALT_PRESSED | KB_CTRL_PRESSED)) == (KB_ALT_PRESSED | KB_CTRL_PRESSED) && ScanCode == 0x53){
    if(*mDisableKBCtrlAltDelReset == 0){
...
--*/
{
  ...
  } else if (KeyChar == 0x00 || KeyChar == 0xe0 || KeyChar == 0xe1) {
    //
  ...
  )
{
  ...
  // event will stop work during the compatibility16. And If a caller recursively invoke this function,
  // e.g. OS loader, other drivers which are driven by timer event will have a bad performance during this period,
  // e.g. usb keyboard driver.
  // Add a stall period can greatly increate other driver performance during the WaitForKey is recursively invoked.
...
//gBS->Stall (1000);
  ...
  if ((UsbKeyboardDevice->KeyState.KeyShiftState & EFI_RIGHT_ALT_PRESSED)||(UsbKeyboardDevice->KeyState.KeyShiftState & EFI_LEFT_ALT_PRESSED)
      ||(UsbKeyboardDevice->KeyState.KeyShiftState & EFI_RIGHT_CONTROL_PRESSED)||(UsbKeyboardDevice->KeyState.KeyShiftState & EFI_LEFT_CONTROL_PRESSED)){
      if(ScanCode < ARRAY_SIZE(ScanCodeConvertTBLForUefi)){
        KeyData->Key.UnicodeChar = ScanCodeConvertTBLForUefi[ScanCode][1];
      }
    }
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "UpdateStatusLights Caps:%d Numlock:%d Scroll:%d LastLedState:0x%x\n", UsbKeyboardDevice->CapsOn, \
    UsbKeyboardDevice->NumLockOn, UsbKeyboardDevice->ScrollOn, UsbKeyboardDevice->LastLedState));
  ...
  )
{
  ...
  State = (*KeyToggleState) & EFI_KEY_TOGGLE_STATE_ALL;
  if ((State & EFI_TOGGLE_STATE_VALID) != EFI_TOGGLE_STATE_VALID) {
    return EFI_UNSUPPORTED;
  }
  ...
  if ((State & EFI_SCROLL_LOCK_ACTIVE) == EFI_SCROLL_LOCK_ACTIVE) {
    UsbKeyboardDevice->ScrollOn = TRUE;
  }
  if ((State & EFI_NUM_LOCK_ACTIVE) == EFI_NUM_LOCK_ACTIVE) {
    UsbKeyboardDevice->NumLockOn = TRUE;
  }
  if ((State & EFI_CAPS_LOCK_ACTIVE) == EFI_CAPS_LOCK_ACTIVE) {
    UsbKeyboardDevice->CapsOn = TRUE;
  }
  if ((State & EFI_KEY_STATE_EXPOSED) == EFI_KEY_STATE_EXPOSED) {
    UsbKeyboardDevice->IsSupportPartialKey = TRUE;
  }

  UsbKeyboardDevice->KeyState.KeyToggleState = State;
