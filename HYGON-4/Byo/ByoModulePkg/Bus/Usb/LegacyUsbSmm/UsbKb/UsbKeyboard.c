...
extern UINT8           *mDisableKBCtrlAltDelReset;
extern UINT8           *gSystemInLegacyPtr;

UINT8  UsageTable[] = {
//   AT code    Usage   index#  Description
    0x00,      //       0       reserved
...
/*
 ...
 Estimate key input and return number of inputting key.
 _________________________________________________________________________________
*/
UINT16
...
UINT16 gLastKey = 0xFFFF;
UINT16 gAltNumberData = 0;
BOOLEAN gHasKey = FALSE;
...
IsSpecialScode (LPBDA_SEG lpBda, UINT16 *ScanCode)
{
  switch (*ScanCode) {
    ...
    case 0xB8:
      ...
      lpBda->ShiftStatus &= ~BIT3;
      if (gAltNumberData) {
        InsertKey (lpBda, gAltNumberData);
        gAltNumberData = 0;
      }       
      break;
...
IsNormalCode (LPBDA_SEG lpBda,UINT16 ScanCode)
{
  ...
  if (ScanCodeCheck[0] == 1) {
      ...
      // with Alt key press
      //
        ExtendScanCode = (UINT16)(ScanCodeCheck[4] * 0x100);
      } else if (lpBda->ShiftStatus & BIT2) {
        ExtendScanCode += (UINT16) (ScanCodeCheck[3]);
        ...
        if (ScanCodeCheck[2] >= 'A' && ScanCodeCheck[2] <= 'Z') {
            ...
            if (lpBda->ShiftStatus & BIT6) {
                ExtendScanCode += (UINT16) (ScanCodeCheck[1]);
            } else {
                ExtendScanCode += (UINT16) (ScanCodeCheck[2]);
            }
        } else {
                ExtendScanCode += (UINT16) (ScanCodeCheck[2]);
        }
    ...
    if (lpBda->ShiftStatus & BIT3) {
      ...
      // With Alt key press
      //
      ExtendScanCode +=  (UINT16) (ScanCodeCheck[4]);
    } else if (lpBda->ShiftStatus & BIT2) {
      //
    ...
    if (lpBda->ShiftStatus & BIT5) {
      ...
      //
      ExtendScanCode = ExtendScanCode * 0x100;
      if((lpBda->ShiftStatus & (BIT3 | BIT2)) == (BIT3 | BIT2)){
        if(ExtendScanCode == 0x5300){
          InsertKey (lpBda, 0xE053);
          return TRUE;
        }
      }
      if (lpBda->ShiftStatus & BIT3) {
        ...
        // With Alt key press
        //
        gAltNumberData = gAltNumberData *10 + ScanCodeCheck[4];
        return TRUE;
    ...
    } else {         // number lock not set
	    ExtendScanCode = 0xE000;
	    ExtendScanCode += ScanCode;
    }

  } else if (ScanCodeCheck[0] == 4 || ScanCodeCheck[0] == 5) {
    //
    ...
    if ((Scan & BIT7) == 0) {
      switch (Scan) {
        ...
        case 0x53:
          ...
          } else if (lpBda->ShiftStatus & BIT2) {        // Gray Del key press
            //
            // With Control key press
            //
            Extend += 0x93;
  ...
  )
{
  ...
  if (ScanCode == 0xE11D) {
    PauseFlag |= BIT0;
    return TRUE;    
  } else if((PauseFlag & BIT0) && ScanCode == 0x45){
    PauseFlag &= (UINT8)~BIT0;
    lpBda->ExtendedKbdStatus ^= BIT3;
    return TRUE;
  }
  ...
  if (ScanCode == 0xE19D) {
    PauseFlag |= BIT1;
    return TRUE;    
  } else if((PauseFlag & BIT1) && ScanCode == 0xC5){
    PauseFlag &= (UINT8)~BIT1;
    return TRUE;
  }  
...
TranslateKey (UINT8 ScanCode)
{
  ...
  if (CheckPauseScanCode (lpBda, ExtendScanCode)) {
    return;
  }
...
ProcExit:
  if(*gSystemInLegacyPtr){
    if((lpBda->ShiftStatus & (BIT2 | BIT3)) == (BIT2 | BIT3) && gLastKey == 0x5300){
      ResetCold();
    }
  }
  ...
  if(gHasKey && (lpBda->ExtendedKbdStatus & BIT3)){
    lpBda->ExtendedKbdStatus ^= BIT3;
  }
}
...
)
{
    ...
    if ((*BdaPtr & WhichLed) == WhichLed) {
      //
      //Do not konw why need to check numlock ,And send E0.FF scancode 
      //mark this and return FALSE
      //return  TRUE;
      //
      return FALSE;
...
)
{
  ...
  if (UsbKeyboardDevice->CmdQueueTail != UsbKeyboardDevice->CmdQueueHead) {
    if (!mLegacyFreeSupport) {
      ...
      //ANC-20200729: only need to check output buffer status for keyboard data,BFT : CHX002-202007101
      if ((KbdCmd & KBD_OBF) == 0) {
...
--*/
{
  ...
  //
    //dell 8115 keyboard in a byte 00, after init
    //if (DataLength == 0 || Data == NULL) {
    if (DataLength != 8 || Data == NULL) {
        return EFI_SUCCESS;
    }
