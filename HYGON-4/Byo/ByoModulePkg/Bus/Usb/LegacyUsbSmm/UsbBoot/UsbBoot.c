...
)
{
  ...
  for (DeviceType = USB_HDD; DeviceType < MaxDevType; DeviceType ++) {
    ...
    while (Link != &mUsbMassStorageList && Index < MAX_UMS_UNM) {
      ...
      *DescStr = 0;
      DescStr = mUmsInfo->UmsInfoEntry[Index].ProductDescAStr;
      AsciiDeleteExtraSpaces(DescStr);
      ...
      mUmsInfo->UmsInfoEntry[Index].Handle[0]    = (UINT32)(UINTN)UsbMass & 0xffffffff;
      mUmsInfo->UmsInfoEntry[Index].Handle[1]    = (UINT32)(RShiftU64((UINT64)(UINTN)UsbMass, 32) & 0xffffffff);
      ...
      Size = GetDevicePathSize (UsbMass->DevicePath);
      mUmsInfo->UmsInfoEntry[Index].UsbDp = (UINT64)UsbAllocatePool (Size);
      if (mUmsInfo->UmsInfoEntry[Index].UsbDp) {
        CopyMem ((VOID*)mUmsInfo->UmsInfoEntry[Index].UsbDp, UsbMass->DevicePath, Size);
      }
      Index++;
      Link = Link->ForwardLink;
    }
...
)
{
    ...
    VOID                      *Buffer = (VOID *)(UINTN)BufferAddress;
