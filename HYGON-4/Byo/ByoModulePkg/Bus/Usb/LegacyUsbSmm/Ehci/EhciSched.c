...
--*/
{
    ...
    // fix the hardware visible parts (EHCI R1.0 page 72).
...
--*/
{
    ...
    // visible part: Only need to loopback the ReclaimHead. The Qh
    // is pointing to ReclaimHead (which is still in the list).
    //
    Head                    = Ehc->ReclaimHead;
...
Returns:
  ...
  Whether the result of URB transfer is finalized.
...
--*/
{
    ...
    BASE_LIST_FOR_EACH_SAFE (Entry, Next, &Ehc->AsyncIntTransfers) {
        ...
        if (EFI_ERROR (Status)) {
            EHCI_DEBUG ((EFI_D_ERROR, "EhcMoniteAsyncRequests: Fail to Flush AsyncInt Mapped Memory\n"));
        }
...
)
{
    ...
    if (UsbSts & USBSTS_SYS_ERROR) {
        EHCI_DEBUG ((EFI_D_INFO, "EhciSmiCallback: host system error occurred \n"));
    }
