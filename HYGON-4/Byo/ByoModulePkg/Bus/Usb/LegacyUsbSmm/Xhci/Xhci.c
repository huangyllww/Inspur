  ...
  )
{
  ...
  USB_DEBUG((EFI_D_INFO, "%a(): Port %d initial state = 0x%x\n", __FUNCTION__, PortNumber, State));
  //
  ...
  )
{
  ...
  case EfiUsbPortReset:
    ...
    if(PcdGetBool(PcdXhciTrainingDelayEnable)){
        XHCI_DEBUG ((EFI_D_INFO,"PcdGetBool(PcdXhciTrainingDelayEnable)111\n"));
  ...
  case EfiUsbPortConfigErrorStateChange:
    //
    // Make sure Host Controller not halt before reset it
    //
    if (XhcIsHalt (Xhc)) {
      Status = XhcRunHC (Xhc, XHC_GENERIC_TIMEOUT);
      if (EFI_ERROR (Status)) {
        USB_DEBUG((EFI_D_ERROR, "%a(): Failed to start HC - %r\n", __FUNCTION__, Status));
        break;
      }
    }
    ...
    USB_DEBUG((EFI_D_ERROR, "%a(): XhcWaitOpRegBit - %r\n", __FUNCTION__, Status));
    break;
  ...
  )
{
  ...
  case EfiUsbPortWarmResetChange:
    //
    // Clear Warm Port Reset Change
    //
    State |= XHC_PORTSC_WRC;
    XhcWriteOpReg (Xhc, Offset, State);
    break;

  case EfiUsbPortLinkStateChange:
    //
    // Clear Port Link State Change
    //
    State |= XHC_PORTSC_PLC;
    XhcWriteOpReg (Xhc, Offset, State);
    break;
  ...
  )
{
  ...
  if (Status == EFI_TIMEOUT) {
    ...
    if (RecoveryStatus == EFI_ALREADY_STARTED) {
      ...
      USB_DEBUG((DEBUG_ERROR, "XhcTransfer[Type=%d]: XhcDequeueTrbFromEndpoint failed!\n", Type));
    }
  }
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if (SlotId == 0) {
    USB_DEBUG((EFI_D_ERROR, "%a(): Error because of SlotId == 0\n", __FUNCTION__));
    goto ON_EXIT;
  }
  ...
  if ((Request->Request     == USB_REQ_GET_DESCRIPTOR) &&
    ...
    // Request->Value indicates AlternateSetting to set
    ...
    PortStatus.PortStatus       = 0;
    PortStatus.PortChangeStatus = 0;
    USB_DEBUG((EFI_D_INFO, "%a(): Request - USB_REQ_GET_STATUS initial State = 0x%x\n", __FUNCTION__, State));
    ...
    Status = XhcPollPortStatusChange (Xhc, Xhc->UsbDevContext[SlotId].RouteString, (UINT8)Request->Index, &PortStatus);

    *(UINT32 *)Data = *(UINT32*)&PortStatus;
  }

ON_EXIT:
  if ((EFI_ERROR (Status)) || (*TransferResult != 0)) {
    USB_DEBUG((EFI_D_INFO, "%a(): Exit - %r, TransferResult = 0x%x\n", __FUNCTION__, Status, *TransferResult));
  }
  return Status;
}
...
)
{
    ...
    if ((UsbSts & USBSTS_PORT_CHANGE) &&
        ...
        USB_DEBUG((EFI_D_INFO, "%a() UsbSts - 0x%x\n", __FUNCTION__, UsbSts));
...
--*/
{
  ...
  XHCI_DEBUG ((DEBUG_INFO, "XhcCreateUsb3Hc: Usb2SupOffset 0x%x\n", Xhc->Usb2SupOffset));
  XHCI_DEBUG ((DEBUG_INFO, "XhcCreateUsb3Hc: Usb3SupOffset 0x%x\n", Xhc->Usb3SupOffset));
