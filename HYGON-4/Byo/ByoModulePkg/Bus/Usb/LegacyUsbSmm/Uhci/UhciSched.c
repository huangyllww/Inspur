...
--*/
{
    ...
    // will remain on the schedule, never got removed
    //
...
Routine Description:
  ...
  Convert the poll rate to the maximum 2^n that is bigger
  than Interval
    ...
    while (Td != NULL) {
      ...
      QhResult->NextToggle = (UINT8) (1 - (UINT8)TdHw->DataToggle);
            ...
            if (!IsLow && (TdHw->ShortPacket == 1) && (Len < Td->DataLen)) {
                UHCI_DEBUG((EFI_D_INFO, "UhciCheckTdStatus: short packet read occurred\n"));
...
Routine Description:
  ...
  Unlink an asynchronous request's from UHC's asynchronous list.
...

{
    ...
    if (FreeNow) {
        ...
        //
        // To synchronize with hardware, mark the queue head as inactive
...
Routine Description:
  ...
  Recycle the asynchronous request. When a queue head
  ...
  moved to UHC's Recyclelist. Then, at another timer
  activation, all the requests on Recycle list is freed.
  This guarantees that each unlink queue head keeps
