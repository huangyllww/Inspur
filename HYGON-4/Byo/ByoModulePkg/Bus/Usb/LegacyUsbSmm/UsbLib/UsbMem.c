...
#define USB_MEM_TRB_RINGS_BOUNDARY  SIZE_64KB
...
)
{
    ...
    StartBit   = 0;
    Available  = 0;
    AlignmentMask = ~((UINTN)USB_MEM_TRB_RINGS_BOUNDARY - 1);
    ...
    for (Byte = 0, Bit = 0; Byte < Block->BitsLen;) {
        ...
        if (!USB_HC_BIT_IS_SET (Block->Bits[Byte], Bit)) {
            if (AllocationForTrbRing && (Available != 0)) {
              MemUnitAddr = (UINTN)Block->BufHost + (Byte * 8 + Bit) * USB_MEM_UNIT;
              if ((MemUnitAddr & AlignmentMask) != ((MemUnitAddr - USB_MEM_UNIT) & AlignmentMask)) {
                //
...
)
{
    ...
    Mem = UsbAllocMemFromBlockXhci (Block, AllocSize / USB_MEM_UNIT,AllocationForRing);
    if (Mem != NULL) {
...
)
{
    ...
    UINTN                   AllocSize;
    //
    //reserve this function for Legacy Ehci/Ohci/Uhci
    //
    Mem       = NULL;
    Block     = mUsbMemPool->Head->Next;
    ASSERT (Block != NULL);
