  ...
  )
{
  ...
  if (Result != EFI_USB_NOERROR) {
    ...
    Status = UsbIo->UsbAsyncInterruptTransfer (
                      ...
                      EpDesc->MaxPacketSize,           // HubIf->NumOfPort / 8 + 1,  follow linux behaviour, patch RENESAS 3.0 hub issue.
                      UsbOnHubInterrupt,
                      HubIf
                      );
  ...
  // practice since UsbOnHubInterrupt is called in the context
  ...
  )
{
  ...
  Status = UsbIo->UsbAsyncInterruptTransfer (
                    ...
                    TRUE,
                    USB_HUB_POLL_INTERVAL,
                    EpDesc->Desc.MaxPacketSize,        // HubIf->NumOfPort / 8 + 1, follow linux behaviour, patch RENESAS 3.0 hub issue.
                    UsbOnHubInterrupt,
                    HubIf
                    );
  ...
  )
{
  ...
  USB_DEBUG((EFI_D_INFO, "%a(): After reset PortStatus - 0x%x, PortChangeStatus - 0x%x\n", __FUNCTION__, PortState.PortStatus, PortState.PortChangeStatus));
