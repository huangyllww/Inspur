  ...
  )
{
  ...
  *Consumed = 0;
  ...
  if (Len <= sizeof (USB_DESC_HEAD)) {
    USB_DEBUG ((DEBUG_ERROR, "UsbCreateDesc: met mal-format descriptor, total length = %d!\n", Len));
    return NULL;
  }
  ...
  while (Offset < Len - sizeof (USB_DESC_HEAD)) {
    ...
    if (Head->Len == 0) {
      USB_DEBUG ((DEBUG_ERROR, "UsbCreateDesc: met mal-format descriptor, Head->Len = 0!\n"));
      return NULL;
    }
    ...
    if (Head->Len > MAX_UINTN - Offset) {
      USB_DEBUG ((DEBUG_ERROR, "UsbCreateDesc: met mal-format descriptor, Head->Len = %d!\n", Head->Len));
      return NULL;
    }
  ...
  //
  if (Len < Offset) {
    USB_DEBUG ((DEBUG_ERROR, "UsbCreateDesc: met mal-format descriptor, Offset/Len = %d/%d!\n", Offset, Len));
    return NULL;
  }

  if ((Head->Type != Type) || (Head->Len < DescLen)) {
    USB_DEBUG ((DEBUG_ERROR, "UsbCreateDesc: descriptor cannot be found, Header(T/L) = %d/%d!\n", Head->Type, Head->Len));
    return NULL;
  }
  ...
  )
{
  ...
  while ((Len >= sizeof (EFI_USB_INTERFACE_DESCRIPTOR)) && (EnumIf < NumIf ) ) {
    ...
    if (Setting == NULL) {
      ...
      USB_DEBUG (( DEBUG_ERROR, "UsbParseConfigDesc: mal-formatted interface descriptor\n"));
  ...
  )
{
  ...
  if(Desc.Length != ((EFI_USB_STRING_DESCRIPTOR *) Buf)->Length) {
    FreePool (Buf);
    return NULL;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG (( EFI_D_INFO, "UsbBuildDescTable: get language ID table %r\n", Status));
  }
