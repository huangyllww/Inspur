/*++
==========================================================================================
      NOTICE: Copyright (c) 2006 - 2022 Byosoft Corporation. All rights reserved.
...
--*/
{
    ...
    *TransferResult = EFI_USB_ERR_SYSTEM;
    ...
    // the TD to corresponding ED, then check
...
--*/
{
    ...
    Ed->EdHw.sKip = 0;
    ((OHCI_ED_SW*)(UINTN)(Ed->PreEDSoft))->EdHw.sKip = 0;
...
--*/
{
    ...
    *TransferResult = EFI_USB_ERR_SYSTEM;
    ...
    Ed->EdHw.sKip = 0;
    ((OHCI_ED_SW*)(UINTN)(Ed->PreEDSoft))->EdHw.sKip = 0;
    Status = OhciExecuteTransfer (Ohc, (OHCI_ED_SW*)(UINTN)Ed->PreEDSoft, TDs, TimeOut, &EdResult);

    OhciUnlinkEdFromEd ((OHCI_ED_SW*)(UINTN)Ed->PreEDSoft, Ed);
    OhciUnlinkTdFromEd (Ed, TDs);
...
--*/
{
    ...
    //
    // Enable "IOC" in Interrupt enable register
    //
...
Routine Description:
  ...
  Disable legacy support for OHCI
...
--*/
{
    ...
    //
    // Disable "IOC" in Interrupt disable register
    //
...
Arguments:
  ...
  Map        - identification of this mapping to return
...
--*/
{
    ...
    Ed->EdHw.FunctionAddress     = DeviceAddress & 0x7f;
...
--*/
{
    ...
    while (NextTd != NULL) {
        ThisTd  = NextTd;
        NextTd  = (OHCI_TD_SW*)(UINTN)ThisTd->NextTDSoft;
...
--*/
{
    ...
    TdTail = Td;
    while (TdTail)
        TdTail = (OHCI_TD_SW*)(UINTN)(((UINT32)TdTail->TdHw.NextTD) << 4);
...
Returns:
  ...
  Whether the TD's result is finalized.
...
--*/
{
    ...
    pTd = Td;
    pEd = (OHCI_ED_SW*)(UINTN)((UINT32)Ed->EdHw.NextED << 4);
    ...
    while (pTd != NULL) {
        ...
        pTd = (OHCI_TD_SW*)(UINTN)((UINT32)pTd->NextTDSoft);
    }
...
--*/
{
    ...
    for (Index = 0; InfiniteLoop || (Index < Delay); Index++) {
        ...
        //
        // Transfer is OK or some error occurred
        //
        if (Finished) {
            break;
        }
...
Arguments:
  ...
  Callback    - Callback after interrupt transfer
...
Routine Description:
  ...
  Unlink an asynchronous request's from OHC's asynchronous list.
...

{
    ...
    OhciUnlinkEdFromEd ((OHCI_ED_SW*)(UINTN)AsyncReq->EdSw->PreEDSoft, AsyncReq->EdSw);
...
--*/
{
    ...
    if (Result == EFI_USB_NOERROR) {
        ...
        for (Td = FirstTd; Td != NULL; Td = (OHCI_TD_SW*)(UINTN)Td->NextTDSoft) {
...
Returns:
   ...
   EFI_SUCCESS - The device is disabled
...
--*/
{
    ...
    OHCI_DEBUG (( EFI_D_INFO, " FunctionAddress      - %x\n",   EdSw->EdHw.FunctionAddress));
    ...
    OHCI_DEBUG (( EFI_D_INFO, " TDs Tail Pointer - %x\n\n", EdSw->EdHw.TDQueueTailPointer << 4));
}
...
--*/
{
    ...
    while (CurTdSw != NULL) {
        ...
        CurTdSw = (OHCI_TD_SW*)(UINTN)CurTdSw->NextTDSoft;
    }
}
