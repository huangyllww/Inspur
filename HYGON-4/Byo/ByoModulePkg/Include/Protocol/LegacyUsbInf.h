...
//
// Disabling 'type cast': conversion from 'UINT32' to 'void *' of greater size
//
  ...
  do { \
    ...
    *ret = 0xFFFFFFFF; \
    if (!IfInSmm()) { \
      ...
      while(*mSmmCallComm->Return == 0xFFFFFFFF){} \
...
struct _EFI_LEGACY_USB_INF_PROTOCOL {
  USB_SMMCALL_COMM                   *SmmCallCommBuf;
  SMMCALL_ENTRY                      *SmmCallTablePtr;
} ;

/*
CurrentMouseData0[0] Save Mouse Button Data
CurrentMouseData0[1] Save Mouse X Data
CurrentMouseData0[2] Save Mouse Y Data

CurrentMouseData2[0] Save Mouse Z Data
*/
