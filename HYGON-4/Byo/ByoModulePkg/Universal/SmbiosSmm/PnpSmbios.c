...
extern SMI_INFO                 *gSmiFlashInfo;
UINT32                          mSmbiosRegionSize = 0;
...
)
{
  ...
  //
  // Scan the smbios table to the end and calculate the total size
  //
...
)
{
  ...
  //
  // If the smbios handle is zero, return the fist smbios structure
  //
  if (*StrucHandle == 0) {
  ...
  switch (DataBufferPtr->StructureHeader.Type) {
    ...
    case EFI_SMBIOS_TYPE_OEM_STRINGS:
      if(!GetSmbiosStructureByType(EFI_SMBIOS_TYPE_OEM_STRINGS, &Hdr)){
        ...
        DEBUG((EFI_D_INFO, "(L%d) L:%X O:%X C:%X D:%X\n", __LINE__, StrCount, \
...
GetFromFlash ()
{
  ...
  Status = Mem2RecordList((UINT8*)SmbiosRecBase, mSmbiosRegionSize);
...
)
{
  ...
  SetMem (mSmbiosScratchPtr, mSmbiosRegionSize, 0xff);
  TotalLen = RecordList2Mem (mSmbiosScratchPtr);
  ASSERT(TotalLen <= mSmbiosRegionSize);
  if(TotalLen > mSmbiosRegionSize){
    TotalLen = mSmbiosRegionSize;
  }
  
  if (pMediaAccessProtocol != NULL) {
    Status = pMediaAccessProtocol->Erase (
                            pMediaAccessProtocol,
                            PcdGet32(PcdFlashNvStorageSmbiosBase),
                            mSmbiosRegionSize,
                            SPI_MEDIA_TYPE
                            );
    Status = pMediaAccessProtocol->Write (
                            pMediaAccessProtocol,
                            PcdGet32(PcdFlashNvStorageSmbiosBase),
                            mSmbiosScratchPtr,
                            TotalLen,
...
)
{
  ...
  if (DataBufferPtr->Command != AddChanged && DataBufferPtr->Command != DeleteChanged) {
    ...
    if(FixHandle && (DataBufferPtr->StructureHeader.Handle == 0 || DataBufferPtr->StructureHeader.Handle == END_HANDLE)) {
      ...
      PnpGetSmbiosStructureByHandle (DataBufferPtr->StructureHeader.Handle, &StrucPtr);
    }
  ...
  //
  ErrorCode = (UINT16)Write2Flash (DataBufferPtr);
