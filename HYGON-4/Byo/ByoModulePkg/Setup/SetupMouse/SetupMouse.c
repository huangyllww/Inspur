  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "SetupMouseStart, Simple Pointer Protocol Count 0x%x\n", Count));
  for (Index = 0; Index < Count; Index++) {
    ...
    if (!EFI_ERROR (Status)) {
      DMSG ((EFI_D_INFO, "Simple Pointer Support.\n"));
  ...
  //
  // Check Absolute pointer protocol.
  //
  Status = gBS->LocateHandleBuffer (
                  ByProtocol,
  ...
  DMSG ((EFI_D_ERROR, "SetupMouseStart, Absolute Pointer Protocol Count 0x%x\n", Count));
  for (Index = 0; Index < Count; Index++) {
    Status = gBS->HandleProtocol (
                HandleBuffer[Index],
                &gEfiDevicePathProtocolGuid,
    ...
    if (!EFI_ERROR (Status)) {
      DMSG ((EFI_D_INFO, "Absolute Pointer Support.\n"));
 ...
 if (!SimplePointerSupport && !AbsPointerSupport) {
  ...
  if (AbsPointerSupport) {
    ...
    DMSG ((EFI_D_ERROR, "SetupMouseStart, Locate Absolute Pointer Protocol :%r. \n", Status));
    if (!EFI_ERROR (Status1)) {
      DMSG ((EFI_D_INFO, "Absolute Pointer Support.\n"));
      AbsPointer->Reset (AbsPointer, TRUE);
      AbsPointer->GetState (AbsPointer, &AbsPointerState);
      Private->AbsPointerProtocol = AbsPointer;
    }
  }
  ...
  for (Index = 0; Index < Count; Index++) {
    ...
    Status = gBS->HandleProtocol(HandleBuffer[Index], &gEfiConsoleOutDeviceGuid, (VOID **)&GraphicsOutput);
    if (EFI_ERROR (Status)) {
      continue;
    }
    ...
    Private->VideoHorizontalResolution = GraphicsOutput->Mode->Info->HorizontalResolution;
    Private->VideoVerticalResolution   = GraphicsOutput->Mode->Info->VerticalResolution;
