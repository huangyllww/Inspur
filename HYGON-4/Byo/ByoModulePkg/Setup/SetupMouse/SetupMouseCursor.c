  ...
  )
{
  ...
  Status = Status1 = EFI_NOT_READY;
  SimplePointerPtr = (EFI_SIMPLE_POINTER_PROTOCOL*)Private ->PointerProtocol;
  if(SimplePointerPtr != NULL){
    Status = SimplePointerPtr->GetState(SimplePointerPtr, &SimplePointerState);
  }

  AbsPointer  = (EFI_ABSOLUTE_POINTER_PROTOCOL*)Private ->AbsPointerProtocol;
  if(AbsPointer != NULL){
    Status1 = AbsPointer->GetState(AbsPointer, &AbsPointerState);
  }
  ...
  // update Coordinate.
  //
  if(EFI_ERROR(Status1)){//Simple mouse
    ...
    RightButton = SimplePointerState.RightButton;
    LeftButton  = SimplePointerState.LeftButton;
  
  }else{//Absolute mouse
    CurX = AbsPointerState.CurrentX * Private->VideoHorizontalResolution;
    CurX = CurX /(0xFFFF+1);
    CurY = AbsPointerState.CurrentY * Private->VideoVerticalResolution;
    CurY = CurY /(0xFFFF+1);
  
    RightButton = ((AbsPointerState.ActiveButtons & BIT1) ? TRUE : FALSE);
    LeftButton  = AbsPointerState.ActiveButtons & BIT0;
  }
  // 
  // update Right button.
  //  
  ...
  // update Left button.
  //  
  if (LeftButton && !LastLeftClick) {
    LastLeftClick = TRUE;
    Private->SaveCursorX = CurX;
    Private->SaveCursorY = CurY;

    TimeCount = 0;
  } else if (LeftButton && LastLeftClick) {
   if (TimeCount > 6) {
  ...
  )
{
  ...
  if ((UINT32) GopEntry->Screen.ImageRc.right != HorizontalResolution || (UINT32) GopEntry->Screen.ImageRc.bottom != VerticalResolution) {
    //
    // Resolution has been changed. Need use real GOP service.
    //
    return GopEntry->OriginalBlt (This, BltBuffer, BltOperation, SourceX, SourceY, DestinationX, DestinationY, Width, Height, Delta);
  }
