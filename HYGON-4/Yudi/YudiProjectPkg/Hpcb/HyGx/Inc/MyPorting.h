...
#define _MY_PORTING_H_
  ...
  // #include <intrin.h>  // MS has built-in functions
    ...
    // Note: Entire HGPI (Project / Solution) will be using pragma pack 1
  ...
  #define TRUE      1
  #define FALSE     0
  // #define CONST const - avoid const until the large job of making hgpi use it consistently is complete
  #define CONST
  #define ROMDATA
  #define CALLCONV
  ...
  #include "MyIntrinsics.h"               // HGPI temp file for GCC until support is added to MinGW

#else
  // -----------------------------------------------------------------------
  // Unknown or unsupported compiler - use for ARM compiler
  //
  #include "MyIntrinsics.h"               // HGPI temp file for ARM GCC
#endif
...
#define HGPI_FORWARD_DECLARATION(x)  typedef struct _ ## x x
