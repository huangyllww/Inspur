...
CONST
HYGON_LOGICAL_BRIDGE_NUMBER HygonLogicalId[18] = {
...
)
{
  ...
  } while(!IsDevicePathEnd(OrgDevPath));
...
)
{
  ...
  ChildHandle = DevHandle;
  /* Get parent bridge handle */
  do {
    ...
    if (!EFI_ERROR(Status)) {
      ...
      /* if pci bridge, break */
      if (((HeaderType & HEADER_LAYOUT_CODE) != HEADER_TYPE_DEVICE) && (0x1D94 == VendorID)) { //found hygon root port
         Status = PciIo->GetLocation(PciIo, &s, &b, &d, &f);
         DEBUG((EFI_D_INFO,"Found Root Port BDF: %X:%X:%X\n",b,d,f));
         Addr = (UINT32)((d << 3) + f);
         for (Index = 0; Index < 18; Index++){
           //Get PCIE physical port
           SmnRead((UINT32)b, TKN_NB_PROG_DEVICE_REMAP+(Index << 2), &Value);
           if (Addr == (Value & 0xFF)){
              return HygonLogicalId[Index].LogicalId;
           }
         }
      }
    }
  ...
  )
{
  ...
  BridgeDp = DuplicateDevicePath(DevicePath);

  while(!IsDevicePathEnd(BridgeDp)) {
    DpTemp = BridgeDp;
...
LegacyBiosConnectPciRootBridgeHook()
{
  ...
  UINTN                              s, b, d, f;
  UINT16                             PirqCount;
  EFI_LEGACY_IRQ_ROUTING_ENTRY       NewEntry[IRQ_ROUTING_COUNT];
  UINTN                              NewEntryCount;
  EFI_DEVICE_PATH_PROTOCOL           *DevicePath = NULL;
  UINT8                              IntPin;
  UINT8                              SlotNum;
  ...
  for(Index=0;Index<HandleCount;Index++){
    ...
    if(PciId == 0x145A1D94 || PciId == 0x14551D94){
      continue;
    }
    ...
    SlotNum = GetPciSlotNum(gBS,DevicePath);
    DEBUG((EFI_D_INFO, "(%x,%x,%x) slotid:%d\n", b, d, f, SlotNum));
    if(SlotNum == 0xFF){
      continue;
    }
    ...
    LogicalBridgeNum = GetLogicalBridgeNum(Handles[Index]);
    DEBUG((EFI_D_INFO, "LogicalBridgeNum:%d\n", LogicalBridgeNum));
    if(LogicalBridgeNum > 17){ // invalid
      continue;
    }
