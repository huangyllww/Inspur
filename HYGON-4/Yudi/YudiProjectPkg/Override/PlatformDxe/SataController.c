  ...
  )
{
  ...
  if (Channel < This->ChannelCount) {
    ...
    if(Channel == 0 && (SataPrivateData->IdePortPresent & 0x3) == 0){
      *Enabled = FALSE;
      *MaxDevices = 0;
    } else if(Channel == 1 && (SataPrivateData->IdePortPresent & 0xC) == 0){
      *Enabled = FALSE;
      *MaxDevices = 0;
    } else {
      *Enabled = TRUE;
  ...
  )
{
  ...
  if(((UINT16)PciId == HG_VENDOR_ID) || PciId == 0x06221B21){
    return EFI_SUCCESS;
  }
