    ...
    if (Index != 16) {
      ...
      if (Index == 16) {
        if (FirstMatch) {
          ...
          //
          sprintf(CurrentFv->FvName, "FV%02d", FvCount);
          ...
          FirstMatch = FALSE;
            ...
            // Store the FV name by found sequence
            //
            sprintf(NewFoundFv->FvName, "FV%02d", FvCount);
  ...
  )
{
  ...
  for(
    ...
    if ((fsize == 0xffffff) || (fsize == 0xffffffff)) {
      break;
    }
...
)
{
  ...
  sprintf (
    TempBuf,
    "-Num%04d-%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X-Level%d",
    *FfsCount,
    ...
    switch (ParentType) {
      ...
      if (LocalEncapData->FvExtHeader != NULL) {
        ...
        */
        ...
        //
        //  Copy old FV file header first 16 bytes to new FV file, on ARM platform first 16 bytes include base address
        //
        FILE *TmpOut;
        UINT32 TmpSize;
        void *TmpBuff;
        ...
        TmpSize = ftell(TmpOut);
        fseek(TmpOut,0,SEEK_SET);
        TmpBuff = malloc(TmpSize);
        if (TmpBuff == NULL) {
          Status = RETURN_OUT_OF_RESOURCES;
          return Status;
        }
        if (fread(TmpBuff, 1, TmpSize, TmpOut) != TmpSize) {
          fclose(TmpOut);
          free(TmpBuff);
          return EFI_ABORTED;
        }
        memcpy(TmpBuff, FvInFd->EncapData->Data, sizeof(GUID));
        fseek(TmpOut,0,SEEK_SET);
        if (fwrite(TmpBuff, 1, TmpSize, TmpOut) != TmpSize) {
          fclose(TmpOut);
          free(TmpBuff);
          return EFI_ABORTED;
        }
