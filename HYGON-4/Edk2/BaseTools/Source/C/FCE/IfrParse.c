  ...
  while (!IsNull (StorageListHead, Link)) {
    ...
    if (!CompareGuid (&StorageNode->Guid, &CurNode->Guid)   \
      ...
      if (mMultiPlatformParam.MultiPlatformOrNot) {
        if (CurNode->Size == StorageNode->Size && 
           CurNode->Type == EFI_IFR_VARSTORE_EFI_OP && 
           CurNode->NewEfiVarstore && 
           ((CurNode->Attributes & EFI_VARIABLE_NON_VOLATILE) != 0)) {
          //
        ...
        if (CurNode->Size == StorageNode->Size && 
           StorageNode->Type == EFI_IFR_VARSTORE_EFI_OP && 
           StorageNode->NewEfiVarstore && 
           ((StorageNode->Attributes & EFI_VARIABLE_NON_VOLATILE) != 0)) {
          //
          // If matched, get the address of EFI_IFR_VARSTORE data.
          //
          CurNode->Type = StorageNode->Type;
          CurNode->NewEfiVarstore = StorageNode->NewEfiVarstore;
          CurNode->Attributes = StorageNode->Attributes;
          return CurNode;
        }
  ...
  )
{
  ...
  if ((((EFI_HII_PACKAGE_HEADER *)UniPackge)->Type == EFI_HII_PACKAGE_STRINGS)) {
    UniBin       = (CHAR8 *) UniPackge;
    UniPackgeEnd = (CHAR8 *) UniPackge + ((EFI_HII_PACKAGE_HEADER *)UniPackge)->Length;
    UniPackge    = UniPackge - 4;
  } else {
  ...
  )
{
  ...
  if ((((EFI_HII_PACKAGE_HEADER *)UniPackge)->Type == EFI_HII_PACKAGE_STRINGS)) {
    UniBin     = (CHAR8 *) UniPackge;
    Index      = 0;
    UniLength  = ((EFI_HII_PACKAGE_HEADER *)UniBin)->Length;
  } else {
    UniBin     = (CHAR8 *) UniPackge + 4;
    Index      = 4;
