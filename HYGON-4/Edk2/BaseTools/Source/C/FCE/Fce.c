  ...
  )
{
  ...
  //
  IfrBinaryData = (UINT8 *)*(BaseAddr + Index);
  PackageLength = ((EFI_HII_PACKAGE_HEADER *) IfrBinaryData)->Length;
...
VOID
InsertSortStorageList (
  IN  LIST_ENTRY          *StorageListHead,
  IN  FORMSET_STORAGE     *StorageInsert
)
{
  LIST_ENTRY        *StorageLink;
  FORMSET_STORAGE   *Storage;
  ...
  while (!IsNull (StorageListHead, StorageLink)) {
    Storage = FORMSET_STORAGE_FROM_LINK (StorageLink);
    CompareValue = FceStrCmp (StorageInsert->Name, Storage->Name);
    if (CompareValue > 0) {
      break;
    } else if (CompareValue == 0) {
      if (memcmp (&(StorageInsert->Guid), &(Storage->Guid), sizeof (EFI_GUID)) > 0) {
        break;
      }
    }
    StorageLink = GetNextNode (StorageListHead, StorageLink);
  ...
  InsertTailList (StorageLink, &StorageInsert->Link);
...
)
{
  ...
  while (!IsNull (StorageListHead2, StorageLink)) {
    ...
    if ((Storage->Type == EFI_IFR_VARSTORE_EFI_OP)
      ...
      //
      InsertSortStorageList (StorageListHead1, StorageCopy);
    }
    StorageLink = GetNextNode (StorageListHead2, StorageLink);
  }
  return EFI_SUCCESS;
    ...
    if (EfiAddr != NULL) {
      ...
      if (Status == EFI_SUCCESS && (!ImageContext.IsTeImage)) {
        ...
        if (MemoryImagePointer == NULL) {
          printf ("Error Memory cannot be allocated for the image\n");
          Status = EFI_ABORTED;
          goto Done;
        }
        ...
        if (Status == EFI_SUCCESS && (ImageContext.HiiResourceData != 0)) {
          ...
          Index = 0;
          VfrBinBaseAddress = malloc (sizeof (UINTN *) * NumberofMachingVfrBin);
          UniBinBaseAddress = malloc (sizeof (UINTN *));
          PackageHeader = (EFI_HII_PACKAGE_HEADER *) (PackageList + 1);
          while (PackageHeader->Type != EFI_HII_PACKAGE_END) {
            if (PackageHeader->Type == EFI_HII_PACKAGE_FORM) {
              VfrBinBaseAddress [Index ++] = (UINTN) (UINTN *) PackageHeader;
            }
            if (PackageHeader->Type == EFI_HII_PACKAGE_STRINGS) {
              if (strcmp (((EFI_HII_STRING_PACKAGE_HDR *)PackageHeader)->Language, "en-US") == 0) {
                *UniBinBaseAddress = (UINTN) (UINTN *) PackageHeader;
              }
            }
            PackageHeader = (EFI_HII_PACKAGE_HEADER *) ((UINT8 *) PackageHeader + PackageHeader->Length);
          }
          EfiBufferHeader->BufferBase = (UINTN) ImageContext.ImageAddress;
          EfiBufferHeader->Length     = (UINTN) ImageContext.ImageSize;
        }
      }
    }
  }
        ...
        if ((Question->Operand == EFI_IFR_ONE_OF_OP)
          ...
          if (*OrderOfQuestion == Index++) {
            ...
            if (Question->Type != EFI_IFR_VARSTORE_EFI_OP) {
              ...
              LIST_ENTRY         *Link;
              LIST_ENTRY         *StorageListHead = &mVarListEntry;
              ...
              while (!IsNull (StorageListHead, Link)) {
                ...
                if (CurNode->NewEfiVarstore &&
                    !CompareGuid (&CurNode->Guid, &Question->Guid) &&
                    (CurNode->Name != NULL) &&
                    (Question->VariableName != NULL) &&
                    !FceStrCmp (CurNode->Name, Question->VariableName)) {
  ...
  )
{
  ...
  //
  // Always generate normal variable storage
  //
  AuthencitatedMonotonicOrNot = FALSE;
  AuthencitatedBasedTimeOrNot = FALSE;
  //
