...
CheckHiiResource (
  ...
  SectionStart       = (VOID*)((UINTN)Ffs + FvBufGetFfsHeaderSize(Ffs));
  TotalSectionsSize  = Len - FvBufGetFfsHeaderSize(Ffs);
  ...
  while (TRUE) {
    ...
    if (EFI_SECTION_PE32 == Section->Type) {
      //
      // Get HII resource data from EFI image
      //
      memset (&ImageContext, 0, sizeof (ImageContext));
      ImageContext.Handle    = (Section + 1);
...
)
{
  ...
  if((FindHandle = FindFirstFile(CurFolderName, &FindFileData)) != INVALID_HANDLE_VALUE){
    ...
    *SizeOptimized                = TRUE;
      ...
      if (EFI_ERROR (Status)) {
        //
        // Check HII resource section
        //
        Status = CheckHiiResource (FfsImage, FileSize);
        if (EFI_ERROR (Status)) {
          free (FfsImage);
        }
      }
