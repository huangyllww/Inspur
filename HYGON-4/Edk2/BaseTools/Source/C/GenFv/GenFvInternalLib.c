...
--*/
{
  ...
  UINT32                              Index;
  UINT32                              LastTextDistance;  //This is used in Printf Function Information,function type3
  ...
  *Cptr2 = '.';
  ...
  while (fgets (Line, MAX_LINE_LEN, PeMapFile) != NULL) {
    ...
    if (FunctionType == 0) {
      ...
      } else if (stricmp (KeyWord, ".text") ==0){
        FunctionType = 3;
        LastTextDistance=1;
      }
      continue;
    ...
    if (FunctionType == 1) {
      if (IsUseClang) {
        ...
        if (FunctionTypeName [1] == '\0' && (FunctionTypeName [0] == 'f' || FunctionTypeName [0] == 'F')) {
          ...
          fprintf (FvMapFile, "  0x%010llx    ", (unsigned long long) (ImageBaseAddress + FunctionAddress - LinkTimeBaseAddress));
          fprintf (FvMapFile, "%s\n", FunctionName);
        } else if ((FunctionAddress > 0) && (IsFunctionSet == FALSE)) {
          fprintf (FvMapFile, "  0x%010llx    ", (unsigned long long) (ImageBaseAddress + FunctionAddress - LinkTimeBaseAddress));
          fprintf (FvMapFile, "%s\n", FunctionName);
        }
      }
      ...
      sscanf (Line, "%s", KeyWord);
      if (strncmp(".text",KeyWord,5)==0){
        //
        // try to copy function name from .text string (.text.function)
        //
        FunctionName[0]=0;
        IsFunctionSet=FALSE;
        if (strlen (KeyWord) > 5) {
          strcpy (FunctionName, KeyWord + 6);
          //
          // ignore unlikely prefix
          //
          if (strncmp("unlikely.",FunctionName,strlen ("unlikely."))==0){
            strcpy (FunctionName, FunctionName + strlen ("unlikely."));
          }
          //
          // ignore .xxx postfix
          //
          for (Index = 0; FunctionName[Index] != 0; Index ++) {
      ...
      //The AARCH 64 map is like 
      // .text(usually the words after .text is .****(FunctionName) ,The second line follows 0x****  0x****,The third line follows 0x***(address) ****(FunctionName))
      // However,sometimes the next of .text is empty and follow 0x**** 0x***,and next line follows 0x****(address) ****(functionname)
      // And,Sometimes the most important !!! the next line still follows 0x****(address) ****(functionname),One .text follows two function.
      //
      // In this case we set LastTextDistance,it means this line since last .text how many lines passed.Its first value is 4
      // When meeting .text,we set LastTextDistance as 1;Every time read a new line,if LastTextDistance<4 it means this line belongs to one .text
      // And when LastTextDistance<4 ,every line passed LastTextDistance++;
      //
      // While reading a new line,read %s %s.check if the first %s contains 0x,the second %s doesnt contain 0x
      // And LastTextDistance<4,we confirm this line contains a function
      //
      if (LastTextDistance<4){
        if (strlen (FunctionName) > 0 && IsFunctionSet) {
          //
          // FunctionName is from .text string (.text.function)
          //
          sscanf (Line, "%s", KeyWord);
          LastTextDistance=4;
        } else {
          //
          // FunctionName is from line 0x****(address) ****(functionname)
          //
          sscanf (Line, "%s %s", KeyWord, FunctionName);
        }
        if ( (strncmp("0x",KeyWord,2)==0)&& (strncmp("0x",FunctionName,2)!=0) ){
          sscanf (Line, "%llx", &TempLongAddress);
          FunctionAddress = (UINT64) TempLongAddress;
