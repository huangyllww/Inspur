...
NeedNewPath(FV_INFORMATION *FvInFd, CHAR8 *FvId, UINT32 FileIndex, BOOLEAN IsAdd) {
  ...
  if (stricmp(FvId, FvInFd->FvName) != 0) {
    return FALSE;
  }
  if (IsAdd) {
    return TRUE;
...
)
{
  ...
  for (i = 0; i < count; i++) {
      for (j = i + 1; j < count; j++){
        if (((FileList + i)->FvId[0] == 0) || ((FileList + j)->FvId[0] == 0)) {
          continue;
        }
        if (stricmp((FileList + j)->FvId, (FileList + i)->FvInFd->FvName) == 0){
...
)
{
  ...
  if ((strcmp(Argv[0], "-v") == 0) || (strcmp(Argv[0], "-V") == 0)) {
    ...
    if ((Argc - 3) % 2 == 0) {
        ...
        for(index = 0; index < (Argc - 3)/2; index ++) {
            ...
            exist = -1;
            for (j = 0; j < count; j ++) {
                if ((stricmp(Argv[2 + index * 2], (FileData + j) -> FvId) == 0)) {
                    exist = j;
                    break;
                }
            }
            if (exist >= 0) {
                ...
                strncpy((FileData + count) -> FvId, Argv[2 + index * 2], MAX_BASENAME_LEN - 1);
        ...
        for (index = 0; index < count; index ++) {
            for (j = index + 1; j < count; j ++) {
                if ((stricmp((FileData + index)->FvId, (FileData + j)->FvId) < 0)) {
                    CHAR8 tmp[MAX_BASENAME_LEN];
                    strcpy(tmp, (FileData + index)->FvId);
                    FILENode *t = (FileData + index)->OldFile;
                    strcpy((FileData + index)->FvId, (FileData + j)->FvId);
                    (FileData + index)-> OldFile = (FileData + j)->OldFile;
                    (FileData + j)-> OldFile = t;
                    strcpy((FileData + j)-> FvId, tmp);
                }
            }
        }
  ...
  for (index = 0; index < count; index ++) {
    ...
    FileData = calloc(1, sizeof (Data) * (Argc - 3)/3);
    if (FileData == NULL) {
      Error ("FMMT", 0, 4001, "Resource: Memory can't be allocated", NULL);
      return 1;
    }
    for(index = 0; index < (Argc - 3)/3; index ++) {
        p = calloc(1, sizeof (FILENode)); //p for old file
        if (p == NULL) {
          Error ("FMMT", 0, 4001, "Resource: Memory can't be allocated", NULL);
          free (FileData);
          return 1;
        }
        q = calloc(1, sizeof (FILENode)); //q for new file
        if (q == NULL) {
          Error ("FMMT", 0, 4001, "Resource: Memory can't be allocated", NULL);
          free (FileData);
          free (p);
        ...
        if (exist >= 0) {
            ...
            (FileData + count) -> FvLevel = 0;
            (FileData + count) -> FvInFd = NULL;
            strncpy((FileData + count) -> FvId, Argv[2 + index * 3], MAX_BASENAME_LEN - 1);
            (FileData + count) -> OldFile = p;
