...
if defined EWDK (
  if NOT defined EWDK_PATH (
    @echo.
    @echo !!! ERROR !!! The EWDK_PATH was not found !!!
    @echo.
    goto end
  )
)
...
) else if defined EWDK (
  call %EDK_TOOLS_PATH%\set_vsprefix_envs.bat EWDK
) else (
...
:Usage
  @echo.
  echo  Usage: "%0 [-h | -help | --help | /h | /help | /?] [ Rebuild | ForceRebuild ] [Reconfig] [base_tools_path [edk_tools_path]] [EWDK] [VS2019] [VS2017] [VS2015] [VS2013] [VS2012]"
  @echo.
  ...
  @echo         EWDK              Set the env for EWDK build.
  @echo.
