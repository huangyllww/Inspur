  ...
  )
{
  ...
  EFI_DEVICE_PATH_PROTOCOL  **OrgDevicePathList;
  EFI_HANDLE                *TempHandleBuffer;
  ...
  OrgDevicePathList = AllocateZeroPool (sizeof (EFI_DEVICE_PATH_PROTOCOL *) * Count);
  if (DevicePathList == NULL) {
    return;
  }

  TempHandleBuffer = AllocateZeroPool (BufferSize);
  if (TempHandleBuffer == NULL) {
    return;
  }

  for (j = 0; j < Count; j++) {
    DevicePathList[j]    = DevicePathFromHandle (HandleBuffer[j]);
    OrgDevicePathList[j] = DevicePathList[j];
    TempHandleBuffer[j]  = HandleBuffer[j];
  }
  ...
  for (j = 0; j < Count; j++) {
    for (k = 0; k < Count; k++) {
      if (DevicePathList[j] == OrgDevicePathList[k]) {
        HandleBuffer[j] = TempHandleBuffer[k];
        break;
      }
    }
  }
  ...
  FreePool (OrgDevicePathList);
  FreePool (TempHandleBuffer);
}
  ...
  )
{
  ...
  SortHandleBuff (HandleBuffer, BufferSize);
  //
