  ...
  )
{
  ...
  //
  if (SystemMiscSetting->SysSetting.Bits.LegacyBiosSupport == 1) {
    HasLegacyImage = ContainImage (PciIoDevice->PciIo.RomImage, PciIoDevice->PciIo.RomSize, PCI_CODE_TYPE_PCAT_IMAGE);
  } else {
  ...
  switch (PciIoDevice->Pci.Hdr.ClassCode[2]) {
    case PCI_CLASS_DISPLAY:
      OptionRomPost = SystemMiscSetting->PciSetting.Bits.VideoOpRomLaunchPolicy;
      break;

    case PCI_CLASS_NETWORK:
      OptionRomPost = SystemMiscSetting->PciSetting.Bits.PxeOpRomLaunchPolicy;
      break;

    case PCI_CLASS_MASS_STORAGE:
      OptionRomPost = SystemMiscSetting->PciSetting.Bits.StorageOpRomLaunchPolicy;
      break;

    default:
      OptionRomPost = SystemMiscSetting->PciSetting.Bits.OtherOpRomLaunchPolicy;
      break;
  }
  ...
  // uefi   : 1
  // no     : 2
  if ((OptionRomPost == ROM_POLICY_LEGACY_FIRST) && HasLegacyImage) {
    // legacy first && has legacy
    HasEfiImage = 0;
  } else if (OptionRomPost == ROM_POLICY_NO_LAUNCH) {
    // no launch
    HasEfiImage = 0;
  } else {
    HasEfiImage = ContainImage (PciIoDevice->PciIo.RomImage, PciIoDevice->PciIo.RomSize, PCI_CODE_TYPE_EFI_IMAGE);
  }

  NvmeAddOnOpRom = (SystemMiscSetting->PciSetting.Bits.NvmeAddOnOprom == 1) ? TRUE : FALSE;
  if (!NvmeAddOnOpRom && HasEfiImage) {
    if (IS_CLASS3 (&PciIoDevice->Pci, PCI_CLASS_MASS_STORAGE, 8, 2)) {
      HasEfiImage = 0;      // BIOS first so ignore NVME Addon option rom
    }
  }

  DEBUG ((
    DEBUG_INFO,
    "PCI(%X,%X,%X,%X) %04X%04X L:%d E:%d P:%d C:%X ROM(%p,%lx)\n", \
    PciIoDevice->PciRootBridgeIo->SegmentNumber,
    PciIoDevice->BusNumber,
    PciIoDevice->DeviceNumber,
    PciIoDevice->FunctionNumber, \
