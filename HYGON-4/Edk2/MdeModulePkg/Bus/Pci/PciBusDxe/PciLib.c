...
PCI_BUS_SCAN_HOOK_FUNC  gPciScanBusHookFuncPtr = NULL;
  ...
  )
{
  ...
  Status = PciRootBridgeIo->Configuration(PciRootBridgeIo, (VOID**)&Descriptor);
  if (EFI_ERROR (Status)) {
    return 0;
  }
  for (; Descriptor->Desc == ACPI_ADDRESS_SPACE_DESCRIPTOR; Descriptor++){
    switch (Descriptor->ResType) {
      case ACPI_ADDRESS_SPACE_TYPE_IO:
        if (Type == 0){
          return Descriptor->AddrTranslationOffset;
        }
        break;
      case ACPI_ADDRESS_SPACE_TYPE_MEM:
        if (Descriptor->AddrSpaceGranularity == 32) {
          if ((Descriptor->SpecificFlag & EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE_PREFETCHABLE) == 0 && Type == 1) {
            return Descriptor->AddrTranslationOffset;
          } else if ((Descriptor->SpecificFlag & EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE_PREFETCHABLE) != 0 && Type == 2){
            return Descriptor->AddrTranslationOffset;
          }
        } else if (Descriptor->AddrSpaceGranularity == 64) {
          if ((Descriptor->SpecificFlag & EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE_PREFETCHABLE) == 0 && Type == 3) {
            return Descriptor->AddrTranslationOffset;
          } else if((Descriptor->SpecificFlag & EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE_PREFETCHABLE) != 0 && Type == 4){
            return Descriptor->AddrTranslationOffset;
          }
        }
        break;
  ...
  )
{
  ...
  if ((BridgeResource != NULL) && (BridgeResource->Length != 0)) {
    if (Translation == 0) {
      ...
      DEBUG ((
        DEBUG_INFO,
        "Type = %s; Base = 0x%lx/0x%lx;\tLength = 0x%lx;\tAlignment = 0x%lx\n",
        mBarTypeStr[MIN (BridgeResource->ResType, PciBarTypeMaxType)],
        BridgeResource->PciDev->PciBar[BridgeResource->Bar].BaseAddress,
        BridgeResource->PciDev->PciBar[BridgeResource->Bar].BaseAddress + ~Translation + 1,
        BridgeResource->Length,
        BridgeResource->Alignment
        ));
    }
      ...
      if (Resource->ResourceUsage == PciResUsageTypical) {
        ...
        if (Translation == 0) {
          DEBUG ((
            ...
            L"PCI",
            Resource->PciDev->PciRootBridgeIo->SegmentNumber,
        ...
        DEBUG ((
        DEBUG_INFO,
          "   Base = 0x%lx/0x%lx;\tLength = 0x%lx;\tAlignment = 0x%lx;\tOwner = %s [%04x|%02x|%02x|%02x:",
          Bar[Resource->Bar].BaseAddress,
          Bar[Resource->Bar].BaseAddress + ~Translation + 1,
          Resource->Length,
          Resource->Alignment,
          IS_PCI_BRIDGE (&Resource->PciDev->Pci)     ? L"PPB" :
          IS_CARDBUS_BRIDGE (&Resource->PciDev->Pci) ? L"P2C" :
  ...
  )
{
  ...
  for (Index = 0; Index < ResourceCount; Index++) {
    DumpBridgeResource (Resources[Index], GetResourceTranslation(Bridge->PciRootBridgeIo, Index));
  }
  ...
  )
{
  ...
  while (TRUE) {
    ...
    while (PciResAlloc->GetNextRootBridge (PciResAlloc, &RootBridgeHandle) == EFI_SUCCESS) {
      ...
      DEBUG ((
        DEBUG_INFO,
        "MaxOptionRomSize:0x%X, Mem32Bridge->Length:0x%lX, Mem32Bridge->Alignment:0x%lX\n", \
        MaxOptionRomSize,
        Mem32Bridge->Length,
        Mem32Bridge->Alignment
        ));
      //
  ...
  while (PciResAlloc->GetNextRootBridge (PciResAlloc, &RootBridgeHandle) == EFI_SUCCESS) {
    ...
    //
    DEBUG ((
      DEBUG_INFO | FixedPcdGet32 (PcdOemDebugPrintErrorLevel),
  ...
  )
{
  ...
  Address = EFI_PCI_ADDRESS (StartBusNumber, Device, Func, PCI_PRIMARY_STATUS_OFFSET);
  ...
  if ((Data16 & EFI_PCI_STATUS_CAPABILITY) == 0) {
    return 0;
  }

  Address = EFI_PCI_ADDRESS (StartBusNumber, Device, Func, PCI_CAPBILITY_POINTER_OFFSET);
  Status  = PciRootBridgeIo->Pci.Read (
                                   PciRootBridgeIo,
                                   EfiPciWidthUint8,
                                   Address,
  ...
  while ((CapabilityPtr >= 0x40) && ((CapabilityPtr & 0x03) == 0x00)) {
    Address = EFI_PCI_ADDRESS (StartBusNumber, Device, Func, CapabilityPtr);
    Status  = PciRootBridgeIo->Pci.Read (
                                     PciRootBridgeIo,
                                     EfiPciWidthUint16,
                                     Address,
    ...
    CapabilityID = (UINT8)Data16;

    if (CapabilityID == EFI_PCI_CAPABILITY_ID_PCIEXP) {
      return CapabilityPtr;
    }

    if (CapabilityPtr == (UINT8)(Data16 >> 8)) {
      break;
    }

    CapabilityPtr = (UINT8)(Data16 >> 8);
  }
  ...
  )
{
  ...
  CapabilityOffset = GetCapabilityOffset (PciRootBridgeIo, StartBusNumber, Device, Func);
  if (CapabilityOffset == 0) {
    return;
  }

  DEBUG ((DEBUG_INFO, "%a() %d CapabilityOffset[%02x] \n", __FUNCTION__, __LINE__, CapabilityOffset));

  Offset  = CapabilityOffset + OFFSET_OF (PCI_CAPABILITY_PCIEXP, LinkControl);
  Address = EFI_PCI_ADDRESS (StartBusNumber, Device, Func, Offset);
  Status  = PciRootBridgeIo->Pci.Read (
                                   PciRootBridgeIo,
                                   EfiPciWidthUint16,
                                   Address,
  ...
  LinkControl.Bits.LinkDisable = 1;
  ...
  )
{
  ...
  for (Device = 0; Device <= PCI_MAX_DEVICE; Device++) {
    ...
    for (Func = 0; Func <= PCI_MAX_FUNC; Func++) {
      if (gPciScanBusHookFuncPtr != NULL) {
        if (Bridge->Parent == NULL) {
          IsRootBridge = TRUE;
        }

        gPciScanBusHookFuncPtr (PciRootBridgeIo, StartBusNumber, Device, Func, IsRootBridge);
      }
      ...
      if (PciDevice->IsAriEnabled != IsAriEnabled) {
        DEBUG ((
          DEBUG_ERROR,
          "ERROR: %04x:%02x:%02x:%02x device ARI Feature(%x) is not consistent with others Function\n",
          Bridge->PciRootBridgeIo->SegmentNumber,
          StartBusNumber,
          Device,
          Func,
      ...
      if (IS_PCI_BRIDGE (&Pci) || IS_CARDBUS_BRIDGE (&Pci)) {
        ...
        if (IS_PCI_BRIDGE (&Pci)) {
          ...
          if (EFI_ERROR (Status)) {
            ...
            if (PcdGetBool (PcdPciBusScanOngoingAtBusResoruceIssue)) {
              DisablePcieLink (PciRootBridgeIo, StartBusNumber, Device, Func);
              *SubBusNumber = SecondBus - 1;
              StopScan      = TRUE;
            } else {
              return Status;
            }
        ...
        if (!StopScan && FeaturePcdGet (PcdPciBusHotplugDeviceSupport) && BusPadding) {
          //
        ...
        DEBUG ((
          DEBUG_INFO,
          "%a() %d LAST SET BUS NUMBER Primary[%02x] - Secondary[%02x] Subordinate[%02x] \n",
          __FUNCTION__,
          __LINE__,
          StartBusNumber,
          SecondBus,
        ...
        //
        if (PciDevice->SrIovCapabilityOffset != 0) {
  ...
  if (StopScan && (StartBusNumber >= *SubBusNumber)) {
    Status = EFI_OUT_OF_RESOURCES;
  } else {
    Status = EFI_SUCCESS;
  }

  DEBUG ((DEBUG_INFO, "%a() %d return Status[%r] \n", __FUNCTION__, __LINE__, Status));
  return Status;
}
