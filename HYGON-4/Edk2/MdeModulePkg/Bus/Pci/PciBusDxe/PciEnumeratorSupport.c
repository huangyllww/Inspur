...
PCI_DEVICE_SUPPORT                     gPciDeviceSupportFuncPtr      = NULL;
PCI_DEVICE_SRIOV_SUPPORT               gPciDeviceSrIovSupportFuncPtr = NULL;
  ...
  )
{
  UINT64      Address;
  EFI_STATUS  Status;
  BOOLEAN     IsDevSupport;

  if (gPciDeviceSupportFuncPtr != NULL) {
    IsDevSupport = gPciDeviceSupportFuncPtr (
                     PciRootBridgeIo,
                     Bridge->BusNumber,
                     Bridge->DeviceNumber,
  ...
  if (!EFI_ERROR (Status) && Bridge->IsCrsEnabled) {
    while ((Pci->Hdr).VendorId == 0x0001 && (Pci->Hdr).DeviceId == 0xffff) {
      // Software should use 100 ms wait periods only if software enables CRS Software 
  ...
  )
{
  ...
  DEBUG ((
    DEBUG_INFO,
    "PciBus: Discovered %s @ [%04x|%02x|%02x|%02x]\n",
  ...
  )
{
  ...
  if (SystemMiscSetting->PciSetting.Bits.PciVgaPaletteSnoopingSupport == 0) {
    Attributes &= ~(UINT64)(EFI_PCI_IO_ATTRIBUTE_VGA_PALETTE_IO | EFI_PCI_IO_ATTRIBUTE_VGA_PALETTE_IO_16);
  }
              ...
              switch (Ptr->AddrSpaceGranularity) {
                ...
                case 64:
                  if (Ptr->SpecificFlag == EFI_ACPI_MEMORY_RESOURCE_SPECIFIC_FLAG_CACHEABLE_PREFETCHABLE) {
                    //
                    // Some PCI Card does not support prefetchable from BAR information and then this BAR will
                    // be degraded to 32-bit non-prefetchable memory. If this PCI device support prefetchable memory,
                    // to support > 4GB memory that could be allocated for such BAR, platform could make use of
                    // Incompatible PCI Device Support Protocol to set SpecificFlag to prefetchable.
                    //
  ...
  )
{
  ...
  //
  // Check platform PCI device SR-IOV support or not
  //
  IsSrIovSupport = TRUE;
  if (gPciDeviceSrIovSupportFuncPtr != NULL) {
    IsSrIovSupport = gPciDeviceSrIovSupportFuncPtr (PciIo);
  }
  ...
  PciIoDevice->IsCrsEnabled = FALSE;
  if (PciIoDevice->IsPciExp && IS_PCI_BRIDGE (&PciIoDevice->Pci)) {
    PCI_REG_PCIE_ROOT_CONTROL  RootControl;
    UINT8                      Offset;
    Offset  = PciIoDevice->PciExpressCapabilityOffset + OFFSET_OF (PCI_CAPABILITY_PCIEXP, RootControl);
    ...
    if (RootControl.Bits.CrsSoftwareVisibility == 1) {
      DEBUG ((DEBUG_INFO, "CrsSoftwareVisibility Enabled on PPB[%04x:%02x:%02x:%02x]\n",
            PciIoDevice->PciRootBridgeIo->SegmentNumber,
            PciIoDevice->BusNumber,
            PciIoDevice->DeviceNumber,
            PciIoDevice->FunctionNumber
            ));
      PciIoDevice->IsCrsEnabled = TRUE;
    }
  }
  ...
  //
  if (PcdGetBool (PcdAriSupport) && !IsRootBridge (Bridge) && IsSrIovSupport) {
    //
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if ((Data32 & EFI_PCIE_CAPABILITY_DEVICE_CAPABILITIES_2_ARI_FORWARDING) != 0) {
        ...
        if ((Data32 & EFI_PCIE_CAPABILITY_DEVICE_CONTROL_2_ARI_FORWARDING) == 0) {
          ...
          DEBUG ((
            DEBUG_INFO,
            " ARI: forwarding enabled for PPB[%04x:%02x:%02x:%02x]\n",
            Bridge->PciRootBridgeIo->SegmentNumber,
            Bridge->BusNumber,
            Bridge->DeviceNumber,
            Bridge->FunctionNumber
  ...
  //
  IsSrIovSupport = PcdGetBool (PcdSrIovSupport) && IsSrIovSupport;

  PciIoDevice->SrIovCapabilityOffset = 0;
  PciIoDevice->SystemPageSize        = 0;
  if (PcdGetBool (PcdPciBusPreAllocateSrIovBusRange) || IsSrIovSupport) {
    Status = LocatePciExpressCapabilityRegBlock (
               PciIoDevice,
    ...
    if (!EFI_ERROR (Status)) {
      ...
      //
      if (PcdGetBool (PcdAriSupport) && (PciIoDevice->AriCapabilityOffset != 0) && (PciIoDevice->IsAriEnabled)) {
        PciIo->Pci.Read (
                     PciIo,
                     EfiPciIoWidthUint16,
      ...
      if (PciIoDevice->InitialVFs == 0) {
        ...
        if (PcdGet8 (PcdSrIovOverrideReservedBusNumber) != 0x0) {
          //
          // Fixed number of reserved bus to override the calculate number from device
          //
          PciIoDevice->ReservedBusNum = (UINT16)PcdGet8 (PcdSrIovOverrideReservedBusNumber);
        } else {
          //
