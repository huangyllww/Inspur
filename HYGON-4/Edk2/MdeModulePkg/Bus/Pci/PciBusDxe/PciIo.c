...
/**
  Detect whether the PCI configuration space is normal before accessing the IO/MEM space.

  @param  This                  A pointer to the EFI_PCI_IO_PROTOCOL instance.

  @retval EFI_SUCCESS           Data was successfully read from the PCI controller.
  @retval EFI_DEVICE_ERROR      Pciio was successfully uninstalled when an error occurred

**/
EFI_STATUS
DetectPciIoConfigSpace (
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdPciBusVerifyPciDevice)) {
    PciIoDevice = PCI_IO_DEVICE_FROM_PCI_IO_THIS (This);
    Address     = EFI_PCI_ADDRESS (PciIoDevice->BusNumber, PciIoDevice->DeviceNumber, PciIoDevice->FunctionNumber, 0);
    ...
    if (EFI_ERROR (Status) || (DidVid == 0xFFFFFFFF)) {
  ...
  )
{
  ...
  Status = DetectPciIoConfigSpace (This);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  if ((BarIndex >= PCI_SPEC_MAX_BAR) || (PciIoDevice->PciBar[BarIndex].BarType == PciBarTypeUnknown)) {
    return EFI_UNSUPPORTED;
  }
