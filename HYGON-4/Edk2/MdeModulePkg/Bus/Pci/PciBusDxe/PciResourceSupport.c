...
PCI_DEVICE_DEGRADE_HOOK_FUNC  gPciDeviceDegradeHookFuncPtr = NULL;
...
BOOLEAN
IsThisPciDeviceNeedDegrade (
  PCI_IO_DEVICE  *PciIo
  )
{
  if (gPciDeviceDegradeHookFuncPtr != NULL) {
    return gPciDeviceDegradeHookFuncPtr (
             &PciIo->Pci,
             PciIo->PciRootBridgeIo,
             gCsmOn,
             PciIo->RomSize,
             PciIo->BusNumber,
             PciIo->DeviceNumber,
             PciIo->FunctionNumber
             );
  }

  if ((PciIo->Pci.Hdr.ClassCode[2] == 0x0C) && (PciIo->Pci.Hdr.ClassCode[1] == 0x03)) {
    return TRUE;
  }

  if ((PciIo->Pci.Hdr.ClassCode[2] == 6) &&
      (PciIo->Pci.Hdr.ClassCode[1] == 4) &&
      (PciIo->Pci.Hdr.ClassCode[0] == 0))
  {
    return FALSE;
  }

  if (gCsmOn) {
    if ((PciIo->Pci.Hdr.ClassCode[2] == 3) && (PciIo->Pci.Hdr.ClassCode[1] == 2) && (PciIo->Pci.Hdr.ClassCode[0] == 0)) {
      return FALSE;
    }

    // new Base Class (12h) is defined for processing accelerators at "PCI Code and ID Assignment Specification Revision 1.5",
    // dated March 6, 2014. No sub-classes or Programming Interfaces are defined.
    if ((PciIo->Pci.Hdr.ClassCode[2] == 0x12) && (PciIo->Pci.Hdr.ClassCode[1] == 0) && (PciIo->Pci.Hdr.ClassCode[0] == 0)) {
      return FALSE;
    }
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdPciDegradeResourceForOptionRom)) {
    ...
    while (ChildDeviceLink != NULL && ChildDeviceLink != &Bridge->ChildList) {
      ...
      if (IsThisPciDeviceNeedDegrade (PciIoDevice)) {
          ...
          while (ChildNodeLink != &Mem64Node->ChildList) {
                ...
                )
            {
              if (IS_PCI_BRIDGE (&PciIoDevice->Pci)) {
                DEBUG ((DEBUG_INFO, "\t\tPPB"));  
              } else {
                DEBUG ((DEBUG_INFO, "\t\tPCI"));
              }
              DEBUG ((DEBUG_INFO, "(%X,%X,%X,%X) Mem64 -> Mem32\n",
        ...
        if (!IsListEmpty (&PMem64Node->ChildList)) {
          ...
          while (ChildNodeLink != &PMem64Node->ChildList) {
                ...
                )
            {
              ...
              DEBUG ((DEBUG_INFO, "(%X,%X,%X,%X) PMem64 -> PMem32\n",
                          PciIoDevice->PciRootBridgeIo->SegmentNumber,
                          PciIoDevice->BusNumber,
                          PciIoDevice->DeviceNumber,
                          PciIoDevice->FunctionNumber
    ...
    if (IsThisPciDeviceNeedDegrade (Bridge)) {
      if ((Bridge->Decodes & (EFI_BRIDGE_PMEM64_DECODE_SUPPORTED | EFI_BRIDGE_MEM64_DECODE_SUPPORTED)) != 0) {
        Bridge->Decodes &= ~EFI_BRIDGE_PMEM64_DECODE_SUPPORTED;
        Bridge->Decodes &= ~EFI_BRIDGE_MEM64_DECODE_SUPPORTED;
        DEBUG ((DEBUG_INFO, "PCI Devices under Bridge(%X,%X,%X,%X) Degraded to 32bit MMIO\n",
