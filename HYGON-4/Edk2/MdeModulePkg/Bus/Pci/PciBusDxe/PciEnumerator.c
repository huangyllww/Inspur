  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    if (PcdGetBool (PcdPciBusScanOngoingAtBusResoruceIssue) && (Status == EFI_OUT_OF_RESOURCES)) {
      DEBUG ((DEBUG_INFO | DEBUG_WARN, "WARNING: PCI Bus is out of resource!!!"));
    } else {
      return Status;
    }
  }
  ...
  for (ResType = 0; ResType < 5; ResType++) {
    ...
    if (Status == EFI_SUCCESS) {
      DEBUG ((
        ...
        PciResNode->PciDev->PciRootBridgeIo->SegmentNumber,
