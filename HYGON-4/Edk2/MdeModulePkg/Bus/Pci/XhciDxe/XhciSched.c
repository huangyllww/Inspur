  ...
  )
{
  ...
  if (Urb->Result == EFI_USB_ERR_BABBLE) {
    ...
    // 1) Send Reset endpoint command to transit from halt to stop state
    //
    DEBUG ((DEBUG_INFO, "Reset Slot = %x, Dci = %x\n", SlotId, Dci));
    Status = XhcResetEndpoint (Xhc, SlotId, Dci);
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "XhcDequeueTrbFromEndpoint: Reset Endpoint Failed, Status = %r\n", Status));
      goto Done;
    }
  } else {
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
    ...
    switch (EvtTrb->Completecode) {
      ...
      case TRB_COMPLETION_SPLIT_TRANSACTION_ERROR:
        CheckedUrb->Result  |= EFI_USB_ERR_BABBLE;
        CheckedUrb->Finished = TRUE;
        DEBUG ((DEBUG_ERROR, "XhcCheckUrbResult: SPLIT_TRANSACTION_ERROR! Completecode = %x\n", EvtTrb->Completecode));
        goto EXIT;
  ...
  )
{
  ...
  if (PcdGetBool (PcdXhciTrainingDelayEnable)) {
    ...
    DEBUG ((DEBUG_INFO, " *setaddress PORTSC Reg = %x \n", State));
    if (!(State & 0x01)) {
      gBS->Stall (XHC_RESET_RECOVERY_DELAY*PcdGet16 (PcdResetRecoveryDelayTimes));
    } else {
      gBS->Stall (XHC_RESET_RECOVERY_DELAY);
    }
  ...
  )
{
  ...
  for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
    ...
    if ((EpDesc->Length < sizeof (USB_ENDPOINT_DESCRIPTOR)) && (EpDesc->Length != 0)) {
      EpDesc = (USB_ENDPOINT_DESCRIPTOR *)((UINTN)EpDesc + EpDesc->Length);
      continue;
    }
