  ...
  )
{
  ...
  if (PcdGetBool (PcdXhciTrainingDelayEnable)) {
    if (Status == EFI_SUCCESS) {
      gBS->Stall (13 * XHC_1_MILLISECOND);         // 12ms is Xhci controller Training stable state
      Offset = (UINT32)(XHC_PORTSC_OFFSET + 0x10); // XHCI PORTSC Register Offset
      State  = XhcReadOpReg (Xhc, Offset);
      if ((((State & 0x1e0) >>5) == 6) && ((State & 3) == 0)) {
        // XHCI Link State is Inactive State and Abnormal state
        State |= 0x80000000;
        XhcWriteOpReg (Xhc, Offset, State);
        gBS->Stall (13 * XHC_1_MILLISECOND);
        State = XhcReadOpReg (Xhc, Offset);
        DEBUG ((DEBUG_INFO, "Warm Reset State=%x\n", State));
      }
    }
  }
