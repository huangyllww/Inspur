  ...
  )
{
  ...
  //
  PhyDetectDelay = PcdGet8 (PcdAtaPhyDetectDelay) * 1000;
  do {
    Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SERR;
...
)
{
  ...
  DEBUG((DEBUG_INFO, "%a(%d)\n",__FUNCTION__, ReadNonAtapi));

  //Polling PxIS first
  Delay = DivU64x32 (Timeout, 1000 ) + 1;  //How much time to timeout
  
  Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_IS;
  Status = EFI_TIMEOUT;                    //Set Status to timeout value
  do {       //Read PxIS
    Value = AhciReadReg (PciIo, Offset);
    //check error, PxIS.TFES, .HBFS, .HBDS, . IFS
    if (Value & (EFI_AHCI_PORT_IS_TFES | EFI_AHCI_PORT_IS_HBFS | EFI_AHCI_PORT_IS_HBDS | EFI_AHCI_PORT_IS_IFS) ) {
      DEBUG((DEBUG_INFO, "PORT_IS:%X\n", Value));
      Status = EFI_DEVICE_ERROR;
    } else {
      //check PSS or DHRS set
      if(ReadNonAtapi) {
        if (Value & EFI_AHCI_PORT_IS_PSS) {
          Status = EFI_SUCCESS;
        }
      } else {
        if (Value & EFI_AHCI_PORT_IS_DHRS)	{
          Status = EFI_SUCCESS;
        }
      }
        }
        ...
        MicroSecondDelay ( 100 );
        Delay --;
      } while ( (Delay > 0) && (Status == EFI_TIMEOUT) );


      // DEBUG ((DEBUG_INFO,"%a(),line:%d,Status:%r\n",__FUNCTION__,__LINE__,Status));
  ...
  )
{
  ...
  //    Do not retry ATA_SECURITY_UNLOCK_CMD in ATA Pass Thru.
  //    Retry here will drain the chance to allow user reenter correct password in HDDPassword which
  //    lays above AtaPassThru Driver
  //
  if (AtaCommandBlock->AtaCommand == 0xF2) {
    CmdRetryTimeOut = 1;
  } else {
    CmdRetryTimeOut = AHCI_COMMAND_RETRIES;
  }

  // BYOSOFT_OVERRIDE_END

  for (Retry = 0; Retry < CmdRetryTimeOut; Retry++) {
    AhciBuildCommand (
      PciIo,
      AhciRegisters,
      Port,
    ...
    if(AhciRegisters->IsAsmediaRaid){
      Status = PatchAsmediaRaid (PciIo, Port, Read && (AtapiCommand == 0), Timeout);
      if (EFI_ERROR (Status)) {
        DEBUG((EFI_D_ERROR, "PatchAsmediaRaid:%r\n", Status));
        break;
      }
    }
    ...
    if (Status == EFI_DEVICE_ERROR) {
      ...
      //
      // If Command Execution fails, Retain Command Error Status
      // and Recover Ahci Port for next command execution
      //
      if (EFI_ERROR (AhciRecoverPortError (PciIo, Port))) {
        break;
      }
    } else {
      break;
  ...
  )
{
  ...
  if (Task == NULL) {
    ...
    for (Retry = 0; Retry < CmdRetryTimeOut; Retry++) {
      ...
      if(AhciRegisters->IsAsmediaRaid){
        Status = PatchAsmediaRaid (PciIo, Port, FALSE, Timeout);
        if (EFI_ERROR (Status)) {
          break;
        }
      }
    ...
    if (!Task->IsStart) {
      ...
      if(AhciRegisters->IsAsmediaRaid){
        PatchAsmediaRaid (PciIo, Port, FALSE, Timeout);
      }
    }
  ...
  )
{
  ...
  if(PciId == 0x06221B21 || PciId == 0x06231B21 || PciId == 0x06251B21){
    Instance->AhciRegisters.IsAsmediaRaid = TRUE;
  } else {
    Instance->AhciRegisters.IsAsmediaRaid = FALSE;
  }
  DEBUG((EFI_D_INFO, "IsAsmediaRaid:%d\n", Instance->AhciRegisters.IsAsmediaRaid));

  if(PcdGetBool(PcdAhciKeepPortSpeedLimit)){
    PortImplementBitMap = AhciReadReg (PciIo, EFI_AHCI_PI_OFFSET);
    for (Port = 0; Port < EFI_AHCI_MAX_PORTS; Port++) {
      ...
      Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SCTL;
      Instance->AhciRegisters.SpeedAllowed[Port] = (UINT8)((AhciReadReg(PciIo, Offset) >> 4) & 0xF);
    }
  }
  ...
  for (Port = 0; Port < EFI_AHCI_MAX_PORTS; Port++) {
    if ((PortImplementBitMap & (((UINT32)BIT0) << Port)) != 0) {
      ...
      if(PcdGetBool(PcdAhciKeepPortSpeedLimit)){
        if(Instance->AhciRegisters.SpeedAllowed[Port] != 0){     // not "No speed negotiation restrictions"
          Offset = EFI_AHCI_PORT_START + Port * EFI_AHCI_PORT_REG_WIDTH + EFI_AHCI_PORT_SCTL;
          Data = AhciReadReg(PciIo, Offset);
          Data = (Data & ~EFI_AHCI_PORT_SCTL_SPD_MASK) | (Instance->AhciRegisters.SpeedAllowed[Port] << 4);
          Data = (Data & ~EFI_AHCI_PORT_SCTL_DET_MASK) | EFI_AHCI_PORT_SCTL_DET_INIT;
          AhciWriteReg(PciIo, Offset, Data);
          MicroSecondDelay (1000);
          AhciAndReg (PciIo, Offset, ~(UINT32)EFI_AHCI_PORT_SSTS_DET_MASK);        
        }
