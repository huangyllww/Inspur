  ...
  )
{
  ...
  if ((DeviceIndex == 0) || (DeviceIndex > Private->ActiveDevices + Private->ActiveOddCount)) {
    return NULL;
  }
  ...
  Node = GetFirstNode (&Private->OddList);
  while (!IsNull (&Private->OddList, Node)) {
    DeviceData = AHCI_PEI_ATA_DEVICE_INFO_FROM_THIS (Node);
    ...
    Node = GetNextNode (&Private->OddList, Node);
  }
  ...
  )
{
  ...
  UINTN       BlocksRemaining = NumberOfBlocks;
  UINT16      SectorCount;
  UINT32      Lba32      = (UINT32)StartLba;
  UINT8       *PtrBuffer = Buffer;

  MaxBlock = (UINT16)(0x10000 / DeviceData->Media.BlockSize);
  ...
  while (BlocksRemaining > 0) {
    ...
    Status = ReadAhciAtapiDevice (DeviceData, PtrBuffer, Lba32, SectorCount);
    if (EFI_ERROR (Status)) {
      break;
    }

    Lba32           += SectorCount;
    PtrBuffer       += SectorCount * DeviceData->Media.BlockSize;
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "AhciRead P:%d (%lx,%x)->%x\n", DeviceData->Port, StartLba, BufferSize, Buffer));
  ...
  if ((BufferSize % BlockSize) != 0) {
    DEBUG ((DEBUG_ERROR, "AhciRead (L%d) %d %d\n", __LINE__, BufferSize, BlockSize));
    return EFI_BAD_BUFFER_SIZE;
  }

  if (StartLba > DeviceData->Media.LastBlock) {
    DEBUG ((DEBUG_ERROR, "AhciRead (L%d) %lx %lx\n", __LINE__, StartLba, DeviceData->Media.LastBlock));
    return EFI_INVALID_PARAMETER;
  }

  NumberOfBlocks = BufferSize / BlockSize;
  if (NumberOfBlocks - 1 > DeviceData->Media.LastBlock - StartLba) {
    DEBUG ((DEBUG_ERROR, "AhciRead (L%d) %x %lx %lx\n", __LINE__, NumberOfBlocks, StartLba, DeviceData->Media.LastBlock));
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  if (DeviceData->DevType == PeiSataHarddisk) {
    Status = AccessAtaDevice (DeviceData, Buffer, StartLba, NumberOfBlocks);
  } else if (DeviceData->DevType == PeiSataCdrom) {
    Status = AccessAtapiDevice (DeviceData, Buffer, StartLba, NumberOfBlocks);
  } else {
  ...
  )
{
  ...
  *NumberBlockDevices = Private->ActiveDevices + Private->ActiveOddCount;
  ...
  )
{
  ...
  if (DeviceData->DevType == PeiSataCdrom) {
    MediaInfo->DeviceType   = IdeCDROM;
    MediaInfo->MediaPresent = DeviceData->Media.MediaPresent;
  } else {
