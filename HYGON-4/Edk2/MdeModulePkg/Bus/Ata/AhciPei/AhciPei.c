  ...
  )
{
  ...
  Status = AhciModeInitializationPei (Private);
  if (EFI_ERROR (Status)) {
    return Status;
  }
...
/*
1. In the S3 path, HddPassword OpalPei and NvmePei  are bound in hardcode fashion which restricts NVMe OPAL to be the only device supporting unlock. If ATA OPAL reports StorageSecurityPpi, OpalPei will ASSERT, 
2. ATA OPALare not widely used, So far as we know, only Samsung ATA SSD 850, 860, 870 EVO/QVO models supports OPAL.. 
 Byo has no plan to support them.
3. We use this short-term solution to stay with ATA Security Feature for ATA OPAL. ATA device SSCP is removed to avoid ASSERT mentioned in 1.
*/
#if 0
