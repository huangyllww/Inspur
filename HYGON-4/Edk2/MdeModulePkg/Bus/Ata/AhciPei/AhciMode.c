  ...
  )
{
  ...
  if (AtapiCommand != NULL) {
    CopyMem (
      &AhciRegisters->AhciCmdTable->AtapiCmd,
      AtapiCommand,
      AtapiCommandLength
      );
    ...
    AhciOrReg (AhciBar, Offset, (AHCI_PORT_CMD_DLAE | AHCI_PORT_CMD_ATAPI));
  } else {
...
EFI_STATUS
PeiAhciCheckFisReceived (
  IN UINTN              AhciBar,
  IN UINT8              Port,
  IN PEI_SATA_FIS_TYPE  FisType
  )
{
  UINT32  Offset;
  ...
  Offset        = AHCI_PORT_START + Port * AHCI_PORT_REG_WIDTH + AHCI_PORT_IS;
  PortInterrupt = AhciReadReg (AhciBar, Offset);
  if ((PortInterrupt & AHCI_PORT_IS_ERROR_MASK) != 0) {
  ...
  //
  if (((FisType == PeiSataFisD2H) && ((PortInterrupt & AHCI_PORT_IS_DHRS) != 0)) ||
      ((FisType == PeiSataFisPioSetup) && ((PortInterrupt & (AHCI_PORT_IS_PSS | AHCI_PORT_IS_DHRS)) != 0)) ||
      ((FisType == PeiSataFisDmaSetup) && ((PortInterrupt & (AHCI_PORT_IS_DSS | AHCI_PORT_IS_DHRS)) != 0)))
  {
    Offset  = AHCI_PORT_START + Port * AHCI_PORT_REG_WIDTH + AHCI_PORT_TFD;
  ...
  )
{
  ...
  // DEBUG((DEBUG_INFO, "P:%d IS:%x\n", Port, AhciReadReg(AhciBar, AHCI_PORT_START + Port * AHCI_PORT_REG_WIDTH + AHCI_PORT_IS)));
  ...
  if (Read) {
    ...
    do {
      ...
      Status = PeiAhciCheckFisReceived (AhciBar, Port, PeiSataFisPioSetup);
      if (!EFI_ERROR (Status)) {
        PioFisReceived = TRUE;
      }
      ...
      //
      Status = PeiAhciCheckFisReceived (AhciBar, Port, PeiSataFisD2H);
      if (!EFI_ERROR (Status)) {
        D2hFisReceived = TRUE;
      }
    ...
    if (EFI_ERROR (Status)) {
      ...
      DEBUG ((
        DEBUG_ERROR,
        "AhciPioTransfer (L%d) %r PioFisReceived:%d, D2hFisReceived:%d\n", \
        __LINE__,
        Status,
        PioFisReceived,
        D2hFisReceived
        ));
      DEBUG ((DEBUG_ERROR, "PrdCount:%d, DataCount:%d PortTfd:%x\n", PrdCount, DataCount, PortTfd));
    }
  } else {
    //
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "AhciNonDataTransfer (L%d) %r PortTfd:%x\n", __LINE__, Status, PortTfd));
  }
  ...
  )
{
  ...
  ZeroMem (&AtapiCmd, sizeof (AtapiCmd));
  AtapiCmd.AtapiCmd[0] = EFI_SCSI_OP_TEST_UNIT_READY;

  Status = AhciNonDataTransfer (
             DeviceData->Private,
             (UINT8)DeviceData->Port,
             (UINT8)DeviceData->PortMultiplier,
             DeviceData->FisIndex,
             &AtapiCmd,
             6,
             &AtaCommandBlock,
...
EFI_STATUS
AhciAtapiRequestSense (
  IN      PEI_AHCI_ATA_DEVICE_DATA  *DeviceData,
  IN OUT  EFI_SCSI_SENSE_DATA       *SenseData
  )
{
  EFI_STATUS              Status;
  EFI_ATA_COMMAND_BLOCK   AtaCommandBlock;
  ...
  if ((DeviceData == NULL) || (SenseData == NULL)) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  ZeroMem (&AtapiCmd, sizeof (AtapiCmd));
  AtapiCmd.AtapiCmd[0] = ATA_CMD_REQUEST_SENSE;
  AtapiCmd.AtapiCmd[4] = sizeof (EFI_SCSI_SENSE_DATA);
...
EFI_STATUS
AhciAtapiReadCapacity10 (
  IN      PEI_AHCI_ATA_DEVICE_DATA     *DeviceData,
  IN OUT  EFI_SCSI_DISK_CAPACITY_DATA  *Cap10
  )
{
  EFI_STATUS              Status;
  EFI_ATA_COMMAND_BLOCK   AtaCommandBlock;
  ...
  ZeroMem (&AtapiCmd, sizeof (AtapiCmd));
  AtapiCmd.AtapiCmd[0] = EFI_SCSI_OP_READ_CAPACITY;
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "IdentifyAtapiDevice\n"));

  Index = 0;
  while (1) {
    Status = AhciAtapiTestUnitReady (DeviceData);
    DEBUG ((DEBUG_INFO, "AhciAtapiTestUnitReady:%r\n", Status));
    if (!EFI_ERROR (Status)) {
      break;
    }

    while (1) {
      Status = AhciAtapiRequestSense (DeviceData, &SenseData);
      DEBUG ((DEBUG_INFO, "AhciAtapiRequestSense:%r\n", Status));
      if (EFI_ERROR (Status)) {
        break;
      }

      DEBUG ((DEBUG_INFO, "Sense_Key:%x %x\n", SenseData.Sense_Key, SenseData.Addnl_Sense_Code));
      if (SenseData.Sense_Key == EFI_SCSI_SK_NO_SENSE) {
        break;
      } else if ((SenseData.Sense_Key == EFI_SCSI_SK_NOT_READY) && (SenseData.Addnl_Sense_Code == EFI_SCSI_ASC_NO_MEDIA)) {
        return EFI_NO_MEDIA;
      }
    }
  ...
  Media  = &DeviceData->Media;
  Status = AhciAtapiReadCapacity10 (DeviceData, &CapacityData10);
  DEBUG ((DEBUG_INFO, "AhciAtapiReadCapacity10:%r\n", Status));
  if (EFI_ERROR (Status)) {
    return EFI_DEVICE_ERROR;
  }

  Media->BlockSize = CapacityData10.BlockSize0 + (CapacityData10.BlockSize1 << 8) +
                     (CapacityData10.BlockSize2 << 16) + (CapacityData10.BlockSize3 << 24);
  Media->LastBlock = CapacityData10.LastLba0 + (CapacityData10.LastLba1 << 8) +
                     (CapacityData10.LastLba2 << 16) + (CapacityData10.LastLba3 << 24);

  Media->ReadOnly       = TRUE;
  Media->MediaPresent   = TRUE;
  Media->InterfaceType  = MSG_SATA_DP;
  Media->RemovableMedia = FALSE;
  DEBUG ((DEBUG_INFO, "BlockSize:%x, LastBlock:%x\n", Media->BlockSize, Media->LastBlock));
...
CreateNewDevice (
  IN OUT PEI_AHCI_CONTROLLER_PRIVATE_DATA  *Private,
  IN     PEI_ATA_DEVICE_TYPE               DevType,
  ...
  )
{
  ...
  DeviceData->DevType        = DevType;

  if (DevType == PeiSataHarddisk) {
  ...
  } else if (DevType == PeiSataCdrom) {
    Status = IdentifyAtapiDevice (DeviceData);
    if (!EFI_ERROR (Status)) {
      Private->ActiveOddCount++;
      InsertTailList (&Private->OddList, &DeviceData->Link);
      return EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  UINTN                DeviceIndex;
  ATA_IDENTIFY_DATA    IdentifyData;
  PEI_ATA_DEVICE_TYPE  DevType = PeiSataUnknown;
  ...
  for (PortIndex = 1; PortIndex <= MaxPortNumber; PortIndex++) {
    ...
    if ((PortImplementBitMap & (BIT0 << Port)) != 0) {
      ...
      if ((Data & AHCI_ATAPI_SIG_MASK) == AHCI_ATA_DEVICE_SIG) {
        ...
        DevType = PeiSataHarddisk;
      } else if ((Data & AHCI_ATAPI_SIG_MASK) == AHCI_ATAPI_DEVICE_SIG) {
        Status = AhciIdentifyPacket (Private, Port, 0, PortIndex - 1, &IdentifyData);
        if (EFI_ERROR (Status)) {
          DEBUG ((DEBUG_ERROR, "%a: AhciIdentifyPacket() failed with %r\n", __FUNCTION__, Status));
          continue;
        }

        DEBUG ((DEBUG_INFO, "%a: ATA ODD found on Port %d.\n", __FUNCTION__, Port));
        DevType = PeiSataCdrom;
      } else {
        continue;
      }
  ...
  )
{
  ...
  if ((DeviceData == NULL) || (Buffer == NULL)) {
    return EFI_INVALID_PARAMETER;
  }

  DEBUG ((DEBUG_INFO, "ReadAhciAtapiDevice (%x,%x)->%x\n", StartLba, NumberOfBlocks, Buffer));
  ...
  SectorSize = (UINT16)NumberOfBlocks;
  SectorSize = SwapBytes16 (SectorSize);
  StartLba   = SwapBytes32 (StartLba);

  ZeroMem (&AtapiCmd, sizeof (AtapiCmd));
  AtapiCmd.AtapiCmd[0] = 0x28;
  CopyMem (&AtapiCmd.AtapiCmd[2], &StartLba, sizeof (StartLba));
  CopyMem (&AtapiCmd.AtapiCmd[7], &SectorSize, sizeof (SectorSize));
  ...
  Status = AhciPioTransfer (
             ...
             &AtaStatusBlock,
             Buffer,
             NumberOfBlocks * DeviceData->Media.BlockSize,
             ATA_TIMEOUT
             );
