  ...
  switch (USB_BOOT_SENSE_KEY (SenseData.SenseKey)) {
    ...
    case USB_BOOT_SENSE_NOT_READY:
      ...
      if (SenseData.Asc == USB_BOOT_ASC_NO_MEDIA) {
        ...
        Status = EFI_NOT_READY;
      } else if (SenseData.Asc == 0x30) {
        // Patch for JINGMI CD-ROM
        Status = EFI_NO_MEDIA;
      }
    ...
    case USB_BOOT_SENSE_UNIT_ATTENTION:
      ...
      if (SenseData.Asc == USB_BOOT_ASC_MEDIA_CHANGE) {
        ...
        if (GetUsbReadOnlySetByPlatform (UsbMass, &Media->ReadOnly) != EFI_SUCCESS) {
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (GetUsbReadOnlySetByPlatform (UsbMass, &PlatformReadOnly) == EFI_SUCCESS) {
      Media->ReadOnly = Media->ReadOnly || PlatformReadOnly;
    }
  }
  ...
  )
{
  ...
  while (TotalBlock > 0) {
    ...
    //
    Timeout = (UINT32) (PcdGet8(PcdUsbReadWriteBlocksTimeoutValueInSeconds) * USB_MASS_1_SECOND);
