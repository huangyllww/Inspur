...
/**
  Check if the USB device matched by USB devcie support rule.

  @param[in]  DevicePath        USB Device device path.
  @param[in]  DevDesc           USB Device Descriptor.
  @param[in]  IfDescriptor      Interface Descriptor for a USB Device Controller.
...
IsUsbDeviceMatched (
  ...
  IN EFI_USB_INTERFACE_DESCRIPTOR  *IfDescriptor,
  IN USB_DEVICE_SUPPORT_RULE       *UsbSupportRule
  )
{
  if (UsbSupportRule->DevicePath != NULL) {
    if (GetDevicePathSize (UsbSupportRule->DevicePath) > GetDevicePathSize (DevicePath)) {
      // If device path of rule is larger
      return FALSE;
    }

    if (CompareMem (UsbSupportRule->DevicePath, DevicePath, GetDevicePathSize (UsbSupportRule->DevicePath) - 4) != 0) {
...
/**
  Get USB read only setting from USB devcie support rule.

  @param[in]  UsbMass           The USB mass storage device.
  @param[out]  ReadOnly          Return USB ReadOnly setting.
  ...
  )
{
  ...
  UINT32                        RuleIndex;
  UINT16                        Priority;
  USB_DEVICE_SUPPORT_RULE       *UsbSupportRule;

  if (mPlatformUsbDeviceSupportTable == NULL) {
    Status = gBS->LocateProtocol (
                    &gPlatformUsbDeviceSupportProtocolGuid,
                    NULL,
                    (VOID **)&mPlatformUsbDeviceSupportTable
                    );
    if (EFI_ERROR (Status) || (mPlatformUsbDeviceSupportTable == NULL)) {
      return EFI_NOT_FOUND;
    }
  }

  Status = UsbMass->UsbIo->UsbGetDeviceDescriptor (UsbMass->UsbIo, &DevDesc);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = UsbMass->UsbIo->UsbGetInterfaceDescriptor (UsbMass->UsbIo, &IfDescriptor);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  *ReadOnly = FALSE;
  ...
  for (RuleIndex = 0; RuleIndex < mPlatformUsbDeviceSupportTable->Header.RuleCount; RuleIndex++) {
    UsbSupportRule = &mPlatformUsbDeviceSupportTable->Rules[RuleIndex];
    if (IsUsbDeviceMatched (UsbMass->UsbIoDevicePath, &DevDesc, &IfDescriptor, UsbSupportRule)) {
      if (UsbSupportRule->Priority > Priority) {
        if (UsbSupportRule->SupportProperty.Bits.Support == 1) {
          //
          // Currently, StorageReadSupport MUST be set 1
          //
          ASSERT (UsbSupportRule->SupportProperty.Bits.StorageReadSupport == 1);
          *ReadOnly = (UsbSupportRule->SupportProperty.Bits.StorageWriteSupport == 0) ? TRUE : FALSE;
          Priority  = UsbSupportRule->Priority;
        }
      }
    }
  }
  ...
  )
{
  ...
  //
  // If media is flagged READ only, reject WRITE operation
  //
  if (Media->ReadOnly) {
  ...
  )
{
  ...
  for (Index = 0; Index <= MaxLun; Index++) {
    ...
    UsbMass->UsbIoDevicePath = DevicePath;
    //
    // Initialize the media parameter data for EFI_BLOCK_IO_MEDIA of Block I/O Protocol.
    //
    ...
    Status = GetUsbReadOnlySetByPlatform (UsbMass, &UsbMass->BlockIoMedia.ReadOnly);
    if (EFI_ERROR (Status)) {
      UsbMass->BlockIoMedia.ReadOnly = FALSE;
    }

    DEBUG ((DEBUG_INFO, "UsbMassInitMedia: Media->ReadOnly = (%d)\n", UsbMass->BlockIoMedia.ReadOnly));
  ...
  )
{
  ...
  Status = gBS->OpenProtocol (
                  Controller,
                  &gEfiDevicePathProtocolGuid,
                  (VOID **)&UsbMass->UsbIoDevicePath,
                  This->DriverBindingHandle,
                  Controller,
                  EFI_OPEN_PROTOCOL_GET_PROTOCOL
                  );
  if (EFI_ERROR (Status) || (UsbMass->UsbIoDevicePath == NULL)) {
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  if (MaxLun == 0) {
    ...
    //
    Status = UsbMassInitMultiLun (This, Controller, Transport, Context, DevicePath, MaxLun, UsbIo);
    if (EFI_ERROR (Status)) {
      gBS->CloseProtocol (
             Controller,
             &gEfiDevicePathProtocolGuid,
  ...
  )
{
  ...
  if (NumberOfChildren == 0) {
    ...
    // BYOSOFT_OVERRIDE_START
    //   Grub2 consumes BlockIo from Hotplug device, But he has no knowledge about whether this protocol becomes VOID due to
    // device unstallment. System GP fault happens when Grub2 continues to consume uninstalled BlockIo as well as underlying UsbIo protocol.
    //   Our solution is that, when device unpluged, we replace previous BlockIo protocol with VOID one and do not free the memory
    // so that Grub2 can continue to consume the VOID version of protocol, without generating a GP exception
    //
    UsbMass->BlockIo.Media->MediaPresent = FALSE;
    UsbMass->BlockIo.Reset               = VoidUsbMassReset;
    UsbMass->BlockIo.ReadBlocks          = VoidUsbMassReadBlocks;
    UsbMass->BlockIo.WriteBlocks         = VoidUsbMassWriteBlocks;
    UsbMass->BlockIo.FlushBlocks         = VoidUsbMassFlushBlocks;
    // BYOSOFT_OVERRIDE_END
    ...
    // FreePool (UsbMass);   // Do not free UsbMass Device, so that it can continue be consumed by Grub2
