  ...
  )
{
  ...
  InitializeListHead (&(UsbMouseAbsolutePointerDevice->InputReport.ReportList));
  ...
  )
{
  ...
  HidFreeReport (UsbMouseAbsolutePointerDevice);
  FreePool (UsbMouseAbsolutePointerDevice);
  ...
  )
{
  ...
  Status = GetResolutionFromGop (&HorizontalResolution, &VerticalResolution);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  UsbMouseAbsolutePointerDev->Mode.AbsoluteMaxX = HorizontalResolution;
  UsbMouseAbsolutePointerDev->Mode.AbsoluteMaxY = VerticalResolution;
  UsbMouseAbsolutePointerDev->Mode.AbsoluteMaxZ = MAX_INT64;
  UsbMouseAbsolutePointerDev->Mode.AbsoluteMinX = 0;
  UsbMouseAbsolutePointerDev->Mode.AbsoluteMinY = 0;
  UsbMouseAbsolutePointerDev->Mode.AbsoluteMinZ = (UINT64)(-MAX_INT64);
  UsbMouseAbsolutePointerDev->Mode.Attributes   = 0x3;

  //
  // Parse report descriptor
  // Initializes the structure and data associated with the report
  //
  Status = HidParseReportDesc (
  ...
  UsbMouseAbsolutePointerDev->State.CurrentZ = 0;
  ...
  )
{
  ...
  MouseHidReportInputPaser (UsbMouseAbsolutePointerDevice, Data, DataLength);
