...
//
// Usage is a combination of usage page and usage id
// Please refer to Chapter 3 of <HID Usage Table> for details.
// <HID Usage Table> link : https://usb.org/document-library/hid-usage-tables-122
//
#define HID_USAGE_BUTTON_1  0x90001
#define HID_USAGE_BUTTON_2  0x90002
#define HID_USAGE_BUTTON_3  0x90003

#define HID_USAGE_AXIS_X        0x10030
#define HID_USAGE_AXIS_Y        0x10031
#define HID_USAGE_MOUSE_WHEEL   0x10038
#define HID_DATA_RELATIVE_FLAG  BIT2

typedef  struct _HID_FIELD   HID_FIELD;
typedef  struct _HID_REPORT  HID_REPORT;

#define HID_MAX_BUFFER_SIZE          SIZE_16KB
#define HID_MAX_USAGES               12288
#define HID_DEFAULT_NUM_COLLECTIONS  16

#define USAGE_ID_PREFIX_PAGE(Id, Page)  (((UINT32)Id & 0xFFFF) | (((UINT32)Page & 0xFFFF) << 16))

/*
 * This is the local environment. It is persistent up the next main-item.
 */
typedef struct {
  UINT32    Usage[HID_MAX_USAGES];                    /* usage array */
  UINT8     UsageSize[HID_MAX_USAGES];                /* usage size array */
  UINT32    UsageIndex;
  UINT32    UsageMin;
  UINT32    Delimiter;
} HID_LOCAL;
...
typedef struct {
  UINT32    HidUsage;             /* hid usage code */
  UINT32    UsageIndex;           /* index into usage array */
} HID_USAGE;

struct _HID_FIELD {
  UINT32       Physical;      /* physical usage for this field */
  UINT32       Logical;       /* logical usage for this field */
  UINT32       Application;   /* application usage for this field */
  HID_USAGE    *Usage;        /* usage table for this function */
  UINT32       UsageMax;      /* maximum usage index */
  UINT32       Flags;         /* main-item flags (i.e. volatile,array,constant) */
  UINT32       BitOffset;     /* bit offset in the report */
  UINT32       ReportSize;    /* size of this field in the report */
  UINT32       ReportCount;   /* number of this field in the report */
  UINT16       ReportType;    /* (input,output,feature) */
  INT32        LogicalMin;
  INT32        LogicalMax;
  // INT32        PhysicalMin;
  // INT32        PhysicalMax;
  // INT32        UnitExponent;
  // UINT32       Unit;
  UINT16       Index;         /* index into report->field[] */
};

#define HID_REPORT_FIELDS_MAX  256
#define HID_REPORT_ID_MAX      256

typedef struct {
  BOOLEAN       PrefixId;
  LIST_ENTRY    ReportList;
  HID_REPORT    *ReportIdMap[HID_REPORT_ID_MAX];
} HID_REPORT_ENUM;
...
typedef struct {
  ...
  HID_REPORT_ENUM                  InputReport;
  UINT32                           ApplicationMax;
} USB_MOUSE_ABSOLUTE_POINTER_DEV;

typedef  USB_MOUSE_ABSOLUTE_POINTER_DEV HID_MOUSE_DEV;

struct _HID_REPORT {
  LIST_ENTRY    List;
  UINT32        Id;                           /* id of this report */
  UINT32        Type;                         /* report type */
  UINT32        Application;                  /* application usage for this report */
  HID_FIELD     *Field[HID_REPORT_FIELDS_MAX];
  UINT16        FieldMax;
  UINT32        BitSize;
};
