...
typedef enum {
  Sync_Volatile_Variable_Cache,
  Sync_NV_Variable_Cache
} CACHE_SYNC_TYPE;
...
/**
 ...
 Synchronize the Volatile Cache to obtain the values of variables in the Volatile Cache
  ...
  @param[in]  SyncType       Synchronizing input parameters.

**/
typedef
EFI_STATUS
(EFIAPI *AUTH_VAR_LIB_SYNCRONIZE_VARIABLE_CACHE)(
  IN CACHE_SYNC_TYPE           SyncType
  );
...
typedef struct {
  ...
  // When the SecureBoot module performs Transition Mode,
  // Synchronize the Volatile Cache to obtain the values of variables in the Volatile Cache
  AUTH_VAR_LIB_SYNCRONIZE_VARIABLE_CACHE    SyncronizeVariableCache;
