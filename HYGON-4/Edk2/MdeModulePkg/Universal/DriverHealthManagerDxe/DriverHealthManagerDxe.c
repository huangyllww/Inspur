...
EFI_HANDLE                           mDriverHealthManagerHandle;
  ...
  )
{
  ...
  mDriverHealthManagerHandle = NULL;
  Status                     = gBS->InstallMultipleProtocolInterfaces (
                                      &mDriverHealthManagerHandle,
  ...
  )
{
  ...
  EFI_GUID                     *ClassGuid;
  EFI_GUID                     VfrFormsetGuid;
  BOOLEAN                      IsFirstFormset = TRUE;
    ...
    if (PackageHeader.Type == EFI_HII_PACKAGE_FORMS) {
      ...
      for (Offset2 = sizeof (EFI_HII_PACKAGE_HEADER); Offset2 < PackageHeader.Length; Offset2 += ((EFI_IFR_OP_HEADER *)OpCodeData)->Length) {
            ...
            (((EFI_IFR_OP_HEADER *)OpCodeData)->Length > OFFSET_OF (EFI_IFR_FORM_SET, Flags)))
        {
          ...
          if (IsFirstFormset) {
            CopyMem (&VfrFormsetGuid, &((EFI_IFR_FORM_SET *)OpCodeData)->Guid, sizeof (EFI_GUID));
  ...
  if (!IsFirstFormset) {
    CopyMem (FormsetGuid, &VfrFormsetGuid, sizeof (EFI_GUID));
    return EFI_SUCCESS;
  }
...
/**
  Unload the DriverHealthManagerDxe form.

  @param[in]  ImageHandle         The driver's image handle.

  @retval     EFI_SUCCESS         The DriverHealthManagerDxe form is unloaded.
  @retval     Others              Failed to unload the form.

**/
EFI_STATUS
EFIAPI
DriverHealthManagerDxeUnload (
  ...
  )
{
  ...
  //
  if (mDriverHealthManagerHiiHandle != NULL) {
    HiiRemovePackages (mDriverHealthManagerHiiHandle);
    mDriverHealthManagerHiiHandle = NULL;
  }

  //
  // Uninstall HII Config Access Protocol
  //
  if (mDriverHealthManagerHandle != NULL) {
