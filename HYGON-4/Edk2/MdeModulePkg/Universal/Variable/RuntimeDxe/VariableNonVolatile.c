...
extern EFI_PHYSICAL_ADDRESS    mNvFlashStorageBase;
extern UINT32                  mNvFlashStorageSize;
  ...
  )
{
  ...
  mNvFlashStorageBase = NvStorageBase;
  mNvFlashStorageSize = NvStorageSize;
  ...
  if ((sizeof (VARIABLE_STORE_HEADER) > VariableStoreLength) || (VariableStore->Size != VariableStoreLength)) {
    FreePool (NvStorageData);
  ...
  )
{
  ...
  if (PcdGetBool (PcdEmuVariableNvModeEnable)) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      //
      // Use Emulation variable storage
      //
      Status = InitEmuNonVolatileVariableStore (&VariableStoreBase);
      if (EFI_ERROR (Status)) {
        return Status;
