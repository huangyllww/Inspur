...
EFI_PHYSICAL_ADDRESS    mNvFlashStorageBase = 0;
UINT32                  mNvFlashStorageSize = 0;
...
BOOLEAN  mDefaultSettingInit = FALSE;
...
AUTH_VAR_LIB_CONTEXT_IN  mAuthContextIn = {
  ...
  // Byo Changes
  // Synchronize the Volatile Cache or NV Cache.
  // When the SecureBoot module performs Transition Mode,
  // Synchronize the Volatile Cache to obtain the values of variables in the Volatile Cache
  VariableExLibSyncronizeVariableCache,
};
  ...
  )
{
  ...
  //
  // Try to find the default variable hob built in DXE phase
  //
  if ((Global->HobVariableBase == 0) && (mDefaultSettingInit == FALSE)) {
    if (mVariableModuleGlobal->VariableGlobal.AuthFormat) {
      GetHobVariableStore (&gEfiAuthenticatedVariableGuid);
    } else {
      GetHobVariableStore (&gEfiVariableGuid);
    }
  }
  ...
  )
{
  ...
  //
  // Call Hook first to get variable data
  //
  Status = BeforeGetVariableHook (
             VariableName,
             VendorGuid,
             Attributes,
             DataSize,
  ...
  if ((Variable.CurrPtr == NULL) || EFI_ERROR (Status)) {
    //
    // Call Hook after to get variable data
    //
    ReturnStatus = AfterGetVariableHook (
                     VariableName,
                     VendorGuid,
                     Attributes,
                     DataSize,
  ...
  )
{
  ...
  //
  // Trig Hook before Set variable success
  //
  Status = BeforeSetVariableHook (VariableName, VendorGuid, Attributes, DataSize, Data);
  if (EFI_ERROR (Status)) {
    goto Done;
  }
  ...
  if (!EFI_ERROR (Status)) {
    //
    // Trig Hook after Set variable success
    //
    AfterSetVariableHook (VariableName, VendorGuid, Attributes, DataSize, Data);
  }
  ...
  )
{
  ...
  if ((Attributes & EFI_VARIABLE_ATTRIBUTES_MASK) == 0) {
    ...
    //
    return EFI_INVALID_PARAMETER;
  } else if ((Attributes & (EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS)) == 0) {
    return EFI_INVALID_PARAMETER;
  ...
  )
{
  ...
  if (GuidHob != NULL) {
    //
    // Default setting has been created.
    //
    mDefaultSettingInit = TRUE;
  }
...
EFI_STATUS
ReInitNonVolatileVariableStore (
  VOID
  )
{
  VARIABLE_HEADER             *Variable;
  ...
  if ((mNvFvHeaderCache == NULL) || (mNvVariableCache == NULL)) {
    return EFI_NOT_READY;
  }
  if ((mNvFlashStorageBase == 0) || (mNvFlashStorageSize == 0)) {
    return EFI_NOT_READY;
  }

  FvHeader = mNvFvHeaderCache;
  CopyMem (FvHeader, (UINT8 *)(UINTN)mNvFlashStorageBase, mNvFlashStorageSize);
  ASSERT (FvHeader->Signature == EFI_FVH_SIGNATURE && CompareGuid (&gEfiSystemNvDataFvGuid, &FvHeader->FileSystemGuid));
  ...
  Variable    = GetStartPointer (mNvVariableCache);
  VariableEnd = GetEndPointer (mNvVariableCache);
  while (IsValidVariableHeader (Variable, VariableEnd)) {
    NextVariable = GetNextVariablePtr (Variable, mVariableModuleGlobal->VariableGlobal.AuthFormat);
    VariableSize = (UINTN)NextVariable - (UINTN)Variable;
...
/**
 ...
 Synchronize the Volatile Cache to obtain the values of variables in the Volatile Cache
  ...
  @param[in]  SyncFlag       Synchronizing input parameters.

**/
EFI_STATUS
EFIAPI
VariableExLibSyncronizeVariableCache (
  CACHE_SYNC_TYPE  SyncType
  )
{
  EFI_STATUS              Status;
  VARIABLE_RUNTIME_CACHE  *VolatileCacheInstance;

  if (SyncType == Sync_NV_Variable_Cache) {
    VolatileCacheInstance = &(mVariableModuleGlobal->VariableGlobal.VariableRuntimeCacheContext.VariableRuntimeNvCache);
  } else if (SyncType == Sync_Volatile_Variable_Cache) {
    VolatileCacheInstance = &(mVariableModuleGlobal->VariableGlobal.VariableRuntimeCacheContext.VariableRuntimeVolatileCache);
  } else {
    return EFI_UNSUPPORTED;
  }
  ...
  //
  // Non-SMM Runtime Variable Solution doesn't need Cache, so synchronization is not required
  //
  if (VolatileCacheInstance->Store == NULL) {
    return EFI_SUCCESS;
  }
