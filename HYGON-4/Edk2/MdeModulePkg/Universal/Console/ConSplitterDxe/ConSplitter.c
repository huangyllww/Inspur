...
EFI_EVENT mVTextOutRegisterEvent = NULL;
VOID *mVTextOutRegistration = NULL;
...
/**
  gEfiVGopSimpleTextOutProtocolGuid registration notification callback
...
VOID
EFIAPI
VTextOutRegisteredCallback (IN  EFI_EVENT    Event,
                            IN  VOID         *Context)
{
  EFI_STATUS                           Status;
  EFI_HANDLE                           ControllerHandle;
  UINTN                                BufferSize      = sizeof(ControllerHandle);
  ...
  Status = ConSplitterTextOutAddDevice (&mConOut, mConOut.VGopTextOut, NULL, NULL);
  ...
  )
{
  ...
  gBS->CreateEvent (EVT_NOTIFY_SIGNAL, TPL_NOTIFY, VTextOutRegisteredCallback, NULL, &mVTextOutRegisterEvent);
  gBS->RegisterProtocolNotify (&gEfiVGopSimpleTextOutProtocolGuid, mVTextOutRegisterEvent, &mVTextOutRegistration);
...
Done:
      ...
      (Mode->VerticalResolution == CurrentGraphicsOutputMode->Info->VerticalResolution))
  {
    ...
    //
    if (Private->Gop != NULL) {
      Status = Private->Gop->SetMode (Private->Gop, (UINT32)CurrentIndex);
    } else {
  ...
  )
{
  ...
  MaxModeInfo.Row    = 0;
  ModeInfo.Column    = 0; // PcdGet32 (PcdConOutColumn);
  ModeInfo.Row       = 0; // PcdGet32 (PcdConOutRow);
  DEBUG ((DEBUG_INFO, "%a(%d) PcdConOutColumn: %d PcdConOutRow: %d\n", __FILE__, __LINE__, ModeInfo.Column, ModeInfo.Row));
  ...
  for (Mode = 0; Mode < MaxMode; Mode++) {
    ...
    if (!EFI_ERROR (Status)) {
      DEBUG ((DEBUG_INFO, "%a(%d) Mode: %d Col: %d Row: %d\n", __FILE__, __LINE__, Mode, Col, Row));
      if ((ModeInfo.Column != 0) && (ModeInfo.Row != 0)) {
        ...
        //
        if (Col >= MaxModeInfo.Column) {
          MaxModeInfo.Column = Col;
          MaxModeInfo.Row    = Row;
          PreferMode         = Mode;
        } else if ((Col == MaxModeInfo.Column) && (Row > MaxModeInfo.Row)) {
          MaxModeInfo.Column = Col;
          MaxModeInfo.Row    = Row;
          PreferMode         = Mode;
        }
  ...
  //
  DEBUG ((DEBUG_INFO, "%a(%d) Set Mode: %d Column: %d Row: %d\n", __FILE__, __LINE__, PreferMode, MaxModeInfo.Column, MaxModeInfo.Row));
  ...
  )
{
  ...
  if (UnInstallFlag) {
    ConSplitterTextOutDeleteDevice (Private, Private->VGopTextOut);
    Private->VGopTextOut = NULL;
  }
  ...
  )
{
  ...
  if (Private->CurrentNumberOfConsoles > 0) {
    if (Private->VGopTextOut != NULL) {
      Private->TextOutMode.CursorColumn = Private->VGopTextOut->Mode->CursorColumn;
      Private->TextOutMode.CursorRow    = Private->VGopTextOut->Mode->CursorRow;
    } else {
  ...
  )
{
  ...
  //
  for (Index = 0, ReturnStatus = EFI_SUCCESS; Index < Private->CurrentNumberOfConsoles; Index++) {
    if (Private->TextOutList[Index].GraphicsOutput != NULL && Private->VGopTextOut != NULL) {
      continue;
    }
  ...
  if (Private->VGopTextOut != NULL) {
    Status = Private->VGopTextOut->TestString (Private->VGopTextOut, WString);
  }
