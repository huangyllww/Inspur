...
CHAR16  mCursorHide[]              = { ESC, '[', '?', '2', '5', 'l', 0 };
CHAR16  mCursorshow[]              = { ESC, '[', '?', '2', '5', 'h', 0 };
  ...
  )
{
  ...
  Columns = TerminalDevice->TerminalConsoleModeData[TerminalDevice->SimpleTextOutput.Mode->Mode].Columns;
  Rows    = TerminalDevice->TerminalConsoleModeData[TerminalDevice->SimpleTextOutput.Mode->Mode].Rows;

  if (Columns == 0 || Rows == 0) {
    DEBUG((DEBUG_ERROR, "%a(%d) QueryMode Error. \n", __FILE__, __LINE__));
    return;
  }

  if (TerminalDevice->StringBuffer != NULL) {
    FreePool(TerminalDevice->StringBuffer);
  }

  if (TerminalDevice->AttributeBuffer != NULL) {
    FreePool(TerminalDevice->AttributeBuffer);
  }

  TerminalDevice->StringBuffer = AllocateZeroPool(Columns * Rows * sizeof(CHAR16));
  TerminalDevice->AttributeBuffer = AllocateZeroPool(Columns * Rows * sizeof(UINTN));
}
  ...
  )
{
  ...
  UINT8                        ValidBytes;
  CHAR8                        CrLfStr[2];
  BOOLEAN                      WideCharacterFlag = FALSE;
  ...
  for ( ; *WString != CHAR_NULL; WString++) {
    if (*WString == NARROW_CHAR) {
      WideCharacterFlag = FALSE;
      continue;
    } else if (*WString == WIDE_CHAR) {
      WideCharacterFlag = TRUE;
      continue;
    }

    if (TerminalDevice->SerialIo->Mode->DataBits < 8) {
    ...
    switch (*WString) {
      case CHAR_BACKSPACE:
        if (Mode->CursorColumn > 0) {
          if (TerminalDevice->OutputEscChar == FALSE && 
              TerminalDevice->AttributeBuffer != NULL && 
              TerminalDevice->StringBuffer != NULL) {
            TerminalDevice->AttributeBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] = This->Mode->Attribute;
            TerminalDevice->StringBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] = L' ';
          }
          Mode->CursorColumn--;
          Length     = 2;
          CrLfStr[1] = CHAR_BACKSPACE;
          CrLfStr[0] = ' ';
          Status     = TerminalDevice->SerialIo->Write (
                                                   TerminalDevice->SerialIo,
                                                   &Length,
      ...
      default:
        ...
        if (TerminalDevice->OutputEscChar == FALSE && 
            ...
            TerminalDevice->StringBuffer != NULL) {
          TerminalDevice->AttributeBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] = This->Mode->Attribute;
          if (WideCharacterFlag) {
            TerminalDevice->AttributeBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] |= EFI_WIDE_ATTRIBUTE;
          }
          TerminalDevice->StringBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] = *WString;
          if (TerminalDevice->AttributeBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn] & EFI_WIDE_ATTRIBUTE) {
            TerminalDevice->StringBuffer[(Mode->CursorRow * MaxColumn) + Mode->CursorColumn + 1] = 0;
          }
        }
  ...
  )
{
  ...
  ReallocateBaupDataPool(TerminalDevice);
  ...
  )
{
  ...
  //
  if ((This->Mode->Attribute & ~EFI_WIDE_ATTRIBUTE) == (INT32)(Attribute & ~EFI_WIDE_ATTRIBUTE)) {
  ...
  )
{
  ...
  Columns = TerminalDevice->TerminalConsoleModeData[This->Mode->Mode].Columns;
  Rows    = TerminalDevice->TerminalConsoleModeData[This->Mode->Mode].Rows;

  if (TerminalDevice->AttributeBuffer != NULL && TerminalDevice->StringBuffer != NULL) {
    SetMemN(TerminalDevice->AttributeBuffer, sizeof(UINTN) * Columns * Rows, This->Mode->Attribute);
    SetMem16(TerminalDevice->StringBuffer, sizeof(CHAR16) * Columns * Rows, L' ');
  }
  ...
  )
{
      ...
      ((UINTN)Mode->CursorRow == Row))
  {
    if ((UINTN)Mode->CursorColumn > Column) {
      mCursorBackwardString[FW_BACK_OFFSET + 0] = (CHAR16)('0' + (((Mode->CursorColumn - Column) / 100) % 10));
      mCursorBackwardString[FW_BACK_OFFSET + 1] = (CHAR16)('0' + (((Mode->CursorColumn - Column) / 10) % 10));
      ...
      mCursorForwardString[FW_BACK_OFFSET + 0] = (CHAR16)('0' + (((Column - Mode->CursorColumn) / 100) % 10));
      mCursorForwardString[FW_BACK_OFFSET + 1] = (CHAR16)('0' + (((Column - Mode->CursorColumn) / 10) % 10));
    ...
    mSetCursorPositionString[ROW_OFFSET + 0]    = (CHAR16)('0' + (((Row + 1) / 100) % 10));
    mSetCursorPositionString[ROW_OFFSET + 1]    = (CHAR16)('0' + (((Row + 1) / 10) % 10));
    mSetCursorPositionString[ROW_OFFSET + 2]    = (CHAR16)('0' + ((Row + 1) % 10));
    mSetCursorPositionString[COLUMN_OFFSET + 0] = (CHAR16)('0' + (((Column + 1) / 100) % 10));
    mSetCursorPositionString[COLUMN_OFFSET + 1] = (CHAR16)('0' + (((Column + 1) / 10) % 10));
...
/**
  ...
  Enables or disables the visibility of the cursor in the  terminal console.
  ...
  )
{
  ...
  TerminalDevice->OutputEscChar = TRUE;
  This->OutputString (This, Visible ? mCursorshow : mCursorHide);
  TerminalDevice->OutputEscChar = FALSE;
