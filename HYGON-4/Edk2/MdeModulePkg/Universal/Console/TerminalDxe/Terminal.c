  ...
  )
{
  ...
  if (RemainingDevicePath != NULL) {
    ...
    if (!IsDevicePathEnd (RemainingDevicePath)) {
      ...
      //
      // only supports AUTO, PC ANSI, VT100, VT100+, VT-UTF8, TtyTerm
  ...
  )
{
  ...
  if (WString == NULL && Attribute == NULL) {
    return EFI_INVALID_PARAMETER;
  }

  TerminalDevice = TERMINAL_TERMINAL_DATA_BACKUP_THIS(This);

  if (TerminalDevice->StringBuffer == NULL || TerminalDevice->AttributeBuffer == NULL) {
    return EFI_UNSUPPORTED;
  }
  ...
  if (*WString == NULL) {
    *WString = AllocatePool(Width * Height * sizeof(CHAR16));
  }

  if (*Attribute == NULL) {
    *Attribute = AllocatePool(Width * Height * sizeof(UINTN));
  }

  for (Index = 0; Index < Height; Index++) {
    CopyMem(*WString + (Index * Width), TerminalDevice->StringBuffer + (((Y + Index) * Columns) + X), sizeof(CHAR16) * Width);
    CopyMem(*Attribute + (Index * Width), TerminalDevice->AttributeBuffer + (((Y + Index) * Columns) + X), sizeof(UINTN) * Width);
  }
  ...
  )
{
  ...
  TerminalDevice->TerminalDataBackup.GetData = GetTerminalBackupData;
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                  ...
                  &TerminalDevice->TerminalDataBackup,
                  NULL
                  );
  if (!EFI_ERROR (Status)) {
    Status = gBS->OpenProtocol (
