  ...
  )
{
  ...
  if(RegsiteredData->KeyState.KeyShiftState == EFI_SHIFT_STATE_VALID &&
     InputData->KeyState.KeyShiftState == 0){
    return TRUE;
  }

  if(((RegsiteredData->Key.ScanCode) < SCAN_F1) || ((RegsiteredData->Key.ScanCode)> SCAN_F12) || (PcdGetBool(PcdTerminalCtrlFnSupport))){
  ...
  )
{
  ...
  while (TRUE) {
    ...
    HasKey = EfiKeyFiFoForNotifyRemoveOneKey (TerminalDevice->EfiKeyFiFoForNotify, &KeyData);
  ...
  )
{
  ...
  if (IsEfiKeyFiFoForNotifyEmpty (EfiKeyFiFo)) {
    ...
    // FIFO is empty
    //
    ZeroMem(Output, sizeof(EFI_KEY_DATA));
    return FALSE;
  }
  ...
  )
{
  ...
  if (CtrlFlag && (PcdGetBool(PcdTerminalCtrlFnSupport)))
  {
    KeyData.KeyState.KeyShiftState |= EFI_LEFT_CONTROL_PRESSED | EFI_SHIFT_STATE_VALID;
  }
...
// ctrl + 'z'('Z') : 0x1A
  if(KeyData.Key.UnicodeChar >= 0x01 && KeyData.Key.UnicodeChar <= 0x1A){
    if(PcdGet32(PcdTerminalCtrlCharSupportMask) & (1 << (KeyData.Key.UnicodeChar - 1))){
      KeyData.KeyState.KeyShiftState |= EFI_LEFT_CONTROL_PRESSED | EFI_SHIFT_STATE_VALID;
  ...
  for (Link = GetFirstNode (NotifyList); !IsNull (NotifyList, Link); Link = GetNextNode (NotifyList, Link)) {
    ...
    if (IsKeyRegistered (&CurrentNotify->KeyData, &KeyData)) {
      ...
      //
      EfiKeyFiFoForNotifyInsertOneKey (TerminalDevice->EfiKeyFiFoForNotify, &KeyData);
  ...
  CopyMem (&TerminalDevice->EfiKeyFiFo->Data[Tail], &KeyData, sizeof (EFI_KEY_DATA));
  ...
  )
{
  ...
  CopyMem (Output, &TerminalDevice->EfiKeyFiFo->Data[Head], sizeof (EFI_KEY_DATA));
  ...
  )
{
  ...
  if ((InputState & INPUT_STATE_ESC) != 0) {
    ...
    Key.UnicodeChar = 0;
    EfiKeyFiFoInsertOneKey (TerminalDevice, &Key, CtrlFlag);
  }
...
Putty function key map:
  ...
  | HOME    | 0x05 | ESC [ 1 ~ | ESC [ 1 ~   | ESC [ 2 ~   | ESC [ 1 ~   | ESC [ H |
  | END     | 0x06 | ESC [ 4 ~ | ESC [ 4 ~   | ESC [ 5 ~   | ESC [ 4 ~   | ESC [ F |
  | INSERT  | 0x07 | ESC [ 2 ~ | ESC [ 2 ~   | ESC [ 1 ~   | ESC [ 2 ~   | ESC [ L |
  | DELETE  | 0x08 | ESC [ 3 ~ | ESC [ 3 ~   | ESC [ 4 ~   | ESC [ 3 ~   | ~       |
  | PG UP   | 0x09 | ESC [ 5 ~ | ESC [ 5 ~   | ESC [ 3 ~   | ESC [ 5 ~   | ESC [ I |
  | PG DOWN | 0x0A | ESC [ 6 ~ | ESC [ 6 ~   | ESC [ 6 ~   | ESC [ 6 ~   | ESC [ G |
  +=========+======+===========+=============+=============+=============+=========+

MobaXterm function key map:
  +=========+======+===========+=============+=============+=============+=========+
  |         | EFI  |           |             |             |             |         |
  |         | Scan |           |             |  Normal     |             |         |
  |   KEY   | Code |  VT100+   | Xterm R6    |  VT400      | Vt100       | SCO     |
  +=========+======+===========+=============+=============+=============+=========+
  | F1      | 0x0B | ESC O P   | ESC O P     | ESC [ 1 1   | ESC O P     | ESC [ M |
  | F2      | 0x0C | ESC O Q   | ESC O Q     | ESC [ 1 2   | ESC O Q     | ESC [ N |
  | F3      | 0x0D | ESC O R   | ESC O R     | ESC [ 1 3   | ESC O R     | ESC [ O |
  | F4      | 0x0E | ESC O S   | ESC O S     | ESC [ 1 4   | ESC O S     | ESC [ P |
  | F5      | 0x0F | ESC O T   | ESC [ 1 5   | ESC [ 1 5   | ESC [ 1 5   | ESC [ Q |
  | F6      | 0x10 | ESC O U   | ESC [ 1 7   | ESC [ 1 7   | ESC [ 1 7   | ESC [ R |
  | F7      | 0x11 | ESC O V   | ESC [ 1 8   | ESC [ 1 8   | ESC [ 1 8   | ESC [ S |
  | F8      | 0x12 | ESC O W   | ESC [ 1 9   | ESC [ 1 9   | ESC [ 1 9   | ESC [ T |
  | F9      | 0x13 | ESC O X   | ESC [ 2 0   | ESC [ 2 0   | ESC [ 2 0   | ESC [ U |
  ...
  | F11     | 0x15 | ESC O Z   | ESC [ 2 3   | ESC [ 2 3   | ESC [ 2 3   | ESC [ W |
  | F12     | 0x16 | ESC O [   | ESC [ 2 4   | ESC [ 2 4   | ESC [ 2 4   | ESC [ X |
  | HOME    | 0x05 | ESC [ H   | ESC [ H     | ESC [ 2 ~   | ESC [ H     | ESC [ H |
  | END     | 0x06 | ESC [ F   | ESC [ F     | ESC [ 5 ~   | ESC [ F     | ESC [ F |
  | INSERT  | 0x07 | ESC [ 2 ~ | ESC [ 2 ~   | ESC [ H     | ESC [ 2 ~   | ESC [ L |
  | DELETE  | 0x08 | ESC [ 3   | ESC [ 3     | ESC [ F     | ESC [ 3     | DEL     |
  | PG UP   | 0x09 | ESC [ 5   | ESC [ 5     | ESC [ 3     | ESC [ 5     | ESC [ I |
  | PG DOWN | 0x0A | ESC [ 6   | ESC [ 6     | ESC [ 6     | ESC [ 6     | ESC [ G |
  +=========+======+===========+=============+=============+=============+=========+

  Electerm function key map:
  +========+======+==============+
  |        | EFI  |              |
  |        | Scan |              |
  | KEY    | Code | PC ANSI      |
  ...
  Ctrl+Fn support need to set PcdTerminalCtrlFnSupport = TRUE
  
  MobaXterm Ctrl+Fn key map:
  +===========+=================+==========+
  |           |                 |          |
  | KEY       |     VT100       |    SCO   |
  +===========+=================+==========+
  | Ctrl+F1   | ESC [ 1 ; 5 P   | ESC [ k  |
  | Ctrl+F2   | ESC [ 1 ; 5 Q   | ESC [ l  |
  | Ctrl+F3   | ESC [ 1 ; 5 R   | ESC [ m  |
  | Ctrl+F4   | ESC [ 1 ; 5 S   | ESC [ n  |
  | Ctrl+F5   |                 | ESC [ o  |
  | Ctrl+F6   |                 | ESC [ p  |
  | Ctrl+F7   |                 | ESC [ q  |
  | Ctrl+F8   |                 | ESC [ r  |
  ...
  Electerm Ctrl+Fn key map:
  +===========+================+================+
  |           |                |                |
  | KEY       |      ANSI      |      VT100     |
  +===========+================+================+
  | Ctrl+F1   | ESC [ 1 ; P    | ESC [ 1 ; P    |  
  | Ctrl+F2   | ESC [ 1 ; Q    | ESC [ 1 ; Q    | 
  | Ctrl+F3   | ESC [ 1 ; R    | ESC [ 1 ; R    |  
  | Ctrl+F4   | ESC [ 1 ; S    | ESC [ 1 ; S    |  
  | Ctrl+F5   | ESC [ 1 5 ; 5  | ESC [ 1 5 ; 5  | 
  | Ctrl+F6   | ESC [ 1 7 ; 5  | ESC [ 1 7 ; 5  | 
  | Ctrl+F7   | ESC [ 1 8 ; 5  | ESC [ 1 8 ; 5  | 
  | Ctrl+F8   | ESC [ 1 9 ; 5  | ESC [ 1 9 ; 5  | 
  | Ctrl+F9   | ESC [ 2 0 ; 5  | ESC [ 2 0 ; 5  | 
  | Ctrl+F10  | ESC [ 2 1 ; 5  | ESC [ 2 1 ; 5  |
  | Ctrl+F11  | ESC [ 2 3 ; 5  | ESC [ 2 3 ; 5  |
  | Ctrl+F12  | ESC [ 2 4 ; 5  | ESC [ 2 4 ; 5  |

  Putty Ctrl+Fn key map:
  +===========+==========+
  |           |          |
  | KEY       |    SCO   |
  +===========+==========+
    ...
    switch (TerminalDevice->InputState) {
      ...
      case INPUT_STATE_ESC:
        ...
        if ((UnicodeChar == 'O') && ((TerminalDevice->TerminalType == TerminalTypeVt100) ||
                                     ...
                                     (TerminalDevice->TerminalType == TerminalTypeAuto) ||
                                     (TerminalDevice->TerminalType == TerminalTypeVt100Plus) ||
                                     (TerminalDevice->TerminalType == TerminalTypePcAnsi)))
        {
      ...
      case INPUT_STATE_ESC | INPUT_STATE_O:
        ...
        } else if (TerminalDevice->TerminalType == TerminalTypeAuto) {
          switch (UnicodeChar) {
            case 'P':
              Key.ScanCode = SCAN_F1;
              break;
            ...
            (TerminalDevice->TerminalType == TerminalTypeSCO))
        {
          switch (UnicodeChar) {
            ...
            case 'H':
              ...
              {
                Key.ScanCode = SCAN_HOME;
              } else if (TerminalDevice->TerminalType == TerminalTypeVt400) {
                Key.ScanCode = SCAN_INSERT;
              }
      ...
      case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_1:
            ...
            (TerminalDevice->TerminalType == TerminalTypePcAnsi))
        {
          switch (UnicodeChar) {
            ...
            case '5':
              ...
              if (UnicodeChar == ';'){
                TerminalDevice->InputState |= INPUT_CTRL_0;
                TerminalDevice->ResetState = RESET_STATE_DEFAULT;
                continue;
              }
        ...
        if ((UnicodeChar == ';') && ((TerminalDevice->TerminalType == TerminalTypeVt100) || (TerminalDevice->TerminalType == TerminalTypePcAnsi)))
        { 
          TerminalDevice->InputState |= INPUT_CTRL_0;
          TerminalDevice->ResetState  = RESET_STATE_DEFAULT;
          continue;
            ...
            (TerminalDevice->TerminalType == TerminalTypePcAnsi))
        {
          switch (UnicodeChar) {
            case '0':
              ...
              ScanCode = Key.ScanCode;
              if (UnicodeChar == ';'){
                TerminalDevice->InputState = INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_2 | INPUT_CTRL_0;
                TerminalDevice->ResetState = RESET_STATE_DEFAULT;
                continue;
              }
      ...
      case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_2 | INPUT_CTRL_0:
        if ((UnicodeChar == '5') && ((TerminalDevice->TerminalType == TerminalTypeVt100) || (TerminalDevice->TerminalType == TerminalTypePcAnsi))){
          TerminalDevice->InputState |= INPUT_CTRL_1;
          TerminalDevice->ResetState  = RESET_STATE_DEFAULT;
          continue;
        }
      ...
      case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_2 | INPUT_CTRL_0 | INPUT_CTRL_1:
        if (((TerminalDevice->TerminalType == TerminalTypeVt100) || (TerminalDevice->TerminalType == TerminalTypePcAnsi))){
          switch (UnicodeChar) {
            case '~':
              CtrlFlag = 1;
      ...
      case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_1 | INPUT_CTRL_0:
        if ((UnicodeChar == '5') && ((TerminalDevice->TerminalType == TerminalTypeVt100) || (TerminalDevice->TerminalType == TerminalTypePcAnsi))){
          TerminalDevice->InputState |= INPUT_CTRL_1;
          TerminalDevice->ResetState  = RESET_STATE_DEFAULT;
          continue;
        ...
        break;
      case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET | INPUT_STATE_1 | INPUT_CTRL_0 | INPUT_CTRL_1:
        if (((TerminalDevice->TerminalType == TerminalTypeVt100) || (TerminalDevice->TerminalType == TerminalTypePcAnsi))){
          switch (UnicodeChar) {
            case 'P':
              CtrlFlag = 1;
