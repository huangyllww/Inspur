  ...
  )
{
  ...
  //
  ValidCount    = 0;
  NewModeBuffer = AllocateZeroPool (sizeof (GRAPHICS_CONSOLE_MODE_DATA) * (Private->GraphicsOutput->Mode->MaxMode + (sizeof (ModeList) / (sizeof (UINTN)*2))));
  ASSERT (NewModeBuffer != NULL);

  //
  // Add default support mode
  //
  Index              = 0;
  ModeList[Index][0] = 80;
  ModeList[Index][1] = 25;
  ...
  for (Index = 0; Index < sizeof (ModeList) / (sizeof (UINTN)*2); Index++) {
    if ((ModeList[Index][0] == 0) && (ModeList[Index][1] == 0)) {
      break;
    }

    if ((ModeList[Index][0] <= MaxColumns) && (ModeList[Index][1] <= MaxRows)) {
      NewModeBuffer[ValidCount].Columns       = ModeList[Index][0];
      NewModeBuffer[ValidCount].Rows          = ModeList[Index][1];
      ...
      ValidCount++;
    } else if ((ModeList[Index][0] == 80) && (ModeList[Index][1] == 50)) {
      NewModeBuffer[ValidCount].Columns       = 0;
      NewModeBuffer[ValidCount].Rows          = 0;
      NewModeBuffer[ValidCount].GopWidth      = HorizontalResolution;
      NewModeBuffer[ValidCount].GopHeight     = VerticalResolution;
      NewModeBuffer[ValidCount].GopModeNumber = GopModeNumber;
  ...
  // Add And Updata Mode
  //
  for (Mode = 0; Mode < Private->GraphicsOutput->Mode->MaxMode; Mode++) {
    Info   = NULL;
    ...
    for (Index = 0; Index < ValidCount; Index++) {
      if ((NewModeBuffer[Index].Columns == Columns) && (NewModeBuffer[Index].Rows == Rows)) {
        break;
      }
    }
    ...
    if (Index == ValidCount) {
      ...
      // Add New Mode
      //
      NewModeBuffer[ValidCount].Columns       = Info->HorizontalResolution / EFI_GLYPH_WIDTH;
      NewModeBuffer[ValidCount].Rows          = Info->VerticalResolution / EFI_GLYPH_HEIGHT;
      NewModeBuffer[ValidCount].GopWidth      = Info->HorizontalResolution;
      NewModeBuffer[ValidCount].GopHeight     = Info->VerticalResolution;
      NewModeBuffer[ValidCount].GopModeNumber = (UINT32)Mode;
      NewModeBuffer[ValidCount].DeltaX        = (Info->HorizontalResolution - (NewModeBuffer[ValidCount].Columns * EFI_GLYPH_WIDTH)) >> 1;
      NewModeBuffer[ValidCount].DeltaY        = (Info->VerticalResolution - (NewModeBuffer[ValidCount].Rows * EFI_GLYPH_HEIGHT)) >> 1;
      ValidCount++;
    } else {
      //
      // Updata Mode
      //
      NewModeBuffer[Index].Columns       = Info->HorizontalResolution / EFI_GLYPH_WIDTH;
      NewModeBuffer[Index].Rows          = Info->VerticalResolution / EFI_GLYPH_HEIGHT;
      NewModeBuffer[Index].GopWidth      = Info->HorizontalResolution;
      NewModeBuffer[Index].GopHeight     = Info->VerticalResolution;
      NewModeBuffer[Index].GopModeNumber = (UINT32)Mode;
      NewModeBuffer[Index].DeltaX        = (Info->HorizontalResolution - (NewModeBuffer[Index].Columns * EFI_GLYPH_WIDTH)) >> 1;
      NewModeBuffer[Index].DeltaY        = (Info->VerticalResolution - (NewModeBuffer[Index].Rows * EFI_GLYPH_HEIGHT)) >> 1;
    }
  ...
  for (Index = 0; Index < ValidCount; Index++) {
    DEBUG ((
      DEBUG_INFO,
      "GopTextMode: %2d->%-2d %3d-%-4d %4d-%-4d DX:%-4d DY:%-4d\n",
      Index,
      NewModeBuffer[Index].GopModeNumber,
      NewModeBuffer[Index].Columns,
      NewModeBuffer[Index].Rows,
      NewModeBuffer[Index].GopWidth,
      NewModeBuffer[Index].GopHeight,
      NewModeBuffer[Index].DeltaX,
      NewModeBuffer[Index].DeltaY
      ));
  }
  ...
  while (*WString != L'\0') {
    if (*WString == CHAR_BACKSPACE) {
      ...
      GraphicsOutput->Blt (
                        ...
                        0,
                        0,
                        This->Mode->CursorColumn * EFI_GLYPH_WIDTH + DeltaX,
                        This->Mode->CursorRow * EFI_GLYPH_HEIGHT + DeltaY,
                        EFI_GLYPH_WIDTH * Count,
                        EFI_GLYPH_HEIGHT,
                        Delta
                        );
      WString                  += Count;
      ...
      if (This->Mode->CursorColumn >= (INT32)MaxColumn) {
        if ((This->Mode->Attribute & BYO_NOT_AUTO_FEED_ATTRIBUTE) == 0) {
          FlushCursor (This);
                       ...
                       IN  VOID         *Context)
{
  ...
  UINTN                                BufferSize = sizeof(ConOutHandle);
  EFI_GRAPHICS_OUTPUT_PROTOCOL         *GraphicsOutput;
  static GRAPHICS_CONSOLE_DEV          *Private = NULL;
  ...
  //
  // Uninstall gEfiVGopSimpleTextOutProtocolGuid
  //
  Status = gBS->LocateHandle(
                      ByProtocol,
                      &gEfiVGopSimpleTextOutProtocolGuid,
                      NULL,
                      &BufferSize,
                      &OldConOutHandle
                      );
  if (!EFI_ERROR(Status)) {
    ...
    if (EFI_ERROR(Status)) {
      DEBUG((DEBUG_ERROR, "%a(%d) HandleProtocol Error.\n", __FILE__, __LINE__));
      return;
    }
  ...
  //
  // Install gEfiVGopSimpleTextOutProtocolGuid
  //
  MaxMode = GraphicsOutput->Mode->MaxMode;
  ...
  if (HorizontalResolution == 0 || VerticalResolution == 0) {
    DEBUG((DEBUG_ERROR, "%a(%d) UnSupported Gop Resolution.\n", __FILE__, __LINE__));
    return;
  }
  ...
  Private->SimpleTextOutput.Mode = &(Private->SimpleTextOutputMode);
  Private->GraphicsOutput = GraphicsOutput;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((DEBUG_ERROR, "%a(%d) InitializeGraphicsConsoleTextMode Error. \n", __FILE__, __LINE__));
    goto Error;
  }
  Private->SimpleTextOutputMode.MaxMode = (INT32) MaxMode;

  DefaultColumn = Private->GraphicsOutput->Mode->Info->HorizontalResolution / 8;
  DefaultRow = Private->GraphicsOutput->Mode->Info->VerticalResolution / 19;
  for (Index = 0; Index < (INT32)MaxMode; Index++) {
      if ((Private->ModeData[Index].Columns == DefaultColumn) &&
          (Private->ModeData[Index].Rows == DefaultRow)) {
        Private->SimpleTextOutput.Mode->Mode = (INT32)Index;
        break;
      }
  }
...
Error:
  if (EFI_ERROR (Status)) {
    ...
    if (Private != NULL && Private->LineBuffer != NULL) {
      FreePool (Private->LineBuffer);
      Private->LineBuffer = NULL;
    }

    if (Private != NULL && Private->ModeData != NULL) {
      FreePool (Private->ModeData);
  ...
  )
{
  ...
  //
  // If PcdSetColorMagentaAsWhite is true,magenta will turn white.
  //
  if (PcdGetBool (PcdSetColorMagentaAsWhite)) {
    CopyMem (&mGraphicsEfiColors[5], &mGraphicsEfiColors[15], sizeof (EFI_GRAPHICS_OUTPUT_BLT_PIXEL));
  }
