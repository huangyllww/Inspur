  ...
  )
{
  ...
  if ((!EFI_ERROR (Status)) && ((OsIndication & EFI_OS_INDICATIONS_FILE_CAPSULE_DELIVERY_SUPPORTED) != 0)){
    DEBUG ((DEBUG_ERROR, " Get OsIndications variable %r!\n", Status));
    return TRUE;
  }
  RelocVarSize   = sizeof (BOOLEAN);
  Status = PPIVariableServices->GetVariable (
                                  PPIVariableServices,
  ...
  if ((!EFI_ERROR (Status)) && (RelocVarSize == sizeof (BOOLEAN)) && CodRelocInfo) {
    DEBUG ((DEBUG_ERROR, " Get CodRelocationInfo variable %r!\n", Status));
    return TRUE;
  }
  return FALSE;
}
  ...
  )
{
  ...
  if (BootMode != BOOT_ON_FLASH_UPDATE)
  {
    ...
    BootMode = GetBootModeHob ();
    DEBUG((DEBUG_ERROR, "%a(%d) BootMode == %r\n", __FILE__, __LINE__,BootMode));
  }
