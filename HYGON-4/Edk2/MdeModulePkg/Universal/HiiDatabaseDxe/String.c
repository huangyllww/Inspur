...
GetStringWorker (
  ...
  OUT EFI_STRING                   *StringInPackage OPTIONAL
  )
{
  UINT8       *StringTextPtr;
  UINT8       BlockType;
  ...
  if (StringInPackage != NULL) {
    *StringInPackage = (EFI_STRING)StringTextPtr;
  }
...
{
  if ((InputChar > 0x3000) && (InputChar < 0xFF20)) {
    return TRUE;
  }

  if ((InputChar == 0x201C) && (InputChar == 0x201D)) {
    return TRUE;
  }
...
/**
  Update String to make sure WIDE prefix for each chinese string

  @param  String       String to be updated with WIDE and NARROW.
  @param  StringSize   String size to be updated with WIDE prefix and NARROW postfix for Chinese font
  @param  UpdateFlag   Flag to specify whether the input string is updated.
  ...
  )
{
  ...
  //
  // Use wide for Chinese font
  //
  OriginSize  = *StringSize;
  UpdateSize  = OriginSize;
  StrArrayNum = UpdateSize / sizeof (CHAR16);
  ChineseFlag = FALSE;
  for (Index = 0; Index < StrArrayNum; Index++) {
    CurrentChar = ReadUnaligned16 (String + Index);

    if (IsWideChar (CurrentChar)) {
      if (ChineseFlag == FALSE) {
        ChineseFlag = TRUE;
        //
        // If no wide char prefix, need to add it
        //
        if ((Index == 0) || (ReadUnaligned16 (String + Index - 1) != WIDE_CHAR)) {
          UpdateSize += sizeof (CHAR16);
        }
      }
    } else {
      if (ChineseFlag == TRUE) {
        ChineseFlag = FALSE;
        //
        // Chinese string needs to use narrow char as the tail
        //
        if (CurrentChar != NARROW_CHAR) {
          UpdateSize += sizeof (CHAR16);
        }
      }
    }
  ...
  //
  // Update output string size
  //
  *StringSize = UpdateSize;

  if ((UpdateFlag == FALSE) || (OriginSize == UpdateSize)) {
    return;
  }
  ...
  // Update String
  //
  NewString = AllocateZeroPool (UpdateSize);
  NewIndex  = 0;

  for (Index = 0; Index < StrArrayNum; Index++) {
    if (IsWideChar (String[Index])) {
      if (ChineseFlag == FALSE) {
        ChineseFlag = TRUE;
        //
        // If no wide char prefix, need to add it
        //
        if ((Index == 0) || (String[Index - 1] != WIDE_CHAR)) {
          NewString[NewIndex++] = WIDE_CHAR;
        }
      }
    } else {
      if (ChineseFlag == TRUE) {
        ...
        if (String[Index] != NARROW_CHAR) {
          NewString[NewIndex++] = NARROW_CHAR;
        }
      }
    }

    NewString[NewIndex++] = String[Index];
  }

  if (NewIndex * sizeof (CHAR16) == UpdateSize) {
    CopyMem (String, NewString, UpdateSize);
  }
  ...
  )
{
  ...
  if (PackageListNode != NULL) {
         ...
         )
    {
      StringPackage = CR (Link, HII_STRING_PACKAGE_INSTANCE, StringEntry, HII_STRING_PACKAGE_SIGNATURE);
      if (HiiCompareLanguage (StringPackage->StringPkgHdr->Language, (CHAR8 *)Language)) {
        Status = GetStringWorker (Private, StringPackage, StringId, String, StringSize, StringFontInfo, &StringInPackage);

        //
        // Update Chinese String with WIDE char prefix
        //
        if (Status == EFI_BUFFER_TOO_SMALL) {
          UpdateChineseString (StringInPackage, StringSize, FALSE);
        } else if (Status == EFI_SUCCESS) {
          UpdateChineseString (String, StringSize, TRUE);
        }
    ...
    {
      StringPackage = CR (Link, HII_STRING_PACKAGE_INSTANCE, StringEntry, HII_STRING_PACKAGE_SIGNATURE);
      Status        = GetStringWorker (Private, StringPackage, StringId, NULL, NULL, NULL, NULL);
      if (!EFI_ERROR (Status)) {
