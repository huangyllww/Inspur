  ...
  )
{
  ...
  *Result      = RequestResp;
  ...
  if (Status == EFI_NOT_FOUND) {
    //
    // Initialize variable as zero data
    //
    BufferSize = EfiVarStoreInfo->Size;
    ...
    DEBUG ((DEBUG_ERROR, "The variable can't be got!"));
    goto Done;
  }
...
Done:
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Save variable failed!\n"));
    DEBUG ((DEBUG_ERROR, "VarStoreName: %s\n", VarStoreName));
    DEBUG ((DEBUG_ERROR, "EfiVarStoreInfo->Guid: %g\n", &EfiVarStoreInfo->Guid));
    DEBUG ((DEBUG_ERROR, "BufferSize: 0x%x\n", BufferSize));
    DEBUG ((DEBUG_ERROR, "EfiVarStoreInfo->Name: %a\n", EfiVarStoreInfo->Name));
    DEBUG ((DEBUG_ERROR, "Status: %r\n", Status));
    DEBUG ((DEBUG_ERROR, "EfiVarStoreInfo->Size: 0x%x\n", EfiVarStoreInfo->Size));
    DEBUG ((DEBUG_ERROR, "EfiVarStoreInfo->Attributes: 0x%x\n", EfiVarStoreInfo->Attributes));
  }
