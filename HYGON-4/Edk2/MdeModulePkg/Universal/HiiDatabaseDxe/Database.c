...
/**
  The function is called by PerformQuickSort to compare  UnicodeWeight of narrow font

  @param  Buffer1                The pointer to first buffer.
  @param  Buffer2                The pointer to second buffer.
...
{
  EFI_NARROW_GLYPH  *ComparatorUnicodeWeight1;
  EFI_NARROW_GLYPH  *ComparatorUnicodeWeight2;

  ComparatorUnicodeWeight1 = (EFI_NARROW_GLYPH *)Buffer1;
  ComparatorUnicodeWeight2 = (EFI_NARROW_GLYPH *)Buffer2;

  if (ComparatorUnicodeWeight1->UnicodeWeight == ComparatorUnicodeWeight2->UnicodeWeight) {
    return 0;
  } else if (ComparatorUnicodeWeight1->UnicodeWeight > ComparatorUnicodeWeight2->UnicodeWeight) {
    return 1;
  } else {
    return -1;
  }
...
/**
  The function is called by PerformQuickSort to compare  UnicodeWeight of Wide font
...
{
  EFI_WIDE_GLYPH  *ComparatorUnicodeWeight1;
  EFI_WIDE_GLYPH  *ComparatorUnicodeWeight2;

  ComparatorUnicodeWeight1 = (EFI_WIDE_GLYPH *)Buffer1;
  ComparatorUnicodeWeight2 = (EFI_WIDE_GLYPH *)Buffer2;
  ...
  )
{
  ...
  //
  VariableStorage = gVariableStorage;
  ...
  if ((VariableStorage == NULL) && (Link == &gVarStorageList)) {
  ...
  )
{
  ...
  if (gNvDefaultStoreSize < sizeof (PCD_NV_STORE_DEFAULT_BUFFER_HEADER)) {
    ...
    gVariableStorage = GET_GUID_HOB_DATA (GuidHob);
  }
    ...
    switch (IfrOpHdr->OpCode) {
      ...
      case EFI_IFR_VARSTORE_OP:
        ...
        if (IfrOpHdr->OpCode == EFI_IFR_VARSTORE_EFI_OP) {
          ...
          Status = FindQuestionDefaultSetting (EFI_HII_DEFAULT_CLASS_STANDARD, EfiVarStoreList[EfiVarStoreNumber], &VarStoreQuestionHeader, NULL, EfiVarStoreList[EfiVarStoreNumber]->Size, FALSE);
        } else {
          //
          // Regard Buffer Var Storage as EFI Var Stoarge if buffer storage matches the default EFI variable.
          //
          IfrVarStore                        = (EFI_IFR_VARSTORE *)IfrOpHdr;
          EfiVarStoreList[EfiVarStoreNumber] = AllocatePool (sizeof (EFI_IFR_VARSTORE_EFI) - 1 + AsciiStrSize ((CHAR8 *)IfrVarStore->Name) * sizeof (CHAR16));
          if (EfiVarStoreList[EfiVarStoreNumber] == NULL) {
            break;
          }

          EfiVarStoreList[EfiVarStoreNumber]->VarStoreId = IfrVarStore->VarStoreId;
          CopyMem (&(EfiVarStoreList[EfiVarStoreNumber]->Guid), &(IfrVarStore->Guid), sizeof (EFI_GUID));
          EfiVarStoreList[EfiVarStoreNumber]->Attributes = EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS;
          EfiVarStoreList[EfiVarStoreNumber]->Size       = IfrVarStore->Size;
          AsciiStrToUnicodeStrS ((CHAR8 *)IfrVarStore->Name, (CHAR16 *)&(EfiVarStoreList[EfiVarStoreNumber]->Name[0]), AsciiStrSize ((CHAR8 *)IfrVarStore->Name) * sizeof (CHAR16));
          Status = FindQuestionDefaultSetting (EFI_HII_DEFAULT_CLASS_STANDARD, EfiVarStoreList[EfiVarStoreNumber], &VarStoreQuestionHeader, NULL, EfiVarStoreList[EfiVarStoreNumber]->Size, FALSE);
          if (EFI_ERROR (Status)) {
            //
            // Try the different buffer varstore attributes to find the match EFI variable.
            //
            EfiVarStoreList[EfiVarStoreNumber]->Attributes = EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS;
            Status                                         = FindQuestionDefaultSetting (EFI_HII_DEFAULT_CLASS_STANDARD, EfiVarStoreList[EfiVarStoreNumber], &VarStoreQuestionHeader, NULL, EfiVarStoreList[EfiVarStoreNumber]->Size, FALSE);
          }
        }
  ...
  )
{
  ...
  UINTN                             HeaderSize;
  UINT8                             *NarrowLocationPtr;
  UINT8                             *WideLocationPtr;
  ...
  NarrowFontSize = SimpleFontPackage->SimpleFontPkgHdr->NumberOfNarrowGlyphs * sizeof (EFI_NARROW_GLYPH);
  HeaderSize     = sizeof (EFI_HII_SIMPLE_FONT_PACKAGE_HDR);

  if (SimpleFontPackage->SimpleFontPkgHdr->NumberOfWideGlyphs > 0 && SimpleFontPackage->SimpleFontPkgHdr->NumberOfWideGlyphs <= 6000) {
    DEBUG ((DEBUG_INFO, "NumberOfWideGlyphs is %d\n", SimpleFontPackage->SimpleFontPkgHdr->NumberOfWideGlyphs));
    WideLocationPtr = (UINT8 *)(SimpleFontPackage->SimpleFontPkgHdr) + HeaderSize + NarrowFontSize;
    PerformQuickSort (WideLocationPtr, SimpleFontPackage->SimpleFontPkgHdr->NumberOfWideGlyphs, sizeof (EFI_WIDE_GLYPH), CompareWideFont);
  }
  if (SimpleFontPackage->SimpleFontPkgHdr->NumberOfNarrowGlyphs > 0 && SimpleFontPackage->SimpleFontPkgHdr->NumberOfNarrowGlyphs <= 6000) {
    DEBUG ((DEBUG_INFO, "NumberOfNarrowGlyphs is %d\n", SimpleFontPackage->SimpleFontPkgHdr->NumberOfNarrowGlyphs));
    NarrowLocationPtr = (UINT8 *)(SimpleFontPackage->SimpleFontPkgHdr) + HeaderSize;
    PerformQuickSort (NarrowLocationPtr, SimpleFontPackage->SimpleFontPkgHdr->NumberOfNarrowGlyphs, sizeof (EFI_NARROW_GLYPH), CompareNarrowFont);
  }
