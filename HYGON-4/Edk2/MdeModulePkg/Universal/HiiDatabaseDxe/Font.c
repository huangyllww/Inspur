  ...
  )
{
  ...
  if (StringInfo != NULL) {
    ...
    for (Link = Private->DatabaseList.ForwardLink; Link != &Private->DatabaseList; Link = Link->ForwardLink) {
           ...
           )
      {
        ...
        MiddlePosition = 0;
        //
        // If the number of narrow glyph array is less than 6000, use dichotomy to search, otherwise search in order
        //
        if (SimpleFont->SimpleFontPkgHdr->NumberOfNarrowGlyphs > 0 && SimpleFont->SimpleFontPkgHdr->NumberOfNarrowGlyphs <= 6000) {
          RightPosition = SimpleFont->SimpleFontPkgHdr->NumberOfNarrowGlyphs -1;
          while (LeftPosition <= RightPosition) {
            MiddlePosition = LeftPosition + ((RightPosition - LeftPosition) / 2);
            CopyMem (&Narrow, NarrowPtr + MiddlePosition, sizeof (EFI_NARROW_GLYPH));
            if (Narrow.UnicodeWeight > Char) {
              RightPosition = MiddlePosition - 1;
            } else if (Narrow.UnicodeWeight < Char) {
        ...
        } else if (SimpleFont->SimpleFontPkgHdr->NumberOfNarrowGlyphs > 6000) {
        ...
        MiddlePosition = 0;
        //
        // If the number of wide glyph array is less than 6000, use dichotomy to search, otherwise search in order
        //
        if (SimpleFont->SimpleFontPkgHdr->NumberOfWideGlyphs > 0 && SimpleFont->SimpleFontPkgHdr->NumberOfWideGlyphs <= 6000) {
          RightPosition = SimpleFont->SimpleFontPkgHdr->NumberOfWideGlyphs -1;
          while (LeftPosition <= RightPosition) {
            MiddlePosition = LeftPosition + ((RightPosition - LeftPosition) / 2);
            CopyMem (&Wide, WidePtr + MiddlePosition, sizeof (EFI_WIDE_GLYPH));
            if (Wide.UnicodeWeight > Char) {
              RightPosition = MiddlePosition - 1;
            } else if (Wide.UnicodeWeight < Char) {
              LeftPosition = MiddlePosition + 1;
            } else {
        ...
        } else if (SimpleFont->SimpleFontPkgHdr->NumberOfWideGlyphs > 6000) {
  ...
  DEBUG ((DEBUG_ERROR, "Can't find font of Char is %x\n", Char));
