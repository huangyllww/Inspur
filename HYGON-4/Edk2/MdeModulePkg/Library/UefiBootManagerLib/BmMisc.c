...
#define     MaxMemoryTypecount      ARRAY_SIZE(gMemoryTypeString)
/**
  ...
  )
{
  ...
  for (Index = 0; PreviousMemoryTypeInformation[Index].Type != EfiMaxMemoryType; Index++) {
    ...
    if (PreviousMemoryTypeInformation[Index].Type < MaxMemoryTypecount) {
      DEBUG ((DEBUG_INFO, " %-25s   %08x  %08x  %08x\n", gMemoryTypeString[PreviousMemoryTypeInformation[Index].Type],Previous, Current, Next));
    }
  }
