  ...
  )
{
  ...
  //
  //Check whether EfiBootManagerConnectAll is executed
  //If it has been executed return
  //
  IsCalled = PcdGet32(PcdBootManagerLibSetting);
  if ((IsCalled & BOOT_MANAGER_CONNECT_ALL) > 0) {
    return ;
  }
  //
  ...
  //
  PcdSet32S(PcdBootManagerLibSetting,IsCalled|BOOT_MANAGER_CONNECT_ALL);
}
  ...
  do {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        //    next connection
        //
        Status = gBS->ConnectController (Handle, NULL, IsDevicePathEnd (RemainingDevicePath) ? NULL : RemainingDevicePath, FALSE);
