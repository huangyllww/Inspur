...
PROMPT_HOTKEY_APP_ENTER_FUNCTION  gPromptHotKeyAppEnterFunc = NULL;
  ...
  )
{
  ...
  if (StrCmp (BootOption->Description, L"Enter Setup") == 0) {
    StrCpyS (Buffer, ARRAY_SIZE (Buffer), L"Entering Setup...");
  } else {
    UnicodeSPrint (Buffer, sizeof (Buffer), L"Entering %s...", BootOption->Description);
  }
  ...
  )
{
  if (mBmHotkeyBootOption.OptionNumber != LoadOptionNumberUnassigned) {
    if (!PcdGetBool (PcdEarlyVideoSupport)) {
      if (gPromptHotKeyAppEnterFunc == NULL) {
        PromptHotKeyAppEnter (&mBmHotkeyBootOption);
      } else {
        gPromptHotKeyAppEnterFunc (&mBmHotkeyBootOption);
      }
    }
  ...
  )
{
  ...
  if (Index < KeyOptionCount) {
    if (KeyOptions[Index].BootOption == BootOptionNumber) {
      return EFI_ALREADY_STARTED;
    } else {
      DEBUG ((DEBUG_INFO, "re-set Key%x\n", Index));
      KeyOptionNumber = Index;
    }
  }
