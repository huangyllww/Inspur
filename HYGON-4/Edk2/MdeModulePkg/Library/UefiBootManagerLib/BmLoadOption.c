...
//
//This value is the same as BM_MENU_TYPE_MAX in BM_MENU_TYPE(Byo\ByoModulePkg\Include\ByoBootGroup.h)
//
#define BM_MENU_TYPE_MAX      0xFF
...
{
  EDKII_PLATFORM_BOOT_MANAGER_PROTOCOL    *BootManager;
  EFI_STATUS                              Status;
  ...
  if (((Attributes & LOAD_OPTION_HIDDEN) != 0) || ((Attributes & LOAD_OPTION_ACTIVE) == 0)) {
    Option->Ignore = TRUE;
  } else {
    Option->Ignore = FALSE;
  }
  if((DevicePathType (Option->FilePath) != BBS_DEVICE_PATH) || (DevicePathSubType(Option->FilePath) != BBS_BBS_DP)){
    Status = gBS->LocateProtocol(&gEdkiiPlatformBootManagerProtocolGuid,NULL,(VOID**)&BootManager);
    if (!EFI_ERROR(Status) && BootManager->Revision >= 0x07 && BootManager->GetUefiBootGroupType != NULL) {
      GroupType = BootManager->GetUefiBootGroupType(FilePath);
    }
  } else {
    Status = gBS->LocateProtocol(&gEdkiiLegacyBootManagerProtocolGuid,NULL,(VOID**)&LegacyBootManager);
    if (!EFI_ERROR(Status) && LegacyBootManager->Revision >= 0x02 && LegacyBootManager->GetLegacyBootGroupType != NULL) {
      GroupType = LegacyBootManager->GetLegacyBootGroupType(FilePath,OptionalData);
    }
  }
  ...
  *OptionCount = 0;
  ...
  if ((LoadOptionType == LoadOptionTypeDriver) || (LoadOptionType == LoadOptionTypeSysPrep) || (LoadOptionType == LoadOptionTypeBoot)) {
    ...
    *OptionCount = OptionOrderSize / sizeof (UINT16);
    ...
    //
    // Check and remove duplicate OptionNumber in XXXXOrder
    //
    for (Index = 0; Index < *OptionCount - 1; Index++) {
      for (OptionIndex = Index + 1; OptionIndex < *OptionCount; OptionIndex++) {
        if (OptionOrder[Index] == OptionOrder[OptionIndex]) {
          // found duplicate OptionNumber
          NeedUpdateOder = TRUE;
          if (OptionIndex != *OptionCount -1) {
            // duplicate OptionNumber is not the last one
            CopyMem (OptionOrder + OptionIndex, OptionOrder + OptionIndex + 1, (*OptionCount - OptionIndex - 1) * sizeof (UINT16));
            OptionIndex--;
          }
  ...
  )
{
  ...
  while (TRUE) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      //
      gBS->SetWatchdogTimer ((UINTN)PcdGet64 (PcdWatchdogTimeoutValueInSeconds), 0, 0, NULL);
