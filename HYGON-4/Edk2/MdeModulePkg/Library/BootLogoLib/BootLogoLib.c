...
/**
  ...
  @param InstanceIndex    Other: Show the specified logo instance.
                          0:     Show all logo instances.
                          
  @retval EFI_SUCCESS     Logo was displayed.
  @retval EFI_UNSUPPORTED Logo was not found or cannot be displayed.
**/
EFI_STATUS
BootLogoEnableLogoWorker (
  ...
  )
{
  ...
  NewDestX      = 0;
  NewDestY      = 0;
  Instance      = (InstanceIndex == 0) ? 0 : InstanceIndex - 1;
  DestX         = 0;
  DestY         = 0;
  while (TRUE) {
    //
    ...
    if ((DestX >= 0) && (DestY >= 0)) {
      ...
      if (LogoState != 0x80 && LogoState != 0 && LogoState != 0xFF) {
        CursorColumn = gST->ConOut->Mode->CursorColumn;
        CursorRow    = gST->ConOut->Mode->CursorRow;
        gST->ConOut->QueryMode(gST->ConOut, gST->ConOut->Mode->Mode, &Columns, &Rows);
        gST->ConOut->SetCursorPosition(gST->ConOut, Columns/4, Rows-8);
        switch (LogoState) {
        case 1:
          Print(L"The user logo is an illegal Bmp image and cannot be displayed\n");
          break;
        case 2:
          Print(L"The user logo is an unsupported Png image and cannot be displayed\n");
          break;
        case 3:
          Print(L"The user logo is an unsupported Jpg image and cannot be displayed\n");
          break;
        case 4:
          Print(L"Unable to display user logo with resolution greater than screen resolution\n");
          break;
        default:
          break;
        }
        gST->ConOut->SetCursorPosition(gST->ConOut, Columns/4, Rows-7);
        Print(L"the default Logo will be used\n");
...
{
  return BootLogoEnableLogoWorker (0);
}

/**
  Show LOGO returned from Edkii Platform Logo protocol on all consoles by specified instance.

  @param InstanceIndex    The specified logo instance index that should begin from 1.
  ...
  )
{
  ...
  return BootLogoEnableLogoWorker (InstanceIndex);
}
  ...
  )
{
  ...
  for (Index = PreviousValue; Index < BlockNum; Index++) {
    ...
    if (PosX >= (SizeOfX - BlockWidth)) {
      PosX = SizeOfX - BlockWidth;
    }
