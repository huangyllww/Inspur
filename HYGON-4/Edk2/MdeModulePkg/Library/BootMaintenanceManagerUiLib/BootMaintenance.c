...
VOID
BackUpConsoleVariable (
  VOID
  )
{
  EFI_STATUS                Status;
  EFI_DEVICE_PATH_PROTOCOL  *ConInVar;
  ...
  Status = GetEfiGlobalVariable2 (L"ConIn", (VOID **)&ConInVar, &ConInSize);
  Status = GetEfiGlobalVariable2 (L"ConOut", (VOID **)&ConOutVar, &ConOutSize);
 ...
 */
  ...
  Var_UpdateConsoleRedirection (L"ConIn", &FilterInMenu, Private);
  Var_UpdateConsoleRedirection (L"ConOut", &FilterOutMenu, Private);
  //
  ...
  )
{
  ...
  BM_LOAD_CONTEXT            *NewLoadContext;
  UINT16                     Index;
  BM_FILTER_MENU_ENTRY       *ConInMenuEntry;
  BM_FILTER_CONSOLE_CONTEXT  *NewConsoleInContext;
  UINT8                      IsHaveTerminal;
  ...
  //
  for (Index = 0; Index < FilterInMenu.MenuNumber; Index++) {
    ConInMenuEntry      = BOpt_GetFilterMenuEntry (&FilterInMenu, Index);
    NewConsoleInContext = (BM_FILTER_CONSOLE_CONTEXT *)ConInMenuEntry->VariableContext;
    if (NewConsoleInContext->IsUart == 1) {
      IsHaveTerminal = 1;
      break;
    }
  }
  ...
  if (IsHaveTerminal == 1) {
    CallbackData->BmmFakeNvData.UCREnable = 1;
  }

  switch (NewConsoleInContext->BaudRate) {
    case 115200:
      CallbackData->BmmFakeNvData.SerialBaudrate = 0;
      break;

    case 57600:
      CallbackData->BmmFakeNvData.SerialBaudrate = 1;
      break;

    case 19200:
      CallbackData->BmmFakeNvData.SerialBaudrate = 3;
      break;

    case 9600:
      CallbackData->BmmFakeNvData.SerialBaudrate = 4;
      break;
  ...
  CallbackData->BmmFakeNvData.Parity           = NewConsoleInContext->Parity;
  CallbackData->BmmFakeNvData.DataBits         = NewConsoleInContext->DataBits;
  CallbackData->BmmFakeNvData.StopBits         = NewConsoleInContext->StopBits;
  CallbackData->BmmFakeNvData.UartTerminalType = NewConsoleInContext->TerminalType;
