...
VOID
ChangeConsoleDevicePath (
  IN OUT EFI_DEVICE_PATH_PROTOCOL  *DevicePath,
  IN     BMM_CALLBACK_DATA         *ConsoleSetup
  )
{
  EFI_DEVICE_PATH_PROTOCOL  *Node;
  UART_DEVICE_PATH          *Uart;
  ...
  switch (ConsoleSetup->BmmFakeNvData.SerialBaudrate) {
    case 0:
      BaudRate = 115200;
      break;
  ...
  DataBits      = ConsoleSetup->BmmFakeNvData.DataBits;
  Parity        = ConsoleSetup->BmmFakeNvData.Parity;
  StopBits      = ConsoleSetup->BmmFakeNvData.StopBits;
  Terminalvalue = ConsoleSetup->BmmFakeNvData.UartTerminalType;
  ...
  while (!IsDevicePathEnd (Node)) {
    ...
    if ((Vendor != NULL) && (DevicePathType (Vendor) == MESSAGING_DEVICE_PATH) && (DevicePathSubType (Vendor) == MSG_VENDOR_DP)) {
      CopyGuid (&Vendor->Guid, &TerminalTypeGuid[Terminalvalue]);
    }
  }
}
  ...
  )
{
  ...
  for (Index = 0; Index < TerminalMenu.MenuNumber; Index++) {
    while (!IsDevicePathEnd (Node)) {
      ...
      if (TerminalNumber > TerminalMenu.MenuNumber) {
        return EFI_NOT_FOUND;
      }
  ...
  )
{
  ...
  UART_DEVICE_PATH           *Uart;
  UINT16                     Index;
  BM_FILTER_MENU_ENTRY       *FilterMenuContext;
  BM_FILTER_CONSOLE_CONTEXT  *FilterConsoleContext;
  UINTN                      Size;
  UINTN                      AllCount;
  VENDOR_DEVICE_PATH         *Vendor;
  UINT8                      TerminalValue;
  ...
  InitializeListHead (&FilterOutMenu.Head);
  FilterOutMenu.MenuNumber = 0;
  DevicePath               = NULL;
  AllCount                 = 0;
  TerminalValue            = 0;
  Status                   = EFI_SUCCESS;
  ...
  Status = GetVariable2 (
             L"BackUpConOut",
             &gNewConsoleRedirectionSpaceGuid,
             (VOID **)&DevicePath,
             NULL
             );
  if (EFI_ERROR (Status)) {
  ...
  AllCount        = EfiDevicePathInstanceCount (DevicePath);
  MultiDevicePath = DevicePath;
  for (Index = 0; Index < AllCount; Index++) {
    DevicePathInst    = GetNextDevicePathInstance (&MultiDevicePath, &Size);
    FilterMenuContext = BOpt_CreateFilterMenuEntry ();
    if (FilterMenuContext == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }

    FilterConsoleContext             = (BM_FILTER_CONSOLE_CONTEXT *)FilterMenuContext->VariableContext;
    FilterConsoleContext->DevicePath = DuplicateDevicePath (DevicePathInst);
    for (Node = FilterConsoleContext->DevicePath; !IsDevicePathEnd (Node); Node = NextDevicePathNode (Node)) {
      if ((DevicePathType (Node) == MESSAGING_DEVICE_PATH) && (DevicePathSubType (Node) == MSG_UART_DP)) {
        FilterConsoleContext->IsUart = 1;
        Uart                         = (UART_DEVICE_PATH *)Node;
        if ((Uart->BaudRate == 0) && (Uart->DataBits == 0) && (Uart->Parity == 0) && (Uart->StopBits == 0)) {
          FilterConsoleContext->BaudRate = 115200;
          FilterConsoleContext->DataBits = 8;
          FilterConsoleContext->Parity   = 1;
          FilterConsoleContext->StopBits = 1;
        } else {
          FilterConsoleContext->BaudRate = Uart->BaudRate;
          FilterConsoleContext->DataBits = Uart->DataBits;
          FilterConsoleContext->Parity   = Uart->Parity;
          FilterConsoleContext->StopBits = Uart->StopBits;
        }

        Node   = NextDevicePathNode (Node);
        Vendor = (VENDOR_DEVICE_PATH *)Node;
        if ((Vendor != NULL) && (DevicePathType (Vendor) == MESSAGING_DEVICE_PATH) && (DevicePathSubType (Vendor) == MSG_VENDOR_DP)) {
          TerminalValue                      = GetTerminalType (Vendor);
          FilterConsoleContext->TerminalType = TerminalValue;
        }
    ...
    InsertTailList (&FilterOutMenu.Head, &FilterMenuContext->Link);
    FilterOutMenu.MenuNumber++;
  }
  ...
  )
{
  ...
  InitializeListHead (&FilterInMenu.Head);
  FilterInMenu.MenuNumber = 0;
  DevicePath              = NULL;
  AllCount                = 0;
  TerminalValue           = 0;
  Status                  = EFI_SUCCESS;
  ...
  for (Index = 0; Index < AllCount; Index++) {
    ...
    InsertTailList (&FilterInMenu.Head, &FilterMenuContext->Link);
    FilterInMenu.MenuNumber++;
  }
...
/**
  ...
  Update console devicepath from gEfiSimpleTextInProtocolGuid and gEfiSimpleTextOutProtocolGuid

  @param ConsoleName     variable is ConIn or ConOut

  @retval  !NULL         There are some devicepath in TextIn or TextOut
  @retval  NULL          There are no devicepath in TextIn or TextOut

**/
EFI_DEVICE_PATH_PROTOCOL *
UpdateConsoleDevicePathFromTextIn (
  ...
  )
{
  ...
  EFI_DEVICE_PATH_PROTOCOL  *TextDevicePath;
  ...
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    while (!IsDevicePathEnd (Node)) {
      if ((DevicePathType (Node) == MESSAGING_DEVICE_PATH) && (DevicePathSubType (Node) == MSG_UART_DP)) {
        TempNewDevicePath = TextDevicePath;
        TextDevicePath    = AppendDevicePathInstance (
                              TextDevicePath,
                              DevicePath
                              );
