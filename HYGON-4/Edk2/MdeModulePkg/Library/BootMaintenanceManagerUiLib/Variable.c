  ...
  )
{
  ...
  BM_FILTER_MENU_ENTRY       *NewMenuEntry;
  BM_FILTER_CONSOLE_CONTEXT  *NewConsoleContext;
  EFI_STATUS                 Status;
  UINTN                      Index;
  BOOLEAN                    IsHaveConsole;
  ...
  for (Index = 0; Index < ConsoleMenu->MenuNumber; Index++) {
    NewMenuEntry      = BOpt_GetFilterMenuEntry (ConsoleMenu, Index);
    NewConsoleContext = (BM_FILTER_CONSOLE_CONTEXT *)NewMenuEntry->VariableContext;

    if (NewConsoleContext->IsUart == 1) {
      IsHaveConsole = TRUE;
      if (ConsoleSetup->BmmFakeNvData.UCREnable == 1) {
        ChangeConsoleDevicePath (NewConsoleContext->DevicePath, ConsoleSetup);
        TempNewDevicePath = ConDevicePath;
  ...
  if ((IsHaveConsole == FALSE) && (ConsoleSetup->BmmFakeNvData.UCREnable == 1)) {
    //
    //  platform uart is disabled when user want to enable console redirection
    //  get console device gEfiSimpleTextInProtocolGuid
    //
    TempDevicePath = UpdateConsoleDevicePathFromTextIn (ConsoleName);
