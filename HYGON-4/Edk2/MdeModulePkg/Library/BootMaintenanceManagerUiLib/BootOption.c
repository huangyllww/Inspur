...
BM_FILTER_MENU_ENTRY *
BOpt_CreateFilterMenuEntry (
  VOID
  )
{
  BM_FILTER_MENU_ENTRY  *MenuEntry;
  UINTN                 ContextSize;

  ContextSize = sizeof (BM_FILTER_CONSOLE_CONTEXT);
  ...
  // Create new menu entry
  //
  MenuEntry = AllocateZeroPool (sizeof (BM_FILTER_MENU_ENTRY));
  if (MenuEntry == NULL) {
    return NULL;
  }
  ...
  )
{
  ...
  NewMenuEntry = CR (List, BM_FILTER_MENU_ENTRY, Link, BM_MENU_ENTRY_SIGNATURE);
