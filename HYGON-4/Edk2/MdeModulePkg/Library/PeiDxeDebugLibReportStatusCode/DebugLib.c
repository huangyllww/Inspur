  ...
  )
{
  ...
  if (TotalSize > EFI_STATUS_CODE_DATA_MAX_SIZE) {
    TotalSize = EFI_STATUS_CODE_DATA_MAX_SIZE;
  }
  ...
  AsciiStrnCpyS (
    FormatString,
    EFI_STATUS_CODE_DATA_MAX_SIZE - (4 + sizeof (EFI_DEBUG_INFO) + 12 * sizeof (UINT64)),
    Format,
    EFI_STATUS_CODE_DATA_MAX_SIZE - (4 + sizeof (EFI_DEBUG_INFO) + 12 * sizeof (UINT64)) - 1
    );
