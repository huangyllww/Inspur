...
//
// Values in percent of of ux.cap height.
//
#define UX_BOTTOM_PADDING    20
  ...
  )
{
  ...
  if (CalculateCheckSum8((UINT8 *)CapsuleHeader, CapsuleHeader->CapsuleImageSize) != 0) {
    DEBUG((DEBUG_INFO, "Defult Linux UX.cap CheckSum is not equal 0\n"));
  }
  ...
  //do not use the UX.cap OffsetX and OffsetY because windows and linux are different
  //avoid overlapping logo and progress bar
  LocalOffsetX = (GraphicsOutput->Mode->Info->HorizontalResolution - (UINT32)Width) / 2;
  LocalOffsetY = GraphicsOutput->Mode->Info->VerticalResolution - ((GraphicsOutput->Mode->Info->VerticalResolution) * UX_BOTTOM_PADDING /100) - (UINT32)Height;
