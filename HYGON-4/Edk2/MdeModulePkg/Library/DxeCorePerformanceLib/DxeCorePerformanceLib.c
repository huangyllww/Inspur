  ...
  if (mFpdtBufferIsReported) {
    ...
    if (mBootRecordSize + RecordSize > mBootRecordMaxSize) {
      if (!mLackSpaceIsReported) {
        ...
        DEBUG ((DEBUG_INFO, "Current Size 0x%08x, Request Size 0x%08x, and MaxSize 0x%08x\n", mBootRecordSize, RecordSize, mBootRecordMaxSize));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "Total memory size 0x%08x, Boot record size 0x%08x, Smm record size 0x%08x, reserved memory size 0x%08x\n", BootPerformanceDataSize, mPerformanceLength, SmmBootRecordDataSize, PcdGet32 (PcdExtFpdtBootRecordPadSize)));
  if (mAcpiBootPerformanceTable == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  DEBUG ((DEBUG_INFO, "DxeCorePerformanceLib: EndOfDxe FPDT Table Current Size 0x%08x and MaxSize 0x%08x\n", mBootRecordSize, mBootRecordMaxSize));
  ...
  )
{
  ...
  EFI_ACPI_5_0_FPDT_PERFORMANCE_RECORD_HEADER      *FpdtPerfRecord;
  ...
  if (mAcpiBootPerformanceTable->Header.Length + SmmBootRecordDataSize > mBootRecordMaxSize) {
    ...
    while(AppendSize< mBootRecordMaxSize - mAcpiBootPerformanceTable->Header.Length){
      FpdtPerfRecord = (EFI_ACPI_5_0_FPDT_PERFORMANCE_RECORD_HEADER*)((UINT8 *)SmmBootRecordData+AppendSize);
      AppendSize = AppendSize + FpdtPerfRecord->Length;  
    }
    AppendSize = AppendSize - FpdtPerfRecord->Length;  
  } else {
  ...
  DEBUG ((DEBUG_INFO, "DxeCorePerformanceLib: ReadyToBoot FPDT Table Current Size 0x%08x and MaxSize 0x%08x\n", mBootRecordSize, (UINT32)mBootRecordMaxSize));
}
