  ...
  )
{
  ...
  if (Image->ImageContext.PdbPointer != NULL) {
  ...
  } else if (Image->Info.FilePath != NULL) {
    EFI_GUID                       *FvNameGuid;
    ...
    FvNameGuid = EfiGetNameGuidFromFwVolDevicePathNode ((MEDIA_FW_VOL_FILEPATH_DEVICE_PATH *)Image->Info.FilePath);
    if (FvNameGuid != NULL) {
      Status = gBS->HandleProtocol (Image->Info.DeviceHandle, &gEfiFirmwareVolume2ProtocolGuid, (VOID **)&FwVol);
      if (!EFI_ERROR (Status)) {
        ...
        if (!EFI_ERROR (Status)) {
          DEBUG ((DEBUG_INFO | DEBUG_LOAD, "%s", UiName));
          gBS->FreePool (UiName);
        }
      }
    }
