  ...
  )
{
  ...
  //
  // Initialize Debug Agent to support source level debug in PEI phase
  //
  if ((OldCoreData != NULL) && (OldCoreData->ShadowedPeiCore != NULL)) {
    InitializeDebugAgent (DEBUG_AGENT_INIT_PEI_CORE, OldCoreData, NULL);
  } else {
    InitializeDebugAgent (DEBUG_AGENT_INIT_PEI_CORE, NULL, NULL);
  }
