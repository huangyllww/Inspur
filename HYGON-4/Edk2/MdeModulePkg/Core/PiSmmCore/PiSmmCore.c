  ...
  if (!InLegacyBoot) {
    ...
    if (CommunicationBuffer != NULL) {
          ...
          IsOverlapped || IsOverUnderflow)
      {
        ...
        if (mInLegacyBoot) {
          if (CompareGuid (&CommunicateHeader->HeaderGuid, &gEfiEventLegacyBootPreExitGuid)) {
            mInLegacyBoot = FALSE;
          }
  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status);
  //
  // Initialize gImageHandle after PiSmmCore installs its loaded image protocol.
  //
  gImageHandle = Handle;
