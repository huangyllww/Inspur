  ...
  )
{
  ...
  mEnterSetupBootIndex        = BootOptionCount;
  ...
  for (StrIndex = 0, Index = 0; Index < BootOptionCount; Index++) {
    if ((mEnterSetupBootIndex == BootOptionCount) && IsBootManagerMenu (&BootOption[Index])) {
      mEnterSetupBootIndex = Index;
      continue;
    }
  ...
  if (mEnterSetupBootIndex < BootOptionCount) {
    //
    // Place Setup Boot Option as the last one
    //
    ASSERT (BootOption[mEnterSetupBootIndex].Description != NULL);
    BootMenuData->PtrTokens[StrIndex++] = HiiSetString (
                                            gStringPackHandle,
                                            0,
                                            BootOption[mEnterSetupBootIndex].Description,
                                            NULL
                                            );
  }
  ...
  )
{
  ...
  if ((Index == BootOptionCount) && (mEnterSetupBootIndex < BootOptionCount)) {
    EfiBootManagerBoot (&BootOptions[mEnterSetupBootIndex]);
  }
}
  ...
  )
{
  ...
  //
  // Suspend frb2 wdt when enter bootmanagermenu.
  //
  Handle = NULL;
  ...
  if (!mModeInitialized) {
    ...
    mBootHorizontalResolution = mSetupHorizontalResolution;
    mBootVerticalResolution   = mSetupVerticalResolution;
    mBootTextModeColumn       = mSetupTextModeColumn;
    mBootTextModeRow          = mSetupTextModeRow;
  }
    ...
    if (!EFI_ERROR (Status)) {
      switch (Key.UnicodeChar) {
        ...
        case CHAR_CARRIAGE_RETURN:
          gST->ConOut->ClearScreen (gST->ConOut);
          //
          // Resume frb2 wdt when exit bootmanagermenu
          //
          Status = gBS->InstallProtocolInterface (
                          &Handle,
                          &gEfiBootMenuExitGuid,
                          EFI_NATIVE_INTERFACE,
                          NULL
                          );
          //
