  ...
  )
{
  ...
  UINTN                            EventIndex;
  CHAR16                           *TmpString;
  BYO_CREATE_POP_UP_PROTOCOL       *CreatePopUpProtocol;
  EFI_SETUP_MOUSE_PROTOCOL         *SetupMouse;
  EFI_EVENT                        WaitEvents[2];
  UINTN                            EventsCount;
  MOUSE_ACTION_INFO                MouseData;

  Status = gBS->LocateProtocol (&gByoCreatePopUpProtocolGuid, NULL, (VOID **)&CreatePopUpProtocol);
  if (!EFI_ERROR (Status)) {
    VA_START (Args, Key);
    CreatePopUpProtocol->CreatePopUp (Attribute, Key, Args);
    VA_END (Args);
    return;
  }
  ...
  Status = gBS->LocateProtocol (&gEfiSetupMouseProtocolGuid, NULL, (VOID **)&SetupMouse);
  if (EFI_ERROR (Status)) {
    SetupMouse = NULL;
  }
  ...
  if (Key != NULL) {
    while (TRUE) {
      if (EventIndex == 0) {
        ...
        Status = SetupMouse->GetData (&MouseData);
        if (!EFI_ERROR (Status) && (MouseData.Action == MOUSE_RIGHT_CLICK)) {
      ...
      if (SetupMouse != NULL) {
        WaitEvents[1] = SetupMouse->WaitForMouse;
        EventsCount++;
      }

      gBS->WaitForEvent (EventsCount, WaitEvents, &EventIndex);
    }
  }
}
