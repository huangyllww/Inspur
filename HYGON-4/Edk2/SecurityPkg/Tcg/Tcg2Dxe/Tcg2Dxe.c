  ...
  )
{
  ...
  TPM2_DEVICE_INFO                 Tpm2DeviceInfo;

  mImageHandle = ImageHandle;

  if (CompareGuid (PcdGetPtr (PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceNoneGuid) ||
      CompareGuid (PcdGetPtr (PcdTpmInstanceGuid), &gEfiTpmDeviceInstanceTpm12Guid) ||
      CompareGuid (PcdGetPtr (PcdTpmInstanceGuid), &gEfiTcmDeviceInstanceGuid))
  {
  ...
  Tpm2DeviceInfo.ManufactureId    = mTcgDxeData.BsCap.ManufacturerID;
  Tpm2DeviceInfo.FirmwareVersion1 = FirmwareVersion1;
  Tpm2DeviceInfo.FirmwareVersion2 = FirmwareVersion2;

  //
  // Save TPM2_DEVICE_INFO to variable so platform driver can get it.
  //
  Status = gRT->SetVariable (
                  TPM2_DEVICE_INFO_NAME,
                  &gTpm2DeviceInfoGuid,
                  EFI_VARIABLE_BOOTSERVICE_ACCESS,
                  sizeof (Tpm2DeviceInfo),
                  &Tpm2DeviceInfo
                  );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Tcg2Dxe: Fail to set TPM2_DEVICE_INFO_NAME\n"));
  }
