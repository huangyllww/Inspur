  ...
  )
{
  ...
  if (PcdGetBool (PcdOpalUpdateDataStoreTable)) {
    //
    // Set User1/Admin1 Datastore Table ACE
    //
    Ret = OpalSetLockingAuthorityDatastoreAccess (Session, &MethodStatus);
    if ((Ret != TcgResultSuccess) || (MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS)) {
      DEBUG ((DEBUG_INFO, "OpalSetLockingAuthorityDatastoreAccess failed\n"));
    }
  }
...
/**
  Update global locking range. return verified authority if password check passes.
  ...
  @param          Authority          Return Authority used to unlock NVMe OPAL Device
**/
TCG_RESULT
EFIAPI
OpalUtilUpdateGlobalLockingRangeEx (
  OPAL_SESSION  *Session,
  const VOID    *Password,
  UINT32        PasswordLength,
  BOOLEAN       ReadLocked,
  BOOLEAN       WriteLocked,
  TCG_UID       *Authority OPTIONAL
  )
{
  UINT8       MethodStatus;
  TCG_RESULT  Ret;

  TCG_UID  SessionAuthority;

  NULL_CHECK (Session);
  NULL_CHECK (Password);

  SessionAuthority = TCG_UID_NULL;
  ...
  if ((Ret == TcgResultSuccess) && (MethodStatus == TCG_METHOD_STATUS_CODE_SUCCESS)) {
    ...
    OpalEndSession (Session);
    if ((Ret == TcgResultSuccess) && (MethodStatus == TCG_METHOD_STATUS_CODE_SUCCESS)) {
      SessionAuthority = OPAL_LOCKING_SP_ADMIN1_AUTHORITY;
      goto done;
    }
  }
  ...
  OpalEndSession (Session);
  SessionAuthority = OPAL_LOCKING_SP_USER1_AUTHORITY;
...
{
  return OpalUtilUpdateGlobalLockingRangeEx (
           Session,
           Password,
           PasswordLength,
           ReadLocked,
  ...
  )
{
  ...
  ZeroMem ((VOID *)&Descriptor, DataSize);
  Ret = OpalGetFeatureDescriptor (Session, TCG_FEATURE_DATA_REMOVAL, &DataSize, &Descriptor);
  if ((Ret != TcgResultSuccess) || (Descriptor.RemovalMechanism == 0)) {
    return TcgResultFailure;
  }
  ...
  )
{
  ...
  ZeroMem ((VOID *)&SupportedAttributes, sizeof (OPAL_DISK_SUPPORT_ATTRIBUTE));
...
/*
  This function reads data from OPAL Table identified by TableUID. Reading range is
  [offset, offset + Size)

  @param[in] Session          OPAL Session used to authorize and read
  @param[in] IsAdmin          Whether Session is authorized by Admin1 or User1
  @param[in] Password         Password
  @param[in] PasswordLength   PasswordLength
  @param[in] TableUid         Table UID
  @param[in] Offset           Offset to Read
  @param[in] Size             Size to Read
  @param[out] Data            DataBuffer
  ...
  )
{
  ...
  //
  // Try to start session with Locking SP as admin1/user1 authority
  //
  Ret = OpalStartSession (
          Session,
          OPAL_UID_LOCKING_SP,
  ...
  if ((Ret != TcgResultSuccess) || (MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS)) {
    DEBUG ((DEBUG_INFO, "StartSession with Locking SP as %a failed\n", (IsAdmin ? "Admin1" : "User1")));
    goto done;
  }

  Ret = OpalGetTableSize (
          Session,
          TableUid,
          &TableSize
          );
  if (Ret != TcgResultSuccess) {
    DEBUG ((DEBUG_INFO, "OpalGetTableSize failed\n"));
    OpalEndSession (Session);
    goto done;
  }

  DEBUG ((DEBUG_INFO, "OpalGetTableSize succeed with TableSize %x\n", TableSize));

  //
  // If offset exceeds real Table Size
  //
  if (TableSize <= Offset) {
    DEBUG ((DEBUG_INFO, "ReadOffset %x exceeds TableSize %x\n", Offset, TableSize));
    MethodStatus = TCG_METHOD_STATUS_CODE_FAIL;
    goto done;
  }

  //
  // If Offset + Read Size exceeds real Table Size, adjust Read Size.
  //
  if (TableSize >= Offset + *Size) {
    SizeToRead = *Size;
  } else {
    SizeToRead = TableSize - Offset;
  }
  ...
  if (Ret != TcgResultSuccess) {
    DEBUG ((DEBUG_INFO, "OpalGetTableSize fail\n"));
  } else {
    DEBUG ((DEBUG_INFO, "OpalGetTableSize succeed\n"));
    *Size = SizeToRead;
  }
...
/*
  This function writes data to OPAL Table identified by TableUID. Write range is
[offset, offset + Size)
  ...
  )
{
  ...
  //
  if (TableSize <= Offset) {
    DEBUG ((DEBUG_INFO, "WriteOffset %x exceeds TableSize %x\n", Offset, TableSize));
    MethodStatus = TCG_METHOD_STATUS_CODE_FAIL;
    goto done;
  }

  //
  // If Offset + Write Size exceeds real Table Size, adjust Write Size.
  //
  if (TableSize >= Offset + *Size) {
    SizeToWrite = *Size;
  } else {
    SizeToWrite = TableSize - Offset;
  }
  ...
  DEBUG ((DEBUG_INFO, "OpalWriteTable succeed\n"));
  OpalEndSession (Session);
...
/**
  Creates a session with OPAL_UID_ADMIN_SP as OPAL_ADMIN_SP_SID_AUTHORITY, then reverts device using Admin SP Revert method.

  @param[in]      Session,           The session info for one opal device.
  @param[in]      Password           Admin Password of device to revert.
  @param[in]      PasswordLength     Length of Admin Password in bytes.
  ...
  )
{
  ...
  RemovalTimeOut = GetRevertTimeOut (Session);
  DEBUG ((DEBUG_INFO, "OpalUtilSidRevert: Timeout value = %d\n", RemovalTimeOut));
  Ret = OpalStartSession (
          Session,
          OPAL_UID_ADMIN_SP,
          TRUE,
