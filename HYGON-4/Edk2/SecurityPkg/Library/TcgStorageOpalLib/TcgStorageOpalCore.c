  ...
  )
{
  ...
  // UINTN ResponseSize = sizeof(TCG_COM_PACKET) + sizeof(TCG_PACKET) + sizeof(TCG_SUB_PACKET) + SwapBytes32(ParseStruct->CurSubPacket->LengthBE);
  ...
  )
{
  ...
  *OpalBaseComId = TCG_RESERVED_COMID;
  ...
  if ((Feat != NULL) && (Size >= sizeof (TCG_BLOCK_SID_FEATURE_DESCRIPTOR))) {
    ...
    SupportedAttributes->SIDBlockedState = Feat->BlockSid.SIDBlockedState;
...
/*
  This function gets Rows of Table which is identified by TableUID.

  @param[in]  Session          OPAL Session used to read
  @param[in]  TableUid         Table UID
  @param[out] TableSize        Table Size (Rows)

  @retval     EFI_SUCCESS - Succeed in getting OPAL table ROWS
  @retval     Others      - Fail to read OPAL table ROWS
...
OpalGetTableSize (
  OPAL_SESSION  *Session,
  IN  TCG_UID   TableUid,
  OUT UINT64    *TableSize
  )
{
  TCG_UID            TableTableUid;
  TCG_CREATE_STRUCT  CreateStruct;
  TCG_PARSE_STRUCT   ParseStruct;
  UINT32             SendSize;
  UINT8              MethodStatus;
  ...
  /* sed-opal UIDs for table table preconfig can be split in two halves:
   *  first:  actual table index
   *  second: relative index in the table
   * so we have to get the first half of the OPAL_TABLE_TABLE and use the
   * first part of the target table as relative index into that table
   */
  TableTableUid = OPAL_TABLE_TABLE;
  CopyMem (((UINT8 *)&TableTableUid + sizeof (TCG_UID)/2), &TableUid, sizeof (TCG_UID)/2);

  // Init Opal Header
  ERROR_CHECK (TcgInitTcgCreateStruct (&CreateStruct, Buffer, BUFFER_SIZE));
  ERROR_CHECK (TcgStartComPacket (&CreateStruct, Session->OpalBaseComId, Session->ComIdExtension));                 // Init ComPacket
  ERROR_CHECK (TcgStartPacket (&CreateStruct, Session->TperSessionId, Session->HostSessionId, 0x0, 0x0, 0x0));      // Init Packet Tsn, Hsn
  ERROR_CHECK (TcgStartSubPacket (&CreateStruct, 0x0));                                                             // Init SubPacket
  ERROR_CHECK (TcgStartMethodCall (&CreateStruct, TableTableUid, TCG_UID_METHOD_GET));                              // Init Table UID & Method Call GET
  ...
  ERROR_CHECK (TcgAddStartList (&CreateStruct));
  ERROR_CHECK (TcgAddStartName (&CreateStruct));
  ERROR_CHECK (TcgAddUINT8 (&CreateStruct, TCG_CELL_BLOCK_START_COLUMN_NAME));                                      // Column START Token
  ERROR_CHECK (TcgAddUINT64 (&CreateStruct, TCG_TABLE_ROWS));                                                       // Set Column START Number
  ERROR_CHECK (TcgAddEndName (&CreateStruct));
  ERROR_CHECK (TcgAddStartName (&CreateStruct));
  ERROR_CHECK (TcgAddUINT8 (&CreateStruct, TCG_CELL_BLOCK_END_COLUMN_NAME));                                        // Column END Token
  ERROR_CHECK (TcgAddUINT64 (&CreateStruct, TCG_TABLE_ROWS));                                                       // Set Column END Number(Reading 1 Row only, therefore START equ END)
  ERROR_CHECK (TcgAddEndName (&CreateStruct));
  ERROR_CHECK (TcgAddEndList (&CreateStruct));
  ...
  ERROR_CHECK (TcgEndSubPacket (&CreateStruct));
  ERROR_CHECK (TcgEndPacket (&CreateStruct));
  ERROR_CHECK (TcgEndComPacket (&CreateStruct, &SendSize));

  //
  // Send Get Method Call to retrieve Table size from Column
  //
  ERROR_CHECK (OpalPerformMethod (Session, SendSize, Buffer, BUFFER_SIZE, &ParseStruct, &MethodStatus, 0));
  METHOD_STATUS_ERROR_CHECK (MethodStatus, TcgResultFailure);

  //
  // Parse Response to get Table Size
  //
  ERROR_CHECK (TcgGetNextStartList (&ParseStruct));
  ERROR_CHECK (TcgGetNextStartList (&ParseStruct));
  ERROR_CHECK (TcgGetNextStartName (&ParseStruct));
  ERROR_CHECK (TcgGetNextUINT32 (&ParseStruct, &Col));
  ERROR_CHECK (TcgGetNextUINT64 (&ParseStruct, TableSize));

  return TcgResultSuccess;
}

/*
  This function reads content of Table which is identified by TableUID.
Read Range is [Offset, Offset + Size)
  ...
  @retval     EFI_SUCCESS - Succeed in reading data from OPAL table
  @retval     Others      - Fail to read data from OPAL table
  ...
  )
{
  ...
  while (RemainingSize > 0) {
    ...
    ERROR_CHECK (TcgStartPacket (&CreateStruct, Session->TperSessionId, Session->HostSessionId, 0x0, 0x0, 0x0));      // Init Packet Tsn, Hsn
    ERROR_CHECK (TcgStartSubPacket (&CreateStruct, 0x0));                                                             // Init SubPacket
    ERROR_CHECK (TcgStartMethodCall (&CreateStruct, TableUid, TCG_UID_METHOD_GET));                                   // Init Table UID & Method Call GET

    /*  Every method call is followed by its parameters enclosed within
           * OPAL_STARTLIST and OPAL_ENDLIST tokens. We automatically open the
           * parameter list here and close it later in TcgEndParameters.
           */
    ERROR_CHECK (TcgStartParameters (&CreateStruct));
    ...
    ERROR_CHECK (TcgAddStartList (&CreateStruct));
    ERROR_CHECK (TcgAddStartName (&CreateStruct));
    ERROR_CHECK (TcgAddUINT8 (&CreateStruct, TCG_CELL_BLOCK_START_ROW_NAME));                                         // Row START Token
    ERROR_CHECK (TcgAddUINT64 (&CreateStruct, Offset + (Size - RemainingSize)));                                      // Set Row START Number
    ERROR_CHECK (TcgAddEndName (&CreateStruct));
    ERROR_CHECK (TcgAddStartName (&CreateStruct));
    ERROR_CHECK (TcgAddUINT8 (&CreateStruct, TCG_CELL_BLOCK_END_ROW_NAME));                                          // Row END Token
    ReadLength = MIN (OPAL_MAX_READ_TABLE, RemainingSize);                                                           // Set Row END Number
    ERROR_CHECK (TcgAddUINT64 (&CreateStruct, Offset + (Size - RemainingSize) + ReadLength - 1));                    //
    ERROR_CHECK (TcgAddEndName (&CreateStruct));
    ERROR_CHECK (TcgAddEndList (&CreateStruct));
    ...
    //
    // Init Get Method Call to retrieve Table size from Column
    //
    ERROR_CHECK (OpalPerformMethod (Session, SendSize, Buffer, BUFFER_SIZE, &ParseStruct, &MethodStatus, 0));
    METHOD_STATUS_ERROR_CHECK (MethodStatus, TcgResultFailure);

    //
    // Parse Response to get Store Table
    //
    ERROR_CHECK (TcgGetNextStartList (&ParseStruct));
    ERROR_CHECK (TcgGetNextByteSequence (&ParseStruct, (const VOID **)&ByteString, &ByteStringLen));

    CopyMem (Data + (Size - RemainingSize), ByteString, ByteStringLen);
    //
    // After Reading a portion of data, goto next read iteration
    //
...
/*
  This function saves data to OPAL Table which is identified by TableUID.
  ...
  @retval     EFI_SUCCESS - Succeed in writing data to OPAL table
  @retval     Others      - Fail to write data to OPAL table
  ...
  )
{
  ...
  while (RemainingSize > 0) {
    ...
    ERROR_CHECK (TcgStartPacket (&CreateStruct, Session->TperSessionId, Session->HostSessionId, 0x0, 0x0, 0x0));      // Init Packet Tsn, Hsn
    ERROR_CHECK (TcgStartSubPacket (&CreateStruct, 0x0));                                                             // Init SubPacket
    ERROR_CHECK (TcgStartMethodCall (&CreateStruct, TableUid, TCG_UID_METHOD_SET));                                   // Init Table UID & Method Call SET
    ...
    //
    ERROR_CHECK (TcgAddStartName (&CreateStruct));
    ERROR_CHECK (TcgAddWhere (&CreateStruct));
    ERROR_CHECK (TcgAddUINT64 (&CreateStruct, Offset + (Size - RemainingSize)));                                      // Set Write Size
    ERROR_CHECK (TcgAddEndName (&CreateStruct));

    ERROR_CHECK (TcgAddStartName (&CreateStruct));
    ERROR_CHECK (TcgAddValues (&CreateStruct));

    WriteLength = MIN (400, RemainingSize);
    ERROR_CHECK (TcgAddByteSequence (&CreateStruct, Data + (Size - RemainingSize), (UINT32)WriteLength, FALSE));            // Copy Write Data
    ERROR_CHECK (TcgAddEndName (&CreateStruct));
    ...
    //
    // Init Set Method Call to Save Table Content
    //
    ERROR_CHECK (OpalPerformMethod (Session, SendSize, Buffer, BUFFER_SIZE, &ParseStruct, &MethodStatus, 0));
    // Exit with success on method failure - user must inspect MethodStatus
    METHOD_STATUS_ERROR_CHECK (MethodStatus, TcgResultSuccess);
    //
    // After writing a portion of data, goto next write iteration
    //
...
/**
  ...
  The function sets ACE attribute to Get/Set Method Datastore Table
  1.  OPAL_LOCKING_SP_ACE_DATASTORE_GET_ALL Admin1 or User1
  2.  OPAL_LOCKING_SP_ACE_DATASTORE_SET_ALL Admin1 Only

  @param[in]      LockingSpSession    OPAL_SESSION with OPAL_UID_LOCKING_SP
  @param[in/out]  MethodStatus        Method status of last action performed.  If action succeeded, it should be TCG_METHOD_STATUS_CODE_SUCCESS.

**/
TCG_RESULT
EFIAPI
OpalSetLockingAuthorityDatastoreAccess (
  OPAL_SESSION  *LockingSpSession,
  UINT8         *MethodStatus
  )
{
  ...
  //
  // allow admin/user1 to get DataStore by modifying ACE_Locking_GlobalRange_Get_All
  //
  ERROR_CHECK (TcgInitTcgCreateStruct (&CreateStruct, Buf, sizeof (Buf)));
  ERROR_CHECK (
    TcgCreateSetAce (
      ...
      LockingSpSession->TperSessionId,
      LockingSpSession->HostSessionId,
      OPAL_LOCKING_SP_ACE_DATASTORE_GET_ALL,
      OPAL_LOCKING_SP_USER1_AUTHORITY,
      TCG_ACE_EXPRESSION_OR,
      OPAL_LOCKING_SP_ADMINS_AUTHORITY
      )
  ...
  if (*MethodStatus != TCG_METHOD_STATUS_CODE_SUCCESS) {
    DEBUG ((DEBUG_INFO, "Update ACE for  failed\n"));
    return TcgResultFailure;
  }

  //
  // allow admin to Set DataStore by modifying ACE_Locking_GlobalRange_Get_All
  //
  ERROR_CHECK (TcgInitTcgCreateStruct (&CreateStruct, Buf, sizeof (Buf)));
  ERROR_CHECK (
    TcgCreateSetAce (
      ...
      LockingSpSession->TperSessionId,
      LockingSpSession->HostSessionId,
      OPAL_LOCKING_SP_ACE_DATASTORE_SET_ALL,
      OPAL_LOCKING_SP_ADMINS_AUTHORITY,
      TCG_ACE_EXPRESSION_OR,
      OPAL_LOCKING_SP_ADMINS_AUTHORITY
      )
