...
EFI_STATUS
EFIAPI
Tpm2CommandDisEndorsementEnStorageHierarchy (
  ...
  )
{
  ...
  Status = Tpm2HierarchyControl (TPM_RH_PLATFORM, AuthSession, TPM_RH_OWNER, YES);
  ...
  )
{
  ...
  switch (CommandCode) {
    ...
    case TCG2_PHYSICAL_PRESENCE_DISABLE_ENDORSEMENT_ENABLE_STORAGE_HIERARCHY:
      Status = Tpm2CommandDisEndorsementEnStorageHierarchy (PlatformAuth);
      if (EFI_ERROR (Status)) {
        return TCG_PP_OPERATION_RESPONSE_BIOS_FAILURE;
      } else {
        return TCG_PP_OPERATION_RESPONSE_SUCCESS;
  ...
  switch (TcgPpData->PPRequest) {
    ...
    case TCG2_PHYSICAL_PRESENCE_DISABLE_ENDORSEMENT_ENABLE_STORAGE_HIERARCHY:
      if (  ((Flags.PPFlags & TCG2_BIOS_TPM_MANAGEMENT_FLAG_PP_REQUIRED_FOR_TURN_ON) == 0) \
         || ((Flags.PPFlags & TCG2_BIOS_TPM_MANAGEMENT_FLAG_PP_REQUIRED_FOR_TURN_OFF) == 0))
      {
        *RequestConfirmed = TRUE;
      }
  ...
  )
{
  ...
  BOOLEAN                           HaveTrustedDisplay;
  CHAR16                            *ResetSystemStr;
  ...
  if (TcgPpData->PPRequest >= TCG2_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    if (!RequestConfirmed) {
      //
      // Get all handleBuffers that support the protocol.Then checks for the existence of its devicepath.
      //
      Status = gBS->LocateHandleBuffer (ByProtocol, &gEfiSimpleTextOutProtocolGuid, NULL, &BufferCount, &HandleBuffer);
      if (EFI_ERROR (Status)) {
        HaveTrustedDisplay = FALSE;
      } else {
      ...
      if (HaveTrustedDisplay == TRUE) {
        //
        // If there is a trusted display device Print confirm text and wait for approval.
        //
        RequestConfirmed = Tcg2UserConfirm (TcgPpData->PPRequest, TcgPpData->PPRequestParameter);
      } else {
        //
        // If no trusted display device exists, confirm the modification directly.(This is a security risk.)
        //
        RequestConfirmed = TRUE;
      }
    }
    ...
    Space = AllocateZeroPool (RightColumn*sizeof (CHAR16)+2);
    if (Space == NULL) {
      return;
    }

    for (Index = 0; Index < RightColumn; Index++ ) {
      Space[Index] = L' ';
    }
  ...
  ResetSystemStr = Tcg2PhysicalPresenceGetStringById (STRING_TOKEN (TPM_RESET_SYSTEM_PROMPT_STR));
  Print (ResetSystemStr);
  FreePool (ResetSystemStr);
...
EFI_STATUS
EFIAPI
DxeTcg2PhysicalPresenceLibConstructor (
...
EFI_STATUS
EFIAPI
DxeTcg2PhysicalPresenceLibDestructor (
  IN EFI_HANDLE                            ImageHandle,
  IN EFI_SYSTEM_TABLE                      *SystemTable
  )
{
