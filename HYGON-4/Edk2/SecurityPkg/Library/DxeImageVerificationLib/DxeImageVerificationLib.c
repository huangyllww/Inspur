...
UINT8  mHashOidValue[] = {
  ...
  0x2A, 0x81, 0x1C, 0xCF, 0x55, 0x01, 0x83, 0x11,       // OBJ_sm3 (1.2.156.10197.1.401)
};
...
HASH_TABLE  mHash[] = {
  ...
  { L"SM3",    32, &mHashOidValue[41], 8, Sm3GetContextSize,    Sm3Init,    Sm3Update,    Sm3Final    },
};
  ...
  )
{
  ...
  for (Index = 0; Index < HASHALG_MAX; Index++) {
    ...
    //
    // Here we need to consider 2 signeddata format. Their only difference is in OID
    //  1. RFC Pkcs7 SignedData. OID  1.2.840.113549.1.7.2. 9-Byte
    //  2. GMT0010 Pkcs7 SM2 SignedData. OID 1.2.156.10197.6.1.4.2.2, 10-byte
    //
    if (CompareMem (AuthData + 32, mHash[Index].OidValue, mHash[Index].OidLength) == 0 ||    // RFC P7 OID
        CompareMem (AuthData + 33, mHash[Index].OidValue, mHash[Index].OidLength) == 0) {    // GMT0010 P7 OID
      break;
    }
  }
  ...
  )
{
  ...
  *SignatureList = NULL;
  ...
  DEBUG ((DEBUG_INFO, "SignatureDataSize %x\n", SignatureDataSize));
  ...
  )
{
  ...
  if ((SecDataDir == NULL) || (SecDataDir->Size == 0)) {
    ...
    // The following condition means that there is no error in finding the image hash in DBX and the image hash is not in DBX
    //
    if (!EFI_ERROR (DbStatus) && !IsFound) {
      ...
      // The following condition means that finished the search without any error and the image hash is not in DB
      //
      if (!EFI_ERROR (DbStatus) && IsFound) {
...
Done:
  if (Status != EFI_SUCCESS) {
    ...
    //
    // When image authentication fails and Policy is DEFER_EXECUTE_ON_SECURITY_VIOLATION
    // return SECURITY_VIOLATION to allow image to load.
    // Otherwise return error to reject image loading.
    //
