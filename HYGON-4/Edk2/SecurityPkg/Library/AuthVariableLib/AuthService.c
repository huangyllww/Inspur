...
CONST UINT8  mSm3OidValue[] = { 0x2A, 0x81, 0x1C, 0xCF, 0x55, 0x01, 0x83, 0x11 };
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "SecureBootMode is %x\n", SecureBootMode));
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "SecureBootMode Update to %x\n", NewMode));
    mSecureBootMode = NewMode;
  } else {
    DEBUG ((DEBUG_ERROR, "SecureBootMode Update failure %x\n", Status));
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Update SecureBootMode Variable fail %x\n", Status));
  }
  ...
  if (NewMode == SecureBootModeTypeDeployedMode) {
    ...
    if (mAuthVarLibContextIn->AtRuntime ()) {
      ...
      //
      //
      // The secure boot module must copy the MEM to switch SecureBootMode
      // and synchronize Volatile Cache to prevent BIOS Setup from obtaining Variable Value incorrectly
      //
      Status = mAuthVarLibContextIn->SyncronizeVariableCache (Sync_Volatile_Variable_Cache);
      if (EFI_ERROR (Status)) {
        ASSERT (FALSE);
      }
    ...
    DEBUG ((DEBUG_ERROR, "Invalid state tranition from %x to %x\n", SecureBootModeTypeAuditMode, NewMode));
    ASSERT (FALSE);
  }
  ...
  )
{
  ...
  switch (NewMode) {
    ...
    case SecureBootModeTypeAuditMode:
      ...
      if (EFI_ERROR (Status)) {
        DEBUG ((DEBUG_ERROR, "UserMode -> AuditMode. Delete PK fail %x\n", Status));
        ASSERT (FALSE);
      }
  ...
  )
{
  ...
  switch (CurMode) {
    case SecureBootModeTypeUserMode:
      DEBUG ((DEBUG_INFO, "SecureBootModeTypeUserMode \n"));
      Status = TransitionFromUserMode (NewMode);
      break;

    case SecureBootModeTypeSetupMode:
      DEBUG ((DEBUG_INFO, "TransitionFromSetupMode \n"));
      Status = TransitionFromSetupMode (NewMode);
      break;

    case SecureBootModeTypeAuditMode:
      DEBUG ((DEBUG_INFO, "SecureBootModeTypeAuditMode \n"));
      Status = TransitionFromAuditMode (NewMode);
      break;

    case SecureBootModeTypeDeployedMode:
      DEBUG ((DEBUG_INFO, "SecureBootModeTypeDeployedMode \n"));
  ...
  )
{
  ...
  Status = AuthServiceInternalFindVariable (EFI_CUSTOM_MODE_NAME, &gEfiCustomModeEnableGuid, (VOID **)&Data, &DataSize);
  ...
  )
{
  ...
  //
  if (StrCmp (VariableName, EFI_AUDIT_MODE_NAME) == 0) {
    DEBUG ((DEBUG_INFO, "Current SecureBootMode %x Transfer to SecureBootMode %x\n", mSecureBootMode, SecureBootModeTypeAuditMode));
  ...
  if (!EFI_ERROR (Status) && IsPk) {
    ...
    if (!Del) {
      if (mSecureBootMode == SecureBootModeTypeSetupMode) {
        ...
        DEBUG ((DEBUG_INFO, "PK is updated in %x mode. No SecureBootMode change.\n", mSecureBootMode));
      }
    } else {
  ...
  )
{
  ...
  Status = EFI_SUCCESS;
  if (((mSecureBootMode == SecureBootModeTypeUserMode) || (mSecureBootMode == SecureBootModeTypeDeployedMode)) && !(InCustomMode () && UserPhysicalPresent ())) {
    //
  ...
  )
{
  ...
  // signature. Digest algorithms supported are SHA-256, SM3.
  //
  ...
  if ((Attributes & EFI_VARIABLE_TIME_BASED_AUTHENTICATED_WRITE_ACCESS) != 0) {
    if (SigDataSize >= (13 + sizeof (mSha256OidValue))) {
      ...
      if((CompareMem (SigData + 13, &mSha256OidValue, sizeof (mSha256OidValue)) != 0) &&
         (CompareMem (SigData + 13, &mSm3OidValue, sizeof (mSm3OidValue)) != 0)) {
        return EFI_SECURITY_VIOLATION;
      }
    } else {
      return EFI_SECURITY_VIOLATION;
