                 ...
                 BIO *indata, BIO *out, int flags)
{
...
#ifndef OPENSSL_NO_EC  // Support SM2 GMT algoritm only when EC is enabled
    EVP_PKEY *pkey = NULL;
#endif  //OPENSSL_NO_EC
    ...
    if (!(flags & PKCS7_NOSIGS))
        for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(sinfos); i++) {
            ...
            //
            // Patch indata when it is signed by SM3withSM2 with signature format to be signeddata(checked at function entry), detached and NULL verify attribute
            // In GMT0010,
            //
            //  Za = H256 (ENTL || IDa || a || b || Xg || Yg || Xa || Ya) where H256 is usually SM3.
            //   M  = indata(detached) or contentinfo (not detached) without verifyAttribute in SignerInfo
            //   M' = (Za || M)
            //    /* Check for no data and no content: no data to verify signature */
            //
            pkey = X509_get0_pubkey(signer);
            if (pkey && is_sm2_verify_pkey(pkey)) {
              if ((si->auth_attr == NULL) || (sk_X509_ATTRIBUTE_num(si->auth_attr) == 0)) {
                sm2_compute_full_digest(p7bio, pkey);
              }
            }
#endif //OPENSSL_NO_EC
...
/*
  Following GM/T0003.5-2012, calculate full SM2 digest 
  based on Za and M(stored in p7bio).Result is returned in p7bio

  
*/
int sm2_compute_full_digest(BIO *p7bio, EVP_PKEY *pkey)
{
    EVP_MD_CTX *mdx, *za_mdx = NULL;
    int ret = 0, i;
    char *buf = NULL;
    ...
    //
    // If SM3withSM2 siganture is detached or without VerifyAttribute 
    // Reset digest and use Za || M to calcuate digest to verify
    // For generic PKCS#7 handling, InData may be NULL if the content is present
    // in PKCS#7 structure. So ignore NULL checking here.
    //
    BIO_ctrl(p7bio, BIO_CTRL_RESET, 0, NULL);
    BIO_ctrl(p7bio, BIO_C_GET_MD_CTX, 0, &mdx);

    //
    // Allocate Za buffer
    //
    za_size = EVP_MD_meth_get_result_size(EVP_MD_CTX_md(mdx));
    ...
    za_mdx = EVP_MD_CTX_new();
    if (za_mdx == NULL) {
        goto err;
    }

    if (!EVP_MD_CTX_copy_ex(za_mdx, mdx))
        goto err;

    ZeroMem(za, za_size);
    //
    // 1. Za = H256 (ENTL || IDa || a || b || Xg || Yg || Xa || Ya)
    //
    sm2_compute_za_digest(za,
                          EVP_MD_CTX_md(za_mdx),
                          SM2_DEFAULT_USERID,
    ...
    //
    // 2. M' = H256(Za || VerifyAttribute)
    //
    if (!EVP_VerifyUpdate(mdx, za, za_size))
        goto err;


    BIO_ctrl(p7bio, BIO_C_SET_MD_CTX, 0, mdx);

    /* 
      We now have to 'read' from p7bio to calculate VerifyAttribute(in p7bio) digests etc.
    */
    if ((buf = OPENSSL_malloc(BUFFERSIZE)) == NULL) {
        PKCS7err(PKCS7_F_PKCS7_VERIFY, ERR_R_MALLOC_FAILURE);
        goto err;
