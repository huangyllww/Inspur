...
int sm2_compute_za_digest(uint8_t *out,
                          const EVP_MD *digest,
                          const uint8_t *id,
                          const size_t id_len,
                          const EC_KEY *key)
                          ...
                          X509 *x509)
{
    ...
    if ((sk != NULL) && (sk_X509_ATTRIBUTE_num(sk) != 0)) {
        ...
        //
        // If using Jit EC SM2 X509 Sign Cert to verify signature
        ...
        if (is_sm2_verify_pkey(pkey)) {
            ...
            za_size = EVP_MD_meth_get_result_size(EVP_MD_CTX_md(mdc_tmp));
            ...
            sm2_compute_za_digest(za,
                                  EVP_MD_CTX_md(mdc_tmp),
                                  SM2_DEFAULT_USERID,
                                  SM2_DEFAULT_USERID_LEN,
                                  EVP_PKEY_get0_EC_KEY(pkey)
                                  );
            //
            // Reset mdc_tmp because it is used before to calculate contentInfo digest
            //
            if (!EVP_VerifyInit_ex(mdc_tmp, EVP_get_digestbynid(md_type), NULL))
                goto err;
            //
            // 2. M' = H256(Za || VerifyAttribute)
            //    First Za, then VerifyAttribute
            //
            if (!EVP_VerifyUpdate(mdc_tmp, za, za_size))
                goto err;
        }
#endif //OPENSSL_NO_SM2
