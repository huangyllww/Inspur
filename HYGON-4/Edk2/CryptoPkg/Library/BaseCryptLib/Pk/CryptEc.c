...
/**
  Initialize EC group with given parameter

  @param[in]       p_hex       Hex String of P.
  @param[in]       a_hex       Hex String of a
  @param[in]       b_hex       Hex String of b
  @param[in]       x_hex       Hex String of Gx
  @param[in]       y_hex       Hex String of Gy
  @param[in]       order_hex   Hex String of Order

  @return  Pointer to created EC group
  ...
  )
{
    ...
    if (BN_hex2bn(&g_x, x_hex) == 0
            || BN_hex2bn(&g_y, y_hex) == 0
            || EC_POINT_set_affine_coordinates(group, generator, g_x, g_y, NULL) ==0)
        goto done;

    if (BN_hex2bn(&order, order_hex) == 0
            || BN_hex2bn(&cof, cof_hex) == 0
            || EC_GROUP_set_generator(group, generator, order, cof) == 0)
        goto done;
...
/**
  Allocates and initializes one EC Key context. It can be used for SM2 schema.

  @return  Pointer to the EC Key context that has been initialized.
           If the allocations fails, EC_KEY_new() returns NULL.
...
{
  //
  // Allocates & Initializes EC Context by OpenSSL EC_KEY_new()
  //
  return (VOID *) EC_KEY_new();
}

/**
  Release the specified EC context used in ECC scheme(SM2 for exmaple).

  @param[in]  EcKey  Pointer to the ECKey context to be released.
...
EcGetKeyInfo (
  ...
  OUT     UINT8        *Xa,
  OUT     UINT8        *Xb,
  IN OUT  UINTN        *BnXaSize,
  IN OUT  UINTN        *BnXbSize
  )
{
  EC_KEY         *EcKey;
  UINTN          XaSize;
  ...
  //
  if (EcContext == NULL || BnXaSize == NULL || BnXbSize == NULL) {
    return FALSE;
  }
  ...
  *BnXbSize = 0;
  ...
  EC_POINT_get_affine_coordinates(group, point, tx, ty, NULL);

  XaSize = BN_num_bytes (tx);
  XbSize = BN_num_bytes (ty);
