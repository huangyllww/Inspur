  ...
  )
{
  ...
  if ((TSToken == NULL) || (TsaCert == NULL) || (TimestampedData == NULL) ||
      (TokenSize > INT_MAX) || (TokenSize <= 16) || (CertSize > INT_MAX) || (DataSize > INT_MAX))
  {
    return FALSE;
  }
  ...
  //
  // TSToken is counter signature. Carried by unauthenticated attribute in up-layer PKCS7 SignedData, TSToken is in RFC PKCS7 signed data format
  //
  // OpensSSL can only process RFC PKCS7 SignedData. While GMT0010 P7 SignedData structure
  // is almost identical to RFC P7 except the difference in SignedData OID,
  // to parse GMT0010 P7 data, our solution is straightforward:
  //   Transition GMT0010 to P7 format by replacing GMT OID with RFC OID, then send to OpenSSL
  //   for parsing.
  //
  Status = WrapPkcs7Data (TSToken, TokenSize, &Wrapped, &SignedData, &SignedDataSize);
  if (!Status || (SignedDataSize > INT_MAX)) {
    goto _Exit;
  }
  TSToken  = SignedData;
  TokenSize = SignedDataSize;
  ...
  )
{
  ...
  if ((DataSize > INT_MAX) || (DataSize <= 16) || (CertSize > INT_MAX)) {
    return FALSE;
  }
