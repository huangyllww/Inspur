...
GLOBAL_REMOVE_IF_UNREFERENCED const UINT8  mGmtOidValue[10] = { 0x2A, 0x81, 0x1C, 0xCF, 0x55, 0x06, 0x01, 0x04, 0x02, 0x02};     // GMT0010 SignedData OID
...
IsGmtSignedData(
  ...
  if ((P7Data[4] == 0x06) && (P7Data[5] == 0x0A)                                // GMT0010 SignedData OID DER-encode header
      && (CompareMem (P7Data + 6, mGmtOidValue, sizeof (mGmtOidValue)) == 0)    // GMT0010 SignedData OID
      && (P7Data[16] == 0xA0) && (P7Data[17] == 0x82)) {                        // DigestAlgorithm DER-encode header
      return TRUE;
  }
  ...
  )
{
  ...
  /*
    2 PKCS7 SignedData Conversion rules
    1. Convert Gmt0010 SignedData to RFC SignedData, using RFC SignedData OID to replace Gmt0010 SignedDataOID. Keep others unchanged.
    2. Wrapper P7 signature generated by MSFT Signtool, adding a signeddata OID.

  */

  //
  // Rule 1: Convert GMT SignedData to RFC counterpart
  //
  if (IsGmtSignedData(P7Data, P7Length)) {
    P7RealLength = (P7Data[2] << 8) + P7Data[3] + 4;
    //
    // P7Data may have extra padding '0x00' at tail. They must be trimmed
    // after converting GMT P7 to RFC P7
    //
    // Actual P7 length should less or equal to P7Length
    //
    if (P7RealLength > P7Length) {
      *WrapFlag = TRUE;
      return FALSE;
    }

    //
    // Converted SignedData is 1 byte smaller
    //
    *WrapDataSize = P7RealLength - 1;
    ...
    *WrapFlag = FALSE;
    ...
    //
    // Part3: RFC OID size 0x06, 0x09.
    //
    SignedData[4] = 0x06;
    SignedData[5] = 0x09;

    //
    // Part4: use RFC OID value -- 0x2A 0x86 0x48 0x86 0xF7 0x0D 0x01 0x07 0x02.
    //
    CopyMem (SignedData + 6, mOidValue, sizeof (mOidValue));

    //
    // Part5: Append rest P7 data.
    //
    CopyMem (SignedData + 15, P7Data + 16, P7RealLength - 16);
