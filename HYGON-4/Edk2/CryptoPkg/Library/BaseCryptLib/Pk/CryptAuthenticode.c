  ...
  )
{
  ...
  if ((DataSize > INT_MAX) || (DataSize <= 16) || (CertSize > INT_MAX) || (HashSize > INT_MAX)) {
    return FALSE;
  }
  ...
  //
  // OpensSSL can only process RFC PKCS7 SignedData. However, GMT0010 P7 SignedData structure
  // is almost same as RFC P7 except the difference in SignedData OID. 
  // Our solution is straightforward: to replace GMT OID with RFC OID, then send to OpenSSL 
  // for parsing.
  //
