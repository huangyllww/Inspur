  ...
  )
{
  ...
  Print (L" >> Start HTTP Boot over IPv%d\n", Private->UsingIpv6 ? 6 : 4);
  ...
  )
{
  ...
  if (Private->BootFileUri == NULL) {
    ...
    if (EFI_ERROR (Status)) {
      AsciiPrint (" >> Error: Could not retrieve NBP file size from HTTP server.\n");
      goto ON_EXIT;
    }
  }
...
ON_EXIT:
  ...
  if (EFI_ERROR (Status)) {
    if (Status == EFI_ACCESS_DENIED) {
      AsciiPrint (" >> Error: Could not establish connection with HTTP server.\n");
    } else if ((Status == EFI_BUFFER_TOO_SMALL) && (Buffer != NULL)) {
      AsciiPrint (" >> Error: Buffer size is smaller than the requested file.\n");
    } else if (Status == EFI_OUT_OF_RESOURCES) {
      AsciiPrint (" >> Error: Could not allocate I/O buffers.\n");
    } else if (Status == EFI_DEVICE_ERROR) {
      AsciiPrint (" >> Error: Network device error.\n");
    } else if (Status == EFI_TIMEOUT) {
      AsciiPrint (" >> Error: Server response timeout.\n");
    } else if (Status == EFI_ABORTED) {
      AsciiPrint (" >> Error: Remote boot cancelled.\n");
    } else if (Status != EFI_BUFFER_TOO_SMALL) {
      AsciiPrint (" >> Error: Unexpected network error.\n");
    }
  }
  ...
  )
{
  ...
  //
  // Prompt Description of Currrent Network BootOption
  //
  ...
  Print (L"\n");
  Status = EfiGetControllerName (VirtualNic->Private->Controller, &NetworkName);
  if (!EFI_ERROR (Status)) {
    Print (L" >> Network Card Name : %s\n", NetworkName);
    FreePool (NetworkName);
  }

  TempDp = VirtualNic->DevicePath;
  while (!IsDevicePathEnd (TempDp) &&
         ((DevicePathType (TempDp) != MESSAGING_DEVICE_PATH) ||
          (DevicePathSubType (TempDp) != MSG_MAC_ADDR_DP))
         )
  {
  ...
  if (!IsDevicePathEnd (TempDp)) {
    Mac = (MAC_ADDR_DEVICE_PATH *)TempDp;
    Print (
  ...
  if (VirtualNic == VirtualNic->Private->Ip6Nic) {
    Print (L" >> Netwrok Versions  : IPv6\n");
  } else {
    Print (L" >> Netwrok Versions  : IPv4\n");
  }

  //
  // Show vendor of netcard
  //
  TempDp = VirtualNic->DevicePath;
  Status = gBS->LocateDevicePath (&gEfiPciIoProtocolGuid, &TempDp, &DevHandle);
  if (!EFI_ERROR (Status)) {
    Status = gBS->HandleProtocol (DevHandle, &gEfiPciIoProtocolGuid, (VOID **)&PciIo);
    if (!EFI_ERROR (Status)) {
      DidVid    = 0xffffffff;
      SubVidDid = 0xffffffff;
      PciIo->Pci.Read (
                   PciIo,
                   EfiPciIoWidthUint32,
                   0,
      ...
      Print (L" >> DidVid Sub-DidVid : 0x%08x 0x%08x\n", DidVid, SubVidDid);
    }
  }
}
  ...
  )
{
  ...
  UINTN                   Size;
  BOOLEAN                 DetectedMediaSuccess = FALSE;
  EFI_STATUS              EventStatus;
  EFI_EVENT               WaitList[2];
  UINTN                   EventIndex;
  EFI_INPUT_KEY           Key;
  BOOLEAN                 ExitHttpBoot = FALSE;
  ...
  //
  PrintNetworkInfo (VirtualNic);

  if (PxeControl.WaitSeconds != 0) {
    while (gST->ConIn->ReadKeyStroke(gST->ConIn,&Key) == EFI_SUCCESS) {};   // flush keyboard buffer

    //
    // Wait for seconds before every http booting and press ESC key to exit from current http booting
    //
    Print (L" >> Wait time in %d seconds to press ESC key to abort HTTP boot\n", PxeControl.WaitSeconds);
    gBS->CreateEvent (EVT_TIMER, 0, NULL, NULL, &WaitList[0]);
    gBS->SetTimer (WaitList[0], TimerRelative, PxeControl.WaitSeconds * 10 * 1000 * 1000); // wait for PxeControl.WaitSeconds seconds
  ...
  Print (L" >> HTTP Boot...\n");
  do {
    Print (L" >> Checking Media Presence...\n");
    MediaStatus = EFI_SUCCESS;
    NetLibDetectMediaWaitTimeout (Private->Controller, HTTP_BOOT_CHECK_MEDIA_WAITING_TIME, &MediaStatus);
    if (MediaStatus != EFI_SUCCESS) {
      PxeControl.DetectMediaTimes--;
    } else {
      DetectedMediaSuccess = TRUE;
    }
  } while ((PxeControl.DetectMediaTimes > 0) && !DetectedMediaSuccess);

  if (MediaStatus != EFI_SUCCESS) {
    AsciiPrint (" >> Error: Could not detect network connection.\n");
    Status = EFI_NO_MEDIA;
    goto Exit;
  }
  ...
  if ((ImageType == ImageTypeVirtualCd) || (ImageType == ImageTypeVirtualDisk)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      AsciiPrint (" >> Error: Could not register RAM disk to the system.\n");
    }
  }
