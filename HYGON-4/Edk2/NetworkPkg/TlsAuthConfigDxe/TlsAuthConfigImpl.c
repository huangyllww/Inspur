    ...
    switch (QuestionId) {
      ...
      case KEY_TLS_AUTH_CONFIG_VALUE_SAVE_AND_EXIT:
        ...
        if (EFI_ERROR (Status)) {
          ...
          String = HiiGetString (Private->RegisteredHandle, STRING_TOKEN (STR_TLS_AUTH_ENROLL_CERT_FAILURE), NULL);
          CreatePopUp (
      ...
      case KEY_TLS_AUTH_CONFIG_CERT_GUID:
        InputSignatureString = HiiGetString (Private->RegisteredHandle, Value->string, NULL);
        ...
        if (RETURN_ERROR (RStatus) || (InputSignatureString[GUID_STRING_LENGTH] != L'\0')) {
          String = HiiGetString (Private->RegisteredHandle, STRING_TOKEN (STR_CORRECT_FORMAT), NULL);
          CreatePopUp (
            EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE,
            &Key,
            String,
