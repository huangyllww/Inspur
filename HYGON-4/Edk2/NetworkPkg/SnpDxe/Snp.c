...
#define INIT_MAX_COUNT 256
#define INCREMENT_OF_COUNT 50
...
SNP_MEM_CTX *gSnpMemCtx = NULL;
UINTN       gSnpMemCtxCountMax = INIT_MAX_COUNT;
UINTN       gSnpMemCtxCount;

VOID *GetSnpDriverAddress(EFI_DEVICE_PATH_PROTOCOL *Dp)
{
  UINTN  Index;

  for(Index=0; Index<gSnpMemCtxCount; Index++){
    if(UefiCompareDevicePath(Dp, gSnpMemCtx[Index].NiiDp)){
      return gSnpMemCtx[Index].SnpDriver;
    }
  }
...
VOID SetSnpDriverAddress(EFI_DEVICE_PATH_PROTOCOL *Dp, VOID *SnpDriver)
{
  UINTN  Index;
  if(gSnpMemCtx == NULL){
    gSnpMemCtx = (SNP_MEM_CTX *)AllocateZeroPool(sizeof(SNP_MEM_CTX) * gSnpMemCtxCountMax);
    ASSERT(gSnpMemCtx != NULL);
    gSnpMemCtxCount = 0;
  }
  ...
  if(Index >= gSnpMemCtxCount){
    if(gSnpMemCtxCount == gSnpMemCtxCountMax){
      gSnpMemCtx = ReallocatePool(sizeof(SNP_MEM_CTX) * gSnpMemCtxCountMax,
        sizeof(SNP_MEM_CTX) * (gSnpMemCtxCountMax + INCREMENT_OF_COUNT), gSnpMemCtx);
      ASSERT(gSnpMemCtx != NULL);
      gSnpMemCtxCountMax = gSnpMemCtxCountMax + INCREMENT_OF_COUNT;
    }

    gSnpMemCtx[gSnpMemCtxCount].SnpDriver = SnpDriver;
    gSnpMemCtx[gSnpMemCtxCount].NiiDp = DuplicateDevicePath(Dp);
    gSnpMemCtxCount++;
  } else {
    gSnpMemCtx[Index].SnpDriver = SnpDriver;
  }
}
  ...
  )
{
  ...
  Snp = (SNP_DRIVER*)GetSnpDriverAddress(NiiDevicePath);
  if(Snp == NULL){
    //
    ...
    SetSnpDriverAddress(NiiDevicePath, Snp);
  }
...
Error_DeleteSNP:
  ...
  SetSnpDriverAddress(NiiDevicePath, NULL);
