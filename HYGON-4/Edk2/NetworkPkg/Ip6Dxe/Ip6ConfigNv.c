  ...
  )
{
  ...
  if ((Action != EFI_BROWSER_ACTION_CHANGED) && (Action != EFI_BROWSER_ACTION_FORM_OPEN)) {
    return EFI_UNSUPPORTED;
  }
  ...
  if (Action == EFI_BROWSER_ACTION_FORM_OPEN) {
    ...
    switch (QuestionId) {
      ...
      case KEY_INTERFACE_ID:
        ...
        if (EFI_ERROR (Status)) {
          String = HiiGetString (
                     ...
                     STRING_TOKEN (STR_INVALID_INTERFACE),
                     NULL
                     );
          CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
          FreePool (String);
        }
      ...
      case KEY_MANUAL_ADDRESS:
        Status = Ipv6AddrCheck (IfrNvData->ManualAddress);
        ...
        if (EFI_ERROR (Status)) {
          IsSetBrowserData = TRUE;
          SetMem (IfrNvData->ManualAddress, (UINTN)(ADDRESS_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_HOST),
                     NULL
                     );
          CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
          FreePool (String);
      ...
      case KEY_GATEWAY_ADDRESS:
        Status = Ipv6AddrCheck (IfrNvData->GatewayAddress);
        if (Status == EFI_SUCCESS) {
          Status = Ip6ParseAddressListFromString (
        ...
        if (EFI_ERROR (Status)) {
          IsSetBrowserData = TRUE;
          SetMem (IfrNvData->GatewayAddress, (UINTN)(ADDRESS_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_GATEWAY),
                     NULL
                     );
          CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
          FreePool (String);
      ...
      case KEY_DNS_ADDRESS:
        Status = Ipv6AddrCheck (IfrNvData->DnsAddress);
        if (Status == EFI_SUCCESS) {
          Status = Ip6ParseAddressListFromString (
        ...
        if (EFI_ERROR (Status)) {
          IsSetBrowserData = TRUE;
          SetMem (IfrNvData->DnsAddress, (UINTN)(ADDRESS_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_DNS),
                     NULL
                     );
          CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
          FreePool (String);
  ...
  if (!EFI_ERROR (Status) || IsSetBrowserData) {
    //
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    UpdateStringWithMac (CallbackInfo->RegisteredHandle, STRING_TOKEN (STR_IP6_CONFIG_FORM_HELP), MacString);
    FreePool (MacString);
