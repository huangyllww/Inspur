  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    if((PcdGetBool(PcdTlsForceServerTrust))==TRUE){
      //
      //Do not get certificates from the variable area if allow to enforce trust server
      //
      Status=EFI_SUCCESS;
    }else{
...
VOID
AutoEnrollCertificate(
  IN  HTTP_PROTOCOL  *HttpInstance,
  IN  UINT8          *BufferData
)
{

  TLS_RECORD_CERTIFICATE    *Certificate;
  UINT32                    CrtSize=0;
  UINT8                     *CrtData;
  EFI_STATUS                Status;
  ...
  Certificate = (TLS_RECORD_CERTIFICATE*)BufferData;
  if((Certificate->TlsRecord.ContentType == TlsContentTypeHandshake)&&(Certificate->TlsRecord.Length!=0)){
    //
    //CrtSize is the size of certificate
    //
    CrtSize |=Certificate->CertificateLen[0]<<16 | Certificate->CertificateLen[1]<<8 | Certificate->CertificateLen[2];
    //
    // HandshakeType is 11 if handshake protocol is Certificate
    //
    if((Certificate->HandshakeType==11)&&(CrtSize!=0)){
      //
      //Get the data of certificate
      //
      CrtData=AllocateZeroPool(CrtSize);
      if (CrtData == NULL) {
        return ;
      }
      CopyMem(CrtData,BufferData+sizeof(TLS_RECORD_CERTIFICATE),CrtSize);
      Status = HttpInstance->TlsConfiguration->SetData (
                                      HttpInstance->TlsConfiguration,
                                      EfiTlsConfigDataTypeCACertificate,
                                      CrtData,
      ...
      if (EFI_ERROR (Status)) {
        DEBUG ((EFI_D_INFO,"AutoEnrollCertificate Failed\n"));
      }
      FreePool(CrtData);
    }
  }
  ...
  )
{
         ...
         ((Timeout == NULL) || EFI_ERROR (gBS->CheckEvent (Timeout))))
  {
    ...
    //
    // Calculates the tls packets received from the server to client
    //
    TlsReceiceTlsCount++;
    ...
    if((PcdGetBool(PcdTlsForceServerTrust))==TRUE){
      //
      // tls handshake step from server to client
      // attention: Each step contains multiple tls records
      // first  : 1> Server Hello, 
      //          2> Certificate
      // second : 1> Server Key Exchange
      //          2> Server Hello Done
      // third  : 1> New Session Ticket
      //          2> Change Cipher Spec
      //          3> Encrypted Handshake Message
      //
      if(TlsReceiceTlsCount==TLS_SERVER_HELLO_CERTIFICATE){
        //
        // We need to enroll the certificate in the first step and second tls record
        //
        AutoEnrollCertificate(HttpInstance,BufferIn);
      }
    }
    Status = HttpInstance->Tls->BuildResponsePacket (
                                  HttpInstance->Tls,
