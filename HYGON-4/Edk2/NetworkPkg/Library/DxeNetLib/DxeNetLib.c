...
/**
  This function updates the Language based on
  the Language supported by HiiHandle.

  @param[in]  HiiHandle     The package list in the HII database to
                            search for the specified string.
  @param[in]  TokenId       A pointer to the token associated with the transaction.
  @param[in]  MacString     The pointer to store the address of the string
                            representation of  the mac address.
...
UpdateStringWithMac (
  ...
  IN EFI_STRING_ID   TokenId,
  IN EFI_STRING      MacString
  )
{
  ...
  //
  // Loop throgh each language that the string supports
  //
  SupportedLanguages = HiiGetSupportedLanguages (HiiHandle);
  for (Supported = SupportedLanguages; *Supported != '\0'; ) {
    //
    // Cache a pointer to the baginning of the current language in the list of languages
    //
    Language = Supported;
    ...
    //
    String = HiiGetString (HiiHandle, TokenId, Language);
    if (String == NULL) {
      String = HiiGetString (HiiHandle, TokenId, NULL);
    }

    ASSERT (String != NULL);

    Size       = (StrSize (String) + StrSize (MacString) + 10 * 2) * sizeof (CHAR16);
    MenuString = AllocateZeroPool (Size);
    UnicodeSPrint (MenuString, Size, L"%s (MAC:%s)", String, MacString);
    HiiSetString (HiiHandle, TokenId, MenuString, Language);
    FreePool (String);
    FreePool (MenuString);
  }
...
/**
  ...
  Filter some invalid IPv4 addresses.

  @param[in]  Ip4Addr  - The pointer to the variable to store the value to
  @param[in]  Type  - The type of the IPv4 addresses
  ...
  )
{
  ...
  for (i = 0; i < 4; i++) {
    if (Ip4Addr->Addr[i] > 255) {
      return EFI_INVALID_PARAMETER;
    }
  }

  if ((Ip4Addr->Addr[0] == 0) && (Ip4Addr->Addr[1] == 0) && (Ip4Addr->Addr[2] == 0) && (Ip4Addr->Addr[3] == 0)) {
    return EFI_INVALID_PARAMETER;
  }

  if (Type != TYPE_IP4_MASK) {
    if ((Ip4Addr->Addr[0] == 255) && (Ip4Addr->Addr[1] == 255) && (Ip4Addr->Addr[2] == 255) && (Ip4Addr->Addr[3] == 255)) {
      return EFI_INVALID_PARAMETER;
    }
  }
...
/**
  ...
  Filter some invalid IPv6 addresses.
  ...
  )
{
  ...
  /*
    if 0000:0000:0000...appear,return invalid
  */
  for (i = 0; i < SrcLen; i++) {
