...
/**
  ...
  @param[in] DevicePath               Pointer to the mnp device path.

  @retval TRUE                        This device is a usb network device.
  @retval FALSE                       This device is not a usb network device.
  ...
  )
{
  ...
  if(*DevicePath==NULL){
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "Open DevicePathProtocol, %r.\n", Status));
      return FALSE;
    }
    gBS->CloseProtocol (
        MnpDeviceData->ControllerHandle,
  ...
  )
{
  ...
  TriggerTime   = MNP_SYS_POLL_INTERVAL;
  ...
  if(IsUsbNetDevicePath((VOID**)&(MnpServiceData->DevicePath),MnpDeviceData)){
    // For usb network card
    // The communication speed of the usb NIC is slow
    // So you need to increase the interval
    TriggerTime=MNP_SYS_POLL_USB_INTERVAL;
  }
  ...
  if (MnpDeviceData->EnableSystemPoll ^ EnableSystemPoll) {
    ...
    Status = gBS->SetTimer (MnpDeviceData->PollTimer, TimerOpType, TriggerTime);
    if (EFI_ERROR (Status)) {
