  ...
  )
{
  ...
  if (PxeControl.WaitSeconds != 0) {
    ...
    //
    // Wait for seconds before every pxe booting and press ESC key to exit from current pxe booting
    //
    Print (L" >> Wait time in %d seconds to press ESC key to abort PXE boot\n", PxeControl.WaitSeconds);
  ...
  //
  // Check media status before PXE start.Retry for PcdDetectMediaTimes times when no media detected.
  //
  Print (L" >> PXE Boot...\n");
  do {
