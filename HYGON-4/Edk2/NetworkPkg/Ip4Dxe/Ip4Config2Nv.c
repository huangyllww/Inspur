  ...
  if (IfrFormNvData->DhcpEnable == TRUE) {
    ...
    if (EFI_ERROR (Status) || ((SubnetMask.Addr[0] != 0) && (GetSubnetMaskPrefixLength (&SubnetMask.v4) == 0))) {
      String = HiiGetString (
                 ...
                 STRING_TOKEN (STR_INVALID_SUBNET),
                 NULL
                 );
        ...
        ((Gateway.Addr[0] != 0) && (SubnetMask.Addr[0] != 0) && !NetIp4IsUnicast (NTOHL (Gateway.Addr[0]), NTOHL (SubnetMask.Addr[0]))))
    {
      String = HiiGetString (
                 CallbackInfo->RegisteredHandle,
                 STRING_TOKEN (STR_INVALID_GATEWAY_IP),
                 NULL
                 );
      CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
      FreePool (String);
  ...
  )
{
  ...
  if (Action == EFI_BROWSER_ACTION_CHANGED) {
    ...
    switch (QuestionId) {
      case KEY_LOCAL_IP:
        ...
        if (Status == EFI_SUCCESS) {
          Status = Ipv4AddrCheck (&StationAddress.v4, TYPE_IP4_ADDR);
        }

        if (EFI_ERROR (Status)) {
          SetMem (IfrFormNvData->StationAddress, (UINTN)(IP4_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_IP),
                     NULL
      ...
      case KEY_SUBNET_MASK:
        Status = Ip4Config2StrToIp (IfrFormNvData->SubnetMask, &SubnetMask.v4);
        if (Status == EFI_SUCCESS) {
          Status = Ipv4AddrCheck (&SubnetMask.v4, TYPE_IP4_MASK);
        }

        if (EFI_ERROR (Status)) {
          SetMem (IfrFormNvData->SubnetMask, (UINTN)(IP4_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_SUBNET),
                     NULL
      ...
      case KEY_GATE_WAY:
        Status = Ip4Config2StrToIp (IfrFormNvData->GatewayAddress, &Gateway.v4);
        if (Status == EFI_SUCCESS) {
          Status = Ipv4AddrCheck (&Gateway.v4, TYPE_IP4_GATEWAY);
        }

        if (EFI_ERROR (Status)) {
          SetMem (IfrFormNvData->GatewayAddress, (UINTN)(IP4_STR_MAX_SIZE), '\0');
          String = HiiGetString (
                     CallbackInfo->RegisteredHandle,
                     STRING_TOKEN (STR_INVALID_GATEWAY_IP),
                     NULL
      ...
      case KEY_DNS:
        ...
        if (!EFI_ERROR (Status) && (DnsCount > 0)) {
          for (Index = 0; Index < DnsCount; Index++) {
            CopyMem (&Ip, &DnsAddress[Index], sizeof (IP4_ADDR));
            if (IP4_IS_UNSPECIFIED (NTOHL (Ip)) || IP4_IS_LOCAL_BROADCAST (NTOHL (Ip))) {
              SetMem (IfrFormNvData->DnsAddress, (UINTN)(ADDRESS_STR_MAX_SIZE), '\0');
              CreatePopUp (EFI_LIGHTGRAY | EFI_BACKGROUND_BLUE, &Key, String, NULL);
              Status = EFI_INVALID_PARAMETER;
              break;
            }
    ...
    if ( Status != EFI_SUCCESS) {
      HiiSetBrowserData (&gIp4Config2NvDataGuid, mIp4Config2StorageName, sizeof (IP4_CONFIG2_IFR_NVDATA), (UINT8 *)IfrFormNvData, NULL);
    }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    UpdateStringWithMac (CallbackInfo->RegisteredHandle, STRING_TOKEN (STR_IP4_CONFIG2_FORM_HELP), MacString);
    FreePool (MacString);
