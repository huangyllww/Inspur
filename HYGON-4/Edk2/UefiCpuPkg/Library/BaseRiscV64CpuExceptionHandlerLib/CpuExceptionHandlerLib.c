...
#include <Library/DebugCallStackLib.h>
...
STATIC EFI_CPU_INTERRUPT_HANDLER  mExceptionHandlers[EXCEPT_RISCV_MAX_EXCEPTIONS + 1] = {0};
STATIC EFI_CPU_INTERRUPT_HANDLER  mIrqHandlers[EXCEPT_RISCV_MAX_IRQS + 1] = {0};
  ...
  )
{
  ...
  if (EXCEPT_RISCV_IS_IRQ (ExceptionType)) {
    ...
    DEBUG ((DEBUG_ERROR, "%a %d: Type:%x Handler: %x\n", __func__, __LINE__, ExceptionType, InterruptHandler));
    mIrqHandlers[EXCEPT_RISCV_IRQ_INDEX (ExceptionType)] = InterruptHandler;
    DEBUG ((DEBUG_ERROR, "%a %d: Type:%lx Handler: %x\n", __func__, __LINE__, EXCEPT_RISCV_IRQ_INDEX (ExceptionType),
                 mIrqHandlers[EXCEPT_RISCV_IRQ_INDEX (ExceptionType)]));
    DEBUG ((DEBUG_ERROR, "%a %d: Type:%x mIrqHandlers[5]: %lx\n", __func__, __LINE__, 5,
                 mIrqHandlers[5]));
    DEBUG ((DEBUG_ERROR, "%a %d: Type:%x &mIrqHandlers[5]: %lp\n", __func__, __LINE__, 5,
                 &mIrqHandlers[5]));
  } else {
    if (ExceptionType > EXCEPT_RISCV_MAX_EXCEPTIONS) {
      return EFI_UNSUPPORTED;
  ...
  )
{
  ...
  //DEBUG ((DEBUG_ERROR, "%a %d: ExceptionType:%lx\n", __func__, __LINE__, ExceptionType));
  if (EXCEPT_RISCV_IS_IRQ (ExceptionType)) {
    IrqIndex = EXCEPT_RISCV_IRQ_INDEX (ExceptionType);
    //DEBUG ((DEBUG_ERROR, "%a %d: IrqIndex:%lx\n", __func__, __LINE__, IrqIndex));
    //DEBUG ((DEBUG_ERROR, "%a %d: mIrqHandlers[%lx]:%p\n", __func__, __LINE__, IrqIndex, mIrqHandlers[IrqIndex]));
  ...
  //
  // Display all call stack trace information
  //
  ShowCallStackByPcFp (SmodeTrapReg->ra, SmodeTrapReg->s0, 0);
  CpuDeadLoop ();
}
