  ...
  )
{
  ...
  if (CpuMpData->WakeupBuffer == (UINTN)-1) {
    CpuMpData->WakeupBuffer = (UINTN)PcdGet32 (PcdCpuS3ApVectorAddress);
    if (CpuMpData->WakeupBuffer == 0) {
      CpuMpData->WakeupBuffer = GetWakeupBuffer (CpuMpData->BackupBufferSize);
    }

    DEBUG ((DEBUG_INFO, "AllocateResetVector Init, WakeupBuffer:0x%8X\n", CpuMpData->WakeupBuffer));
  ...
  )
{
  ...
  if (OldCpuMpData == NULL) {
    if (MaxLogicalProcessorNumber > 1) {
      ...
      //
      CpuMpData->MicroCodeNeedLoad = TRUE;
    }
  } else {
    //
