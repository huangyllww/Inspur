  ...
  )
{
  ...
  if (PcdGet32 (PcdCpuS3ApVectorAddress) != 0) {
    //
    // Try to load microcode by itself
    //
    if (CpuMpData->MicroCodeNeedLoad == FALSE) {
      //
      // No micro code is required to be loaded.
      //
      return;
    }
    ...
    if (ProcessorNumber == CpuMpData->BspNumber) {
      //
      // Prepare MicroCode for AP
      //
      PrepareMicroCode (CpuMpData);

      //
      // Skip BSP Microcode loading, Microcode has been loaded in Sec.
      //
      return;
    }
    ...
    if (CpuMpData->MicroCode != NULL) {
      //
      // Load MicroCode for AP
      //
      AsmWriteMsr64 (
        MSR_IA32_BIOS_UPDT_TRIG,
        (UINTN)CpuMpData->MicroCode + sizeof (CPU_MICROCODE_HEADER)
        );
