...
CpuSmmDebugEntryAbsAddr:
    ...
    mov     rcx, rbx
    mov     qword [rsp], 0              ;   clear top of stack
    mov     rax, strict qword 0         ;   call    ASM_PFX(SmiRendezvousHook)
SmiRendezvousHookAbsAddr:
    call    rax
...
ASM_PFX(PiSmmCpuSmiEntryFixupAddress):
    ...
    lea    rax, [ASM_PFX(SmiRendezvousHook)]
    lea    rcx, [SmiRendezvousHookAbsAddr]
    mov    qword [rcx - 8], rax
...
global ASM_PFX(SmiRendezvousHook)
ASM_PFX(SmiRendezvousHook):
    add     rsp, -0x28  ; stack alignment to 16 bytes 
