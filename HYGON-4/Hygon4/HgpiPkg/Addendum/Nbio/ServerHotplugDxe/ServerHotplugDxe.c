...
EFI_STATUS CheckI2cDeviceValid (UINT8 I2cAddress)
{
  EFI_STATUS               Status;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((DEBUG_INFO, "ERROR: I2C protocol not ready\n"));
    return Status;
  }
  DEBUG((DEBUG_INFO, "I2C protocol = %d\n", (UINT8)NumberOfHandles));
  ...
  if (EFI_ERROR (Status)) {
    DEBUG((DEBUG_INFO, "ERROR: I2C protocol can not be handle\n"));
    return Status;
  }
  ...
  Operation[0].Buffer           = (UINT8 *)&Offset;
  Operation[1].Flags            = I2C_FLAG_READ;
  Operation[1].LengthInBytes    = sizeof (UINT16);
  Operation[1].Buffer           = (UINT8 *)&Data[0];

  Status = I2cMaster->StartRequest (I2cMaster, I2cAddress, RequestPacket, NULL, NULL);
...
EFI_STATUS SetNbioHotplugDescriptorProtocol(VOID)
{
    ...
    HotplugDescriptor = AllocateZeroPool (HotplugUserDescriptorCount * sizeof(HOTPLUG_DESCRIPTOR));
    if(HotplugDescriptor == NULL) {
        ASSERT(FALSE);
    ...
    {
        IsValidDevice = TRUE;
        Status = CheckI2cDeviceValid(HotplugUserDescriptor[i].I2CSlaveAddress);
        if (EFI_ERROR(Status)) {
            DEBUG((DEBUG_INFO, "ERROR: the I2C(0x%x) is %r!!! Please check it...\n", HotplugUserDescriptor[i].I2CSlaveAddress, Status));
            IsValidDevice = FALSE;
        }
        ...
        HotplugDescriptor[i].Mapping.PortActive               = IsValidDevice ? 1 : 0;
...
)
{
    ...
    if (PcdGetBool (PcdCfgPcieHotplugSupport) == FALSE) {
        DEBUG((DEBUG_ERROR, "HotplugDescEntry hotplug unsupport\n"));
        return EFI_SUCCESS;
    }
