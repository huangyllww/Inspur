...
#include <Pi/PiBootMode.h>                                     // byo230831 +
#include <Pi/PiHob.h>                                          // byo230831 +
#include <Library/HobLib.h>                                    // byo230831 +
#include <Library/ByoSharedSmmData.h>                          // byo230906 +
  ...
  )
{
  ...
  *Progress = Configuration;
  ...
  if (!EFI_ERROR (Status)) {
    if(gByoSharedSmmData->CbsSyncMmLibOn == 0){           // byo230906 +
      HygonSaveCbsConfigData (Private->IfrData);
    }                                                     // byo230906 +
  }
  ...
  )
{
  ...
  if ((Action == EFI_BROWSER_ACTION_FORM_OPEN) || (Action == EFI_BROWSER_ACTION_FORM_CLOSE) ||     // byo230831 -
       Action == EFI_BROWSER_ACTION_SUBMITTED) {                                                   // byo230831 +
    return EFI_SUCCESS;
  }
...
// byo230831 + <<
  ...
  Status = HygonCbsSetupCallbackRoutine (This, Action, QuestionId, Type, Value, ActionRequest, (UINT8 *)Private->IfrData, Private->HiiHandle);
  CbsOptionControlDependOnStepping (Private->IfrData);
  if (!EFI_ERROR (Status)) {
    //
  ...
  )
{
  ...
  if (HpcbRecoveryFlag) {
    DEBUG ((EFI_D_ERROR, "HPCB Recovery enabled\n"));
    //
    // Get Buffer Storage data from EFI variable.
    // Try to get the current setting from variable.
    //
...
// byo230831 - >>
  ...
  HobData = (CBS_CONFIG*)(GuidHob.Guid + 1);
  CopyMem(Private->IfrData, HobData, CbsVariableSize);
// byo230831 - <<
