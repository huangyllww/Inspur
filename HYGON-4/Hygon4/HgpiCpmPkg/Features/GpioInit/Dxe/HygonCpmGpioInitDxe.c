...
VOID
EFIAPI
HygonCpmGpioInitLate (
  IN      EFI_EVENT        Event,
  IN      VOID             *Context
  );

VOID
HygonCpmDxePcieClockInit (
  IN      HYGON_CPM_TABLE_PROTOCOL  *CpmTableProtocolPtr
  );

VOID
HygonCpmDxeGeventInit (
  IN      HYGON_CPM_TABLE_PROTOCOL  *CpmTableProtocolPtr
  );

VOID
HygonCpmGeventSmiControlInit (
  IN      HYGON_CPM_TABLE_PROTOCOL  *CpmTableProtocolPtr
  );
...
/**
 * Entry point of the HYGON CPM GPIO Init DXE driver
 *
  ...
  )
{
  ...
  HygonCpmGpioInitMid (CpmTableProtocolPtr);
  HygonCpmGpioInitMid (CpmTableProtocolPtr);
...
{
  STATIC BOOLEAN  HygonCpmGpioInitLateInvoked = FALSE;

  if (!HygonCpmGpioInitLateInvoked) {
    HygonCpmGeventSmiControlInit (Context);
    HygonCpmDxePcieClockInit (Context);
  }

  HygonCpmGpioInitLateInvoked = TRUE;
}
...
{
  STATIC BOOLEAN  HygonCpmGpioInitMidInvoked = FALSE;

  if (!HygonCpmGpioInitMidInvoked) {
    HygonCpmDxeGeventInit (Context);
  }

  HygonCpmGpioInitMidInvoked = TRUE;
}

/*----------------------------------------------------------------------------------------*/
VOID
HygonCpmDxeAddOneItemInClkList (
  ...
  )
{
  ...
  if (CpuModel == HYGON_EX_CPU) {
    ...
    ExtClkGenTablePtr    = CpmTableProtocolPtr->CommonFunction.GetTablePtr (CpmTableProtocolPtr, HYEX_CPM_SIGNATURE_EXT_CLKGEN);
    ...
    ExtClkGenTablePtr    = CpmTableProtocolPtr->CommonFunction.GetTablePtr (CpmTableProtocolPtr, HYGX_CPM_SIGNATURE_EXT_CLKGEN);
  }
  ...
  if (CpmPcieClockTablePtr && CpmTableProtocolPtr->MainTablePtr->UnusedGppClkOffEn) {
    ...
    while (TablePtr->ClkId != 0xFF) {
      ...
      if (ClockType) {
        ...
        HygonCpmDxeAddOneItemInClkList (ExtClkGenTablePtr, &ClkGen[0], &Index, TablePtr->ClkIdExt, TablePtr->ClkReqExt);
      }
