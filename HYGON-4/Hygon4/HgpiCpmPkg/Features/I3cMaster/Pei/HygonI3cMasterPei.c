...
#include <Ppi/I3cMasterPpi.h>
#include <I3cRegs.h>
#include "HygonI3cMasterPei.h"
...
Routine Description:
  ...
  I3cMaster Driver Entry. Publish I3cMaster Ppi
...
EFI_STATUS
EFIAPI
HygonI3cMasterPeiInit (
  IN       EFI_PEI_FILE_HANDLE  FileHandle,
  IN CONST EFI_PEI_SERVICES     **PeiServices
  )
{
  EFI_STATUS                       Status = EFI_SUCCESS;
  HYGON_PEI_I3C_MASTER_PRIVATE     *I3cPrivate;
  EFI_PEI_PPI_DESCRIPTOR           *PpiListI3c;
  ...
  Status = (*PeiServices)->AllocatePool (
                             PeiServices,
                             sizeof (HYGON_PEI_I3C_MASTER_PRIVATE),
                             &I3cPrivate
                             );
  ...
  Status = (*PeiServices)->AllocatePool (
                             PeiServices,
                             sizeof (EFI_I3C_CONTROLLER_CAPABILITIES),
                             &I3cControllerCapabilities
                             );
  ASSERT (!EFI_ERROR (Status));
  ...
  for (Index = 0; Index < I3C_CONTROLLER_COUNT; Index++) {
    I3cPrivate->I3cMasterPpi[Index].PeiServices   = (EFI_PEI_SERVICES **)PeiServices;
    I3cPrivate->I3cMasterPpi[Index].I3cSdpAddress = I3cSdpAddressList[Index];
    I3cPrivate->I3cMasterPpi[Index].I3cPpi.SetBusFrequency = SetBusFrequency;
    I3cPrivate->I3cMasterPpi[Index].I3cPpi.Reset = Reset;
    I3cPrivate->I3cMasterPpi[Index].I3cPpi.StartRequest = StartRequest;
    I3cPrivate->I3cMasterPpi[Index].I3cPpi.I3cControllerCapabilities = I3cControllerCapabilities;
    I3cPrivate->I3cMasterPpi[Index].I3cI2cMode = I3C_MODE;
    CopyGuid (&I3cPrivate->I3cMasterPpi[Index].I3cPpi.Identifier, &HygonI3cMasterID);
    I3cInit (&I3cPrivate->I3cMasterPpi[Index]);
  }

  // Install the PPIs
  Status = (*PeiServices)->AllocatePool (
                             PeiServices,
                             (sizeof (EFI_PEI_PPI_DESCRIPTOR) * I3C_CONTROLLER_COUNT),
                             &PpiListI3c
                             );
  ...
  for (Index = 0; Index < I3C_CONTROLLER_COUNT; Index++) {
    ...
    PpiListI3c[Index].Guid = &gEfiPeiI3cMasterPpiGuid;
    PpiListI3c[Index].Ppi  = &(I3cPrivate->I3cMasterPpi[Index].I3cPpi);

    if (Index != (I3C_CONTROLLER_COUNT - 1)) {
      PpiListI3c[Index].Flags = (EFI_PEI_PPI_DESCRIPTOR_PPI);
    } else {
      PpiListI3c[Index].Flags = (EFI_PEI_PPI_DESCRIPTOR_PPI | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST);
    }
  }
...
/**
 *
 * This function use for change I3C SDA_HOLD_SWITCH_DLY_TIMING register default
 * value from I3cSdaHoldTimeList.
 *
 *
 * @param[in]   I3cSdpAddress
 * @param[out]  EFI_STATUS
 *
 */
EFI_STATUS
I3cSetSdaHold (
  IN UINT32               I3cSdpAddress
  )
{
  UINT8  I3cBus;

  for (I3cBus = 0; I3cBus < (sizeof (I3cSdpAddressList) / sizeof (UINT32)); I3cBus++) {
    if (I3cSdpAddress == I3cSdpAddressList[I3cBus]) {
      if(I3cSdaHoldTimeList[I3cBus].OverrideEn) {
        MmioWrite32 (I3cSdpAddress + SDA_HOLD_SWITCH_DLY_TIMING, I3cSdaHoldTimeList[I3cBus].SdaHold);
      }
      break;
    }
  }
...
EFI_STATUS
I3cInit (
  IN OUT   I3CMASTER_PPI_PRIVATE    *Private
  )
{
  EFI_STATUS                 Status;
  UINT32                     Base;
  ...
  // Set I3c Frequency
  Status = SetBusFrequency (&Private->I3cPpi, &BusClockHertz);
 ...
 *
 * @param This  Pointer to EFI_PEI_I3C_MASTER_PPI
  ...
  )
{
  ...
  Private = (I3CMASTER_PPI_PRIVATE *)This;
  Base    = Private->I3cSdpAddress;
  ...
  )
{
  ...
  Status = I3cAccess ((I3CMASTER_PPI_PRIVATE *)This, (UINT32)SlaveAddress, RequestPacket);
 ...
 * @brief This function transmit/receive Data.
 *
 * @param This  Pointer to I3CMASTER_PPI_PRIVATE
 * @param SlaveAddress   Slave Device Address
 * @param RequestPacket  Pointer to EFI_I3C_REQUEST_PACKET
 *
 * @returns EFI_STATUS EFI_SUCCESS   success.
...
EFI_STATUS
I3cAccess (
  IN I3CMASTER_PPI_PRIVATE         *Private,
...
/**
*  @brief This function operate I3c Software Reset
*  @param Base - Base address of I3cController
*
*  @returns EFI_STATUS
...
*  @retval EFI_SUCCESS   Master Idle
*/
EFI_STATUS
I3cWaitMasterIdle (
  IN  UINT32   Base
...
*
*  @returns EFI_STATUS
*  @retval EFI_TIMEOUT - aborts fail
*  @retval EFI_SUCCESS - aborts success
*/
EFI_STATUS
I3cAbort (
  IN  UINT32   Base
...
/**
  ...
  @param Private - pointer to the I3C master private PPI
