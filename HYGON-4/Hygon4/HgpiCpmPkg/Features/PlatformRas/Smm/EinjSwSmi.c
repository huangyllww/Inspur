...
UINT8   mCddNumForType4Einj = 0xFF;
...
UINT8
GetNBIOBusNum (
  UINT8 Socket, UINT8 RbNumber
  )
{
  UINT8  IohcBusNum;

  // Get IOHC Bus number based on socket and root bridge number
  IohcBusNum = (UINT8)FabricTopologyGetHostBridgeBusBase (Socket, 0, RbNumber);
...
UINT16
EFIAPI
HygonSmiEinjExecuteActionProcessor (
  VOID
  )
{
...
UINT16
EFIAPI
HygonSmiEinjExecuteActionMemory (
  VOID
  )
{
  APEI_EINJ_REGISTERS     *EinjActMemRegAddr;
  ...
  // Find Cdds and Channel ID use TranslateSysAddrToCS function.
  mHygonRasSmmProtocol->TranslateSysAddrToCS (&gSysMemAddr, &NormalizedAddress, &DimmInfo);
  SocketNum  = NormalizedAddress.normalizedSocketId;
  CddNum     = NormalizedAddress.normalizedCddId;
  Channel    = NormalizedAddress.normalizedChannelId;
  SubChannel = NormalizedAddress.normalizedSubChannelId;
  ChipSelect = DimmInfo.ChipSelect;

  mCddNumForType4Einj = CddNum;
  //
  // Execute error injection based upon requested Error Type
  //
  switch (ErrorType) {
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_CORRECTABLE: // Correctable Memory Error
      DEBUG ((EFI_D_ERROR, "Memory ECC Error Injection to Socket %x, Cdd %x, UMC %x, SubChannel %x, CS %x\n", SocketNum, CddNum, Channel, SubChannel, ChipSelect));
      // Inject correctable error
      OperationStatus = MemEccErrInject (EINJ_ERROR_VECTOR_CORR, SocketNum, CddNum, Channel, SubChannel, ChipSelect, gSysMemAddr);
    ...
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_NONFATAL:
      DEBUG ((EFI_D_ERROR, "[Type 4] Memory ECC Error Injection to Socket %x, Cdd %x, UMC %x, CS %x\n", SocketNum, CddNum, Channel, ChipSelect));
      // Inject un-correctable error
      OperationStatus = MemEccErrInject (EINJ_ERROR_VECTOR_UNCORR, SocketNum, CddNum, Channel, SubChannel, ChipSelect, gSysMemAddr);
      if (OperationStatus == EFI_ACPI_6_0_EINJ_STATUS_INVALID_ACCESS) {
        DEBUG ((EFI_D_ERROR, "EINJ Invalid Access\n"));
        break;
      }
      ...
      // Get IOHC bus number
      IohcBusNum = GetNBIOBusNum (SocketNum, 0);
      // Get UMC registers base address
      if (SubChannel == 0) {
        UmcRegBase = (UINT32)UMC_SPACE (CddNum, Channel, UMC0_CH_REG_BASE);
      } else {
        UmcRegBase = (UINT32)UMC_SPACE (CddNum, Channel, UMC0_SUBCH1_REG_BASE);
      }

      // DF::DramScrubBaseAddr ==> 0x0  (disable the DRAM Scrubber)
      RegValue = 0;
      SmnRegisterRead (IohcBusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_BASEADDR_REG), &RegValue);
      RegValue &= ~BIT0;
      SmnRegisterWrite (IohcBusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_BASEADDR_REG), &RegValue, 0);
      ...
      SmnRegisterWrite (IohcBusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ADDR_HI), &DramScrubAddrHi.Value, 0);
      ...
      SmnRegisterWrite (IohcBusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ADDR_LO), &DramScrubAddrLo.Value, 0);
      ...
      mBusNumForType4Einj = IohcBusNum;
    ...
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_FATAL: // Uncorrectable Memory Error
      DEBUG ((EFI_D_ERROR, "Memory ECC Error Injection to Socket %x, Cdd %x, UMC %x, CS %x\n", SocketNum, CddNum, Channel, ChipSelect));
      // Inject un-correctable error
      OperationStatus = MemEccErrInject (EINJ_ERROR_VECTOR_UNCORR, SocketNum, CddNum, Channel, SubChannel, ChipSelect, gSysMemAddr);
      if (OperationStatus == EFI_ACPI_6_0_EINJ_STATUS_INVALID_ACCESS) {
        DEBUG ((EFI_D_ERROR, "EINJ Invalid Access\n"));
...
UINT16
EFIAPI
HygonSmiEinjExecuteActionPlatform (
  VOID
  )
{
  APEI_EINJ_REGISTERS     *EinjActMemRegAddr;
    ...
    case EFI_ACPI_6_0_EINJ_ERROR_PLATFORM_CORRECTABLE:
      ...
      for (i = 0; i < sizeof (gNbioParityErrorTable) / sizeof (NBIO_PARITY_ERROR_TABLE); i++) {
        if (gNbioParityErrorTable[i].GroupType == PARITY_CORRECTABLE_GROUP) {
          RegValue = 0;
          SmnRegisterWrite (IohcBusNum, (CpuModel==HYGON_EX_CPU)?RAS_PARITY_CONTROL_1_HYEX:RAS_PARITY_CONTROL_1_HYGX, &RegValue, 0);
          do {
            SmnRegisterRead (IohcBusNum, (CpuModel==HYGON_EX_CPU)?RAS_PARITY_CONTROL_1_HYEX:RAS_PARITY_CONTROL_1_HYGX, &ParityControl1.Value);
          ...
          SmnRegisterWrite (IohcBusNum, (CpuModel==HYGON_EX_CPU)?RAS_PARITY_CONTROL_1_HYEX:RAS_PARITY_CONTROL_1_HYGX, &ParityControl1.Value, 0);
        }
      }
...
/*********************************************************************************
 * Name: HygonSmiEinjExecuteActionCallback
 *
 * Description
...
EFI_STATUS
EFIAPI
HygonSmiEinjExecuteActionCallback (
  ...
  )
{
  ...
  switch (ErrorType) {
    ...
    case EFI_ACPI_6_0_EINJ_ERROR_PROCESSOR_UNCORRECTABLE_NONFATAL:
    case EFI_ACPI_6_0_EINJ_ERROR_PROCESSOR_UNCORRECTABLE_FATAL:
      OperationStatus = HygonSmiEinjExecuteActionProcessor ();
      break;
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_CORRECTABLE: // Correctable Memory Error
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_NONFATAL:
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_FATAL: // Uncorrectable Memory Error
      OperationStatus = HygonSmiEinjExecuteActionMemory ();
      break;
    case EFI_ACPI_6_0_EINJ_ERROR_PLATFORM_CORRECTABLE:
    case EFI_ACPI_6_0_EINJ_ERROR_PLATFORM_UNCORRECTABLE_FATAL:
      OperationStatus = HygonSmiEinjExecuteActionPlatform ();
      break;
...
EFI_STATUS
EFIAPI
HygonSmiEinjTriggProcessorError (
  VOID
  )
{
  ...
  )
{
  ...
  // DF::DramScrubBaseAddr[DramScrubEn] ==> 1  (Enable the DRAM Scrubber)
  SmnRegisterRead (mBusNumForType4Einj, (UINT32)CDD_SPACE (mCddNumForType4Einj, DF_DFF6_REG_BASE + DRAM_SCRUBBER_BASEADDR_REG), &RegValue);
  RegValue |= BIT0;
  SmnRegisterWrite (mBusNumForType4Einj, (UINT32)CDD_SPACE (mCddNumForType4Einj, DF_DFF6_REG_BASE + DRAM_SCRUBBER_BASEADDR_REG), &RegValue, 0);
  ...
  )
{
  ...
  switch (ErrorType) {
    ...
    case EFI_ACPI_6_0_EINJ_ERROR_PROCESSOR_UNCORRECTABLE_NONFATAL:
    case EFI_ACPI_6_0_EINJ_ERROR_PROCESSOR_UNCORRECTABLE_FATAL:
      Status = HygonSmiEinjTriggProcessorError ();
      break;

    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_CORRECTABLE:  // Correctable Memory Error
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_NONFATAL:
    case EFI_ACPI_6_0_EINJ_ERROR_MEMORY_UNCORRECTABLE_FATAL:  // Uncorrectable Memory Error
      Status = HygonSmiEinjTriggMemoryError ();
      break;
  ...
  )
{
  ...
  SwRegisterContext.HygonSwValue = EinjSwSmiData;  // EINJ software SMI ID
  ...
  SwRegisterContext.HygonSwValue = EinjTrigErrSwSmiId;  // EINJ software SMI ID
  SwRegisterContext.Order = 0x80;
  Status = HygonSwDispatch->Register (
                              HygonSwDispatch,
                              HygonSmiEinjTriggErrCallback,
  ...
  )
{
  ...
  // Get IOHC bus number
  BusNum = GetNBIOBusNum (SocketNum, 0);

  // Get UMC registers base address
  if (SubChannelId == 0) {
    UmcRegBase = (UINT32)UMC_SPACE (CddNum, ChannelId, UMC0_CH_REG_BASE);
  } else {
    UmcRegBase = (UINT32)UMC_SPACE (CddNum, ChannelId, UMC0_SUBCH1_REG_BASE);
  }

  // Init thresholding count to 0xFFE
  // debug  BspMcaThresholdInit (MCA_UMC0_BANK);
  // Put memory error injection code here
  // Injection single bit error to Socket 0 Cdd 0 UmcChannel 0
  // 1. UmcMiscCfg_ch0[DisErrInj] ==> `0b

  SmnRegisterRead (BusNum, UmcRegBase + UMC_MISC_CFG, &UmcMiscCfg.Value);
  if (UmcMiscCfg.Field.DisErrInj) {
    return EFI_ACPI_6_0_EINJ_STATUS_INVALID_ACCESS;
  }

  // 2. check if ECC enabled
  SmnRegisterRead (BusNum, UmcRegBase + UMC_ECC_CTRL, &UmcEccCtrl.Value);
  ...
  RegValue = 0;
  SmnRegisterWrite (BusNum, UmcRegBase + UMC_ECC_ERR_INJ_CTRL, &RegValue, 0);

  // 3. DF::DramScrubBaseAddr ==> 0x0  (disable the DRAM Scrubber)
  RegValue = 0;
  SmnRegisterWrite (BusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_BASEADDR_REG), &RegValue, 0);
  ...
  SmnRegisterWrite (BusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ERRORADDR_LO), &DramScrubErrorAddrLo.Value, 0);
  ...
  SmnRegisterWrite (BusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ERRORADDR_HI), &DramScrubErrorAddrHi.Value, 0);
  ...
  SmnRegisterWrite (BusNum, UmcRegBase + (ChipSelect * 4) + UMC_ECC_ERR_INJ, &UmcEccErrInj.Value, 0);

  // 7. UmcCH::EccErrInjCtrl ==> 0x04  (for Address based error injection)
  SmnRegisterRMW (BusNum, UmcRegBase + UMC_ECC_ERR_INJ_CTRL, ~ECC_ERR_ADDR_EN, ECC_ERR_ADDR_EN, 0);
  ...
  SmnRegisterRMW (BusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ERRORADDR_LO), ~ERR_INJ_EN, ERR_INJ_EN, 0);

  do {
    SmnRegisterRead (BusNum, (UINT32)CDD_SPACE (CddNum, DF_DFF6_REG_BASE + DRAM_SCRUBBER_ERRORADDR_LO), &DramScrubErrorAddrLo.Value);
