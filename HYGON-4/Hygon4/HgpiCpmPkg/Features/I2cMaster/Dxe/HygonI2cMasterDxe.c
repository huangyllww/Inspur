...
/*********************************************************************************
 * Name: HygonI2cMasterDxeInit
 *
 * Description
 *   Entry point of the HYGON I2cMaster DXE driver
  ...
  )
{
  ...
  EFI_HANDLE                       Handle;
  HYGON_I2C_MASTER_PRIVATE         *I2cPrivate;
  ...
  Status = gBS->AllocatePool (EfiBootServicesData, sizeof (HYGON_I2C_MASTER_PRIVATE), &I2cPrivate);
  if (EFI_ERROR (Status)) {
    return Status;
  } else {
    // clear instances content
    gBS->SetMem (I2cPrivate, sizeof (HYGON_I2C_MASTER_PRIVATE), 0);
  }
  ...
  for (Index = 0; Index < I2cCount; Index++) {
    ...
    Status = gBS->InstallMultipleProtocolInterfaces (
                    &I2cPrivate->I2cMaster[Index].Handle,
                    &gEfiI2cMasterProtocolGuid,
                    &I2cPrivate->I2cMaster[Index],
                    NULL
                    );
  }
  ...
  )
{
  ...
  if (Status & DW_IC_INTR_TX_ABRT) {
    ...
    DEBUG ((EFI_D_ERROR, "TX_ABORT Error: %08x\n", TxAbrt));
  }
