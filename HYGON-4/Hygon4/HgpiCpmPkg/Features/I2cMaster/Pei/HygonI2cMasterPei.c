...
EFI_STATUS
EFIAPI
HygonI2cMasterPeiInit (
  IN       EFI_PEI_FILE_HANDLE  FileHandle,
  IN CONST EFI_PEI_SERVICES     **PeiServices
  )
{
  EFI_STATUS                       Status = EFI_SUCCESS;
  HYGON_PEI_I2C_MASTER_PRIVATE     *I2cPrivate;
  ...
  I2cCount = (CpuModel==HYGON_EX_CPU) ? I2C_CONTROLLER_COUNT : (I2C_CONTROLLER_COUNT - 2);
  //
  // Create the PPIs
  //
  Status = (*PeiServices)->AllocatePool (
                             PeiServices,
                             sizeof (HYGON_PEI_I2C_MASTER_PRIVATE),
                             &I2cPrivate
                             );
  ...
  for (Index = 0; Index < I2cCount; Index++) {
    ...
    CopyGuid (&I2cPrivate->I2cMasterPpi[Index].I2cPpi.Identifier, &HygonI2cMasterID);
  ...
  //Status = HygonCddI2cMasterPeiInit (PeiServices);
  ...
  )
{
  ...
  for (I2cBus = 0; I2cBus < (sizeof (I2cSdpAddressList) / sizeof (UINT32)); I2cBus++) {
    if (I2cSdpAddress == I2cSdpAddressList[I2cBus]) {
  ...
  )
{
  ...
  if (Status & DW_IC_INTR_TX_ABRT) {
    TxAbrt = MmioRead32 (Base + DW_IC_TX_ABRT_SOURCE);
    MmioRead32 (Base + DW_IC_CLR_TX_ABRT);
    DEBUG ((EFI_D_ERROR, "I2cTX_ABORT Error: %08x\n", TxAbrt));
  }
