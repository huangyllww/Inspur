...
#define SVC_SAVE_HPOB_DATA                 0x15
...
#define SVC_SPILOAD_VALIDATE_HMAC256_HPOB  0x1E
...
#define SVC_SET_MIN_CSV_ASID               0x3A
#define SVC_GET_MIN_CSV_ASID               0x3B
...
typedef enum _BIOS_DIRECTORY_ENTRY_TYPE {
  BIOS_PUBLIC_KEY    = 0x05,                // BIOS OEM public key, signed by HYGON root private key
  BIOS_RTM_SIGNATURE = 0x07,                // Signed hash of RTM volume concatenated with BIOS DIRECTORY
  BIOS_HPCB_INFO     = 0x60,                // Hgpi PSP Customization Block (HPCB)
  BIOS_HPOB_INFO     = 0x61,                // Hgpi PSP Output Block (HPOB) target location
  BIOS_FIRMWARE      = 0x62,                // BIOS Firmware volumes, also referred as RTM volume with reset flag
  BIOS_HPOB_DATA     = 0x63,                // Hgpi PSP Output Block data and HMAC
  BIOS_HPPB_IMEM     = 0x64,                // Hgpi PMU FW IMEM
  BIOS_HPPB_DMEM     = 0x65,                // Hgpi PMU FW DMEM
  BIOS_HPCB_RO_INFO  = 0x68,                // Hgpi RO PSP Customization Block (HPCB)
  BIOS_HBL_VGA       = 0x6C,                // Hgpi loading VGA information
...
// Master status bits [31:24] assigned as HGPI status bits. It is up to HGPI team
// to define individual bit values.
#define HGPI_MASTER_STATUS_BITS  0xFF000000

#define CLIENT_ID_HGPI  0

/*-----------------------------------------------------------------------------
    Exit to the main Boot Loader. This does not return back to user application.

    Parameters:
        Status  -   either Ok or error code defined by HGPI
*/
__svc (SVC_EXIT) void
Svc_Exit (
  unsigned int Status
...
*/
__svc (SVC_MAP_USER_STACK) unsigned int
Svc_MapUserStack (
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_LOAD_BINARY) unsigned int
Svc_LoadBinary (
  BIOS_DIRECTORY_ENTRY_TYPE_E    PspDirType,
  void *pDest,
...
/*-----------------------------------------------------------------------------
    ...
    Return value:
        ...
        on error    -   NULL
*/
__svc (SVC_MAP_SMN_ON_DIE_NUM) void *Svc_MapSmnOnDieNum (
                                       unsigned int    SmnAddress,
                                       unsigned int    DieNum
                                       );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_UNMAP_SMN) unsigned int
Svc_UnmapSmn (
  void *pAxiAddress
  );
...
*/
__svc (SVC_SET_DEBUG_STATE) void
Svc_SetDebugState (
  uint32_t fEnable
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_UNMAP_SYSHUB) unsigned int
Svc_UnmapSysHub (
  void *pAxiAddress
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_READ_SYSHUB) unsigned int
Svc_ReadSysHub (
  unsigned int    SyshubAddressLo,
  unsigned int    SyshubAddressHi,
  unsigned int *pValue,
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_WRITE_SYSHUB) unsigned int
Svc_WriteSysHub (
  unsigned int SyshubAddressLo,
  unsigned int SyshubAddressHi,
  unsigned int Value,
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_MODEXP) unsigned int
Svc_MODEXP (
  MOD_EXP_PARAMS *pModExParams
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_RSAPSS_VERIFY) unsigned int
Svc_RSAPSSVerify (
  RSAPSS_VERIFY_PARAMS *pRSAPSSParams
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_AES_ECB_ENCRYPT) unsigned int
Svc_AesEcbEncrypt (
  AES_ECB_ENCRYPT_PARAMS *pAesEcbEncryptParam
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_SHA256_OTP) unsigned int
Svc_Sha256OTP (
  char *pHash
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_GET_MCM_INFO) unsigned int
Svc_GetMcmInfo (
  MCM_INFO *pMcmInfo
  );
...
/*-----------------------------------------------------------------------------
    ...
    Note: MasterStatusBit should be one of the bits within XXX_MASTER_STATUS_BITS
          value defined for the specific client, i.e. HGPI_MASTER_STATUS_BITS.
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_SET_MASTER_STATUS) unsigned int
Svc_McmSetMasterStatus (
  unsigned int ClientId, unsigned int MasterStatusBit
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_WAIT_FOR_MASTER_STATUS) unsigned int
Svc_McmWaitForMasterStatus (
  unsigned int ClientId, unsigned int MasterStatusBit
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_MASTER_BROACAST) unsigned int
Svc_McmBroadcastDataToSlaves (
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_SLAVE_GET_BROACAST_DATA) unsigned int
Svc_McmGetBroadcastDataFromMaster (
  unsigned int    ClientId,
  unsigned int    MasterStatusBit,
  unsigned int *pOut,
  unsigned int    Num
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_SAVE_HPOB_DATA) unsigned int
Svc_SaveHpobData (
  void *pHpobData, unsigned int HpobDataSize
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_SEARCH_BIOS_DIR) uint32_t
SVC_BIOSDirectorySearch (
  BIOS_DIR_SEARCH_PARMS *pParms
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_SEARCH_BIOS_DIR_V2) uint32_t
SVC_BIOSDirectorySearchV2 (
  BIOS_DIR_SEARCH_PARMS_V2 *pParms
  );
...
*/
__svc (SVC_GET_DATA_FROM_SLAVES) uint32_t
Svc_GetDataFromSlaves (
  uint32_t ClientId, uint32_t *pData,
...
*/
__svc (SVC_SEND_DATA_TO_MASTER) uint32_t
Svc_SendDataToMaster (
  uint32_t ClientId, uint32_t *pData,
...
*/
__svc (SVC_WAIT_10NS_MULTIPLE) uint32_t
Svc_Wait10nsMultiple (
  uint32_t Multiple
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_GET_BOOT_MODE) uint32_t
Svc_GetBootMode (
  uint32_t *pBootMode
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_GET_DEBUG_UNLOCK_INFO) uint32_t
Svc_GetDebugUnlockInfo (
  void *pData, void *pKdsPubKey, uint32_t KeyLen
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_SLAVE_UNLOCK_REQUEST) uint32_t
Svc_SlaveUnlockRequest (
  void *pData
  );
...
//
// -----------------------------------------------------------------------------
__svc (SVC_SET_DEBUG_UNLOCK_INFO) uint32_t
Svc_SetDebugUnlockInfo (
  void *pData
  );

// Description      : Each die should call this SVC call to PSP boot loader to sign
// S3 data for each die. The master should call Svc_SaveHPOBData()
...
// Return values    : BL_OK or error code
// -----------------------------------------------------------------------------
__svc (SVC_OTP_HMAC256_SIGN) uint32_t
Svc_OTPHmac256Sign (
  uint8_t *pHmac, uint32_t DataSize, uint8_t *pS3Data
  );

// -----------------------------------------------------------------------------
// Description      : Validates HMAC and loads S3 data. Each die should call
// this during S3 resume to get the HPOB data
//
// Parameters       : DataHpobOffset [in]  - S3 DataInfo offset from HPOB entry 0x63
// pDestAddr      [out] - SRAM location to be copied
// DataSize       [in]  - S3 DataSize in bytes to read
// HmacHpobOffset [in]  - SpiOffset from HPOB entry where HMAC is stored
// Will read 32 bytes from offset value provided
//
// Return Values    : BL_OK or error code
//
// ----------------------------------------------------------------------------
__svc (SVC_SPILOAD_VALIDATE_HMAC256_HPOB) uint32_t
Svc_LoadValidateHPOBHmac256 (
  uint32_t DataHpobOffset, uint8_t *pDestAddr, uint32_t DataSize, uint32_t HmacHpobOffset
  );
...
// before HGPI calls Svc_ProgramUmcKeys().
//
...
//
// -----------------------------------------------------------------------------
__svc (SVC_PROGRAM_UMC_KEYS) uint32_t
Svc_ProgramUmcKeys (
  uint32_t Flags, uint16_t AddrTweakEn
  );
...
*/
__svc (SVC_READ_SMBUS_BYTE) uint32_t
Svc_ReadSmbusByte (
  char SlaveAddr, char Command, char *pByteRead
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK, or a BL_ERROR code
*/
__svc (SVC_WRITE_SMBUS_BYTE) uint32_t
Svc_WriteSmbusByte (
  char SlaveAddr, char Command, char ByteWrite
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK, or a BL_ERROR code
*/
__svc (SVC_SYNC_UNLOCK_INFO) uint32_t
Svc_SyncUnlockInfo (
  void *pUnlockInfo
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK, or a BL_ERROR code
*/
__svc (SVC_MCM_SYNC_ON_DATA) uint32_t
Svc_McmSyncOnData (
  uint32_t *data
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value:
        ...
        on error    -   NULL
*/
__svc (SVC_MAP_SYSHUB_EX) void *Svc_MapSysHubEx (
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_READ_SYSHUB_EX) unsigned int
Svc_ReadSysHubEx (
  SYSHUB_RW_PARMS_EX *pParms
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_WRITE_SYSHUB_EX) unsigned int
Svc_WriteSysHubEx (
  SYSHUB_RW_PARMS_EX *pParms
  );
...
/*-----------------------------------------------------------------------------
    ...
    Return value: BL_OK or error code
*/
__svc (SVC_SEND_PSP_SMU_MSG) unsigned int
Svc_SendPspSmuMsg (
  uint32_t MsgId, uint32_t MsgArg, uint32_t *pMsgResp
  );
...
*/
__svc (SVC_MCM_SYNCHRONIZATION_POINT) uint32_t
Svc_McmSynchronizationPoint (
  uint32_t SyncPointFlag
  );

// HDM - remove this when no more need of PCIe dGPU memory hack
__svc (SVC_SET_PSP_SMU_RESERVED) uint32_t
Svc_SetPspSmuReservedAddrs (
  uint32_t PspSecurePhysAddrLo,
  uint32_t PspSecurePhysAddrHi,
  uint32_t SmuReservedPhysAddrLo,
  uint32_t SmuReservedPhysAddrHi
  );

__svc (SVC_HMI_NON_FUNCTIONAL) uint32_t
Svc_HmiNonFunctional (
  void
  );
...
Svc_LoadBinaryByInstance (
  BIOS_DIRECTORY_ENTRY_TYPE_E    BiosDirType,
  void                           *pDest,
...
Svc_EcdsaVerify (
  bool *pIsSigVerified, void *pPublicPointQ, uint8_t *pMsgHash, void *pSignature
  );
...
Svc_EcdsaSign (
  void *pEcdsaSig, void *pPrivate_d, uint8_t *pMsgHash
  );

/*-----------------------------------------------------------------------------
    Programs HMI and xHMI keys, according to parameters, and enables encryption.
    Three out of six CAKEs on each die are programmed for HMI, and one CAKE on each die is programmed for
    xHMI.

    Parameters:
        IsHmiEncrypted - True -> Ciphertext HMI transactions, False -> Plaintext HMI transactions
        IsXHmiEncrypted - True -> Ciphertext xHMI transactions, False -> Plaintext xHMI transactions

*/
__svc (SVC_HMI_NONCE_KEY_PROGRAM) uint32_t
Svc_HmiXHmiLinkKeyProgram (
  bool IsHmiEncrypted, bool IsXHmiEncrypted
  );
...
Svc_SevMasterToSlave (
  uint32_t targetDie, void *pCmdBuf, uint32_t bufSize
  );
...
Svc_CacheOperation (
  uint32_t Operation, uint32_t CacheType, uint32_t VirtAddr, uint32_t Size
  );
...
Svc_GetSecureGasketMode (
  SECURE_GASKET_MODE *pMode
  );
...
Svc_CcpInflateOut (
  uint32_t x86_addr_lo, uint32_t x86_addr_hi, uint32_t psp_addr, size_t size
  );
...
Svc_SetWaflThresholdParams (
  uint32_t Fclk, uint32_t PerfBound
  );

/* This should be called by ABL to set the MIN_CSV_ASID value*/
__svc (SVC_SET_MIN_CSV_ASID) void
Svc_SetMinCsvAsid (
  uint32_t MinCsvAsid
  );

/*
 * This should be called by SEV to get the MIN_CSV_ASID value.
 * Will return 0xFFFFFFFF if no Svc_SetMinCsvAsid was called previously
 */
__svc (SVC_GET_MIN_CSV_ASID) uint32_t
Svc_GetMinCsvAsid (
  void
  );
