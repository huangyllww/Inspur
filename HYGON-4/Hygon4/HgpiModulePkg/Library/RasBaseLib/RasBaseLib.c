  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    MAIN_FLOW,
    "noofbank = %d, noofrm = %d, noofrwolo = %d, noofrowhi = %d, noofcol = %d, noofbg = %d\n",
    noofbank,
    noofrm,
    noofrowlo,
    noofrowhi,
  ...
  IDS_HDT_CONSOLE (
    MAIN_FLOW,
    "RowLoBits = %d, RowHiBits = %d, ColBits = %d, CsBits = %d, BankBits = %d, BangGpBits = %d\n",
    numrowlobits,
    numrowhibits,
    numcolbits,
    numcsbits,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MAIN_FLOW, "calcSysAddr Dxe entry \n");
  ...
  IDS_HDT_CONSOLE (
    MAIN_FLOW,
    "intLvAddrBit = %d, numSocketsInterleaved = %d, numCddInterleaved = %d, numChanInterleaved = %d, numInterleaveBits = %d, hashEnabled = %d\n",
    intLvAddrBit,
    numSocketsInterleaved,
    numCddInterleaved,
    numChanInterleaved,
    numInterleaveBits,
    hashEnabled
  ...
  )
{
  ...
  if (EnDDR5) {
    ...
    if (EnDramChanHash) {
      ...
      normalizedAddress.normalizedSubChannelId = (UINT8)(getBit (12, (UINT32)sysAddr) ^
                                                         getBit (21, (UINT32)sysAddr) ^
                                                         getBit (30, (UINT32)sysAddr) ^
                                                         getBit (Ddr5StartBit, (UINT32)sysAddr));
    } else {
      normalizedAddress.normalizedSubChannelId = (UINT8)(getBit (Ddr5StartBit, (UINT32)sysAddr));
    }
