 ...
 * @file
 *
 * HYGON Fabric IDS hook implementation
 *
 *
...
#define FILECODE  LIBRARY_FABRICIDSHOOKSTLIB_PEI_FABRICIDSHOOKSTLIBPEI_FILECODE
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "FabricStPeiSyncFlood Option\n");
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "RegData = 0x%X\n", RegData);
  ...
  for (i = 0; i < FabricTopologyGetNumberOfSocketPresent (); i++) {
    ...
    for (k = 0; k < MAX_CDDS_PER_SOCKET; k++) {
      if (!IS_CDD_PRESENT (k, CddsPresent)) {
        continue;
      }

      RegValue  = CddFabricRegisterAccRead (i, k, 0x5, 0x104, FABRIC_REG_ACC_BC);
      RegValue &= 0xFFF7FFFF;
      RegValue |= RegData;
      CddFabricRegisterAccWrite (i, k, 0x5, 0x104, FABRIC_REG_ACC_BC, RegValue, FALSE);
    }
  }
...
IDS_HOOK_STATUS
FabricStPeiDfFreezeModuleQueuesOnError (
  HOOK_ID HookId,
  VOID    *Handle,
  VOID    *Data
  )
{
  ...
  // IDS_HDT_CONSOLE (CPU_TRACE, "FabricStFreezeModuleQueuesOnError Option\n");
...
IDS_HOOK_STATUS
FabricStPeiDfRedirectScrubCtrl (
  HOOK_ID HookId,
  VOID    *Handle,
  VOID    *Data
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "FabricStPeiRedirectScrubCtrl Option\n");
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "FabricStPeiDramScrubTime Option\n");
  ...
  #define FABRIC_SAT_IDS_HOOKS_INT_PEI
#else
  #include "Internal/FabricIdsHookSatLibIntPei.h"
#endif

IDS_HOOK_ELEMENT  FabricSatIdsHooksPei[] = {
  {
  ...
  FABRIC_SAT_IDS_HOOKS_INT_PEI
  IDS_HOOKS_END
};

IDS_HOOK_TABLE  FabricSatIdsHookTablePei = {
  IDS_HOOK_TABLE_HEADER_REV1_DATA,
  FabricSatIdsHooksPei
};
...
{
  *IdsHookTable = &FabricSatIdsHookTablePei;
  return HGPI_SUCCESS;
}
