...
#define FILECODE  LIBRARY_CCXSETMCASATLIB_CCXSETMCASATLIB_FILECODE
  ...
  )
{
  ...
  McaCount = (AsmReadMsr64 (MSR_MCG_CAP) & 0xFF);
  for (McaIndex = 0; McaIndex < McaCount; McaIndex++) {
    ...
    McaIpid.Value = AsmReadMsr64 ((MCA_EXTENSION_BASE + (McaIndex * 0x10) | MCA_IPID_OFFSET));
    // write all MCA_MISC0[BlkPtr] = 0 in all MCA banks except MCA_MISC0_UMC (Bank 23-28)
    if (McaIpid.Field.HardwareID == MCA_UMC_ID) {
      McaMiscAddr   = MSR_MCA_MISC1_BANK0 + 0x10 * McaIndex;
      McaMisc.Value = AsmReadMsr64 (McaMiscAddr);
      McaMisc.Field.BlkPtr = 0;
      AsmWriteMsr64 (McaMiscAddr, McaMisc.Value);
    } else {
      McaMiscAddr   = MSR_MCA_MISC0_BANK0 + 0x10 * McaIndex;
      McaMisc.Value = AsmReadMsr64 (McaMiscAddr);
      McaMisc.Field.BlkPtr = 0;
      AsmWriteMsr64 (McaMiscAddr, McaMisc.Value);
    }
    ...
    if (CcxIsDiePrimary (NULL)) {
      if ((McaIpid.Field.HardwareID == MCA_L3_ID) && (McaIpid.Field.McaType == L3_MCA_TYPE)) {
        AsmMsrOr64 ((MCA_CTL_MASK_BASE + McaIndex), BIT7);
      }
    }
  }
  ...
  // bit [3] = 1
  AsmMsrOr64 (MSR_MCA_INTR_CFG, BIT3);

  // MCA_CTL_MASK_LS (C001_0400)
  // bit [10] = 1
  // bit [9]  = 1
  AsmMsrOr64 (MCA_CTL_MASK_BASE, (BIT10 | BIT9));
}
