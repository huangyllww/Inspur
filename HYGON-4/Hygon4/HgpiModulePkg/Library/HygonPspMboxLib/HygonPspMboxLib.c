...
#define FILECODE  LIBRARY_HYGONPSPMBOXLIB_HYGONPSPMBOXLIB_FILECODE
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpPspC2PmboxBeforeSendCmdBase + Cmd, NULL);
  ...
  HGPI_TESTPOINT (TpPspC2PmboxWaitCmdBase + Cmd, NULL);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE_PSP_TRACE (
    "SmmTrigInfo::Addr:0x%x AddrType:0x%x Width:0x%x AndMask:0x%x OrMask:0x%x\n",
  ...
  IDS_HDT_CONSOLE_PSP_TRACE (
    "SmiEnb.Address:0x%x AddrType:0x%x Width:0x%x Mask:0x%x ExpectValue:0x%x\n",
  ...
  IDS_HDT_CONSOLE_PSP_TRACE (
    "Eos.Address:0x%x AddrType:0x%x Width:0x%x Mask:0x%x ExpectValue:0x%x\n",
  ...
  IDS_HDT_CONSOLE_PSP_TRACE (
    "FakeSmiEn.Address:0x%x AddrType:0x%x Width:0x%x Mask:0x%x ExpectValue:0x%x\n",
  ...
  IDS_HDT_CONSOLE_PSP_TRACE (
    "PspMboxSmmBufferAddress:0x%x PspMboxSmmFlagAddress:0x%x\n",
  ...
  )
{
  ...
  Buffer->Req.Capabilities = 1;      // 1 to identify the command is sending
...
/*----------------------------------------------------------------------------------------*/
/**
 * Psp service request for 4A
 *
 *
 * @param[in]      IohcBus         IOHC Bus
...
{
  PSP_MBOX_CMD_STATUS         PspCmd;
  UINT32                      Data32;
  UINT32                      Ret;

  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyEx Enter\n");

  if (RequestId >= MboxCmdAbort) {
    IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyEx INVALID!!\n");
    return PSP_SERVICE_RESULT_OK;
  }
  ...
  //1 Wait till mailbox is ready and PSP finish processing command
  do {
    SmnRegisterRead (IohcBus, C2P_CMD_STATUS_REG_HYEX, &Data32);
    PspCmd.Value = Data32;
  ...
  SmnRegisterWrite (IohcBus, C2P_ARG0_REG_HYEX, &RequestArgument[0], AccessFlags);
  SmnRegisterWrite (IohcBus, C2P_ARG1_REG_HYEX, &RequestArgument[1], AccessFlags);
  SmnRegisterWrite (IohcBus, C2P_ARG2_REG_HYEX, &RequestArgument[2], AccessFlags);
  SmnRegisterWrite (IohcBus, C2P_ARG3_REG_HYEX, &RequestArgument[3], AccessFlags);
  SmnRegisterWrite (IohcBus, C2P_ARG4_REG_HYEX, &RequestArgument[4], AccessFlags);
  SmnRegisterWrite (IohcBus, C2P_ARG5_REG_HYEX, &RequestArgument[5], AccessFlags);

  // 3 Write message ID
  PspCmd.Field.CommandId = RequestId;
  Data32 = PspCmd.Value;
  
  SmnRegisterWrite (IohcBus, C2P_CMD_STATUS_REG_HYEX, &Data32, AccessFlags);

  // 4 Poll CommandId until non-zero
  do {
    SmnRegisterRead (IohcBus, C2P_CMD_STATUS_REG_HYEX, &Data32);
    PspCmd.Value = Data32;
  } while (PspCmd.Field.CommandId != 0);

  while (PspCmd.Field.Status == PSP_SERVICE_RESULT_FATAL) {
    IDS_HDT_CONSOLE (GNB_TRACE, "PSP Assert AccessFlags\n");
    ASSERT (FALSE);   // ASSERT if asserts are enabled
  }

  // 5 Read updated PSP message arguments
  SmnRegisterRead (IohcBus, C2P_ARG0_REG_HYEX, &RequestArgument[0]);
  SmnRegisterRead (IohcBus, C2P_ARG1_REG_HYEX, &RequestArgument[1]);
  SmnRegisterRead (IohcBus, C2P_ARG2_REG_HYEX, &RequestArgument[2]);
  SmnRegisterRead (IohcBus, C2P_ARG3_REG_HYEX, &RequestArgument[3]);
  SmnRegisterRead (IohcBus, C2P_ARG4_REG_HYEX, &RequestArgument[4]);
  SmnRegisterRead (IohcBus, C2P_ARG5_REG_HYEX, &RequestArgument[5]);

  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyEx Exit RequestId = 0x%x, Response = 0x%x\n", RequestId, PspCmd.Field.Status);
  Ret = (PspCmd.Field.Status == 0) ? PSP_SERVICE_RESULT_OK : (UINT32)(0x80000000 | PspCmd.Field.Status);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyGx Enter\n");

  if (RequestId >= MboxCmdAbort) {
    IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyGx INVALID!!\n");
    return PSP_SERVICE_RESULT_OK;
  }
  ...
  //1 Wait till mailbox is ready and PSP finish processing command
  do {
    SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_CMD_STATUS_REG_HYGX), &Data32);
    PspCmd.Value = Data32;
  } while ( (!PspCmd.Field.Ready) ||  (PspCmd.Field.CommandId));
  ...
  // 2 Write message arguments
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG0_REG_HYGX), &RequestArgument[0], AccessFlags);
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG1_REG_HYGX), &RequestArgument[1], AccessFlags);
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG2_REG_HYGX), &RequestArgument[2], AccessFlags);
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG3_REG_HYGX), &RequestArgument[3], AccessFlags);
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG4_REG_HYGX), &RequestArgument[4], AccessFlags);
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG5_REG_HYGX), &RequestArgument[5], AccessFlags);

  // 3 Write message ID
  PspCmd.Field.CommandId = RequestId;
  Data32 = PspCmd.Value;
  
  SmnRegisterWrite (IohcBus, IOD_SPACE(PhysicalDieId, C2P_CMD_STATUS_REG_HYGX), &Data32, AccessFlags);
  ...
  // 5 Read updated PSP message arguments
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG0_REG_HYGX), &RequestArgument[0]);
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG1_REG_HYGX), &RequestArgument[1]);
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG2_REG_HYGX), &RequestArgument[2]);
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG3_REG_HYGX), &RequestArgument[3]);
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG4_REG_HYGX), &RequestArgument[4]);
  SmnRegisterRead (IohcBus, IOD_SPACE(PhysicalDieId, C2P_ARG5_REG_HYGX), &RequestArgument[5]);

  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPspServiceRequestHyGx Exit RequestId = 0x%x, Response = 0x%x\n", RequestId, PspCmd.Field.Status);
  Ret = (PspCmd.Field.Status == 0) ? PSP_SERVICE_RESULT_OK : (UINT32)(0x80000000 | PspCmd.Field.Status);
  ...
  )
{
  ...
  Ret      = PSP_SERVICE_RESULT_FATAL;
  
  if (CpuModel == HYGON_EX_CPU) {
    Ret = NbioPspServiceRequestHyEx(IohcBus, PhysicalDieId, RequestId, RequestArgument, AccessFlags);
  } else if (CpuModel == HYGON_GX_CPU) {
    Ret = NbioPspServiceRequestHyGx(IohcBus, PhysicalDieId, RequestId, RequestArgument, AccessFlags);
  }
  return Ret;
}