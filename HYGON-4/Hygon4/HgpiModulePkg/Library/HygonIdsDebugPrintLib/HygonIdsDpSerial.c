...
#define FILECODE  LIBRARY_HYGONIDSDEBUGPRINTLIB_HYGONIDSDPSERIAL_FILECODE

#define IDS_SERIAL_PORT  (PcdGet64 (PcdHygonIdsDebugPrintSerialPort))
...
#define IDS_SERIAL_PORT_LSR_READ(Value) \
  (IDS_SERIAL_PORT > 0xFFFF ? \
   LibHygonMemRead (AccessWidth8, (UINT64)IDS_SERIAL_PORT_LSR, &Value, NULL) : \
   LibHygonIoRead (AccessWidth8, (UINT16)IDS_SERIAL_PORT_LSR, &Value, NULL))

#define IDS_SERIAL_PORT_WRITE(ByteSended) \
  (IDS_SERIAL_PORT > 0xFFFF ? \
   LibHygonMemWrite (AccessWidth8, (UINT64)IDS_SERIAL_PORT, &ByteSended, NULL) : \
   LibHygonIoWrite (AccessWidth8, (UINT16)IDS_SERIAL_PORT, &ByteSended, NULL))
...
#define IDS_SERIAL_PORT_MSR_READ(Value) \
  (IDS_SERIAL_PORT > 0xFFFF ? \
   LibHygonMemRead (AccessWidth8, (UINT64)IDS_SERIAL_PORT_MSR, &Value, NULL) : \
   LibHygonIoRead (AccessWidth8, (UINT16)IDS_SERIAL_PORT_MSR, &Value, NULL))
 ...
 **/
BOOLEAN
HygonIdsDpSerialSupport (
  VOID
  )
{
  UINT8  Value;

  if (PcdGetBool (PcdHygonIdsDebugPrintSerialPortEnable)) {
    if (PcdGetBool (PcdHygonIdsDebugPrintSerialPortDetectCableConnection)) {
  ...
  )
{
  ...
  if (PcdGet8 (PcdBiosLogPrint) == 2) {
    ...
    AsmWriteMsr64 (BIOS_SCRATCH_MSR_REG, Value);
  } else if (PcdGet8 (PcdBiosLogPrint) == 1){
    Value = REDIRECT_IO_DATA_BEGIN;
    ...
    while (BufferSize--) {
      if (*Buffer == '\n') {
        SendStatus = HygonIdsDpSerialSendByte ('\r');
      }

      SendStatus = HygonIdsDpSerialSendByte (*Buffer);
      Buffer++;
    }
  }
}

CONST HYGON_IDS_DEBUG_PRINT_INSTANCE  HygonIdsDebugPrintSerialInstance =
{
  HygonIdsDpSerialSupport,
  HygonIdsDpSerialConstructor,
  HygonIdsDpSerialGetFilter,
