...
#define FILECODE  LIBRARY_HYGONIDSDEBUGPRINTLIB_HYGONIDSDPHDTOUTLIB_FILECODE
  ...
  )
{
  ...
  if ((Status == HGPI_SUCCESS) && (LocateHeapParam.BufferPtr != NULL)) {
  ...
  )
{
  ...
  Result = FALSE;
  if (PcdGetBool (PcdHygonIdsDebugPrintHdtOutEnable)) {
    if (PcdGetBool (PcdHygonIdsDebugPrintHdtOutForceEnable) == TRUE) {
      Result = TRUE;
    } else {
      DR2reg = AsmReadDr2 ();
      if (DR2reg == 0x99CC) {
 ...
 **/
BOOLEAN
HygonIdsDpHdtOutGetFilter (
  IN OUT   UINT64 *Filter
  )
{
  HDTOUT_HEADER  *HdtoutHeaderPtr;

  if (HygonIdsDpHdtOutGetHeader (&HdtoutHeaderPtr) == TRUE) {
 ...
 **/
VOID
HygonIdsDpHdtOutRegisterRestore (
...
VOID
HygonIdsDpHdtOutBufferModeInit (
  )
{
  ...
  if ((!HygonIdsDpHdtOutSupport ()) ||
      (!CcxIsBsp (StdHeader)) ||
      (HygonIdsDpHdtOutGetHeader (&HdtOutHeaderPtr) == TRUE)) {
    return;
  }

  HygonIdsDpHdtOutRegisterInit ();
  LibHygonMemFill (&HdtoutHeader, 0, sizeof (HDTOUT_HEADER), StdHeader);
  ...
  // if (!IsBsp (StdHeader)) {
  // HygonIdsDpHdtOutRegisterRestore (StdHeader);
  // return;
  // }
  // }
  ...
  // if (HygonIdsDpHdtOutGetHeader (&pHdtoutHeader, StdHeader)) {
  ...
  if (Status == HGPI_SUCCESS) {
    ...
    LibHygonMemCopy (AllocateHeapParams.BufferPtr, &HdtoutHeader, sizeof (HdtoutHeader), StdHeader);
  ...
  )
{
  ...
  if (HygonIdsDpHdtOutSupport ()) {
    ...
    if (HygonIdsDpHdtOutRegInitCheck () == FALSE) {
  ...
  )
{
  ...
  if (HygonIdsDpHdtOutSupport ()) {
    if (HygonIdsDpHdtOutGetHeader (&HdtoutHeaderPtr)) {
  ...
  )
{
  ...
  if (HygonIdsDpHdtOutSupport ()) {
    // IdsApLateTask.ApTask = (PF_IDS_AP_TASK) HygonIdsDpHdtOutExitCoreTask;
    // IdsApLateTask.ApTaskPara = NULL;
    // IdsHgpiRunFcnOnAllCoresLate (&IdsApLateTask, StdHeader);
...
**/
VOID
HygonIdsDpHdtOutPrintWithBuffer (
  IN      CHAR8   *Buffer,
  IN      UINTN BufferSize,
  ...
  )
{
  ...
  LibHygonIoWrite (AccessWidth16, 0xFFED, Buffer, NULL);
  ...
  if (HygonIdsDpHdtOutGetHeader (&HdtoutHeaderPtr)) {
    ...
    if (HdtoutHeaderPtr->PrintCtrl == HDTOUT_PRINTCTRL_ON) {
      if (HdtoutHeaderPtr->OutBufferMode == HDTOUT_BUFFER_MODE_ON) {
        HygonIdsDpHdtOutPrintWithBuffer (Buffer, BufferSize, HdtoutHeaderPtr);
      } else {
    ...
    // HygonIdsDpHdtOutBreakPoint (Buffer, BufferSize, HdtoutHeaderPtr);
    // if (debugPrintPrivate->saveContext) {
    // HygonIdsDpHdtOutSaveContext (Buffer, BufferSize, HdtoutHeaderPtr);
...
CONST HYGON_IDS_DEBUG_PRINT_INSTANCE  HygonIdsDebugPrintHdtoutInstance =
{
  HygonIdsDpHdtOutSupport,
  HygonIdsDpHdtOutRegisterInit,
  HygonIdsDpHdtOutGetFilter,
