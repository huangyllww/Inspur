...
#define FILECODE  LIBRARY_HYGONHEAPDXELIB_HYGONHEAPDXELIB_FILECODE
...
typedef struct _HEAP_BUFFER_NODE {
  ...
  UINT8     Persist;                              ///< A flag. If marked, to be stored and passed to HygonInitLate.
...
#define HYGON_HEAP_PROTOCOL_REV1  0x00000001ul      ///< HYGON Heap Protocol Rev 1.0
/// Heap Protocol definition
typedef struct _HYGON_HEAP_PROTOCOL {
...
extern EFI_GUID  gHygonHeapHobGuid;
/// Protocol used to locate the Header of HEAP LinkList
extern EFI_GUID  gHygonHeapHeaderProtocolGuid;

/// Init Protocol data
HYGON_HEAP_PROTOCOL  mHygonHeapProtocolStruct = {
  HYGON_HEAP_PROTOCOL_REV1,
  {
    NULL,
    NULL
  }
 ...
 * This function will only be called once on 1st enter of HygonHeapDxeLib, when
 * gHygonHeapHeaderProtocolGuid protocol hasn't been installed
 *
 */
VOID
EFIAPI
  ...
  )
{
  ...
  GuidHob = GetFirstGuidHob (&gHygonHeapHobGuid);
  // Traversal the GUID HOB to find HYGON HEAP GUID HOB Node
  while (GuidHob != NULL) {
    ...
    GuidHob = GetNextGuidHob (&gHygonHeapHobGuid, GuidHob);
  }
}
  ...
  )
{
  EFI_STATUS           Status;
  EFI_HANDLE           Handle;
  HYGON_HEAP_PROTOCOL  *HygonHeapProtocol;

  HygonHeapProtocol = NULL;
  Status = gBS->LocateProtocol (&gHygonHeapHeaderProtocolGuid, NULL, &HygonHeapProtocol);
  if (EFI_ERROR (Status)) {
    // Heap List haven't been created, rebuild Heap List by copy from HOB
    InitializeListHead (&mHygonHeapProtocolStruct.HeapListHead);
    mHeapLinkedListHead = &mHygonHeapProtocolStruct.HeapListHead;
    BuildHeapList ();

    Handle = NULL;
    Status = gBS->InstallProtocolInterface (
                    &Handle,
                    &gHygonHeapHeaderProtocolGuid,
                    EFI_NATIVE_INTERFACE,
                    &mHygonHeapProtocolStruct
                    );
    ASSERT (Status == EFI_SUCCESS);
    return Status;
  }

  mHeapLinkedListHead = &HygonHeapProtocol->HeapListHead;
  return EFI_SUCCESS;
}
 ...
 *
 * @retval         HGPI_SUCCESS     No error
 * @retval         HGPI_BOUNDS_CHK  Handle already exists, or not enough
 *                                   free space
 * @retval         HGPI_UNSUPPORTED Input Parameter check fail
 * @retval         HGPI_ERROR       Locate Heap Fail
 *
 */
HGPI_STATUS
HeapAllocateBuffer (
  ...
  )
{
  ...
  if (HeapLocateBuffer (&LocateHeapPtr, NULL) == HGPI_SUCCESS) {
  ...
  Status = (EFI_ERROR (EfiStatus)) ? HGPI_ERROR : HGPI_SUCCESS;
  return Status;
}
 ...
 *
 * @retval         HGPI_SUCCESS     No error
 * @retval         HGPI_BOUNDS_CHK  Handle does not exist on the heap
 *
 */
HGPI_STATUS
