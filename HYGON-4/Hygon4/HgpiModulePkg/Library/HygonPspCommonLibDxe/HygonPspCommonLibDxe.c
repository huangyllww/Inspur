...
#define FILECODE  LIBRARY_HYGONPSPCOMMONLIBDXE_HYGONPSPCOMMONLIBDXE_FILECODE

HYGON_PSP_COMMON_SERVICE_PROTOCOL  *mPspServicePtr = NULL;
  ...
  )
{
  ...
  if (mPspServicePtr == NULL) {
    Status = gBS->LocateProtocol (&gHygonPspCommonServiceProtocolGuid, NULL, &mPspServicePtr);
    if (EFI_ERROR (Status)) {
      ASSERT (FALSE);
      return Status;
    }
