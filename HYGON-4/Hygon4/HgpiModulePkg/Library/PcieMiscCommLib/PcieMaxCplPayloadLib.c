  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  PcieSetMaxCplPayload for Device = %d:%d:%d\n",
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  PcieSetMaxCplPayload: Return from GnbLibPciScan"
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  PcieGetMaxCplPayloadCallback for Device = %d:%d:%d\n",
    Device.Address.Bus,
  ...
  )
{
  ...
  if ((MaxPayloadEnable != 0) &&
    ...
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "PcieMaxCplPayloadInitCallback, EngineMaxPayload = %x\n",
      EngineMaxPayload
      );
  }
  ...
  while (PcieWrapper != NULL) {
    ...
    while (PcieEngine != NULL) {
      ...
      if (PcieLibIsEngineAllocated (PcieEngine)) {
        ...
        SmnAddress = ConvertPciePortAddress (PCIE0_GPP0_LINK_CTL_STS_ADDRESS_HYGX, GnbHandle, PcieWrapper, MacPortId);
      ...
      if (MaxCplPayload != 0xff) {
        ...
        PCIEPSMNAddress = ConvertPciePortAddress (PCIEP_PORT_CNTL_ADDRESS_HYGX, GnbHandle, PcieWrapper, MacPortId);
