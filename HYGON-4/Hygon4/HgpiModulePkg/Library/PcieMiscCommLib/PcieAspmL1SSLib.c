  ...
  )
{
  ...
  if (CapValue & BIT4) {
    ...
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "  Set Aspm L1 SS for Device = %d:%d:%d Value = %08x\n",
      Device.Address.Bus,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  GetPcieAspmL1SSCallback for Device = %d:%d:%d (AspmL1_1=%d, AspmL1_2=%d)\n",
    Device.Address.Bus,
    Device.Address.Device,
    Device.Address.Function,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  SetPcieAspmL1SSCallback for Device = %d:%d:%d (AspmL1_1=%d, AspmL1_2=%d)\n",
    Device.Address.Bus,
    Device.Address.Device,
    Device.Address.Function,
    PcieAspmL1SSData->AspmL1_1,
  ...
  )
{
  ...
  if (PcieCapPtr != 0) {
    ...
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "  Set Snoop/non-snoop latency for Device = %d:%d:%d Value = %08x\n",
      Device.Address.Bus,
      Device.Address.Device,
      Device.Address.Function,
      Value
  ...
  )
{
  ...
  if (PcieCapPtr != 0) {
    ...
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "  Set LC_LTR_L1_2_thresholds for Device = %d:%d:%d Value = %08x\n",
      Device.Address.Bus,
      Device.Address.Device,
      Device.Address.Function,
      Value
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  GetPcieLTRCallback for Device = %d:%d:%d (LTR_SUPPORTED=%x)\n",
    Device.Address.Bus,
    Device.Address.Device,
    Device.Address.Function,
    PcieLTRData->LTREn
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  SetPcieLTRCallback for Device = %d:%d:%d to %d\n",
    Device.Address.Bus,
    Device.Address.Device,
    Device.Address.Function,
    PcieLTRData->LTREn
      ...
      if (PcieLibIsEngineAllocated (PcieEngine)) {
        if (((PcieEngine->Type.Port.LinkAspmL1_1 == 1) || (PcieEngine->Type.Port.LinkAspmL1_2 == 1)) &&
          ...
          if (PcieEngine->Type.Port.LinkAspmL1_2 == 1) {
            ...
            SmnAddress = ConvertPciePortAddress (PCIEP_STRAP_MISC_instPCIE0_link_ADDRESS_HYGX, GnbHandle, PcieWrapper, (PcieEngine->Type.Port.PortId % 8));
            NbioRegisterRMW (
              GnbHandle,
              TYPE_SMN,
              SmnAddress,
              (UINT32) ~(PCIEP_STRAP_MISC_STRAP_LTR_SUPPORTED_MASK),
              1 << PCIEP_STRAP_MISC_STRAP_LTR_SUPPORTED_OFFSET,
