...
#include <Library/HygonPspFlashAccLib.h>
...
#define FILECODE  LIBRARY_HYGONPSPFLASHACCLIBDXE_HYGONPSPFLASHACCLIBDXE_FILECODE
  ...
  )
{
  ...
  if (mPspFlashAccSmmCommReady == FALSE) {
    ...
    if (EFI_ERROR (Status)) {
      ...
      IDS_HDT_CONSOLE_PSP_TRACE ("HygonPspFlashAccLibDxe locate gPspFlashAccSmmCommReadyProtocol fail \n");
      return Status;
    }
  ...
  if (mSmmCommunication == NULL) {
    ...
    ASSERT (Status == EFI_SUCCESS);
    if (EFI_ERROR (Status)) {
      IDS_HDT_CONSOLE_PSP_TRACE ("HygonPspFlashAccLibDxe locate SmmCommunicationProtocol fail \n");
      return Status;
    }
  }
  ...
  )
{
  ...
  // Init SMM communication buffer header
  RwBuffer = (UINT8 *)(UINTN)PcdGet64 (PcdHygonSmmCommunicationAddress);
