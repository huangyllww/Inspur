...
#define FILECODE  LIBRARY_CCXIDSCUSTOMPSTATESDHARMLIB_CCXIDSCUSTOMPSTATESDHARMLIB_FILECODE
...
/**
 * CcxDmSatIdsSetPstateMsrs - routine to update Pstate MSRs to specified values
 *
 *
  ...
  )
{
  ...
  HPOB_CCX_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_HYEX  HpobCcxLogToPhysMapHyEx;
  HPOB_CCX_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_HYGX  HpobCcxLogToPhysMapHyGx;
        ...
        while (i <= (PS_MAX_REG - PS_MIN_REG)) {
          for (SocketLoop = 0; SocketLoop < NumberOfSockets; SocketLoop++) {
            ...
            for (CddLoop = 0; CddLoop < MAX_CDDS_PER_SOCKET; CddLoop++) {
              ...
              NbioSmuServices->HygonSmuRegisterRMW (
                                 NbioSmuServices,
                                 (UINT32)SocketLoop,
                                 (UINT32)CDD_SPACE (CddLoop, (PWR_PSATE0_HIGH + i * 8)),
                                 0x7FFFFFFF,
                                 0
                                 );
            }
  ...
  for (SocketLoop = 0; SocketLoop < NumberOfSockets; SocketLoop++) {
    ...
    for (CddLoop = 0; CddLoop < MAX_CDDS_PER_SOCKET; CddLoop++) {
      ...
      if (CoreTopologyServices->GetCoreTopologyOnCdd (CoreTopologyServices, SocketLoop, CddLoop, &NumberOfComplexes, &NumberOfCores, NULL) != EFI_SUCCESS) {
        return;
      }
      ...
      if (HygonCpuModel == HYGON_EX_CPU) {
        CopyMem (&HpobCcxLogToPhysMapHyEx, HpobEntry, sizeof (HPOB_CCX_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_HYEX));

        for (ComplexLoop = 0; ComplexLoop < NumberOfComplexes; ComplexLoop++) {
          if (HpobCcxLogToPhysMapHyEx.ComplexMap[ComplexLoop].PhysComplexNumber == CCX_NOT_PRESENT) {
            return;
          }

          for (CoreLoop = 0; CoreLoop < NumberOfCores; CoreLoop++) {
            if (HpobCcxLogToPhysMapHyEx.ComplexMap[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber  == CCX_NOT_PRESENT) {
              return;
            }
            
            PmcPlrRegAddr = CCX_SPACE_HYEX((UINT32) CddLoop, 
                                    (UINT32) HpobCcxLogToPhysMapHyEx.ComplexMap[ComplexLoop].PhysComplexNumber,
                                    (UINT32) HpobCcxLogToPhysMapHyEx.ComplexMap[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber,
                                    CCX0_PMC_PLR_ADDRESS);
            NbioSmuServices->HygonSmuRegisterRMW (NbioSmuServices, (UINT32)(SocketLoop), PmcPlrRegAddr, 0xFFFFFF88, ((MaxPstate << 4) | MaxPstate));
          }
        }
      }

      if (HygonCpuModel == HYGON_GX_CPU) {
        CopyMem (&HpobCcxLogToPhysMapHyGx, HpobEntry, sizeof (HPOB_CCX_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_HYGX));

        for (ComplexLoop = 0; ComplexLoop < NumberOfComplexes; ComplexLoop++) {
          if (HpobCcxLogToPhysMapHyGx.ComplexMap[ComplexLoop].PhysComplexNumber == CCX_NOT_PRESENT) {
            return;
          }

          for (CoreLoop = 0; CoreLoop < NumberOfCores; CoreLoop++) {
            if (HpobCcxLogToPhysMapHyGx.ComplexMap[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber  == CCX_NOT_PRESENT) {
              return;
            }
            
            PmcPlrRegAddr = CCX_SPACE_HYGX((UINT32) CddLoop, 
                                    (UINT32) HpobCcxLogToPhysMapHyGx.ComplexMap[ComplexLoop].PhysComplexNumber,
                                    (UINT32) HpobCcxLogToPhysMapHyGx.ComplexMap[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber,
                                    CCX0_PMC_PLR_ADDRESS);
            NbioSmuServices->HygonSmuRegisterRMW (NbioSmuServices, (UINT32)(SocketLoop), PmcPlrRegAddr, 0xFFFFFF88, ((MaxPstate << 4) | MaxPstate));
          }
        }
