  ...
  )
{
  ...
  AllocHeapParams.BufferHandle = HYGON_GNB_IOMMU_SCRATCH_MEM_HANDLE;
  AllocHeapParams.Persist = HEAP_RUNTIME_SYSTEM_MEM;
  Status = HeapAllocateBuffer (&AllocHeapParams, (HYGON_CONFIG_PARAMS *)NULL);
  ...
  while (GnbHandle != NULL) {
    if (GnbCheckIommuPresent (GnbHandle, (HYGON_CONFIG_PARAMS *)NULL)) {
      IDS_HDT_CONSOLE (GNB_TRACE, "Set Iommu Scratch Memory for Socket %d Physical Die %d\n", GnbGetSocketId (GnbHandle), GnbGetDieNumber (GnbHandle));
