 ...
 * @file
 *
 * HYGON CPU AP related utility functions.
 *
  ...
  )
{
  ...
  LibHygonMemRead (AccessWidth32, Address, &TempVar_a, StdHeader);
  ...
  )
{
  ...
  do {
    ...
    LibHygonMemWrite (AccessWidth32, ApicAddr, &TargetApicId, StdHeader);
    ApicAddr     = ApicBase + APIC_CMD_LO_REG;
    ApicRegister = CMD_REG_TO_READ | (UINT32)RegAddr;
    LibHygonMemWrite (AccessWidth32, ApicAddr, &ApicRegister, StdHeader);
    do {
      LibHygonMemRead (AccessWidth32, ApicAddr, &ApicRegister, StdHeader);
  ...
  )
{
  ...
  LibHygonMemWrite (AccessWidth32, ApicAddr, &Value, StdHeader);
}
 ...
 * @param[in]      StdHeader    Configuration parameters pointer
 *
 * @retval         HGPI_SUCCESS Transaction was successful
 * @retval         HGPI_ALERT   The non-NULL desired location to place
 *                               the buffer was not used as the buffer
 *                               resides in a shared memory space.  The
 *                               input data pointer has changed.
 * @retval         HGPI_ERROR   There is not enough room to receive the
 *                               buffer.
 *
 */
HGPI_STATUS
  ...
  )
{
  ...
  HGPI_STATUS         ReturnStatus;
  AP_WAIT_FOR_STATUS  WaitForStatus;

  ReturnStatus = HGPI_SUCCESS;
  MyCore = CcxGetInitialApicID (StdHeader);
  ...
  // HeapMalloc.BufferHandle = HYGON_CPU_AP_TASKING_HANDLE;
  ...
  // if (HeapAllocateBuffer (&HeapMalloc, StdHeader) == HGPI_SUCCESS) {
