...
#define FILECODE  LIBRARY_HYGONTABLELIB_PEI_HYGONTABLESMNPEILIB_FILECODE
...
PEI_HYGON_NBIO_SMU_SERVICES_PPI  *NbioSmuServices   = NULL;
UINTN                            SystemSocketNumber = 0xFF;
  ...
  )
{
  ...
  if (NbioSmuServices == NULL) {
    ...
    (**PeiServices).LocatePpi (PeiServices, &gHygonNbioSmuServicesPpiGuid, 0, NULL, &NbioSmuServices);
  }

  if (InstanceId == SMN_ON_ALL_SOCKETS) {
    if (SystemSocketNumber == 0xFF) {
      // Get SystemSocketNumber
      (*PeiServices)->LocatePpi (PeiServices, &gHygonFabricTopologyServicesPpiGuid, 0, NULL, &FabricTopologyServices);
      FabricTopologyServices->GetSystemInfo (&SystemSocketNumber, NULL, NULL);
    }

    for (SocketLoop = 0; SocketLoop < SystemSocketNumber; SocketLoop++) {
      NbioSmuServices->SmuRegisterRMW (NbioSmuServices, SocketLoop, RegisterIndex, RegisterANDValue, RegisterORValue);
    }
  } else {
