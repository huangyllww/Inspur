...
#include <Library/HygonRegisterLib.h>
...
#define FILECODE  LIBRARY_HYGONTABLELIB_HYGONTABLELIB_FILECODE
  ...
  )
{
  ...
  ASSERT ((PlatformFeatEntry->TypeFeats.PlatformValue & ~((HYGON_PF_ALL) | (HYGON_PF_AND))) == 0);

  GetPlatformFeatures (&PlatformFeatures, StdHeader);

  if (DoesEntryTypeSpecificInfoMatch (
        PlatformFeatures.PlatformValue,
  ...
  )
{
  ...
  if (!mPlatformFeatsInited) {
    ...
    if (CddPerSocket == 1) {
      PlatformFeats.PlatformFeatures.PlatformDm1 = 1;
    }

    // bit 2 - PlatformX2Apic: x2APIC mode desired
 ...
 *
 *  @retval      HGPI_SUCCESS         Register table successfully loaded
 *  @retval      HGPI_UNSUPPORTED     Register table is not supported
 */
HGPI_STATUS
  ...
  )
{
  ...
  if (TimePoint >= MaxHygonRegisterTableTps) {
    ASSERT (FALSE);
    return HGPI_BOUNDS_CHK;
  }

  RegisterTableList = NULL;
  while (RegTableListAtGivenTP->TimePoint != MaxHygonRegisterTableTps) {
