...
#define FILECODE  LIBRARY_HYGONTABLELIB_DXE_HYGONTABLESMNDXELIB_FILECODE
...
DXE_HYGON_NBIO_SMU_SERVICES_PROTOCOL  *NbioSmuServices   = NULL;
  ...
  )
{
  ...
  if (InstanceId == SMN_ON_ALL_SOCKETS) {
    if (SystemSocketNumber == 0xFF) {
      ...
      FabricTopologyServices->GetSystemInfo (FabricTopologyServices, &SystemSocketNumber, NULL, NULL);
    }

    for (SocketLoop = 0; SocketLoop < SystemSocketNumber; SocketLoop++) {
      NbioSmuServices->HygonSmuRegisterRMW (NbioSmuServices, SocketLoop, RegisterIndex, RegisterANDValue, RegisterORValue);
    }
  } else {
    NbioSmuServices->HygonSmuRegisterRMW (NbioSmuServices, InstanceId, RegisterIndex, RegisterANDValue, RegisterORValue);
  }
}
