...
#define FILECODE  CCX_DHARMA_CCXDHARMADXE_CCXDHARMAPREFETCH_FILECODE
...
/**
 *
 *  CcxDharmaInitializePrefetchMode
 *
 *  Description:
 *    This funtion provides for performance tuning to optimize for specific
 *    workloads. For general performance use the recommended settings.
  ...
  )
{
  ...
  PrefetchMode.HardwarePrefetchMode = PcdGet8 (PcdHygonHardwarePrefetchMode);
  PrefetchMode.SoftwarePrefetchMode = PcdGet8 (PcdHygonSoftwarePrefetchMode);
  if ((PrefetchMode.HardwarePrefetchMode != HARDWARE_PREFETCHER_AUTO) ||
      (PrefetchMode.SoftwarePrefetchMode != SOFTWARE_PREFETCHES_AUTO)) {
    IDS_HDT_CONSOLE (CPU_TRACE, "  CcxDharmaInitializePrefetchMode:\n");
    ...
    CcxDharmaPrefetchModeAps (&PrefetchMode);
  }
