...
//
//
// HGPI Types and Definitions
//
//

// HGPI BASIC CALLOUTS
#define HGPI_MEM_RELEASE  0x00028000ul

// HGPI ADVANCED CALLOUTS, Processor
#define HGPI_CHECK_UMA           0x00028100ul
#define HGPI_DO_RESET            0x00028101ul
#define HGPI_ALLOCATE_BUFFER     0x00028102ul
#define HGPI_DEALLOCATE_BUFFER   0x00028103ul
#define HGPI_LOCATE_BUFFER       0x00028104ul
#define HGPI_RUNFUNC_ONAP        0x00028105ul
#define HGPI_RUNFUNC_ON_ALL_APS  0x00028106ul
#define HGPI_IDLE_AN_AP          0x00028107ul
#define HGPI_WAIT_FOR_ALL_APS    0x00028108ul

// HGPI ADVANCED CALLOUTS, Memory
#define HGPI_READ_SPD                       0x00028140ul
#define HGPI_HOOKBEFORE_DRAM_INIT           0x00028141ul
#define HGPI_HOOKBEFORE_DQS_TRAINING        0x00028142ul
#define HGPI_READ_SPD_RECOVERY              0x00028143ul
#define HGPI_HOOKBEFORE_EXIT_SELF_REF       0x00028144ul
#define HGPI_HOOKBEFORE_DRAM_INIT_RECOVERY  0x00028145ul
#define HGPI_EXTERNAL_2D_TRAIN_VREF_CHANGE  0x00028146ul
#define HGPI_EXTERNAL_VOLTAGE_ADJUST        0x00028147ul

// HGPI IDS CALLOUTS
#define HGPI_GET_IDS_INIT_DATA  0x00028200ul

// HGPI GNB CALLOUTS
#define HGPI_GNB_PCIE_SLOT_RESET      0x00028301ul
#define HGPI_GNB_GFX_GET_VBIOS_IMAGE  0x00028302ul
#define HGPI_GNB_PCIE_CLK_REQ         0x00028303ul

// HGPI FCH CALLOUTS
#define HGPI_FCH_OEM_CALLOUT  0x00028401ul

// HGPI MAX DOMAIN COUNT
#define MAX_PROXIMITY_DOMAINS  (MAX_SOCKETS_SUPPORTED * MAX_CDDS_PER_SOCKET)

/// Configuration values for IOD type
typedef enum {
  IOD_NULL = 0,
  IOD_SATORI,
  IOD_DUJIANG,
...
typedef enum {
  ...
  SataAhci7804,                       ///< AHCI mode as 7804 ID (HYGON driver)
  SataIde2Ahci7804                    ///< IDE->AHCI mode as 7804 ID (HYGON driver)
} SATA_CLASS;
...
typedef struct {
  ...
  IN       UINT32        SataEnable;         ///< SATA controller function
                                             ///< bit0~bit3  - Socket0 DJ0~3 SATA0
                                             ///< bit4~bit7  - Socket1 DJ0~3 SATA0
                                             ///< bit8~bit11 - Socket2 DJ0~3 SATA0
                                             ///< bit12~bit15- Socket3 DJ0~3 SATA0
                                             ///< bit16~bit19- Socket4 DJ0~3 SATA0
                                             ///< bit20~bit23- Socket5 DJ0~3 SATA0
                                             ///< bit24~bit27- Socket6 DJ0~3 SATA0
                                             ///< bit28~bit31- Socket7 DJ0~3 SATA0
...
typedef struct  {
  ...
  IN       UINT8                     LinkSpeedCapability;    /**< PCIe link speed/
                                                               ...
                                                               *  @li @b 3 - Gen3
                                                               */
  IN       UINT8                     LinkAspm;               /**< ASPM control. (see HgpiPcieLinkAspm for additional option to control ASPM)
                                                               *  @li @b 0 - Disabled
                                                               *  @li @b 1 - L0s only
                                                               *  @li @b 2 - L1 only
                                                               *  @li @b 3 - L0s and L1
  ...
  IN       UINT8                     ResetId;                /**< Arbitrary number greater than 0 assigned by platform firmware for GPIO
                                                               ...
                                                               *   see HgpiPcieSlotResetContol.
                                                               */
  IN       UINT16                    SlotNum;                ///< PHYSICAL_SLOT_NUM
...
typedef struct {
  ///< Structure that discribe lane mapping
...
/// VBIOS image info
typedef struct {
  IN      HYGON_CONFIG_PARAMS    StdHeader;               ///< Standard configuration header
...
typedef enum {
  ...
  ConnectorTypeEDPToLvds,                                 ///< 3rd party common eDP-to-LVDS translator chip without HYGON SW init
  ConnectorTypeEDPToLvdsSwInit,                           ///< 3rd party eDP-to-LVDS translator which requires HYGON SW init
...
#define IGPU_DISABLE_AUTO      0          ///< Auto setting - disable iGPU if ANY PCI graphics or non-HYGON PCIe graphics
...
typedef struct {
  ...
  IN        UINT8      GnbIoapicId;                     ///< GNB IOAPIC ID, platform BIOS needs to pass correct id number, default is 0xFF.
                                                        ///< If BLDCFG_GNB_IOAPIC_ADDRESS == NULL or BLDCFG_IOMMU_SUPPORT == NULL or GnbIoapicId == default(0xFF), HGPI will skip it anyway.
  ...
  IN        UINT8      FchIoapicId;                     ///< Fch IOAPIC ID, platform BIOS needs to pass correct id number, default is 0xFF.
                                                        ///< If BLDCFG_IOMMU_SUPPORT == NULL or or FchIoapicId == default(0xFF), HGPI will skip it anyway.
                                                        ///< @li 0~n = IOAPIC ID number for IVRS which should be matched with MADT
...
//
//
// HGPI MAXIMIUM VALUES
//
// These Max values are used to define array sizes and associated loop
// counts in the code.  They reflect the maximum values that HGPI
...
#define MAX_IOD_PER_SOCKET            4  ///< Max number of IOD per socket
#define MAX_LITE_IOD_PER_SOCKET       4  ///< Max number of LiteIOD per socket
#define MAX_RBS_PER_IOD               4  ///< Max number of root bridges per IOD
#define MAX_RBS_OF_SYSTEM             16 ///< Max number of root bridges in system
#define MAX_CDDS_PER_SOCKET           8  ///< Max number of cdds per socket

#ifndef MAX_CHANNELS_PER_CDD
#define MAX_CHANNELS_PER_CDD          3  ///< Max Channels per sockets for HyEx and HyGx
#endif
#define MAX_CHANNELS_PER_CDD_HYEX      3  ///< Max Channels per core die for HyEx
#define MAX_CHANNELS_PER_CDD_HYGX      2  ///< Max Channels per core die for HyGx

#ifndef MAX_CHANNELS_PER_SOCKET
#define MAX_CHANNELS_PER_SOCKET       12  ///< Max Channels per sockets for HyEx and HyGx
#endif
#define MAX_CHANNELS_PER_SOCKET_HYEX   12  ///< Max Channels per sockets for HyEx
#define MAX_CHANNELS_PER_SOCKET_HYGX   8  ///< Max Channels per sockets for HyGx

#define MAX_DIMMS_PER_CHANNEL         2  ///< Max DIMMs on a memory channel (independent of platform)
#define MAX_IODIE_PER_SOCKET          4  ///< Max number of IOD per socket
#define MAX_NBIO_PER_EMEI             4  ///< Max number of NBIO per EMEI IOD
#define MAX_NBIO_PER_DUJIANG          1  ///< Max number of NBIO per DUJIANG IOD
#define MAX_CCX_PER_CDD_HYEX           4
#define MAX_CCX_PER_CDD_HYGX           2  ///< Max number of ccx per core die
#define MAX_CORE_PER_CCX_HYEX          4 
#define MAX_CORE_PER_CCX_HYGX          8  ///< Max number of core per ccx
#define MAX_THREAD_PER_CORE           4  ///< Max number of thread per core
...
#define POWER_DOWN_MODE_AUTO       2  ///< HGPI to select power down mode
...
/// Build Configuration values for BLDCFG_HYGON_PLATFORM_TYPE
typedef enum {
  HYGON_PLATFORM_SERVER  = 0x8000,    ///< Server
  HYGON_PLATFORM_DESKTOP = 0x10000,   ///< Desktop
  HYGON_PLATFORM_MOBILE  = 0x20000,   ///< Mobile
} HYGON_PLATFORM_TYPE;
...
typedef enum {
  ...
  VOLT_DDR5_RANGE_START,            ///< Start of DDR5 Voltage Range
  VOLT1_1 = VOLT_DDR5_RANGE_START,  ///< 1.1 Volt
...
typedef struct _CH_DEF_STRUCT {
  ...
  OUT UINT8     *MemClkDisMap;    ///< This pointer will be set to point to an array that describes
                                  ///< the routing of M[B,A]_CLK pins to the DIMMs' ranks. HGPI will
  ...
  OUT UINT8    *CKETriMap;        ///< This pointer will be set to point to an array that describes
                                  ...
                                  ///< HGPI will base on this array to disable unused CKE pins to save power.
  ...
  OUT UINT8    *ODTTriMap;        ///< This pointer will be set to point to an array that describes
                                  ...
                                  ///< HGPI will base on this array to disable unused ODT pins to save power.
  ...
  OUT UINT8    *ChipSelTriMap;    ///< This pointer will be set to point to an array that describes
                                  ...
                                  ///< HGPI will base on this array to disable unused Chip select pins to save power.
...
typedef struct _DIE_STRUCT {
  ...
  OUT HGPI_STATUS    ErrCode;             ///< Current error condition of Node
                                          ///< 0x0 = HGPI_SUCCESS
                                          ///< 0x1 = HGPI_UNSUPPORTED
                                          ///< 0x2 = HGPI_BOUNDS_CHK
                                          ///< 0x3 = HGPI_ALERT
                                          ///< 0x4 = HGPI_WARNING
                                          ///< 0x5 = HGPI_ERROR
 ...
 **********************************************************************/
/// HygonInitResume, HygonS3LateRestore, and HygonInitRtb param structure
typedef struct {
  ...
  IN OUT   VOID      *VolatileStorage; ///< Pointer to remaining HYGON save state data
...
typedef struct _MEM_PARAMETER_STRUCT {
  ...
  OUT VDDP_VDDR_VOLTAGE          VddpVddrVoltage; ///< For a given configuration, request is made to change the VDDP/VDDR
                                                  ///< voltage in platform BIOS via HgpiHookBeforeDramInit callout and
  ...
  // 'HygonMemInitDataStructDef' based on HYGON defaults. It is up
  ...
  ///< HGPI initializes this pointer with DefaultPlatformMemoryConfiguration that
  ...
  ///< FALSE = perform memory init as normal (HYGON default)
  ///< TRUE = restore memory context and skip training. This requires
  ///< MemContext is valid before HygonInitPost
  ///<
  IN  BOOLEAN             SaveMemContextCtl; ///< Control switch to save memory context at the end of MemAuto
  ///< TRUE = HGPI will setup MemContext block before exit HygonInitPost
  ///< FALSE = HGPI will not setup MemContext block. Platform is
  ...
  ///< TRUE = HGPI will use the function defined in "HGPI_EXTERNAL_VREF_CHANGE" in function list
  ///< to change the vref
  ///< FALSE = HGPI will will use the internal vref control.
  ...
  IN     FORCE_TRAIN_MODE    ForceTrainMode; ///< Training Mode
                                             ...
                                             ///< 2 = Auto - HGPI will control 1D or 2D
  IN     TECHNOLOGY_TYPE     DimmTypeUsedInMixedConfig; ///< Select the preferred technology type that HGPI will enable
  ...
  ///< TRUE = Enable, platform BIOS requests to enable memory overclocking function, and HGPI
  ...
  ///<
  ///< AmpVoltageValid :
  ///< TRUE - HGPI does enable AMP function, so use AmpVoltage for voltage adjustment
  ///< FALSE - HGPI does not enable AMP function, so use DDRVoltage for voltage adjustment
  ///<
  ...
  ///< 3 = HGPI control type of training depend on configurations
...
typedef struct _MEM_FUNCTION_STRUCT {
  ...
  IN OUT BOOLEAN (*hygonMemGetPsCfgU)(
    VOID *pMemData
    );                                                  ///< Proc for Unbuffered DIMMs, platform specific
  IN OUT BOOLEAN (*hygonMemGetPsCfgR)(
    VOID *pMemData
    );                                                  ///< Proc for Registered DIMMs, platform specific

  // PUBLIC optional functions

  IN OUT VOID (*hygonMemEccInit)(
    VOID *pMemData
    );                                                            ///< NB proc for ECC feature
  IN OUT VOID (*hygonMemChipSelectInterleaveInit)(
    VOID *pMemData
    );                                                            ///< NB proc for CS interleave feature
  IN OUT VOID (*hygonMemDctInterleavingInit)(
    VOID *pMemData
    );                                                            ///< NB proc for Channel interleave feature
  IN OUT VOID (*hygonMemMctInterleavingInit)(
    VOID *pMemData
    );                                                            ///< NB proc for Node interleave feature
  IN OUT VOID (*hygonMemParallelTraining)(
    VOID *pMemData
    );                                                            ///< NB proc for parallel training feature
  IN OUT VOID (*hygonMemEarlySampleSupport)(
    VOID *pMemData
    );                                                            ///< NB code for early sample support feature
  IN OUT VOID (*hygonMemMultiPartInitSupport)(
    VOID *pMemData
    );                                                            ///< NB code for 'multi-part'
  IN OUT VOID (*hygonMemOnlineSpareSupport)(
    VOID *pMemData
    );                                                            ///< NB code for On-Line Spare feature
  IN OUT VOID (*hygonMemUDimmInit)(
    VOID *pMemData
    );                                                            ///< NB code for UDIMMs
  IN OUT VOID (*hygonMemRDimmInit)(
    VOID *pMemData
    );                                                            ///< NB code for RDIMMs
  IN OUT VOID (*hygonMemLrDimmInit)(
    VOID *pMemData
    );                                                            ///< NB code for LRDIMMs
...
typedef struct _MEM_DATA_STRUCT {
  ...
  IN OUT HGPI_STATUS (*GetPlatformCfg[MAX_PLATFORM_TYPES])(
    struct _MEM_DATA_STRUCT *MemData, UINT8 SocketID, CH_DEF_STRUCT *CurrentChannel
    );                                                                                                                                         ///< look-up platform info

  IN OUT BOOLEAN (*ErrorHandling)(
    struct _DIE_STRUCT *MCTPtr, UINT8 DCT, UINT16 ChipSelMask, HYGON_CONFIG_PARAMS *StdHeader
    );                                                                                                                      ///< Error Handling
...
// HGPI_CACHE_SIZE_REDUCED                            2101
// HGPI_CACHE_REGIONS_ACROSS_1MB                      2102
// HGPI_CACHE_REGIONS_ACROSS_4GB                      2103
// HGPI_REGION_NOT_ALIGNED_ON_BOUNDARY                2104
// HGPI_CACHE_START_ADDRESS_LESS_D0000                2105
// HGPI_THREE_CACHE_REGIONS_ABOVE_1MB                 2106
// HGPI_DEALLOCATE_CACHE_REGIONS                      2107
...
// HGPI_SCS_HEAP_ENTRY_MISSING                        2401
// HGPI_SCS_BUFFER_EMPTY                              2402
// HGPI_SCS_WEIGHTS_MISMATCH                          2403
...
#define HYGON_AP_MTRR_FIX64k_00000  0x00000250ul
#define HYGON_AP_MTRR_FIX16k_80000  0x00000258ul
#define HYGON_AP_MTRR_FIX16k_A0000  0x00000259ul
#define HYGON_AP_MTRR_FIX4k_C0000   0x00000268ul
#define HYGON_AP_MTRR_FIX4k_C8000   0x00000269ul
#define HYGON_AP_MTRR_FIX4k_D0000   0x0000026Aul
#define HYGON_AP_MTRR_FIX4k_D8000   0x0000026Bul
#define HYGON_AP_MTRR_FIX4k_E0000   0x0000026Cul
#define HYGON_AP_MTRR_FIX4k_E8000   0x0000026Dul
#define HYGON_AP_MTRR_FIX4k_F0000   0x0000026Eul
#define HYGON_AP_MTRR_FIX4k_F8000   0x0000026Ful
...
///
typedef struct {
  IN    UINT16    IdsNvIdSAT;      ///< Unified ID for IDS Setup Option.
...
/************************************************************************
 *
 *  HGPI interface Call-Out function parameter structures
 *
 ***********************************************************************/

/// Parameters structure for interface call-out HgpiRunCodeOnAp
typedef struct {
  IN OUT    HYGON_CONFIG_PARAMS    StdHeader;          ///< Standard configuration header
...
/// Buffer Handles
typedef enum {
  HYGON_DMI_INFO_BUFFER_HANDLE = 0x000D000,       ///< Assign 0x000D000 buffer handle to DMI function
  HYGON_PSTATE_DATA_BUFFER_HANDLE,                ///< Assign 0x000D001 buffer handle to Pstate data
  HYGON_PSTATE_ACPI_BUFFER_HANDLE,                ///< Assign 0x000D002 buffer handle to Pstate table
  HYGON_BRAND_ID_BUFFER_HANDLE,                   ///< Assign 0x000D003 buffer handle to Brand ID
  HYGON_WHEA_BUFFER_HANDLE,                       ///< Assign 0x000D004 buffer handle to WHEA function
  HYGON_S3_INFO_BUFFER_HANDLE,                    ///< Assign 0x000D005 buffer handle to S3 function
  HYGON_S3_NB_INFO_BUFFER_HANDLE,                 ///< Assign 0x000D006 buffer handle to S3 NB device info
  HYGON_ACPI_ALIB_BUFFER_HANDLE,                  ///< Assign 0x000D007 buffer handle to ALIB SSDT table
  HYGON_ACPI_IVRS_BUFFER_HANDLE,                  ///< Assign 0x000D008 buffer handle to IOMMU IVRS table
  HYGON_CRAT_INFO_BUFFER_HANDLE,                  ///< Assign 0x000D009 buffer handle to CRAT function
  HYGON_ACPI_CDIT_BUFFER_HANDLE,                  ///< Assign 0x000D00A buffer handle to CDIT function
  HYGON_GNB_SMU_MEMORY_INFO_BUFFER_HANDLE,        ///< Assign 0x000D00B buffer handle to SMU memory parameters function
  HYGON_CSTATE_DATA_BUFFER_HANDLE                 ///< Assign 0x000D00C buffer handle to Cstate data
} HYGON_BUFFER_HANDLE;

/************************************************************************
 *
 *  HGPI interface Call-Out function prototypes
 *
 ***********************************************************************/
...
/************************************************************************
 *
 *  HGPI interface structure definition and function prototypes
 *
 ***********************************************************************/
...
typedef struct {
  IN  HYGON_CODE_HEADER                  VersionString;   ///< HYGON embedded code version string
  ...
  IN UINT32                              CfgHygonPlatformType;    ///< Designate the platform as a Server, Desktop, or Mobile.
  IN UINT32                              CfgHygonPowerCeiling;    ///< PowerCeiling, specifies a maximum power usage limit for the platform
  ...
  IN DP_FIXED_VOLT_SWING_TYPE            CfgDpFixedVoltSwingType;///< To indicate fixed voltage swing value
                                                                 ///< @BldCfgItem{BLDCFG_DP_FIXED_VOLT_SWING}
  IN TECHNOLOGY_TYPE                     CfgDimmTypeUsedInMixedConfig; ///< Select the preferred technology type that HGPI will enable
                                                                     ///< when it is mixed with other technology types.
  IN BOOLEAN                             CfgDimmTypeDdr4Capable;     ///< Select DDR4 as technology type that HGPI will enable
                                                                     ///< @BldCfgItem{BLDCFG_DIMM_TYPE_DDR4_CAPABLE}
  IN BOOLEAN                             CfgDimmTypeDdr3Capable;     ///< Select DDR3 as technology type that HGPI will enable
                                                                     ///< @BldCfgItem{BLDCFG_DIMM_TYPE_DDR3_CAPABLE}
...
/**********************************************************************
 * Interface call:  HygonGetAvailableExeCacheSize
 **********************************************************************/
/// Get available Cache remain
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS    StdHeader;             ///< Standard configuration header
  OUT   UINT32                    AvailableExeCacheSize; ///< Remain size
} HYGON_GET_EXE_SIZE_PARAMS;

HGPI_STATUS
HygonGetAvailableExeCacheSize (
  IN OUT   HYGON_GET_EXE_SIZE_PARAMS *HygonGetExeSizeParams
  );
...
/************************************************************************
 *
 *  HGPI Basic Level interface structure definition and function prototypes
 *
 ***********************************************************************/

/**********************************************************************
 * Interface call:  HygonCreateStruct
 **********************************************************************/
HGPI_STATUS
HygonCreateStruct (
  IN OUT   HYGON_INTERFACE_PARAMS *InterfaceParams
  );

/**********************************************************************
 * Interface call:  HygonReleaseStruct
 **********************************************************************/
HGPI_STATUS
HygonReleaseStruct (
  IN OUT   HYGON_INTERFACE_PARAMS *InterfaceParams
...
/**********************************************************************
 * Interface call:  HygonInitEarly
 **********************************************************************/
/// InitEarly param structure
///
/// Provide defaults or customizations to each service performed in HygonInitEarly.
///
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;        ///< Standard configuration header
...
HGPI_STATUS
HygonInitEarly (
  IN OUT   HYGON_EARLY_PARAMS     *EarlyParams
  );

/**********************************************************************
 * Interface call:  HygonInitPost
 **********************************************************************/
/// HygonInitPost param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;        ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig; ///< platform operational characteristics.
...
HGPI_STATUS
HygonInitPost (
  IN OUT   HYGON_POST_PARAMS      *PostParams         ///< Hygon Cpu init param
  );

/**********************************************************************
 * Interface call:  HygonInitEnv
 **********************************************************************/
/// HygonInitEnv param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;           ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig;      ///< platform operational characteristics.
...
HGPI_STATUS
HygonInitEnv (
  IN OUT   HYGON_ENV_PARAMS       *EnvParams
  );

/**********************************************************************
 * Interface call:  HygonInitMid
 **********************************************************************/
/// HygonInitMid param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;           ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig;      ///< platform operational characteristics.
...
HGPI_STATUS
HygonInitMid (
  IN OUT   HYGON_MID_PARAMS       *MidParams
  );

/**********************************************************************
 * Interface call:  HygonInitLate
 **********************************************************************/
/// HygonInitLate param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS                 StdHeader;               ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION              PlatformConfig;          ///< platform operational characteristics.
...
HGPI_STATUS
HygonInitLate (
  IN OUT   HYGON_LATE_PARAMS      *LateParams
  );

/**********************************************************************
 * Interface call:  HygonInitRecovery
 **********************************************************************/
...
/// HygonInitRecovery param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS          StdHeader;         ///< Standard configuration header
  IN       MEM_PARAMETER_STRUCT         MemConfig;         ///< Memory post param
  IN       EXECUTION_CACHE_REGION       CacheRegion[3];    ///< The cached memory region. And the max cache region is 3
  IN       HYGON_CPU_RECOVERY_PARAMS    CpuRecoveryParams; ///< Params for CPU related recovery init.
} HYGON_RECOVERY_PARAMS;

HGPI_STATUS
HygonInitRecovery (
  IN OUT   HYGON_RECOVERY_PARAMS    *RecoveryParams
  );

/**********************************************************************
 * Interface call:  HygonInitResume
 **********************************************************************/
/// HygonInitResume param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;     ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig; ///< Platform operational characteristics
...
HGPI_STATUS
HygonInitResume (
  IN       HYGON_RESUME_PARAMS    *ResumeParams
  );

/**********************************************************************
 * Interface call:  HygonS3LateRestore
 **********************************************************************/
/// HygonS3LateRestore param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;        ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig; ///< Platform operational characteristics.
  IN       S3_DATA_BLOCK             S3DataBlock;    ///< Save state data
...
HGPI_STATUS
HygonS3LateRestore (
  IN OUT   HYGON_S3LATE_PARAMS    *S3LateParams
  );

/**********************************************************************
 * Interface call:  HygonS3FinalRestore
 **********************************************************************/
/// HygonS3FinalRestore param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;        ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig; ///< Platform operational characteristics.
  IN       S3_DATA_BLOCK             S3DataBlock;    ///< Save state data
} HYGON_S3FINAL_PARAMS;

HGPI_STATUS
HygonS3FinalRestore (
  IN OUT   HYGON_S3FINAL_PARAMS    *S3FinalParams
  );

/**********************************************************************
 * Interface call:  HygonInitRtb
 **********************************************************************/
/// HygonInitRtb param structure
typedef struct {
  IN OUT   HYGON_CONFIG_PARAMS       StdHeader;               ///< Standard configuration header
  IN       PLATFORM_CONFIGURATION    PlatformConfig;          ///< platform operational characteristics.
  IN       VOID                      *OptionMemoryInstallPtr; ///< Pointer to OptionMemoryInstall
...
HGPI_STATUS
HygonInitRtb (
  IN OUT   HYGON_RTB_PARAMS    *HygonInitRtbParams
  );

/**********************************************************************
 * Interface call:  HygonLateRunApTask
 **********************************************************************/
/**
...
HGPI_STATUS
HygonLateRunApTask (
  IN       AP_EXE_PARAMS  *HygonApExeParams
  );
...
/**********************************************************************
 * Interface service call:  HygonGetApicId
 **********************************************************************/
...
HGPI_STATUS
HygonGetApicId (
  IN OUT HYGON_APIC_PARAMS *HygonParamApic
  );

/**********************************************************************
 * Interface service call:  HygonGetPciAddress
 **********************************************************************/
...
HGPI_STATUS
HygonGetPciAddress (
  IN OUT   HYGON_GET_PCI_PARAMS *HygonParamGetPci
  );

/**********************************************************************
 * Interface service call:  HygonIdentifyCore
 **********************************************************************/
...
HGPI_STATUS
HygonIdentifyCore (
  IN OUT  HYGON_IDENTIFY_PARAMS *HygonParamIdentify
  );

/**********************************************************************
 * Interface service call:  HygonReadEventLog
 **********************************************************************/
/// An Event Log Entry.
typedef struct {
  IN       HYGON_CONFIG_PARAMS    StdHeader;    ///< Standard configuration header
  OUT   UINT32                    EventClass;   ///< The severity of this event, matches HGPI_STATUS.
...
/**********************************************************************
 * Interface service call:  HygonIdentifyDimm
 **********************************************************************/
...
HGPI_STATUS
HygonIdentifyDimm (
  IN OUT   HYGON_IDENTIFY_DIMM *HygonDimmIdentify
  );
...
/**********************************************************************
 * Interface service call:  HygonGet2DDataEye
 **********************************************************************/
/// Request the training data eye on Socket, Channel, Dimm.

typedef struct _HYGON_GET_DATAEYE {
  IN OUT HYGON_CONFIG_PARAMS    StdHeader;       ///< Standard configuration header
  IN OUT HYGON_POST_PARAMS      *PostParamsPtr;  ///< Pointer to HYGON_POST_PARAMS
...
HGPI_STATUS
HygonGet2DDataEye (
  IN OUT   HYGON_GET_DATAEYE *HygonGetDataEye
  );

#define HGPI_IDS_DFT_VAL  0xFFFF                   ///< Default value of every uninitlized NV item, the action for it will be ignored
#define HGPI_IDS_NV_END   0xFFFF                   ///< Flag specify end of option structure
/// WARNING: Don't change the comment below, it used as signature for script
/// HGPI IDS NV ID Definitions
typedef enum {
  HGPI_IDS_EXT_ID_START = 0x0000,                  ///< 0x0000                                                             specify the start of external NV id

  HGPI_IDS_NV_UCODE,                               ///< 0x0001                                                            Enable or disable microcode patching

  HGPI_IDS_NV_TARGET_PSTATE,                       ///< 0x0002                                                        Set the P-state required to be activated
  HGPI_IDS_NV_POSTPSTATE,                          ///< 0x0003                                           Set the P-state required to be activated through POST

  HGPI_IDS_NV_BANK_INTERLEAVE,                     ///< 0x0004                                                               Enable or disable Bank Interleave
  HGPI_IDS_NV_CHANNEL_INTERLEAVE,                  ///< 0x0005                                                            Enable or disable Channel Interleave
  HGPI_IDS_NV_NODE_INTERLEAVE,                     ///< 0x0006                                                               Enable or disable Node Interleave
  HGPI_IDS_NV_MEMHOLE,                             ///< 0x0007                                                                  Enables or disable memory hole

  HGPI_IDS_NV_SCRUB_REDIRECTION,                   ///< 0x0008                                           Enable or disable a write to dram with corrected data
  HGPI_IDS_NV_DRAM_SCRUB,                          ///< 0x0009                                                   Set the rate of background scrubbing for DRAM
  HGPI_IDS_NV_DCACHE_SCRUB,                        ///< 0x000A                                            Set the rate of background scrubbing for the DCache.
  HGPI_IDS_NV_L2_SCRUB,                            ///< 0x000B                                           Set the rate of background scrubbing for the L2 cache
  HGPI_IDS_NV_L3_SCRUB,                            ///< 0x000C                                           Set the rate of background scrubbing for the L3 cache
  HGPI_IDS_NV_ICACHE_SCRUB,                        ///< 0x000D                                             Set the rate of background scrubbing for the Icache
  HGPI_IDS_NV_SYNC_ON_ECC_ERROR,                   ///< 0x000E                                    Enable or disable the sync flood on un-correctable ECC error
  HGPI_IDS_NV_ECC_SYMBOL_SIZE,                     ///< 0x000F                                                                             Set ECC symbol size

  HGPI_IDS_NV_ALL_MEMCLKS,                         ///< 0x0010                                                      Enable or disable all memory clocks enable
  HGPI_IDS_NV_DCT_GANGING_MODE,                    ///< 0x0011                                                                             Set the Ganged mode
  HGPI_IDS_NV_DRAM_BURST_LENGTH32,                 ///< 0x0012                                                                    Set the DRAM Burst Length 32
  HGPI_IDS_NV_MEMORY_POWER_DOWN,                   ///< 0x0013                                                        Enable or disable Memory power down mode
  HGPI_IDS_NV_MEMORY_POWER_DOWN_MODE,              ///< 0x0014                                                                  Set the Memory power down mode
  HGPI_IDS_NV_DLL_SHUT_DOWN,                       ///< 0x0015                                                                   Enable or disable DLLShutdown
  HGPI_IDS_NV_ONLINE_SPARE,                        ///< 0x0016      Enable or disable the Dram controller to designate a DIMM bank as a spare for logical swap

  HGPI_IDS_NV_HDTOUT,                              ///< 0x0017                                                                Enable or disable HDTOUT feature

  HGPI_IDS_NV_GNBHDAUDIOEN,                        ///< 0x0018                                                                  Enable or disable GNB HD Audio

  HGPI_IDS_NV_CPB_EN,                              ///< 0x0019                                                                          Core Performance Boost

  HGPI_IDS_NV_HTC_EN,                              ///< 0x001A                                                                                      HTC Enable
  HGPI_IDS_NV_HTC_OVERRIDE,                        ///< 0x001B                                                                                    HTC Override
  HGPI_IDS_NV_HTC_PSTATE_LIMIT,                    ///< 0x001C                                                                        HTC P-state limit select
  HGPI_IDS_NV_HTC_TEMP_HYS,                        ///< 0x001D                                                                      HTC Temperature Hysteresis
  HGPI_IDS_NV_HTC_ACT_TEMP,                        ///< 0x001E                                                                             HTC Activation Temp

  HGPI_IDS_NV_DRAMCON,                             ///< 0x001F                                                  Specify the mode for controller initialization
  HGPI_IDS_NV_ALL_CKE,                             ///< 0x0020                                                                       Enable or disable all CKE
  HGPI_IDS_NV_ALL_CS,                              ///< 0x0021                                                                        Enable or disable all CS
  HGPI_IDS_NV_MSR_DE_CFG_BIT16,                    ///< 0x0022                                               Workaround GCC/C000005 issue for XV Core on CZ A0
  HGPI_IDS_EXT_ID_END,                             ///< 0x0023                                                               specify the end of external NV ID
} IDS_EX_NV_ID;

#define IDS_NUM_EXT_NV_ITEM  (HGPI_IDS_EXT_ID_END - HGPI_IDS_EXT_ID_START + 1)
