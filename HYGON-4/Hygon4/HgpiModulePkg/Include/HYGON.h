...
#define IMAGE_SIGNATURE  'GYH$'
#define IOCF8            0xCF8
#define IOCFC            0xCFC

/// The return status for all HGPI public services.
///
...
///
typedef enum {
  HGPI_SUCCESS = 0,             ///< The service completed normally. Info may be logged.
  HGPI_UNSUPPORTED,             ///< The dispatcher or create struct had an unimplemented function requested.
                                ///< Not logged.
  HGPI_BOUNDS_CHK,              ///< A dynamic parameter was out of range and the service was not provided.
                                ///< Example, memory address not installed, heap buffer handle not found.
                                ///< Not Logged.
  // HGPI_STATUS of greater severity (the ones below this line), always have a log entry available.
  HGPI_ALERT,                   ///< An observed condition, but no loss of function.  See Log.
  HGPI_WARNING,                 ///< Possible or minor loss of function.  See Log.
  HGPI_ERROR,                   ///< Significant loss of function, boot may be possible.  See Log.
  HGPI_CRITICAL,                ///< Continue boot only to notify user.  See Log.
  HGPI_FATAL,                   ///< Halt booting.  See Log, however Fatal errors pertaining to heap problems
                                ///< may not be able to reliably produce log events.
  HgpiStatusMax                 ///< Not a status, for limit checking.
} HGPI_STATUS;

/// For checking whether a status is at or above the mandatory log level.
#define HGPI_STATUS_LOG_LEVEL  HGPI_ALERT
 ...
 */
typedef HGPI_STATUS (*CALLOUT_ENTRY) (
  IN       UINT32  Function,
  IN       UINTN   FcnData,
  IN OUT   VOID    *ConfigPtr
  );

typedef HGPI_STATUS (*IMAGE_ENTRY) (
  VOID *ConfigPtr
  );
typedef HGPI_STATUS (*MODULE_ENTRY) (
  VOID *ConfigPtr
  );

/// This allocation type is used by the HygonCreateStruct entry point
typedef enum {
...
/// HGPI struct name
typedef enum {
  // HGPI BASIC FUNCTIONS
  HYGON_INIT_RECOVERY = 0x00020000,                           ///< HygonInitRecovery entry point handle
  HYGON_CREATE_STRUCT,                                        ///< HygonCreateStruct handle
  HYGON_INIT_EARLY,                                           ///< HygonInitEarly entry point handle
  HYGON_INIT_ENV,                                             ///< HygonInitEnv entry point handle
  HYGON_INIT_LATE,                                            ///< HygonInitLate entry point handle
  HYGON_INIT_MID,                                             ///< HygonInitMid entry point handle
  HYGON_INIT_POST,                                            ///< HygonInitPost entry point handle
  HYGON_INIT_RESET,                                           ///< HygonInitReset entry point handle
  HYGON_INIT_RESUME,                                          ///< HygonInitResume entry point handle
  HYGON_RELEASE_STRUCT,                                       ///< HygonReleaseStruct handle
  HYGON_S3LATE_RESTORE,                                       ///< HygonS3LateRestore entry point handle
  HYGON_GET_APIC_ID,                                          ///< HygonGetApicId entry point handle
  HYGON_GET_PCI_ADDRESS,                                      ///< HygonGetPciAddress entry point handle
  HYGON_IDENTIFY_CORE,                                        ///< HygonIdentifyCore general service handle
  HYGON_READ_EVENT_LOG,                                       ///< HygonReadEventLog general service handle
  HYGON_GET_EXECACHE_SIZE,                                    ///< HygonGetAvailableExeCacheSize general service handle
  HYGON_LATE_RUN_AP_TASK,                                     ///< HygonLateRunApTask entry point handle
  HYGON_IDENTIFY_DIMMS,                                       ///< HygonIdentifyDimm general service handle
  HYGON_GET_2D_DATA_EYE,                                      ///< HygonGet2DDataEye general service handle
  HYGON_S3FINAL_RESTORE,                                      ///< HygonS3FinalRestore entry point handle
  HYGON_INIT_RTB                                              ///< HygonInitRtb entry point handle
} HGPI_STRUCT_NAME;
...
/// The standard header for all HGPI services.
/// For internal HGPI naming conventions, see @ref hygonconfigparamname .
typedef struct {
  IN       UINT32           ImageBasePtr;           ///< The HGPI Image base address.
  IN       UINT32           Func;                   ///< The service desired
  IN       UINT32           AltImageBasePtr;        ///< Alternate Image location
  IN       CALLOUT_ENTRY    CalloutPtr;             ///< For Callout from HGPI
...
typedef struct {
  ...
  IN       HGPI_STRUCT_NAME       HgpiFunctionName; ///< The service to init
...
/// HGPI Binary module header structure
typedef struct {
...
/// HGPI Binary module header structure
typedef struct _HYGON_MODULE_HEADER {
  ...
  IN  struct _HYGON_MODULE_HEADER    *NextBlock;            ///< Next module header link
} HYGON_MODULE_HEADER;

// HYGON_CODE_HEADER Signatures.
// Clear HGPI V5 code signature
#define HGPI_CODE_SIGNATURE    { 0, 0, 0, 0, 0, 0, 0, 0 }
