...
typedef enum {
  ...
  LogicalNonVolatileMemType,                            ///< Assign 31 to Logical non-volatile device
  HBMMemType,                                           ///< Assign 32 to High Bandwidth Memory
  HBM2MemType,                                          ///< Assign 33 to High Bandwidth Memory Generation 2
...
typedef struct {
  ...
  OUT CHAR8                                       DeviceLocator[12];    // byo230831 - ///< The string number of the string that identifies the physically labeled socket or board position where the memory device is located.
  ...
  OUT UINT32                                      ExtendedSpeed;                 ///< Extended speed of the memory device. Identifies the maximum capable speed of the device
  OUT UINT32                                      ExtendedConfiguredMemorySpeed; ///< Extended configured memory speed of the memory device. identifies the configured speed of the memory device
} TYPE17_DMI_INFO;

/// Memory DMI Type 17 - for memory use
typedef struct {
  OUT UINT16                  Socket      : 3;           ///< Socket ID
  OUT UINT16                  Channel     : 4;           ///< Channel ID
  OUT UINT16                  Dimm        : 2;           ///< DIMM ID
  OUT UINT16                  DimmPresent : 1;           ///< Dimm Present
  OUT UINT16                  Reserved    : 6;           ///< 
  OUT UINT16                 Handle;                    ///< The temporary handle, or instance number, associated with the structure
  OUT UINT16                 TotalWidth;                ///< Total Width, in bits, of this memory device, including any check or error-correction bits.
  OUT UINT16                 DataWidth;                 ///< Data Width, in bits, of this memory device.
  OUT UINT16                 MemorySize;                ///< The size of the memory device.
...
typedef struct {
  ...
  UINT16    SubSysRevisionId;           ///< High byte reserved, set to 0
  UINT16    ManufacturingIdCode;        ///<
  UINT16    ManufacturingLocation;      ///< High byte reserved, set to 0
  UINT16    ManufacturingDate;          ///<
  UINT8     SerialNumber[4];            ///< SPD 325 ~ 328
  UINT16    RevisionId;                 ///< High byte reserved, set to 0
