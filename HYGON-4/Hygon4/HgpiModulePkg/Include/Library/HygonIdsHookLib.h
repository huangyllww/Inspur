 ...
 * @file
 *
 * HYGON Integrated Debug Hook Routines
 *
...
#ifndef _HYGON_IDS_HOOKLIB_H_
#define _HYGON_IDS_HOOKLIB_H_
#include <IdsHookId.h>
#include <IdsNvTable.h>
#include <Library/HygonIdsHookExtLib.h>
 ...
 *
 * @retval    HGPI_SUCCESS     Successfully get the IDS_HOOK_TABLE
 * @retval    HGPI_ERROR       Error happens in get the IDS_HOOK_TABLE
 *
 */
HGPI_STATUS
GetIdsHookTable (
...
/**
 * Used to get the value for a specific IdsNvIdSAT
 *
 *
 * @param[in] IdsNvIdSAT            ID used to identify the IDS HOOK
 * @param[in,out] IdsNvValue     Pointer to be filled with corresponding IdsNvIdSAT value
 *
 * @retval    HGPI_SUCCESS     Find the according IDSNVID value successfully
 * @retval    HGPI_ERROR       Error occurs during finding the according IDSNVID value
 *
 */
HGPI_STATUS
HygonIdsNvReader (
  IN       IDS_NV_ID IdsNvIdSAT,
  IN OUT   UINT64 *IdsNvValue
  );

#define IDS_NV_READ_SKIP(Nvid, NvValue) \
  if ((HygonIdsNvReader ((Nvid), (NvValue)) == HGPI_SUCCESS))

#endif //#define _HYGON_IDS_HOOKLIB_H_
