...
typedef struct _S3_MEM_NB_BLOCK {
  ...
  _4BYTE_ALIGN VOID (*MemS3ExitSelfRefReg)(
    MEM_NB_BLOCK *NBPtr,
    HYGON_CONFIG_PARAMS *StdHeaderPtr
    ); ///< S3 Exit self refresh register
  _4BYTE_ALIGN VOID (*MemS3GetConPCIMask)(
    MEM_NB_BLOCK *NBPtr,
    DESCRIPTOR_GROUP *DescriptPtr
    ); ///< Get conditional mask for PCI register setting
  _4BYTE_ALIGN VOID (*MemS3GetConMSRMask)(
    MEM_NB_BLOCK *NBPtr,
    DESCRIPTOR_GROUP *DescriptPtr
    ); ///< Get conditional mask for MSR register setting
  _4BYTE_ALIGN UINT16 (*MemS3GetRegLstPtr)(
    MEM_NB_BLOCK *NBPtr,
    DESCRIPTOR_GROUP *DescriptPtr
    ); ///< Get register list pointer for both PCI and MSR register
  _4BYTE_ALIGN BOOLEAN (*MemS3Resume)(
    struct _S3_MEM_NB_BLOCK *S3NBPtr,
    UINT8 NodeID
    );///< Exit Self Refresh
  _4BYTE_ALIGN VOID (*MemS3RestoreScrub)(
    MEM_NB_BLOCK *NBPtr,
    UINT8 NodeID
    );///< Restore scrubber base
  _4BYTE_ALIGN HGPI_STATUS (*MemS3GetDeviceRegLst)(
    UINT32 ReigsterLstID,
    VOID **RegisterHeader,
    MEM_NB_BLOCK *NBPtr
    ); ///< Get register list for a device
  _4BYTE_ALIGN BOOLEAN (*MemS3PspDetect)(
    MEM_NB_BLOCK *NBPtr
    ); ///< PSP detection
  _4BYTE_ALIGN BOOLEAN (*MemS3PspPlatformSecureBootEn)(
    MEM_NB_BLOCK *NBPtr
    ); ///< PSP Platform Secure Boot detection
} S3_MEM_NB_BLOCK;
