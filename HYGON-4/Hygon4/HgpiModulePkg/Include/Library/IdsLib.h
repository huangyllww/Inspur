 ...
 * HYGON IDS Routines
 *
 * Contains HYGON HGPI Integrated Debug Macros
 *
 ...
 *  IBVs can change HGPI's default port by defining IDS_DEBUG_PORT to desired port
 *  in OptionsIds.h in their build tip.
 *
 *  @param[in]      TestPoint       The value for display indicating progress
 *  @param[in,out]  StdHeader       Pointer of HYGON_CONFIG_PARAMS
 *
 **/

#define HGPI_TESTPOINT(TestPoint, StdHeader)  IdsHgpiTestPoint ((TestPoint), (StdHeader))
...
/**
 * @def IDS_ERROR_TRAP
 * Trap HGPI Error events with stop code display.
 *
...
///// HGPI tracing service
// #if IDSOPT_TRACING_ENABLED == TRUE
// #ifdef VA_ARGS_SUPPORTED
// #if IDSOPT_C_OPTIMIZATION_DISABLED == TRUE
// #define IDS_HDT_CONSOLE(f, s, ...)        HygonIdsDebugPrint (f, s, __VA_ARGS__)
// #else
// #pragma warning(disable: 4127)
// #define IDS_HDT_CONSOLE(f, s, ...)        if      (f == MEM_FLOW) HygonIdsDebugPrintMem (s, __VA_ARGS__); \
//                                                  else if (f == CPU_TRACE) HygonIdsDebugPrintCpu (s, __VA_ARGS__); \
//                                                   else if (f == TOPO_TRACE) HygonIdsDebugPrintTopology (s, __VA_ARGS__); \
//                                                   else if (f == GNB_TRACE) HygonIdsDebugPrintGnb (s, __VA_ARGS__); \
//                                                  else HygonIdsDebugPrint (f, s, __VA_ARGS__)
// #endif
// #else
// #define IDS_HDT_CONSOLE    HygonIdsDebugPrint
// #endif
// #define CONSOLE            HygonIdsDebugPrintAll
// #define IDS_HDT_CONSOLE_DEBUG_CODE(Code)  Code
// #define IDS_TIMEOUT_CTL(t)                IdsMemTimeOut (t)
// #else
//// #define IDS_HDT_CONSOLE           1 ? (VOID) 0 : HygonIdsDebugPrint
// #define IDS_HDT_CONSOLE
...
// #define IDS_HDT_CONSOLE_INIT(x)           HygonIdsDpHdtOutInit (x)
// #define IDS_HDT_CONSOLE_EXIT(x)           HygonIdsDpHdtOutExit (x)
// #define IDS_HDT_CONSOLE_S3_EXIT(x)        HygonIdsDpHdtOutS3Exit (x)
// #define IDS_HDT_CONSOLE_S3_AP_EXIT(x)     HygonIdsDpHdtOutS3ApExit (x)

// #define IDS_HDT_CONSOLE_FLUSH_BUFFER(x)   HygonIdsDpHdtOutBufferFlush (x)
// #define IDS_HDT_CONSOLE_ASSERT(x)   HygonIdsDebugPrintAssert (x)
...
IdsLibDebugPrint (
...
#if 0
  VOID
  HygonIdsDpHdtOutInit (
  IN OUT   HYGON_CONFIG_PARAMS *StdHeader
  );

  VOID
  HygonIdsDpHdtOutExit (
  IN OUT   HYGON_CONFIG_PARAMS *StdHeader
  );

  VOID
  HygonIdsDpHdtOutS3Exit (
  IN OUT   HYGON_CONFIG_PARAMS *StdHeader
  );

  VOID
  HygonIdsDpHdtOutS3ApExit (
  IN OUT   HYGON_CONFIG_PARAMS *StdHeader
  );
  ...
  VOID
  HygonIdsDebugPrintTopology (
  IN      CONST CHAR8 *Format,
  IN      ...
  );

  VOID
  HygonIdsDebugPrintCpu (
  IN      CONST CHAR8 *Format,
  IN      ...
  );

  VOID
  HygonIdsDebugPrintMem (
  IN      CONST CHAR8 *Format,
  IN      ...
  );

  VOID
  HygonIdsDebugPrintGnb (
  IN      CONST CHAR8 *Format,
  IN      ...
  );

  VOID
  HygonIdsDebugPrintAll (
  IN      CONST CHAR8 *Format,
  IN      ...
  );

  VOID
  HygonIdsDpHdtOutBufferFlush (
  IN OUT   HYGON_CONFIG_PARAMS *StdHeader
  );
