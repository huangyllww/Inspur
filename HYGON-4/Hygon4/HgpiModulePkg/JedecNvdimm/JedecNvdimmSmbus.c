...
#define NUM_DIMM_INFO_ENTRIES  sizeof (DimmSmbus)/sizeof (HYGON_DIMM_INFO_SMBUS)

// ---- PORTING REQUIRED ------
// Below is configuration for CRB

HYGON_DIMM_INFO_SMBUS  DimmSmbus[] =
{
  // DIMM_SPD_MAP
  { 0, 0, 0xA0 },
  { 0, 1, 0xA2 },
  ...
  )
{
  ...
  if (Page == 1) {
    ...
    Status = mSmbus->SmbusController.Execute (
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "JedecNvdimmSmbusInit Begin\n"));           // byo230914 -
  ...
  DEBUG ((EFI_D_INFO, "JedecNvdimmSmbusInit End\n"));             // byo230914 -
