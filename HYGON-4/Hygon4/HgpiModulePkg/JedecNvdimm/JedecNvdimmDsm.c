  ...
  )
{
  ...
  for (i = 0; i < TotalNvdimm; i++) {
    ...
    if (Data & BIT0) {
      EventNotifications |= BIT0;                        // Persistency notification
    }

    if (Data & BIT1) {
      EventNotifications |= BIT1;                        // Warning threshold notification
    }

    if (Data & BIT2) {
      EventNotifications |= BIT2;                        // Voltage regulator failed notification
    }
  ...
  )
{
  ...
  if (Data & BIT7) {
    TimeoutUs *= 1000000;                   // Convert seconds to microseconds
  } else {
    TimeoutUs *= 1000;                      // Convert milliseconds to microseconds
  }
  ...
  )
{
  ...
  Checksum = (UINT16)Crc16 (
                       ((FIRMWARE_UPDATE_DATA *)pDsmAcpiSmmData->InputData)->FirmwareData,
  ...
  )
{
  ...
  switch (pDsmAcpiSmmData->FunctionIndex) {
    ...
    case 16:
      ...
      ReadNvdimm (Socket, Channel, Dimm, 0, PAGE0_CAPABILITIES, &Data, FALSE);
      if (!(Data & BIT2)) {
        gDsmErrorInjectionEnable = 0;                           // Disable error injection if it's not supported
      }
