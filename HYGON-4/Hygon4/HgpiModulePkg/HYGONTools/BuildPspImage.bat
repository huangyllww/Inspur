...
@echo D | xcopy  %PSP_FW_PATH%\*.*   %TEMP_PATH%\HgpiModulePkg\Firmwares /S /Y
@echo copy  %PSPKIT_PATH%\*.*       %TEMP_PATH% /Y
copy  %PSPKIT_PATH%\*.*       %TEMP_PATH% /Y
IF NOT "%CUSTOM_HPCB_PATH%" == "" (
  @echo copy  %CUSTOM_HPCB_PATH%\*.*   %TEMP_PATH% /Y
  mkdir %TEMP_PATH%\Build\Hpcb\Release
  copy  %CUSTOM_HPCB_PATH%\*.*   %TEMP_PATH%\Build\Hpcb\Release /Y
)
