...
#define FILECODE  FABRIC_HYEX_FABRICPEI_FABRICSCRUBINIT_FILECODE
...
#define mGetIntLvNumDies(DramLimitAddr)     (1 << DramLimitAddr.HyExField.IntLvNumDies)
  ...
  )
{
  ...
  HPOB_MEM_GENERAL_CONFIGURATION_INFO_TYPE_STRUCT_HYEX  HpobMemGenCfg;
  ...
  for (Socket = 0; Socket < NumOfSocketPresent; Socket++) {
    ...
    for (Cdd = 0; Cdd < MAX_CDDS_PER_SOCKET; Cdd++) {
      ...
      CopyMem (&HpobMemGenCfg, HpobEntry, sizeof (HPOB_MEM_GENERAL_CONFIGURATION_INFO_TYPE_STRUCT_HYEX));

      IsEccEnabled = FALSE;
      for (i = 0; i < HPOB_MAX_CHANNELS_PER_CDD_HYEX; i++) {
  ...
  )
{
  ...
  for (AddrMapPair = 0; AddrMapPair < 2; AddrMapPair++) {
    ...
    if ((DramLimitAddr.HyExField.DstFabricID & FABRIC_ID_SOCKET_DIE_MASK_HYEX) != FabricId) {
      continue;
    }
