 ...
 * @file
 *
 * HYGON Fabric SATORI API, and related functions.
 *
...
#define FILECODE  FABRIC_HYEX_FABRICPEI_HYGONFABRICSTPEI_FILECODE

EFI_STATUS
EFIAPI
FabricHyExEndOfPeiSignalPpiCallback (
  IN  EFI_PEI_SERVICES                **PeiServices,
  IN  EFI_PEI_NOTIFY_DESCRIPTOR       *NotifyDesc,
  IN  VOID                            *InvokePpi
  );

EFI_PEI_NOTIFY_DESCRIPTOR  mFabricHyExEndOfPeiSignalPpiCallback = {
...
Routine Description:
  ...
  Fabric - Satori Driver Entry.  Initialize the core complex.
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpDfPeiEntry, NULL);

  Status = EFI_SUCCESS;

  IDS_HDT_CONSOLE (CPU_TRACE, "  HygonFabricHyExPeiInit Entry\n");

  if (IsGroupMode ()) {
    IDS_HDT_CONSOLE (CPU_TRACE, "  Access Fabric Registers by group mode \n");
  } else {
    IDS_HDT_CONSOLE (CPU_TRACE, "  Access Fabric Registers by flatting mode \n");
  }

  IDS_HDT_CONSOLE (CPU_TRACE, "  Socket Number = %d \n", FabricTopologyGetNumberOfSocketPresent ());
  IDS_HDT_CONSOLE (CPU_TRACE, "  Cdd Number per socket = %d \n", FabricTopologyGetNumberOfCddsOnSocket (0));
  IDS_HDT_CONSOLE (CPU_TRACE, "  IOMS Number per socket = %d \n", FabricTopologyGetNumberOfRootBridgesOnSocket (0));
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "    Pci Bus Map Init\n");
  FabricPciBusMapInit ();

  IDS_HOOK (IDS_HOOK_FABRIC_PEI_INIT_END, NULL, NULL);

  Status = (**PeiServices).NotifyPpi (PeiServices, &mFabricHyExEndOfPeiSignalPpiCallback);
  
  IDS_HDT_CONSOLE (CPU_TRACE, "  HygonFabricHyExPeiInit End\n");

  HGPI_TESTPOINT (TpDfPeiExit, NULL);
...
{
  IDS_HDT_CONSOLE_PSP_TRACE ("HygonFabricHyExPei.EndOfPeiSignalPpiCallback\n");
