...
#define FILECODE  FABRIC_HYGX_FABRICPEI_FABRICDFCLKGATINGINIT_FILECODE
  ...
  )
{
  ...
  UINT8                       DfInstanceIdToEnableClkGatingTableHyGx[] = 
                                {CS0_INSTANCE_ID, 
                                 CS1_INSTANCE_ID, 
                                 CS2_INSTANCE_ID, 
                                 CS3_INSTANCE_ID,
                                 ...
                                 CCM1_INSTANCE_ID,
                                 PIE_INSTANCE_ID,
                                 TCDX0_INSTANCE_ID_HYGX, 
                                 TCDX1_INSTANCE_ID_HYGX, 
                                 TCDX2_INSTANCE_ID_HYGX, 
                                 TCDX3_INSTANCE_ID_HYGX,
                                 TCDX4_INSTANCE_ID_HYGX,
                                 TCDX5_INSTANCE_ID_HYGX,
                                 TCDX6_INSTANCE_ID_HYGX,
                                 TCDX7_INSTANCE_ID_HYGX,
                                 TCDX8_INSTANCE_ID_HYGX,
                                 TCDX9_INSTANCE_ID_HYGX
                                };
  ...
  for (i = 0; i < FabricTopologyGetNumberOfSocketPresent (); i++) {
    for (j = 0; j < FabricTopologyGetNumberOfLogicalDiesOnSocket (i); j++) {
      ...
      if (!IsIomTcbEnabled && (FabricTopologyGetNumberOfSystemDies () == 1)) {
        ...
        IDS_HOOK (IDS_HOOK_DF_CLK_GATING, NULL, (VOID *)&DfGlblClkGater);
        for (k = 0; k < sizeof (DfInstanceIdToEnableClkGatingTableHyGx); k++) {
          FabricRegisterAccWrite (
            i, 
            j, 
            DF_GLBL_CLK_GATER_FUNC, 
            DF_GLBL_CLK_GATER_REG, 
            (UINTN) DfInstanceIdToEnableClkGatingTableHyGx[k], 
            DfGlblClkGater.Value, 
            FALSE
          );
        }
