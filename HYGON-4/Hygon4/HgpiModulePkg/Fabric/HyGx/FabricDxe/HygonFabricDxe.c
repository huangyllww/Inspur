...
#define FILECODE  FABRIC_HYGX_FABRICDXE_HYGONFABRICSTDXE_FILECODE
...
EFI_STATUS
EFIAPI
HygonFabricHyGxDxeInit (
  IN       EFI_HANDLE         ImageHandle,
  IN       EFI_SYSTEM_TABLE   *SystemTable
  )
{
  UINT32                         NumaDomainCount;
  UINT32                         CxlNumaDomainCount;
  EFI_STATUS                     Status;
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  HygonFabricHyGxDxeInit Entry\n");
  ...
  if (gBS->LocateProtocol (&gHygonFabricNumaServicesProtocolGuid, NULL, (VOID **)&FabricNuma) == EFI_SUCCESS) {
    if (FabricNuma->GetDomainInfo (FabricNuma, &NumaDomainCount, NULL, &CxlNumaDomainCount, NULL) == EFI_SUCCESS) {
      IDS_HDT_CONSOLE (CPU_TRACE, "  DramNumaDomainCount %d, CxlNumaDomainCount %d\n", NumaDomainCount, CxlNumaDomainCount);
      if ((NumaDomainCount + CxlNumaDomainCount) > 1) {
  ...
  if (PcdGetBool (PcdHygonAcpiHmat)) {
    Status = gBS->CreateEventEx (
                          EVT_NOTIFY_SIGNAL,
                          TPL_NOTIFY,
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  HygonFabricHyGxDxeInit End\n");
