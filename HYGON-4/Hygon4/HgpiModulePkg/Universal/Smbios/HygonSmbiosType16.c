...
#define FILECODE  UNIVERSAL_SMBIOS_HYGONSMBIOSTYPE16_FILECODE
  ...
  )
{
  ...
  HYGON_SMBIOS_TABLE_TYPE16  *SmbiosTableType16;

  Status = EFI_SUCCESS;
  // Calculate size of DMI type 16
  StructureSize = sizeof (HYGON_SMBIOS_TABLE_TYPE16);
  ...
  SmbiosTableType16->Hdr.Type   = HYGON_EFI_SMBIOS_TYPE_PHYSICAL_MEMORY_ARRAY;
  SmbiosTableType16->Hdr.Length = (UINT8)StructureSize;
  SmbiosTableType16->Hdr.Handle = HYGON_SMBIOS_HANDLE_PI_RESERVED;
  ...
  SmbiosTableType16->MaximumCapacity = PcdGet32 (PcdHygonSmbiosT16MaximumCapacity);
  if (SmbiosTableType16->MaximumCapacity == 0x80000000) {
    SmbiosTableType16->ExtendedMaximumCapacity = PcdGet64 (PcdHygonSmbiosT16ExtMaximumCapacity);
  } else {
