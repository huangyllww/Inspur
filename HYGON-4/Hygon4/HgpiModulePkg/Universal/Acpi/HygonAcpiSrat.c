...
#define FILECODE  UNIVERSAL_ACPI_HYGONACPISRAT_FILECODE
  ...
  )
{
  ...
  LibHygonMemCopy ((VOID *)SratHeaderStructPtr, (VOID *)&SratHeaderStruct, (UINTN)(sizeof (SRAT_HEADER)), StdHeader);
  // Update table OEM fields.
  ASSERT (AsciiStrnLenS (PcdGetPtr (PcdHygonAcpiTableHeaderOemId), 100) <= 6);
  ASSERT (AsciiStrnLenS (PcdGetPtr (PcdHygonAcpiSratTableHeaderOemTableId), 100) <= 8);

  LibHygonMemCopy (
    (VOID *)&SratHeaderStructPtr->OemId,
    (VOID *)PcdGetPtr (PcdHygonAcpiTableHeaderOemId),
    AsciiStrnLenS (PcdGetPtr (PcdHygonAcpiTableHeaderOemId), 6),
    StdHeader
    );
  LibHygonMemCopy (
    (VOID *)&SratHeaderStructPtr->OemTableId,
    (VOID *)PcdGetPtr (PcdHygonAcpiSratTableHeaderOemTableId),
    AsciiStrnLenS (PcdGetPtr (PcdHygonAcpiSratTableHeaderOemTableId), 8),
    StdHeader
    );
...
{
  EFI_STATUS                             Status;
  HYGON_CCX_ACPI_SRAT_SERVICES_PROTOCOL  *SratLocalApic;

  Status = EFI_ABORTED;
  if (gBS->LocateProtocol (&gHygonCcxAcpiSratServicesProtocolGuid, NULL, (VOID **)&SratLocalApic) == EFI_SUCCESS) {
...
{
  EFI_STATUS                             Status;
  HYGON_CCX_ACPI_SRAT_SERVICES_PROTOCOL  *SratX2Apic;

  Status = EFI_ABORTED;
  if (gBS->LocateProtocol (&gHygonCcxAcpiSratServicesProtocolGuid, NULL, (VOID **)&SratX2Apic) == EFI_SUCCESS) {
