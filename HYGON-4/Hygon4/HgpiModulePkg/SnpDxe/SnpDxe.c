  ...
  )
{
  ...
  if (EFI_SUCCESS == Status) {
    if (Private->RecyclTxBufCount < MAX_XMIT_BUFFERS) {
      Private->RecyclTxBufCount++;
    }
    ((PXE_DB_GET_STATUS*)(Private->Db))->TxBuffer[Private->RecyclTxBufCount-1] = (UINTN) (UINT8 *) Buffer;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "HygonEnet Private Data inital failure\r\n"));
    FreePool (Private);
    Private = NULL;
    return Status;
  }
