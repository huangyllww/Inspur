...
#define FILECODE  MEM_HYGONMEMCHANXLATHyExPEI_MEMCHANXLATHyExPEI_FILECODE
...
HOST_TO_HPCB_CHANNEL_XLAT SL2ChannelXlatTable[] = {
...
HOST_TO_HPCB_CHANNEL_XLAT SL2r2ChannelXlatTable[] = {
  // Channel A = 0, Channel B = 1, Channel C = 2, Channel D = 3
  // Channel E = 4, Channel F = 5, Channel G = 6, Channel H = 7
  // Requested   Translated
  { 0,          3 },
  ...
  )
{
  ...
  if (CddNumberPerSocket ==  4) {
    mMemChanXLatPpi->XLatTab = (VOID *)&SL2ChannelXlatTable[0];
  }
  ...
  if (CddNumberPerSocket ==  2) {
    mMemChanXLatPpi->XLatTab = (VOID *)&SL2r2ChannelXlatTable[0];
  }
