 ...
 * @file
 *
 * HYGON Memory API for CZ DDR4, and related functions.
 *
...
#include "HygonMemSmbiosDxe.h"
#include "Library/UefiBootServicesTableLib.h"
#include "Filecode.h"

#define FILECODE  MEM_HYGONMEMSMBIOSDXE_HYGONMEMSMBIOSDXE_FILECODE
 ...
 *----------------------------------------------------------------------------------------
 */
STATIC HYGON_MEM_SMBIOS_SERVICES_PROTOCOL  mMemDmiServicesProtocol = {
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (MAIN_FLOW, "  HygonMemSmbiosDxe Entry\n");

  // Install SMBIOS services protocol
  Status = gBS->InstallProtocolInterface (
                  &ImageHandle,
                  &gHygonMemSmbiosServicesProtocolGuid,
                  EFI_NATIVE_INTERFACE,
                  &mMemDmiServicesProtocol
                  );

  IDS_HDT_CONSOLE (MAIN_FLOW, "  HygonMemSmbiosDxe Exit\n");
  return Status;
}
 ...
 *                                                  HYGON_MEM_SMBIOS_SERVICES_PROTOCOL instance.
  ...
  )
{
  ...
  LocateHeapParams.BufferHandle = HYGON_DMI_INFO_BUFFER_HANDLE;
  ...
  if (Status == HGPI_SUCCESS) {
    ...
    LibHygonMemCopy (MemDmiInfo, LocateHeapParams.BufferPtr, LocateHeapParams.BufferSize, &StdHeader);
