...
#include <Protocol/HygonSocProtocol.h>
#include "HygonSocDxe.h"

#define FILECODE  SOC_HYGONSOCSL2STDXE_HYGONSOCSL2STDXE_FILECODE
...
/*********************************************************************************
 * Name: HygonSocDxeInit
 *
 * Description
 *   Entry point of the HYGON SOC Satori SL2 DXE driver
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "*****************************DXE SOC SL2 ST Driver Entry*********************\n"));         // byo230914 -
  ...
  if (CpuModel == HYGON_EX_CPU) {
    for (i = 0; i < HyExSiliconDriverProtocolListNumber; i++) {
      Status = gBS->InstallProtocolInterface (
                      &Handle,
                      HyExSiliconDriverProtocolList[i],
  ...
  if (CpuModel == HYGON_GX_CPU) {
    for (i = 0; i < HyGxSiliconDriverProtocolListNumber; i++) {
      Status = gBS->InstallProtocolInterface (
                      &Handle,
                      HyGxSiliconDriverProtocolList[i],
                      EFI_NATIVE_INTERFACE,
                      NULL
                      );
    }
  ...
  DEBUG ((EFI_D_INFO, "*****************************DXE SOC SL2 ST Driver Exit*********************\n"));          // byo230914 -
  return (Status);
}
