...
HYGON_USB_DISABLE_MAP gHygonUsbDisableMap[] = {
  // SocketNum, RootBridgeNum, Usb0DisableMap, Usb1DisableMap
  // Socket0
  { 0, 0, 0x00, 0x00 },
  { 0, 1, 0x00, 0x00 },
  { 0, 2, 0xFF, 0xFF },     // may need debug
  { 0, 3, 0xFF, 0xFF },
  ...
  )
{
  ...
  // Program USB3 PHY0 PHYCFG0(address=0x0A508200) to 0x00000039
  // Program USB3 PHY1 PHYCFG0(address=0x0A508600) to 0x00000039
  // Program USB3 PHY2 PHYCFG0(address=0x0A508A00) to 0x00000039
  // Program USB3 PHY3 PHYCFG0(address=0x0A508E00) to 0x00000039
  // Program USB3 PHY0 LANEPARACTL0(address=0x0A50C000) to 0x01B3C153
  // Program USB3 PHY1 LANEPARACTL0(address=0x0A50C400) to 0x01B3C153
  // Program USB3 PHY2 LANEPARACTL0(address=0x0A50C800) to 0x01B3C153
  // Program USB3 PHY3 LANEPARACTL0(address=0x0A50CC00) to 0x01B3C153
  // Program Interrupt control (address=0x0A58_0118) to 0x00001110 to enable SMI

  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY0_PHYCFG0_HYGX), 0x00, 0x00000039, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY1_PHYCFG0_HYGX), 0x00, 0x00000039, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY2_PHYCFG0_HYGX), 0x00, 0x00000039, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY3_PHYCFG0_HYGX), 0x00, 0x00000039, LocalCfgPtr->StdHeader);

  if (0 == UsbIndex) {  //optmize the usb2.0 EQ of the nbio1 usb phy
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY0_LANEPARACTL0_HYGX), 0x00, 0x0183C153, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY1_LANEPARACTL0_HYGX), 0x00, 0x0183C153, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY2_LANEPARACTL0_HYGX), 0x00, 0x0183C153, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY3_LANEPARACTL0_HYGX), 0x00, 0x0183C153, LocalCfgPtr->StdHeader);
  } else if (1 == UsbIndex) { 
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY0_LANEPARACTL0_HYGX), 0x00, 0x0183C353, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY1_LANEPARACTL0_HYGX), 0x00, 0x0183C353, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY2_LANEPARACTL0_HYGX), 0x00, 0x0183C353, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PHY3_LANEPARACTL0_HYGX), 0x00, 0x0183C353, LocalCfgPtr->StdHeader);
  }

  // Program USB TX EQ
  for (PhyIndex = 0; PhyIndex < 4; PhyIndex++){
    //Program USB 3.1 Gen2 TX EQ
    SmnAddress = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_MAP_PHY_BASE_HYGX + PhyIndex * 0x400 + PHY_EXT_TX_EQ0_OFFSET);
    ...
    SmnAddress = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_MAP_PHY_BASE_HYGX + PhyIndex * 0x400 + PHY_EXT_TX_EQ1_OFFSET);
  ...
  SmnAddress = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY_CR_SELECT_HYGX);
  PhyIndex   = 0;
  ...
  if (RegValue) {
    FchSmnRW (
      DieBusNum,
      USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_INTERRUPT_CONTROL_HYGX), 
      0xffffefff,
      BIT12,
  ...
  )
{
  ...
  // Program GUCTL1(address=0x0A40C11C) to 0x20001908
  // Program GUCTL(address=0x0A40C12C) to 0x0A434802  // clearing GUCTL[15].CMdevAddr to 0
  // Program GUSB3PIPECTL0(address=0x0A40C2C0) to 0x01000202
  // Program GUSB3PIPECTL1(address=0x0A40C2C4) to 0x01000202
  // Program GUSB3PIPECTL2(address=0x0A40C2C8) to 0x01000202
  // Program GUSB3PIPECTL3(address=0x0A40C2CC) to 0x01000202

  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUCTL1_HYGX), 0x00, 0x20001908, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUCTL_HYGX), 0x00, 0x0A434802, LocalCfgPtr->StdHeader);
  ...
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUSB3PIPECTL0_HYGX), 0x00, 0x01020202, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUSB3PIPECTL1_HYGX), 0x00, 0x01020202, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUSB3PIPECTL2_HYGX), 0x00, 0x01020202, LocalCfgPtr->StdHeader);
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUSB3PIPECTL3_HYGX), 0x00, 0x01020202, LocalCfgPtr->StdHeader);
    
  // workaroud
  FchSmnRead (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GSNPSID_HYGX), &Value32, NULL);
  if (Value32 == GSNPSID_REVISION_320A) {
      FchSmnRW (
        DieBusNum,
        USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_GUCTL2_HYGX), 
        0xFFFFEFFF,
  ...
  for (PhyIndex = 0; PhyIndex < 4; PhyIndex++){
    ...
    SmnAddress = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SUP_DIG_LVL_OVER_IN_HYGX);
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, " Polling PhysicalDieId %d Bus (0x%X) PHY %d sram_init_done \n", PhysicalDieId, DieBusNum, PhyNum));
  Reg = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY0_SRAMINITDONE_HYGX + PhyNum * 0x400);
  ...
  if (UpdateFlag) {
    if (FwBuffer == NULL) {
      ...
      FchSmnRW (
        DieBusNum,
        USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY_CR_SELECT_HYGX), 
        0xfffffffc,
        PhyNum,
        LocalCfgPtr->StdHeader
        );
            
      //Step 2.2 Write FW to PCS raw RAM 0A40_0000h ~ 0A40_5FFCh,
      //write data = {16'h0, 16-bit FW data}
      //Loop 6000h/4 = 1800h times
      Buffer = (UINT16 *)FwBuffer;
      for (Index = 0; Index < 0x1800; Index++) {
        Reg = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY_PCS_RAW_RAM_HYGX + Index * 4);
  ...
  Reg = USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PHY0_EXTIDDONE_HYGX + PhyNum * 0x400);
 ...
 * configuration
 *
 * @param[in] SocketId       Socket Id of lite IOD.
 * @param[in] PhysicalDieId  Physical Die Id of lite IOD.
 * @param[in] UsbIndex       USB index 0 or 1.
 * @param[in] DieBusNum      IOHC bus number on current Die.
 * @param[in] FchDataPtr     Fch configuration structure pointer.
  ...
  )
{
  ...
  UINT8                       SocketPresent;
  UINT8                       DujiangDieId;
  UINTN                       DujiangDieCountPerSocket;
  UINTN                       LogicalDieId;
  UINTN                       DieType;

  LocalCfgPtr = (FCH_RESET_DATA_BLOCK *)FchDataPtr;

  UsbDisableMap      = gHygonUsbDisableMap;
  UsbDisableMapCount = sizeof(gHygonUsbDisableMap) / sizeof(HYGON_USB_DISABLE_MAP);

  DujiangDieCountPerSocket = PcdGet8(PcdHygonDujiangDieCountPerProcessor);
  ...
  for (NodeIndex = 0; NodeIndex < UsbDisableMapCount; NodeIndex++) {
    ...
    DujiangDieId  = UsbDisableMap[NodeIndex].PhysicalDieId;

    if(((SocketId != SocketNum) || 
       (PhysicalDieId !=DujiangDieId)) ||
       (SocketNum >= SocketPresent) || 
       (DujiangDieId >= DujiangDieCountPerSocket)
       ) {
      continue;
    }

    Usb2PortDisable = (UINT32)(UsbDisableMap[NodeIndex].UsbDisableMap[UsbIndex] & 0xF);
    Usb3PortDisable = (UINT32)((UsbDisableMap[NodeIndex].UsbDisableMap[UsbIndex] & 0xF0) >> 4);

    FabricTopologyGetPhysicalIodDieInfo ((UINTN)PhysicalDieId, &LogicalDieId, &DieType);
    if ((UINT8)DieType == IOD_DUJIANG){
      if (DieBusNum == ReadSocDieBusNum (SocketId, LogicalDieId, 0)) {
        ...
        DEBUG ((DEBUG_INFO, "FchTSXhciPlatformPortDisable on Bus 0x%x - UsbIndex 0x%x PortNum 0x%x(Usb2PortNum=0x%x, Usb3PortNum=0x%x) - UsbPortDisable 0x%x\n", 
                             DieBusNum, UsbIndex, PortNum, Usb2PortNum,Usb3PortNum, UsbPortDisable));
        FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_CONTROL_HYGX), 0x00, (UINT32)PortNum, LocalCfgPtr->StdHeader);
        FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_DISABLE0_HYGX), 0x00, UsbPortDisable, LocalCfgPtr->StdHeader);
      }
    }
  }
}
  ...
  )
{
  ...
  FchSmnRW (
    DieBusNum,
    USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_CONTROL_HYGX),
    (UINT32)~BIT8,
    (UINT32)BIT8,
    LocalCfgPtr->StdHeader
    );

  //Get SocketId/PhysicalDieId, USB Port, OCPin Array by Pcd
  //*pUsbPortOCPinArray = UINT32[OcPin:PortNum:PhysicalDieId:SocetNum]
  pUsbPortOCPinArray = (UINT8 *)PcdGetPtr (PcdDieNumUsbPortAndOCPinMapHyGx);

  while (*pUsbPortOCPinArray != 0xFF) {
    //Get SocketId/PhysicalDieId/Port/OCPin Num
    UsbPortOCPinArray = *(UINT32 *)pUsbPortOCPinArray;

    SocNumPcd  = (UINT8)((UsbPortOCPinArray >> 0) & 0xFF);
    DieIdPcd   = (UINT8)((UsbPortOCPinArray >> 8) & 0xFF);
    PortNum    = (UINT8)((((UsbPortOCPinArray >> 16) & 0xFF) >> (UsbIndex * 4)) & 0xF);
    OCPin      = (UINT8)((((UsbPortOCPinArray >> 24) & 0xFF) >> (UsbIndex * 4)) & 0xF);

    if ((PortNum == 0xF) || (OCPin == 0xF)) {
      continue;
    }

    if ((SocketId == SocNumPcd) && (PhysicalDieId == DieIdPcd)) {
      //Set OC Pin is low when OC occurs.
      FchSmnRW (
        DieBusNum,
        USB_SPACE_HYGX(PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_CONTROL_HYGX),
 ...
 * Power-On
 *
 * @param[in] Socket         Socket Id of lite IOD.
 * @param[in] PhysicalDieId  Physical Die Id of lite IOD.
 * @param[in] UsbIndex       USB index 0 or 1.
  ...
  )
{
  ...
  if (PcdGet8 (PcdRunEnvironment) == 0) {
  ...
  #if 0
    if(UsbIndex == 0){
      FchSmnRW (DieBusNum, RCC_NBIF2_STRAP1_ADDRESS_HYGX, 0xffffffef, 0x10, LocalCfgPtr->StdHeader);
      FchSmnRW (DieBusNum, NBIF2_SMN_VWR_VCHG_DIS_CTRL_1_ADDRESS_HYGX, 0xffffffbf, 0x40, LocalCfgPtr->StdHeader);
      FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL_HYGX), 0xffffffff, 0x0, LocalCfgPtr->StdHeader);
    }else if(UsbIndex == 1){
      FchSmnRW (DieBusNum, RCC_NBIF3_STRAP1_ADDRESS_HYGX, 0xffffffef, 0x10, LocalCfgPtr->StdHeader);
      FchSmnRW (DieBusNum, NBIF3_SMN_VWR_VCHG_DIS_CTRL_1_ADDRESS_HYGX, 0xffffffbf, 0x40, LocalCfgPtr->StdHeader);
      FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SMU_ASSIST_POWER_STATE_CTL_HYGX), 0xffffffff, 0x0, LocalCfgPtr->StdHeader);
    }
  #endif
    ...
    FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SHADOWED_POWERSTATE_HYGX), 0x00, 0x00, LocalCfgPtr->StdHeader);
    // 1.2.  Wait until Current_power_state is read as D0 (read as 2'b00)
    for (Retry = 0; Retry < MAX_RETRY_NUM; Retry++) {
      FchSmnRead (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_CURRENT_PWR_STS_HYGX), &CurrentPowerState, LocalCfgPtr->StdHeader);
    ...
    FchSmnRW (
      DieBusNum,
      USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), 
    ...
    // a.   To change USB port numbers, program Port Control (0x0A58_0130), and Port Disable Write Once (0x0A58_0128) and/or Port Disable Read Write (0x0A58_012C)
    // b.   To program over current mapping, program Overcurrent Map register (0x0A58_0120 or 0x0A58_0120)
    // c.   To enable SMI/NMI (0x0A58_0118)
    ...
    FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), ~((UINT32)FCH_TS_USB_S5_RESET), 0, LocalCfgPtr->StdHeader);
    ...
    FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), ~((UINT32)(FCH_TS_USB_CONTROLLER_RESET)), 0, LocalCfgPtr->StdHeader);
    // 11. Program internal registers
    FchTSXhciGen31ProgramInternalRegStepTwo (PhysicalDieId, UsbIndex, DieBusNum, FchDataPtr);

    //reset usb PHY
    FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), ~((UINT32)(FCH_TS_USB_PHY_RESET)),
                    (FCH_TS_USB_PHY_RESET), LocalCfgPtr->StdHeader);
    FchStall (200, LocalCfgPtr->StdHeader);
    FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), ~((UINT32)(FCH_TS_USB_PHY_RESET)), 0, LocalCfgPtr->StdHeader); 
  ...
  )
{
  ...
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_RESET_HYGX), ~((UINT32)FCH_TS_USB_S5_RESET), 0, LocalCfgPtr->StdHeader);  // Temp
  ...
  FchSmnRW (
    DieBusNum,
    USB_RSMU_SPACE_HYGX (UsbIndex, FCH_TS_USB_RSMU_PGFSM_CNTL),
    0x00,
  ...
  for (Retry = 0; Retry < MAX_RETRY_NUM; Retry++) {
    ...
    FchSmnRead (
      DieBusNum,
      USB_RSMU_SPACE_HYGX (UsbIndex, FCH_TS_USB_RSMU_PGFSM_RD_DATA), 
      &Pgfsm_RegData,
      LocalCfgPtr->StdHeader
      );
  ...
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PGFSM_OVERRIDE_HYGX), 0x00, 0x33, LocalCfgPtr->StdHeader);
  ...
  )
{
  ...
  for (Retry = 0; Retry < MAX_RETRY_NUM; Retry++) {
    FchSmnRead (
      DieBusNum,
      USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_CURRENT_PWR_STS_HYGX), 
      &CurrentPowerState,
      NULL
      );
  ...
  FchSmnRW (
    DieBusNum,
    USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PGFSM_OVERRIDE_HYGX), 
    0x00,
    0xC8,
    NULL
    );
  ...
  FchSmnRW (
    DieBusNum,
    USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_OCMAPPING0_HYGX), 
    (UINT32)0x0,
    (UINT32) ~0x0,
    NULL
    );
  ...
  )
{
  ...
  if (UsbIndex == 0) {
    FchSmnRW (
      DieBusNum, 
      NBIO_SPACE2 (PhysicalDieId, 0, PCICFG_NBIF2_USB_ADAPTER_ID_W_ADDRESS_HYGX) + 0x4C, 
      0x00, 
      Ssid, 
      NULL
      );
    ...
    FchSmnRW (
      DieBusNum, 
      NBIO_SPACE2 (PhysicalDieId, 0, PCICFG_NBIF3_USB_ADAPTER_ID_W_ADDRESS_HYGX) + 0x4C, 
      0x00, 
      Ssid, 
      NULL
      );
  ...
  )
{
  ...
  FchSmnRead (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_DISABLE0_HYGX), &PortDisableReg, NULL);
  ...
  FchSmnRW (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_PORT_DISABLE0_HYGX), 0x00, PortDisableMap, NULL);
  return TRUE;
}
 ...
 * @param[in] PhysicalDie    Physical Die Id of lite IOD.
 * @param[in] UsbIndex       USB index 0 or 1.
  ...
  )
{
  ...
  if(PcdGetBool (PcdFchUsbPmeOffSupport) == FALSE) {
    return;
  }

  FchSmnRead (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SHADOWED_PMCSR_HYGX), &Data32, NULL);
  DEBUG ((DEBUG_INFO, "TurnOffUsbPme write Bus %x SMN: %x\n", DieBusNum, Data32));

  if(Data32 & FCH_TS_USB_PMCSR_PME_EN) {
    Data32 &= ~FCH_TS_USB_PMCSR_PME_EN;
    FchSmnWrite (DieBusNum, USB_SPACE_HYGX (PhysicalDieId, UsbIndex, FCH_TS_USB_SHADOWED_PMCSR_HYGX), &Data32, NULL);
  }
}
