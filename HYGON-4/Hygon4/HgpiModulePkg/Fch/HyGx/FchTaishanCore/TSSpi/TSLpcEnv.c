  ...
  )
{
  ...
  HGPI_TESTPOINT (TpFchInitEnvLpc, NULL);
  //
  // LPC CFG programming
  //
  //
  ...
  if (LocalCfgPtr->Spi.LpcSsid != NULL ) {
    PciAndThenOr32 ((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG2C, 0x00, LocalCfgPtr->Spi.LpcSsid);
  }
  ...
  if ( LocalCfgPtr->Spi.LpcMsiEnable ) {
    PciAndThenOr32 ((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG78, ~(UINT32)BIT1, BIT1);
  }
}
