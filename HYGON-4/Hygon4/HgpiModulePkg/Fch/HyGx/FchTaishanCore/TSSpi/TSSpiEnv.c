...
SPI_DEVICE_PROFILE      DefaultSpiDeviceTable[] = {
  ...
  { 0x001920C2, 32 << 20, 4096, 33,  104, 168, 432, 0x05, 0x01, 0x1, 0x0040 }, // Macronix_MX25L25673GMX25L25673G3V
  ...
  { 0x001940EF, 32 << 20, 4096, 104, 104, 208, 416, 0x35, 0x01, 0x2, 0x0200 }, // Wnbond_W25Q256 SPI
  { 0x001960EF, 32 << 20, 4096, 104, 104, 208, 416, 0x35, 0x01, 0x2, 0x0200 }, // Wnbond_W25Q256 QPI
  ...
  { 0x0019BB20, 32 << 20, 4096, 108, 108, 216, 432, 0x00, 0x00, 0x1, 0x0000 }, // N25Q256
  ...
  if (!((Opcode == 0x9f) && (!DataFlag))) {
    ...
    do {
      ...
      if (AddressFlag) {
        ...
        if (PcdGetBool (PcdFchDetectSpiAddressMode) && ((ACPIMMIO8 (SPI_BASE + FCH_SPI_MMIO_REG50_ADDR32_CTRL0) & BIT0) == 1)) {
          // 4 byte mode
          ACPIMMIO8 (SPI_BASE + FCH_SPI_MMIO_REG80_FIFO) = ACPIMMIO8 (SPI_BASE + FCH_SPI_MMIO_REG5C_ADDR32_CTRL3);
          for (i = 1; i < 4; i++) {
            ACPIMMIO8 (SPI_BASE + FCH_SPI_MMIO_REG80_FIFO + index) = (UINT8)((Addr >> (16 - (i-1) * 8)) & 0xff);
            index++;
          }
...
{
  BOOLEAN       IsLpcEnable;
  HGPI_TESTPOINT (TpFchInitEnvSpi, NULL);
  
  IsLpcEnable = IsLpcModeEnabled();
  //If config eSPI, disable LPC config
  if(IsLpcEnable == TRUE){
