 ...
 * @file
 *
 * HYGON Hudson-2 POWER BUTTON Dispatcher Driver
 *
 *
    ...
    while (CurrentFchSmmPwrBtnNodePtr->FchPwrBtnNodePtr != NULL) {
      if (CurrentFchSmmPwrBtnNodePtr->Context.Phase == EfiPowerButtonEntry) {
        if (CurrentFchSmmPwrBtnNodePtr->CallBack2Function != NULL) {
          ...
          DEBUG ((
            DEBUG_INFO,
            "[FchSmmDispatcher] PwrBtn SMM handler dispatched (Phase = Entry) Order = 0x%x, \
                  return - %r\n",
            CurrentFchSmmPwrBtnNodePtr->Context.Order,
            Status
            ));
        }
    ...
    while (CurrentFchSmmPwrBtnNodePtr->FchPwrBtnNodePtr != NULL) {
      if (CurrentFchSmmPwrBtnNodePtr->Context.Phase == EfiPowerButtonExit) {
        if (CurrentFchSmmPwrBtnNodePtr->CallBack2Function != NULL) {
          ...
          DEBUG ((
            DEBUG_INFO,
            "[FchSmmDispatcher] PwrBtn SMM handler dispatched (Phase = Exit) Order = 0x%x, \
                  return - %r\n",
            CurrentFchSmmPwrBtnNodePtr->Context.Order,
            Status
            ));
  ...
  )
{
  ...
  *DispatchHandle = &NewFchSmmPwrBtnNodePtr->DispatchHandle;
  ...
  DEBUG ((
    DEBUG_INFO,
    "[FchSmmDispatcher] Registering PwrBtn SMM handler: Phase = 0x%x, Order = 0x%x\n", \
