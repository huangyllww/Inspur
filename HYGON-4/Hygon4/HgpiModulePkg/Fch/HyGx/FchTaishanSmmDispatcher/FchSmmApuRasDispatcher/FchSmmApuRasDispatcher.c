  ...
  )
{
  ...
  *DispatchHandle = &NewFchSmmApuRasNodePtr->DispatchHandle;
  ...
  if (HeadFchSmmApuRasNodePtr->FchApuRasNodePtr == NULL) {
    ...
    while (CurrentFchSmmApuRasNodePtr->FchApuRasNodePtr != NULL) {
      if (NewFchSmmApuRasNodePtr->Context.Order <= CurrentFchSmmApuRasNodePtr->Context.Order &&
          ...
          CurrentFchSmmApuRasNodePtr->Context.LogicalDie == NewFchSmmApuRasNodePtr->Context.LogicalDie &&
  ...
  RbBusNum = (UINT8)FabricTopologyGetHostBridgeBusBase (ApuRasRegisterContext->Socket, ApuRasRegisterContext->LogicalDie, ApuRasRegisterContext->Rb);

  FchSmnRW ((UINT32)RbBusNum, NBIO_SPACE2 (ApuRasRegisterContext->IohubPhysicalDie, ApuRasRegisterContext->Rb, RAS_GLOBAL_STATUS_LO_HYGX), 0xfffffeff, BIT8, NULL); // HWSMI_STS
  FchSmnRW ((UINT32)RbBusNum, NBIO_SPACE2 (ApuRasRegisterContext->IohubPhysicalDie, ApuRasRegisterContext->Rb, MISC_RAS_CONTROL_HYGX), 0xffffbfff, BIT14, NULL);    // PCIE_SMI_EN
  FchSmnRW ((UINT32)RbBusNum, NBIO_SPACE2 (ApuRasRegisterContext->IohubPhysicalDie, ApuRasRegisterContext->Rb, IOHC_INTERRUPT_EOI_HYGX), 0xfffffffe, BIT0, NULL);   // SMI_EOI
  ...
  if (!EFI_ERROR (Status)) {
    ...
    for (SocketId = 0; SocketId < NumberOfSockets; SocketId++) {
      ...
      for (LogicalDieId = 0; LogicalDieId < NumberOfLogicalDies; LogicalDieId++) {
        FabricTopology->GetDieInfo (FabricTopology, SocketId, LogicalDieId, &NumberOfRootBridges, NULL, NULL);

        IohubPhysicalDieId = FabricTopologyGetIohubPhysicalDieId (LogicalDieId);

        for (RbId = 0; RbId < NumberOfRootBridges; RbId++) {
          FabricTopology->GetRootBridgeInfo (FabricTopology, SocketId, LogicalDieId, RbId, NULL, &RbBusNum, NULL);
          FchSmnRW ((UINT32)RbBusNum, NBIO_SPACE2 (IohubPhysicalDieId, RbId, IOHC_INTERRUPT_EOI_HYGX), 0xfffffffe, BIT0, NULL);
          FchSmnRW ((UINT32)RbBusNum, NBIO_SPACE2 (IohubPhysicalDieId, RbId, RAS_GLOBAL_STATUS_LO_HYGX), 0xfffffeff, BIT8, NULL);
        }
      }
    }
  }
