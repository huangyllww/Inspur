...
{
  HYGON_SMBUS_PPI_PRIVATE  *PeiPrivate;
  HYGON_SMBUS_PPI_PRIVATE  Private;
  EFI_STATUS               Status;
  EFI_PEI_CPU_IO_PPI       *CpuIo;
  UINT16                   LoopCount;
  UINT8                    retryCount;
  ...
  PeiPrivate = (HYGON_SMBUS_PPI_PRIVATE *)(This);
  ...
  for (retryCount = 0; retryCount < TRY_TIMES; retryCount++) {
    ...
    Private.IoWrite8 (
              &Private,
              SMB_IOREG02,
              (UINT8)(SmbusOperation + BIT6 + ((PecCheck) ? (StsReg | 0xA0) : 0))
              );  // [6] = Start
...
/*********************************************************************************
 * Name: HygonSmbusPeiInit
 *
 * Description
 *   Entry point of the HYGON SMBUS PEIM
...
{
  EFI_STATUS                   Status;
  HYGON_PEI_FCH_SMBUS_PRIVATE  *SmbusPrivate;
  ...
  Status = (*PeiServices)->AllocatePool (
                             (CONST EFI_PEI_SERVICES **)PeiServices,
                             sizeof (HYGON_PEI_FCH_SMBUS_PRIVATE),
                             (VOID **)&SmbusPrivate
                             );
  ...
  // Enable ACPI MMIO range (FED8_0000h-FED8_1FFFh)
  Value16 = ReadPmio16 (
              (CONST EFI_PEI_SERVICES **)PeiServices,
              CpuIo,
              FCH_ML_PMIOA_REG04
  ...
  // Enable SMBus and ASF IO decoding. SMBUS and ASF IO ranges are defined
  // in Smbus0AsfIobase
  Value16 = ReadPmio16 (
              (CONST EFI_PEI_SERVICES **)PeiServices,
              CpuIo,
  ...
  //
  // Set Smbus0 timing. Frequency = 66 MHz/(SMBusTiming * 4)
  //
  FchSmbusIoWrite8 (&SmbusPrivate->SmbusPpi0, SMB_IOREG0E, PcdGet8 (PcdSmbusTiming0));

  //
  // Smbus debounce time is too long to support 1M frequency.Update smbus debounce time value,
  // for default value is 'h1f same as previous design, for 1M speed, it is recommened config below two vaule with `h0a.
  //
  if (PcdGet8 (PcdSmbusTiming0) <= SMBUS_TIMING_1M_DIVISOR) {
    FchSmbusIoWrite8 (&SmbusPrivate->SmbusPpi0, SMB_IOREG18, PcdGet8 (PcdSmbus0SdaDebounceCount1M));
    FchSmbusIoWrite8 (&SmbusPrivate->SmbusPpi0, SMB_IOREG19, PcdGet8 (PcdSmbus0SclDebounceCount1M));
    FchSmbusIoWrite8 (&SmbusPrivate->SmbusPpi0, SMB_IOREG1A, PcdGet8 (PcdSlaveTxHoldCount1M));
  }
