...
/********************************************************************************
 * Name: InstallHygonSmbusLightEntryPoint
 *
 * Description
 *   Smbus Entrypoint
 *
...
EFI_STATUS
EFIAPI
InstallHygonSmbusLightEntryPoint (
  ...
  )
{
  ...
  UINT16                       SmbusIoBase;
  UINT8                        SmbusTiming;
  UINT8                        SMBusSdaDebounceCount;
  UINT8                        SMBusSclDebounceCount;
  UINT8                        SMBusSlaveTxHoldCount;
  ...
  // Set Smbus0 Clock Speed
  SmbusTiming = PcdGet8 (PcdSmbusTiming0);
  LibHygonMemWrite (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_ML_PMIOA_REG0E), &SmbusTiming, NULL);

  if (SmbusTiming <= 16) {
    SMBusSdaDebounceCount = PcdGet8 (PcdSmbus0SdaDebounceCount1M);
    SMBusSclDebounceCount = PcdGet8 (PcdSmbus0SclDebounceCount1M);
    SMBusSlaveTxHoldCount = PcdGet8 (PcdSlaveTxHoldCount1M);
    LibHygonMemWrite (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_ML_PMIOA_REG18), &SMBusSdaDebounceCount, NULL);
    LibHygonMemWrite (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_ML_PMIOA_REG19), &SMBusSclDebounceCount, NULL);
    LibHygonMemWrite (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_ML_PMIOA_REG1A), &SMBusSlaveTxHoldCount, NULL);
  }

  // read SMBus base address from HW
  LibHygonMemRead (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_ML_PMIOA_REG00), &SmbusIoBase, NULL);
  ...
  )
{
  ...
  LibHygonIoRead (AccessWidth8, Private->SmbusIoBase + Offset, &FchSmbusData8, NULL);
...
{
  LibHygonIoWrite (AccessWidth8, Private->SmbusIoBase + Offset, &Data, NULL);
}
  ...
  )
{
  ...
  LoopCount = 500;
  while (LoopCount) {
    LibHygonIoRead (AccessWidth8, Smbus->SmbusIoBase + SMB_IOREG08, &dbSmbusTempVar, NULL);
  ...
  dbSmbusTempVar |= BIT4;
  LibHygonIoWrite (AccessWidth8, Smbus->SmbusIoBase + SMB_IOREG08, &dbSmbusTempVar, NULL);
  ...
  while (retryCount--) {
    ...
    do {
      dbSmbusTempVar = 0x1f;
      LibHygonIoWrite (AccessWidth8, Smbus->SmbusIoBase, &dbSmbusTempVar, NULL);
      LibHygonIoRead (AccessWidth8, 0xed, &bValue, NULL);
      LibHygonIoRead (AccessWidth8, 0xed, &bValue, NULL);
      LibHygonIoRead (AccessWidth8, Smbus->SmbusIoBase, &dbSmbusTempVar, NULL);
    ...
    dbSmbusTempVar = BIT1;
    LibHygonIoWrite (AccessWidth8, Smbus->SmbusIoBase + 2, &dbSmbusTempVar, NULL);
  }
