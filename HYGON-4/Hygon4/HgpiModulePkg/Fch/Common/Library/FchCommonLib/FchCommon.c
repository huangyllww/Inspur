 ...
 *
 *
 * @param[in] StdHeader - Hygon Configuration Parameters.
 *
 */
UINT8
  ...
  )
{
  ...
  FchChipSetRevision = PciRead8 ((SMBUS_BUS_DEV_FUN << 12) + FCH_CFG_REG08);
  ...
  )
{
  ...
  SocId = PciRead32 ((HYGON_D0F0 << 12) + FCH_CFG_REG00);

  return SocId;
}

/**
 * CheckHygonSoc - Check if SAT Soc
 *
 *
 *
 */
...
{
  if ((ReadSocType () & 0x0000FFFF) == HYGON_VID) {
    return TRUE;
  } else {
    return FALSE;
  }
  ...
  )
{
  ...
  DieBusNum = (UINT32)FabricTopologyGetHostBridgeBusBase (SocketId, LogicalDieId, RbId);
  return DieBusNum;
}
