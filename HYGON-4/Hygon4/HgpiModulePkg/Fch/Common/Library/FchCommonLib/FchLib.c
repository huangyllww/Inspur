  ...
  )
{
  ...
  LibHygonMemRead (AccessWidth16, (UINT64)(ACPI_MMIO_BASE + PMIO_BASE + FCH_PMIOA_REG64), &timerAddr, StdHeader);
  if ( timerAddr == 0 ) {
    uSec = uSec / 2;
    while ( uSec != 0 ) {
      LibHygonIoRead (AccessWidth8, FCHOEM_IO_DELAY_PORT, (UINT8 *)(&startTime), StdHeader);
      uSec--;
    }
  } else {
    LibHygonIoRead (AccessWidth32, timerAddr, &startTime, StdHeader);
    for ( ; ; ) {
      LibHygonIoRead (AccessWidth32, timerAddr, &elapsedTime, StdHeader);
  ...
  )
{
  ...
  if ( timerAddr == 0 ) {
    uSec = uSec / 2;
    while ( uSec != 0 ) {
      LibHygonIoRead (AccessWidth8, FCHOEM_IO_DELAY_PORT, (UINT8 *)&elapsedTime, StdHeader);
      uSec--;
    }
  } else {
    LibHygonIoRead (AccessWidth32, timerAddr, &startTime, StdHeader);
  ...
  )
{
  ...
  PciRstValue = 0x06;
  LibHygonIoWrite (AccessWidth8, FCH_PCIRST_BASE_IO, &PciRstValue, StdHeader);
}
...
{
  LibHygonIoWrite (AccessWidth8, FCHOEM_OUTPUT_DEBUG_PORT, &pcode, StdHeader);
  return;
}
...
{
  LibHygonIoWrite (AccessWidth32, 0x1080, &pcode, StdHeader);
  return;
}
  ...
  )
{
  ...
  *RomSigPtr = 0;
  ...
  do {
    ...
    LibHygonMemRead (AccessWidth32, (UINT64)RomPtr, &RomSig, StdHeader);
  ...
  )
{
  ...
  LibHygonIoWrite (AccessWidth32, ALINK_ACCESS_INDEX, &Index, StdHeader);
  LibHygonIoRead (AccessWidth32, ALINK_ACCESS_DATA, &Data, StdHeader);
  // Clear Index
  Index = 0;
  LibHygonIoWrite (AccessWidth32, ALINK_ACCESS_INDEX, &Index, StdHeader);
  return Data;
...
{
  LibHygonIoWrite (AccessWidth32, ALINK_ACCESS_INDEX, &Index, StdHeader);
  LibHygonIoWrite (AccessWidth32, ALINK_ACCESS_DATA, &Data, StdHeader);
  // Clear Index
  Index = 0;
  LibHygonIoWrite (AccessWidth32, ALINK_ACCESS_INDEX, &Index, StdHeader);
}
  ...
  )
{
  ...
  OpFlag = 1 << (OpFlag - 1);
  for (i = 0; i < OpFlag; i++) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD6, &Address, StdHeader);
    Address++;
    LibHygonIoRead (AccessWidth8, FCH_IOMAP_REGCD7, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  for (i = 0; i < OpFlag; i++) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD6, &Address, StdHeader);
    Address++;
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD7, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  OpFlag = 1 << (OpFlag - 1);
  for ( i = 0; i < OpFlag; i++ ) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD0, &Address, StdHeader);
    Address++;
    LibHygonIoRead (AccessWidth8, FCH_IOMAP_REGCD1, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  for ( i = 0; i < OpFlag; i++ ) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD0, &Address, StdHeader);
    Address++;
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD1, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  OpFlag = 1 << (OpFlag - 1);
  for (i = 0; i < OpFlag; i++) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD4, &Address, StdHeader);
    Address++;
    LibHygonIoRead (AccessWidth8, FCH_IOMAP_REGCD5, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  for (i = 0; i < OpFlag; i++) {
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD4, &Address, StdHeader);
    Address++;
    LibHygonIoWrite (AccessWidth8, FCH_IOMAP_REGCD5, (UINT8 *)Value + i, StdHeader);
  }
}
  ...
  )
{
  ...
  LibHygonIoRead (AccessWidth8, 0xB0, &SwSmiValue, StdHeader);
  ...
  )
{
  ...
  LibHygonMemRead (AccessWidth32, (UINT64)(ACPI_MMIO_BASE + MISC_BASE + FCH_MISC_REG80), &Value, NULL);
  if (Value & BIT4) {
    return TRUE;
  }
  return FALSE;
