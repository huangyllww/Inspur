  ...
  )
{
  ...
  SataSgpio32              = PcdGet32 (PcdSataSgpioHyEx);
  SataMultiDiePortShutDown = (UINT8 *)PcdGetPtr (PcdSataMultiDiePortShutDownHyEx);
  SataMultiDiePortESP      = (UINT8 *)PcdGetPtr (PcdSataMultiDiePortESPHyEx);
  SataMultiDieDevSlp       = PcdGet64 (PcdSataMultiDieDevSlpHyEx);
  ...
  for (SocketIndex = 1, LogicalSocket = 1; SocketIndex < NumberOfSockets; SocketIndex++, LogicalSocket++) {
    ...
    FchDataPtr->HwAcpi.FchAcpiMmioBase = (UINT32) FchMfData->FchAcpiMmioBase[LogicalSocket][0];
    ...
    for (RbIndex = 0; RbIndex < RootBridgesNumberPerIoDie; RbIndex++) {
      ...
      //SATA
      FchTSSecondaryFchInitSataDxe (0, (UINT8)RbIndex, NbioBusNum, FchDataPtr);

      SataMultiDiePortShutDown++;
      SataMultiDiePortESP++;
    }

    NbioBusNum = (UINT8)ReadSocDieBusNum (SocketIndex, 0, 0);
    // Ethernet
    FchTSSecondaryFchInitXgbeDxe (0, NbioBusNum, FchDataPtr);
  ...
  )
{
  ...
  //
  LogicalSocket = 1;
  for (SocketIndex = 1; SocketIndex < NumberOfSockets; SocketIndex++) {
    //
    // Update local Data Structure for each Die
    //

    FchDataPtr->HwAcpi.FchAcpiMmioBase = (UINT32) FchMfInit->FchMfData.FchAcpiMmioBase[LogicalSocket][0];
    ...
    for (RbIndex = 0; RbIndex < RootBridgesNumberPerIoDie; RbIndex++) {
      ...
      FchTSSecondaryFchInitSataRtb (0, (UINT8)RbIndex, NbioBusNum, FchDataPtr);
