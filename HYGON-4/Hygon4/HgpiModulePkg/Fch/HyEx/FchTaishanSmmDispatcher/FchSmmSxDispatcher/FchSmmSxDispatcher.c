 ...
 * @file
 *
 * HYGON FCH SMM Sleep Type Dispatcher Driver
 *
 *
...
#include "Filecode.h"
#include <Library/ReportStatusCodeLib.h>                                               // byo230831 +
...
// byo230831 + >>
  if(SleepTypeValue == SxS3){
    DEBUG ((DEBUG_INFO, "IsS3\n"));
    ...
    while (CurrentFchSmmSxNodePtr->FchSxNodePtr != NULL) {
      if (CurrentFchSmmSxNodePtr->Context.Type == SleepTypeValue &&
        ...
        if (CurrentFchSmmSxNodePtr->CallBack2Function != NULL) {
          ...
          DEBUG ((
            DEBUG_INFO,
            "[FchSmmDispatcher] Sx SMM handler dispatched (Phase = SxEntry) SleepType = 0x%x, Order = 0x%x, return - %r\n", \
...
// byo230831 + >>
      if(IsS3){
        DEBUG((DEBUG_INFO, "GoToS3\n"));
        REPORT_STATUS_CODE(EFI_PROGRESS_CODE, PcdGet32(PcdProgressCodeS3SuspendEnd));
      }
      AsmWbinvd();
// byo230831 + <<	  

      // SAT to send MSG to SMU instead of write PmCnt directly
      ...
      while (CurrentFchSmmSxNodePtr->FchSxNodePtr != NULL) {
        if (CurrentFchSmmSxNodePtr->Context.Phase == SxExit) {
          if (CurrentFchSmmSxNodePtr->CallBack2Function != NULL) {
            ...
            DEBUG ((
              DEBUG_INFO,
              "[FchSmmDispatcher] Sx SMM handler dispatched (Phase = SxExit) SleepType = 0x%x, Order = 0x%x, return - %r\n", \
              CurrentFchSmmSxNodePtr->Context.Type,
              CurrentFchSmmSxNodePtr->Context.Order,
              Status
              ));
  ...
  )
{
  ...
  *DispatchHandle = &NewFchSmmSxNodePtr->DispatchHandle;
  ...
  DEBUG ((
    DEBUG_INFO,
    "[FchSmmDispatcher] Registering Sx SMM handler: SleepType = 0x%x, Phase = 0x%x, Order = 0x%x\n", \
    RegisterContext->Type,
    RegisterContext->Phase,
