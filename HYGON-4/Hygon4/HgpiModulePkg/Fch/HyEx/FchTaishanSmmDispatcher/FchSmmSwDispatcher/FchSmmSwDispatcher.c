 ...
 * @file
 *
 * HYGON Hudson-2 Child SMI Dispatcher Driver
 *
 *
  ...
  )
{
  ...
  if (HeadFchSmmSwNodePtr->FchSwNodePtr == NULL) {
    ...
    while (CurrentFchSmmSwNodePtr->FchSwNodePtr != NULL) {
      if ((UINT8)CurrentFchSmmSwNodePtr->Context.HygonSwValue == (UINT8)SwSmiValue) {
        if (CurrentFchSmmSwNodePtr->CallBack2Function != NULL) {
          ...
          //       CurrentFchSmmSwNodePtr->Context.HygonSwValue, CurrentFchSmmSwNodePtr->Context.Order, Status));
        }
  ...
  if (HeadFchSmmSwNodePtr->FchSwNodePtr == NULL) {
    ...
    while (CurrentFchSmmSwNodePtr->FchSwNodePtr != NULL) {
      if (NewFchSmmSwNodePtr->Context.Order <= CurrentFchSmmSwNodePtr->Context.Order &&
          CurrentFchSmmSwNodePtr->Context.HygonSwValue == NewFchSmmSwNodePtr->Context.HygonSwValue) {
  ...
  )
{
  ...
  while ((CurrentFchSmmSwNodePtr->FchSwNodePtr != NULL) && !Result) {
    Result = (CurrentFchSmmSwNodePtr->Context.HygonSwValue == Value);
    CurrentFchSmmSwNodePtr = CurrentFchSmmSwNodePtr->FchSwNodePtr;
  }
  ...
  if (RegisterContext->SwSmiInputValue == (UINTN)- 1) {
    for (Index = 1u; Index < MAX_SW_SMI_VALUE; Index++) {
      if (!IsSwSmiValueUsed (Index)) {
        FchRegisterContext.HygonSwValue  = Index;
    ...
    FchRegisterContext.HygonSwValue = RegisterContext->SwSmiInputValue;
  }
