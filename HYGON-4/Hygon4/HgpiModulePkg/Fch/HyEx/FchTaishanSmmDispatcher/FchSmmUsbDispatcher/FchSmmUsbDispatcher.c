...
VOID
HyExFchSmmUsbSmiEnable (
  VOID
  );

VOID
HyExFchSmmUsbClearSmiSts (
  VOID
  );
  ...
  for (SocketIndex = 0 ; SocketIndex < SocketPresent; SocketIndex++) {
    for (XhciPerSocketIndex = 0 ; XhciPerSocketIndex < MAX_XHCI_CONTROLLERS_PER_SATORI; XhciPerSocketIndex++) {
      ...
      if ((FchUsbSmiSysInfo[XhciIndex].RbBusNum != 0xFF) && FchUsbSmiSysInfo[XhciIndex].XhciEnable) {
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, NBIO_SPACE2 (0, XhciPerSocketIndex, IOHC_INTERRUPT_EOI_HYEX), 0xfffffffe, BIT0, NULL);
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, NBIO_SPACE2 (0, XhciPerSocketIndex, RAS_GLOBAL_STATUS_LO_HYEX), 0x00, BIT8, NULL);
      }
    }
  }
...
{
  UINT32    SocketIndex,XhciPerSocketIndex;
  UINTN     SocketPresent;
  UINT32    XhciIndex;
  //
  ...
  for (SocketIndex = 0 ; SocketIndex < SocketPresent; SocketIndex++) {
    for (XhciPerSocketIndex = 0 ; XhciPerSocketIndex < MAX_XHCI_CONTROLLERS_PER_SATORI; XhciPerSocketIndex++) {
      ...
      if ((FchUsbSmiSysInfo[XhciIndex].RbBusNum != 0xFF) && FchUsbSmiSysInfo[XhciIndex].XhciEnable) {
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, NBIO_SPACE2 (0, XhciPerSocketIndex, RAS_GLOBAL_STATUS_LO_HYEX), 0x00, BIT8, NULL);
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, NBIO_SPACE2 (0, XhciPerSocketIndex, MISC_RAS_CONTROL_HYEX), 0xffffbfff, BIT14, NULL);
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, NBIO_SPACE2 (0, XhciPerSocketIndex, IOHC_INTERRUPT_EOI_HYEX), 0xfffffffe, BIT0, NULL);
        FchSmnRW (FchUsbSmiSysInfo[XhciIndex].RbBusNum, USB_SPACE_HYEX (XhciPerSocketIndex, XHCI_INTERRUPT_CONTROL_HYEX), 0xfffff000, BIT8, NULL);
      }
    }
  }
