  ...
  )
{
  ...
  base  = PciRead32 ((LPC_BUS_DEV_FUN << 12) + 0xA0);
  base &= 0xFFFFFFC0;
  base += 0x00010000;
  ...
  )
{
   RwMem (EspiBase + 0x00, AccessWidth32, ~(UINT32)BIT3, 0x00);
   wait4stClear (EspiBase);
   RwMem (EspiBase + 0x0C, AccessWidth32, 0x0, 0x2203);
   RwMem (EspiBase + 0x00, AccessWidth32, 0xFFFFFFFA, 0x5);     // [2:0] DNCMD_TYPE PC Channel Down Stream
  ...
  )
{
  ...
  ESPI_SL20_SLAVE_PC_CAPCFG       SlavePCCapCfg;
  ...
  RwMem (EspiBase + 0x68, AccessWidth32, 0x3FFFFFFF, BIT30|BIT31); //Set ESPI_Alert and CRC_check_en
  //
...
{
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x4A, AccessWidth8, 0x00, 0x02);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x68, AccessWidth8, 0x00, 0x02);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x69, AccessWidth8, 0x00, 0x02);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x6A, AccessWidth8, 0x00, 0x02);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x6B, AccessWidth8, 0x00, 0x02);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x6C, AccessWidth8, 0x00, 0x01);
  RwMem (ACPI_MMIO_BASE + IOMUX_BASE + 0x6D, AccessWidth8, 0x00, 0x02);

  // To enable dedicated Alert pin. (ESPI_ALERT)
  // D14F3x78[3,2] = 00b;
  // D14F3x78[9,  LDRQ0_PU_EN] = 1b; Reset 0;
  // D14F3x78[10, LDRQ0_PD_EN] = 0b; Reset 1;
  PciAndThenOr32 (((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG78), 0xFFFFF9F3, 0x00000200);
}

/**
 * EspiDisableLpcDecoding - Config ESPI controller
 *
 *
 */
VOID
...
{
  PciAndThenOr32 (((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG44), 0x00, 0x0);          // 60h/64h, 6xxh
  PciAndThenOr32 (((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG48), 0x00, 0xFF00);       // 2E/2F, 4E/4F
}
  ...
  )
{
  ...
  if ( Enable ) {
    PciAndThenOr32 (((LPC_BUS_DEV_FUN << 12) + FCH_LPC_REG48), ~(UINT32)BIT29, 0x00);          // disable LPC 60h/64h
  ...
  )
{
  ...
  RwMem (EspiBase + 0x44, AccessWidth32, 0x00, 0x004E004F); 
  RwMem (EspiBase + 0x48, AccessWidth32, 0x00, 0x0CA00600); 
  RwMem (EspiBase + 0x4C, AccessWidth32, 0x00, 0x10FF1010);  
  RwMem (EspiBase + 0x40, AccessWidth32, 0xFFFFF0FF, 0x00001F07);                         // Enable IO range
  RwMem (EspiBase + 0x40, AccessWidth32, ~(UINT32)BIT1, BIT1);                            // 60h/64h
