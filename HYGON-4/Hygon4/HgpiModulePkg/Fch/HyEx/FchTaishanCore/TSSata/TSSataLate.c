  ...
  )
{
  ...
  HGPI_TESTPOINT (TpFchInitLateSata, NULL);
  //
  ...
  for(SataIndex = 0; SataIndex < RootBridge; SataIndex++) {
    if ((LocalCfgPtr->Sata.SataMode.SataEnable & BIT (SataIndex)) == 0) {
      continue;
    }

    DieBusNum = ReadSocDieBusNum (0, LogicalDieId, SataIndex);
    ...
    if ((LocalCfgPtr->Sata.SataClass == SataAhci7804) || (LocalCfgPtr->Sata.SataClass == SataAhci)) {
      FchInitLateSataAhci (PhysicalDieId, SataIndex, DieBusNum, LocalCfgPtr);
    }

    if ( LocalCfgPtr->Sata.SataClass == SataRaid) {
      FchInitLateSataRaid (PhysicalDieId, SataIndex, DieBusNum, LocalCfgPtr);
    }

    FchInitLateProgramSataRegs (PhysicalDieId, SataIndex, DieBusNum, LocalCfgPtr);
