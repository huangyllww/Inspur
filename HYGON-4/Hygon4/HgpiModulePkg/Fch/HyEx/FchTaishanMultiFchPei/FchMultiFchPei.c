...
{
  UINT8                                   LogicalSocket;
  UINT8                                   FchBusNum, NbioBusNum;
  UINT32                                  SataEnable32;
  UINT32                                  IOBaseRegValOld;
  UINT32                                  IOLimitRegValOld;
  UINT32                                  IOBaseRegValNew;
  UINT32                                  IOLimitRegValNew;
  EFI_STATUS                              Status;
  ...
  DEBUG ((DEBUG_INFO, "MultiFchPeiHyExInit Entry \n"));
  ...
  FabricTopologyServicesPpi->GetSystemInfo (&NumberOfSockets, &TotalNumberOfIoDie, &TotalNumberRootBridges);

  DEBUG ((
    DEBUG_INFO,
    "MultiFchPeiHyExInit NumberOfSockets=%d, TotalNumberOfIoDie=%d, TotalNumberRootBridges=%d\n",
    (UINT32)NumberOfSockets,
    (UINT32)TotalNumberOfIoDie,
    (UINT32)TotalNumberRootBridges
    ));
  ...
  //
  SataEnable32 = PcdGet32 (PcdSataEnableHyEx);

  //
  // Check Socket# and Rb#
  //
  for (SocketIndex = 1, LogicalSocket = 1; SocketIndex < NumberOfSockets; SocketIndex++, LogicalSocket++) {
    // enable IO 0xCxx on the Socket N Die N
    IOBaseRegValOld = CddFabricRegisterAccRead (
                          0,                             // Socket0
                          0,                             // Cdd0
                          DF_X86IOBASE_FUNC,             // 0x00
                          DF_X86IOBASE_OFFSET,           // 0x220
                          FABRIC_REG_ACC_BC              // BC
                          );
    IOBaseRegValNew = 0x00000003;
    ...
    IOLimitRegValOld = CddFabricRegisterAccRead (
                          0,                             // Socket0
                          0,                             // Cdd0
                          DF_X86IOBASE_FUNC,             // 0x00
                          DF_X86IOBASE_OFFSET + 4,       // 0x224
                          FABRIC_REG_ACC_BC              // BC
                          );
    IOLimitRegValNew = IOMS0_FABRIC_ID + (SocketIndex << FABRIC_ID_SOCKET_SHIFT_HYEX) + (0 << FABRIC_ID_DIE_SHIFT);
    CddFabricRegisterAccWrite (
      0,                                                 // Socket0
      0,                                                 // Cdd0
      DF_X86IOBASE_FUNC,                                 // 0x00
    ...
    FchBusNum = (UINT8)ReadSocDieBusNum (SocketIndex, 0, 0);
    ...
    DEBUG ((DEBUG_INFO, "MultiFchPeiHyExInit Socket %d, IOBaseRegValOld=0x%X, IOBaseRegValOld=0x%X\n", SocketIndex, IOBaseRegValOld, IOBaseRegValOld));

    // skip socket 0
    SataEnable32 = SataEnable32 >> 4;
    FchResetDataPtr->FchReset.SataEnable = (UINT8)(SataEnable32 & 0x0F);
    for (RbIndex = 0; RbIndex < RootBridgesNumberPerIoDie; RbIndex++) {
      //
      // Update local Data Structure for each NBIO
      //

      NbioBusNum = (UINT8)ReadSocDieBusNum (SocketIndex, 0, RbIndex);
      // USB
      if (PcdGetBool (PcdHygonUsbSupport) == TRUE) {
        FchTSSecondaryFchInitUsbPei (SocketIndex, 0, RbIndex, NbioBusNum, FchResetDataPtr);
      }

      // SATA
      FchTSSecondaryFchInitSataPei (0, RbIndex, NbioBusNum, FchResetDataPtr);
    }

    // Update Globle Data
    FchMfResetData->FchAcpiMmioBase[LogicalSocket][0] = (UINT64)FchResetDataPtr->Misc2.FchAcpiMmioBase;
  ...
  DEBUG ((DEBUG_INFO, "MultiFchPeiHyExInit Exit \n"));
  ...
  )
{
  ...
  // For Boundary Scan
  FchSmnRW (DieBusNum32, USB_SPACE_HYEX(UsbIndex, FCH_TS_USB_SPARE2_REG_HYEX), ~ (UINT32) BIT31, BIT31, NULL);
  //Raise USB Axi Clock to 400M
  FchSmnRW (DieBusNum32, USB_SPACE_HYEX(UsbIndex, FCH_SMN_MISC_BASE_HYEX + FCH_MISC_REG40), ~ (UINT32) BIT2, 0, NULL);

  if (UsbIndex >= 2) {
    //
    //Disable USB controller on IOMS2/IOMS3
    //
    IdsLibDebugPrint (MAIN_FLOW, "Disable Socket %d Rb %d XHCI controller \n", Socket, UsbIndex);
      
    FchSmnRW (
      DieBusNum32,
      NBIO_SPACE2(PhysicalDieId, UsbIndex, NBIF0_USB_STRAP0_ADDRESS_HYEX),
      ~(UINT32) BIT28,
      0,
      NULL
      );
    ...
    FchSmnRW (
      ...
      ~ ((UINT32) (FCH_TS_USB_CONTROLLER_RESET + FCH_TS_USB_PHY_RESET + FCH_TS_USB_S5_RESET)), 
      (FCH_TS_USB_CONTROLLER_RESET + FCH_TS_USB_PHY_RESET + FCH_TS_USB_S5_RESET),
      NULL
      );
  ...
  //
  // Enable USB controller according to PCD
  //

  if (((UsbIndex == 0) && (PcdGetBool (PcdXhci2EnableHyEx) == TRUE)) ||
      ((UsbIndex == 1) && (PcdGetBool (PcdXhci3EnableHyEx) == TRUE))) {
      
    IdsLibDebugPrint (MAIN_FLOW, "Enable Socket %d Rb %d XHCI controller \n", Socket, UsbIndex);     
    FchSmnRW (DieBusNum32, NBIO_SPACE2 (PhysicalDieId, UsbIndex, NBIF0_USB_STRAP0_ADDRESS_HYEX), ~(UINT32)BIT28, BIT28, NULL);
  } else if((UsbIndex == 0) && (PcdGetBool (PcdXhci2EnableHyEx) == FALSE)){  //Disable XHCI2 according to PCD  
    FchSmnRW (DieBusNum32, NBIO_SPACE2 (PhysicalDieId, UsbIndex, NBIF0_USB_STRAP0_ADDRESS_HYEX), ~(UINT32)BIT28, 0, NULL);
  } else if((UsbIndex == 1) && (PcdGetBool (PcdXhci3EnableHyEx) == FALSE)){  //Disable XHCI3 according to PCD
    FchSmnRW (DieBusNum32, NBIO_SPACE2 (PhysicalDieId, UsbIndex, NBIF0_USB_STRAP0_ADDRESS_HYEX), ~(UINT32)BIT28, 0, NULL);
  } 
  ...
  )
{
  ...
  FchAcpiMmio = LocalCfgPtr->Misc2.FchAcpiMmioBase;
  //
  //Step 4 - If it is slave die (SL2/SL2r2) or master die in socket1 package: (100M refclk)
