...
EFI_STATUS
EFIAPI
HygonLegacyUartTrapSmiCallback (
  IN       EFI_HANDLE   DispatchHandle,
  IN       CONST VOID   *Context        OPTIONAL,
  IN OUT   VOID         *CommBuffer     OPTIONAL,
  IN OUT   UINTN        *CommBufferSize OPTIONAL
  )
{
  ...
  DEBUG ((DEBUG_ERROR, "HygonLegacyUartTrapSmiCallback for IO Addr 0x%X \n", IoAddress));
...
{
  EFI_STATUS                          Status;
  FCH_SMM_IO_TRAP_DISPATCH2_PROTOCOL  *HygonIoTrapDispatch;
  ...
  ASSERT (!EFI_ERROR (Status));
    ...
    Status = HygonIoTrapDispatch->Register (
                                    HygonIoTrapDispatch,
                                    HygonIoTrapSmiCallback,
