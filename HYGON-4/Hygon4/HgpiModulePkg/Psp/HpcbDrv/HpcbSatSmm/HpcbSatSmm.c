...
#include <Filecode.h>
#include <Library/HygonPspBaseLib.h>                                  // byo230831 +

#define FILECODE  PSP_HPCB_HPCBSATSMM_HPCBSATSMM_FILECODE

extern EFI_GUID  gHygonHpcbSmmServiceProtocolGuid;

STATIC HYGON_HPCB_SERVICE_PROTOCOL  mHpcbSmmServiceProtocol = {
...
extern UINT64 mHpcbEntryAddress;
extern UINT32 mHpcbEntrySize;
  ...
  )
{
  ...
  IDS_HDT_CONSOLE_PSP_TRACE("mHpcbReload, mHpcbInstance:%d\n", mHpcbInstance);   // byo231107 -

  HpcbEntryExist = BIOSEntryInfo(BIOS_HPCB_INFO, INSTANCE_IGNORED, &TypeAttrib, &HpcbEntryAddress, &HpcbEntrySize, &EntryDest);
  if (!HpcbEntryExist) {
    IDS_HDT_CONSOLE_PSP_TRACE("Hpcb not found\n");
    return EFI_NOT_FOUND;
  }
  IDS_HDT_CONSOLE_PSP_TRACE("Hpcb(%x,%x)\n", HpcbEntryAddress, HpcbEntrySize);

  NewHpcb = AllocatePool(HpcbEntrySize);
  if(NewHpcb == NULL){
  ...
  CopyMem(NewHpcb, (VOID*)(UINTN)HpcbEntryAddress, HpcbEntrySize);
  SetHpcbShadowCopy(NewHpcb);

  mHpcbEntryAddress = HpcbEntryAddress;
  mHpcbEntrySize    = HpcbEntrySize;
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpHpcbSmmEntry, NULL);
  IDS_HDT_CONSOLE_PSP_TRACE ("HPCB SMM Driver Entry\n");
  Handle = NULL;
  ...
  Status = gSmst->SmmInstallProtocolInterface (
                    ...
                    &mHpcbSmmServiceProtocol
                    );
  if (EFI_ERROR (Status)) {
    ASSERT (FALSE);
  }

  IDS_HDT_CONSOLE_PSP_TRACE ("HPCB SMM Driver Exit\n");
  HGPI_TESTPOINT (TpHpcbSmmExit, NULL);
  return EFI_SUCCESS;
}
