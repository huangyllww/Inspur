...
#define FILECODE  PSP_HYGONPSPSMM_HYGONPSPRESUMEHANDLING_FILECODE
...
// after PSP restore the memory. PSP release BSP and BSP will go through PEI Hgpi
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpPspSmmApRsmEntry, NULL);
  ...
  if (ResumeType == ResumeFromS3) {
    ...
    HGPI_TESTPOINT (TpPspSmmApRsmExit, NULL);
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpPspSmmBspRsmEntry, NULL);
  ...
  if (ResumeType == ResumeFromS3) {
    ...
    HGPI_TESTPOINT (TpPspSmmBspRsmExit, NULL);
