 ...
 ******************************************************************************
 */
#include "HygonPspSmm.h"
#include "HygonPspResumeServices.h"
#include <Library/BaseLib.h>

#define FILECODE  PSP_HYGONPSPSMMV2_HYGONPSPSMMV2_FILECODE
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpPspSmmSwSmiCallbackEntry, NULL);
  ...
  if (mPspSwSmiEntered == FALSE) {
    ...
    //
    UpdateS3SaveAreaMap ();
    HGPI_TESTPOINT (TpPspSmmSwSmiCallbackExit, NULL);
  ...
  )
{
  ...
  if (HygonCapsuleHookFlag == PSP_CAPSULE_HOOK_FLAG_SXINFO_NOCLRSMMLCK) {
    // Send PSP about S3 entering
    PspMboxBiosCmdSxInfo (0x3, mPspMboxSmmBuffer, mPspMboxSmmFlagAddr);
  } else if (HygonCapsuleHookFlag == PSP_CAPSULE_HOOK_FLAG_SXINFO_CLRSMMLCK) {
...
HYGON_CAPSULE_SMM_HOOK_PROTOCOL  mHygonCapsuleSmmHookProtocol = {
  HygonCapsuleSmmHook
};
  ...
  )
{
  ...
  HGPI_TESTPOINT (TpPspSmmEntry, NULL);
  ...
  IDS_HDT_CONSOLE_PSP_TRACE ("Psp.Drv.HygonPspSmm Enter\n");
  ...
  // Declare the HGPI capsule entry hook
  Handle = NULL;
  IDS_HDT_CONSOLE_PSP_TRACE ("\tInstall HGPI capsule entry hook protocol\n");
  Status = gSmst->SmmInstallProtocolInterface (
                    &Handle,
                    &gHygonCapsuleSmmHookProtocolGuid,
                    EFI_NATIVE_INTERFACE,
                    &mHygonCapsuleSmmHookProtocol
                    );
  if (EFI_ERROR (Status)) {
  ...
  if (CheckPspPlatformSecureEnable () == 0) {
  ...
  if ((mPspPlatformProtocolCopy.CpuContextResumeEnable != TRUE) || (HygonCapsuleGetStatus ())) {
  ...
  // For SAT, only require to assign Stack for BSP, AP will halt after exit AP SMM Vector which no stack needed
  ...
  SwRegisterContext.HygonSwValue = mPspPlatformProtocolCopy.SwSmiCmdtoBuildContext;
  ...
  IDS_HDT_CONSOLE_PSP_TRACE ("HygonPspSmm Exit\n");
  HGPI_TESTPOINT (TpPspSmmExit, NULL);
  return Status;
}
