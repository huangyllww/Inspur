...
#define FILECODE  NBIO_PCIE_HYGX_HYGONNBIOPCIESTPEI_HIDEPORTSST_FILECODE
 ...
 */
VOID
PciePortsVisibilityControlHyGx (
  IN      PCIE_PORT_VISIBILITY    Control,
  IN      GNB_HANDLE              *GnbHandle
  )
{
  switch (Control) {
    case UnhidePorts:
      ...
      PcieControlPortsHyGx (UnhidePorts, GnbHandle);
      break;
    case HidePorts:
      IDS_HDT_CONSOLE (GNB_TRACE, "Hide Ports\n");
      PcieControlPortsHyGx (HidePorts, GnbHandle);
      PcieEnablePortsHyGx (GnbHandle);
      break;
    default:
      ASSERT (FALSE);
  }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieControlPortsHyGx Entry \n");
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieControlPortsHyGx Exit\n");
  return;
}
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEnablePortsHyGx Entry \n");

  RunEnv = PcdGet8 (PcdRunEnvironment);
  PcieWrapper = PcieConfigGetChildWrapper (GnbHandle);
  while (PcieWrapper != NULL) {
    PcieEngine = PcieConfigGetChildEngine (PcieWrapper);
    while (PcieEngine != NULL) {
      if (RunEnv != 0) {
        // Simnow, EMU, FPGA
        if ((PcieEngine->Type.Port.LogicalBridgeId == 0) || (PcieEngine->Type.Port.LogicalBridgeId == 16)) {
          PcieEngine->InitStatus = INIT_STATUS_PCIE_TRAINING_SUCCESS;
        }
      }

      IDS_HDT_CONSOLE (GNB_TRACE, "PcieEnablePortsHyGx Wrapper %d LogicalBridgeId %d, InitStatus %d, LinkHotplug %d \n", 
         PcieWrapper->WrapId, 
         PcieEngine->Type.Port.LogicalBridgeId,
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "PcieEnablePortsHyGx Exit \n");
  return;
}
