  ...
  )
{
  ...
  PhyRegBase = (CoreId == 0) ? PCS_NBIO0_PCIE0_BASE_HYGX + (GnbHandle->RbId << 20) : ((CoreId == 2) ? PCS_NBIO0_PCIE2_BASE_HYGX : 0xFFFFFFFF); 

  PhyOft = LaneOft / 4;
  PhyLaneOft = LaneOft % 4;
  PhyRegAddr = (PhyLaneReg) ? PhyRegBase + ((PhyOft * 4) << 16) + ((PhyLaneOft * 4) << 8) + RegOft * 2 : 
                              PhyRegBase + ((PhyOft * 4) << 16) + RegOft * 2;
  *Address = PhyRegAddr;

  return (PhyRegBase != 0xFFFFFFFF) ? EFI_SUCCESS : EFI_UNSUPPORTED;
}
  ...
  )
{
  ...
  Status = CalculatePhyRegAddress (GnbHandle, Engine, LaneNumber, RegOft, PhyLaneReg, &PhyRegAddr);
  if (EFI_ERROR(Status))
    return Status;
  ...
  )
{
  ...
  for (Lane = StartLane; Lane <= EndLane; Lane += Step) {
    Status = ReadPhyReg (GnbHandle, Engine, Lane, RegOft, PhyLaneReg, &Value);
    if (EFI_ERROR(Status))
      return Status;
    Value &= AndMask;
    Value |= OrMask;
    Status = WritePhyReg (GnbHandle, Engine, Lane, RegOft, PhyLaneReg, &Value);
    if (EFI_ERROR(Status))
      return Status;
  }
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_COEFF_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_COEFF2_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_COEFF_32GT_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_EQ_REQ_COEFF_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_EQ_REQ_COEFF2_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_FORCE_EQ_REQ_COEFF_32GT_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_GPP0_PCIE_LC_CNTL4_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_PCIE_LC_CNTL8_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  ...
  SmnAddress = ConvertPciePortAddress(PCIE0_LC_32GT_EQUALIZATION_CNTL_ADDRESS_HYGX, GnbHandle, PcieWrapper, MappingPortID);
  ...
  )
{
  RwPhyReg (
    ...
    Engine, 
    0xFF, 
    SUP_DIG_SUP_OVRD_IN_1_ADDRESS_HYGX, 
    FALSE,
  ...
  )
{
  RwPhyReg (
    ...
    Engine, 
    0xFF, 
    SUP_DIG_SUP_OVRD_IN_2_ADDRESS_HYGX,
    FALSE, 
  ...
  )
{
  ...
  HYGON_CPM_EQ_CONFIG_TABLE   *pEqCfgTable = NULL;
  ...
  *EqCfgTable = pEqCfgTable;
  return (pEqCfgTable != NULL) ? EFI_SUCCESS : EFI_NOT_FOUND;
}
...
)
{
  ...
  if ((PcieCurrentLinkSpeed (GnbHandle,Engine) == 5) || PcdGetBool (PcdPcieSITestMode)) {
    PcieRxSscOffPhug1 (GnbHandle, Engine, 0);
    PcieRxSscOffFrug1 (GnbHandle, Engine, 0);
    PcieRxIctrl (GnbHandle, Engine, 0);
    *RequestRedoEq = TRUE;
  }
  SmnAddress = ConvertPcieCoreAddress (PCIE2_PCIE_HW_DEBUG_ADDRESS_HYGX, GnbHandle, PcieWrapper);
  NbioRegisterRMW (GnbHandle, TYPE_SMN, SmnAddress, (UINT32)(~BIT9), BIT9, 0);
