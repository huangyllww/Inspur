...
#define FILECODE  NBIO_PCIE_HYGONNBIOPCIE_HYGX_DXE_HYGONNBIOPCIEAERST_FILECODE
...
NbioSetPcieAerFeature (
  IN       HYGON_NBIO_PCIE_AER_PROTOCOL  *This,
 ...
 *----------------------------------------------------------------------------------------
 */
HYGON_NBIO_PCIe_AER_CONFIG  NbioPcieAerConfigEmei = {
  NUMBER_OF_PCIE_PORTS_EMEI,
  {
...
HYGON_NBIO_PCIe_AER_CONFIG  NbioPcieAerConfigDj = {
  NUMBER_OF_PCIE_PORTS_DJ,
  {
    { 0xFF,       ///< PortAerEnable, 0=Disable 1=Enable 0xFF=do nothing
      5,          ///< PciDev
      1,          ///< PciFunc
 ...
 *----------------------------------------------------------------------------------------
 */
HYGON_NBIO_PCIE_AER_PROTOCOL  mHygonNbioPcieAerProtocol = {
  NbioSetPcieAerFeature
};
    ...
    while (PcieEngine != NULL) {
      if (PcieLibIsEngineAllocated (PcieEngine)) {
        if ((PcieEngine->Type.Port.PortData.DeviceNumber == PortAerConfig->PciDev) &&
          ...
          MacPortId = PcieEngine->Type.Port.PortId % 8;
          PcieErrCntlAddr    = ConvertPciePortAddress (PCIE0_GPP0_ERR_CNTL_ADDRESS_HYGX, GnbHandle, PcieWrapper, MacPortId);
          CorrMaskAddr       = ConvertPciePortAddress (PCIE0_GPP0_CORR_ERR_MASK_HYGX, GnbHandle, PcieWrapper, MacPortId);
          UnCorrMaskAddr     = ConvertPciePortAddress (PCIE0_GPP0_UNCORR_ERR_MASK_HYGX, GnbHandle, PcieWrapper, MacPortId);
          UnCorrSeverityAddr = ConvertPciePortAddress (PCIE0_GPP0_UNCORR_ERR_SEVERITY_HYGX, GnbHandle, PcieWrapper, MacPortId);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPcieAerInitST Enter\n");
  if (GnbHandle->DieType == IOD_EMEI) {
    NbioPcieAerConfigEmei.NumberOfPorts = NUMBER_OF_PCIE_PORTS_EMEI;
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "Socket %d Die P%d L%d Rb %d, Number of ports: %d\n",
      GnbHandle->SocketId,
      GnbHandle->PhysicalDieId,
      GnbHandle->LogicalDieId,
      GnbHandle->RbId,
      NbioPcieAerConfigEmei.NumberOfPorts
      );
    IDS_HOOK (IDS_HOOK_NBIO_PCIE_AER_INIT, NULL, (void *)&NbioPcieAerConfigEmei);

    for (Index = 0; Index < NbioPcieAerConfigEmei.NumberOfPorts; Index++) {
      DumpPcieAerSetting ((NBIO_PCIe_PORT_AER_CONFIG *)&NbioPcieAerConfigEmei.PortAerConfig[Index]);
      NbioPcieAerFeatureEnable (GnbHandle, (NBIO_PCIe_PORT_AER_CONFIG *)&NbioPcieAerConfigEmei.PortAerConfig[Index]);
    }
  }

  if (GnbHandle->DieType == IOD_DUJIANG) {
    NbioPcieAerConfigDj.NumberOfPorts = NUMBER_OF_PCIE_PORTS_DJ;
    IDS_HDT_CONSOLE (
      GNB_TRACE,
      "Socket %d Die P%d L%d Rb %d, Number of ports: %d\n",
      GnbHandle->SocketId,
      ...
      GnbHandle->LogicalDieId,
      GnbHandle->RbId,
      NbioPcieAerConfigDj.NumberOfPorts
      );
    IDS_HOOK (IDS_HOOK_NBIO_PCIE_AER_INIT, NULL, (void *)&NbioPcieAerConfigDj);

    for (Index = 0; Index < NbioPcieAerConfigDj.NumberOfPorts; Index++) {
      DumpPcieAerSetting ((NBIO_PCIe_PORT_AER_CONFIG *)&NbioPcieAerConfigDj.PortAerConfig[Index]);
      NbioPcieAerFeatureEnable (GnbHandle, (NBIO_PCIe_PORT_AER_CONFIG *)&NbioPcieAerConfigDj.PortAerConfig[Index]);
    }
  }

  IDS_HDT_CONSOLE (GNB_TRACE, "NbioPcieAerInitST Exit\n");
}
  ...
  )
{
  ...
  if ((ParentCapFlag & (BIT5 | BIT7)) != 0) {
          ...
          if (GnbLibPciIsBridgeDevice (Device.AddressValue, NULL)) {
            ...
            if (ChildCapPtr != 0) {
              ...
              if ((ChildCapFlag & (BIT5 | BIT7)) != 0) {
                ...
                IDS_HDT_CONSOLE (
                  PCIE_MISC,
                  "Set EndPoint ECRC Enable Bus=%02X, Dev=%02X, Fun=%02X, Reg=%03X\n", \
                  Device.Address.Bus,
                  Device.Address.Device,
                  Device.Address.Function,
                  (Device.AddressValue | (ChildCapPtr + 0x18))
                  );
                EcrcEnabled = TRUE;
    ...
    if (EcrcEnabled) {
      ...
      IDS_HDT_CONSOLE (
        PCIE_MISC,
        "Set Bridget ECRC Enable Bus=%02X, Dev=%02X, Fun=%02X, Reg=%03X\n", \
        PciDevice.Address.Bus,
        PciDevice.Address.Device,
        PciDevice.Address.Function,
        (PciDevice.AddressValue | (ParentCapPtr + 0x18))
        );
      if (PcdGetBool (PcdPcieEcrcSeverityFatal)) {
        // Set ECRC Severity as Fatal
