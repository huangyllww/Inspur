...
#define FILECODE  NBIO_PCIE_HYGONNBIOPCIE_HYGX_DXE_HYGONNBIOPCIESRISST_FILECODE
...
/**
 * Find PCIE Engine port ID by device and function number
  ...
  )
{
  ...
  for (Index = 0; Index < (sizeof (DefaultPortDevMap) / sizeof (DefaultPortDevMap[0])); Index++) {
    if (DefaultPortDevMap[Index] == DevFunc) {
      if (Index < 12) {
        EnginePortId = Index % 8;
      } else {
        EnginePortId = Index - 12;
      }
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  PcieGetSrisCapabilityCallback for Device = %d:%d:%d\n",
    Device.Address.Bus,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (
    GNB_TRACE,
    "  PcieSrisInitCallback for Device = %d:%d:%d\n",
  ...
  if (PcieConfigCheckPortStatus (Engine, INIT_STATUS_PCIE_TRAINING_SUCCESS)) {
    ...
    EnginePortId = PcieGetEnginePortId (Engine->Type.Port.PortData.DeviceNumber, Engine->Type.Port.PortData.FunctionNumber);
    Address0     = ConvertPciePortAddress (PCIE0_GPP0_LINK_CTL3_ADDRESS_HYGX, GnbHandle, PcieWrapper, EnginePortId);
    Address1     = ConvertPciePortAddress (PCIE0_GPP0_LC_CNTL6_ADDRESS_HYGX, GnbHandle, PcieWrapper, EnginePortId);
