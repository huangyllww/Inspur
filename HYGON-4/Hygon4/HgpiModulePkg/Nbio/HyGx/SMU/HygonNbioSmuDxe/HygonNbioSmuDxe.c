...
#define FILECODE  NBIO_SMU_HYGX_HYGONNBIOSMUDXE_HYGONNBIOSMUDXE_FILECODE
  ...
  )
{
  ...
  if (GnbHandle != NULL) {
    Status = NbioRegisterRead (
               ...
               (UINT32)CDD_SPACE (Cdd, THREAD_CONFIGURATION_ADDRESS_HYGX),
               &ThreadConfiguration,
               0
               );
  ...
  )
{
  ...
  HPOB_MEM_NVDIMM_INFO_STRUCT_HYGX          *HpobNvDimmInfo;
  ...
  IDS_HDT_CONSOLE (MAIN_FLOW, "HygonNbioSmuHyGxDxeEntry\n");
  Handle = NULL;
  FanPolicyTable = NULL;
  ...
  if (PcdGetBool (PcdHygonDeterminismControl) && ((CddPerSocket >= 4) || (CddPerSocket == 2))) {
