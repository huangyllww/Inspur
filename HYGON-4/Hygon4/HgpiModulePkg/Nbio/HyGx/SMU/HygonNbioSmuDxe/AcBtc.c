...
#define FILECODE  NBIO_SMU_HYGX_HYGONNBIOSMUDXE_ACBTC_FILECODE
  ...
  )
{
  ...
  for (Index = 0; Index < 3; Index++) {
    ...
    CcxNumber     = (Cpuid.EBX_Reg >> 29) & 0xF;
    while (CcxNumber != 0) {
  ...
  while (NbioHandle != NULL) {
    ...
    for (CddIndex = 0; CddIndex < MAX_CDDS_PER_SOCKET; CddIndex++) {
      ...
      NbioRegisterRead (
        ...
        (UINT32)CDD_SPACE (CddIndex, THREAD_CONFIGURATION_ADDRESS_HYGX),
        &ThreadConfiguration,
        0
        );
      ...
      TotalThreads += (ThreadConfiguration.HyGxField.ComplexCount + 1) * (ThreadConfiguration.HyGxField.CoreCount + 1) * NumberOfThreads;
    }
  ...
  if (PcdGet8 (PcdAcBtc)) {
    ...
    for (AcBtcIndex = 0; AcBtcIndex < 3; AcBtcIndex++) {
      ...
      NbioRegisterRead (
        NbioHandle,
        THREAD_ENABLE_TYPE,
        (UINT32)CDD_SPACE (CddId, THREAD_ENABLE_ADDRESS_HYGX),
        &ThreadEnable,
        0
        );
