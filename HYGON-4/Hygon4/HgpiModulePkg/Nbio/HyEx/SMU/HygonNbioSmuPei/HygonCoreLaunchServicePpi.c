...
#define FILECODE  NBIO_SMU_HYEX_HYGONNBIOSMUPEI_HYGONCORELAUNCHSERVICEPPI_FILECODE
  ...
  )
{
  ...
  if (GnbHandle != NULL) {
    Status = NbioRegisterRead (
               ...
               CDD_SPACE (Cdd, THREAD_CONFIGURATION_ADDRESS_HYEX),
               &ThreadConfiguration,
               0
               );
  ...
  )
{
  ...
  ASSERT (GnbHandle != NULL);
  if (GnbHandle != NULL) {
      Status = NbioRegisterRead (GnbHandle,
              THREAD_ENABLE_TYPE,
              CDD_SPACE(Cdd, THREAD_ENABLE_ADDRESS_HYEX),
              &ThreadEnable,
              0);
      Status = NbioRegisterRead (GnbHandle,
              THREAD_CONFIGURATION_TYPE,
