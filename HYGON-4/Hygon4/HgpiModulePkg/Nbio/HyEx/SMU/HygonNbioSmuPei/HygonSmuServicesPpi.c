...
#define FILECODE  NBIO_SMU_HYEX_HYGONNBIOSMUPEI_HYGONSMUSERVICESPPI_FILECODE
    ...
    for (i = 0; i < SocketCount; i++) {
      ...
      for (j = 0; j < MAX_CDDS_PER_SOCKET; j++) {
        ...
        if (GnbHandle != NULL) {
          ...
          Status = NbioRegisterRead (GnbHandle, TYPE_SMN, CDD_SPACE (j, THREAD_CONFIGURATION_ADDRESS_HYEX), &ThreadConfiguration, 0);
          ThreadConfiguration.HyExField.SMTDisable = (SmtMode == 0? 1 : 0);
          Status = NbioRegisterWrite (GnbHandle, TYPE_SMN, CDD_SPACE (j, THREAD_CONFIGURATION_ADDRESS_HYEX), &ThreadConfiguration.Value, 0);
        }
      }
