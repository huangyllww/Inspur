...
#define FILECODE NBIO_PCIE_HYGONNBIOPCIE_HYEX_DXE_HYGONNBIOPCIEAERST_FILECODE
/*----------------------------------------------------------------------------------------
 ...
 *----------------------------------------------------------------------------------------
 */
HYGON_NBIO_PCIe_AER_CONFIG NbioPcieAerConfig = {
  NUMBER_OF_PORTS,
  {
    ...
    while (PcieEngine != NULL) {
      if (PcieLibIsEngineAllocated (PcieEngine)) {
        if ((PcieEngine->Type.Port.PortData.DeviceNumber == PortAerConfig->PciDev) &&
          ...
          PcieErrCntlAddr = ConvertPciePortAddress(PCIE0_GPP0_ERR_CNTL_ADDRESS_HYEX, GnbHandle, PcieWrapper, MacPortId);
          CorrMaskAddr = ConvertPciePortAddress(PCIE0_GPP0_CORR_ERR_MASK_HYEX, GnbHandle, PcieWrapper, MacPortId);
          UnCorrMaskAddr = ConvertPciePortAddress(PCIE0_GPP0_UNCORR_ERR_MASK_HYEX, GnbHandle, PcieWrapper, MacPortId);
          UnCorrSeverityAddr = ConvertPciePortAddress(PCIE0_GPP0_UNCORR_ERR_SEVERITY_HYEX, GnbHandle, PcieWrapper, MacPortId);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (GNB_TRACE, "Socket:%x, Die:%x, Instance:%x, Number of ports in NBIO: %d\n",  GnbHandle->SocketId, GnbHandle->PhysicalDieId, GnbHandle->InstanceId, NbioPcieAerConfig.NumberOfPorts);
