...
#define FILECODE NBIO_PCIE_HYEX_HYGONNBIOPCIESTPEI_PCIEREMAPST_FILECODE
  ...
  )
{
  ...
  if (Status == HGPI_SUCCESS) {
    if (Engine->Type.Port.PortId < 16)  {
        //PCIE Core 0/1 port 0-7
        PortDevMap[Engine->Type.Port.PortId] = DevFunc;
    } else {
        //PCIE Core 2 Port 6,7
        PortDevMap[Engine->Type.Port.PortId-6] = DevFunc;
    }
  ...
  )
{
  ...
  for (Index = 0; Index < sizeof (DefaultPortDevMap); ++Index) {
    ...
    if (Index < 16) {
        NbioRegisterRead (GnbHandle, TYPE_SMN, NBIO_SPACE(GnbHandle, NB_PROG_DEVICE_REMAP_HYEX) + (Index << 2), &DeviceRemap.Value, 0);
    } else {
        NbioRegisterRead (GnbHandle, TYPE_SMN, NBIO_SPACE(GnbHandle, NB_PROG_DEVICE_REMAP_N18_HYEX) + ((Index-16) << 2), &DeviceRemap.Value, 0);
    }
    
    DeviceRemap.Field.DevFnMap = PortDevMap[Index];
    
    if (Index < 16) {
        NbioRegisterWrite (GnbHandle, TYPE_SMN, NBIO_SPACE(GnbHandle, NB_PROG_DEVICE_REMAP_HYEX) + (Index << 2), &DeviceRemap.Value, 0);
    } else {
        NbioRegisterWrite (GnbHandle, TYPE_SMN, NBIO_SPACE(GnbHandle, NB_PROG_DEVICE_REMAP_N18_HYEX) + ((Index-16) << 2), &DeviceRemap.Value, 0);
    }  
  }
}
