...
VOID AssertPciReset (GNB_HANDLE *GnbHandle)
{
  ...
  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1a, &IoMuxSelect, 0);        //IoMux function1 
  SmnRegisterWrite (GnbHandle->Address.Address.Bus, 0x2D02568, &GpioLow, 0);             //GPIO26

  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1b, &IoMuxSelect, 0);        //IoMux function1
  SmnRegisterWrite (GnbHandle->Address.Address.Bus, 0x2D0256c, &GpioLow, 0);             //GPIO27
}

VOID DeAssertPciReset (GNB_HANDLE *GnbHandle)
{
  ...
  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1a, &IoMuxSelect, 0);        //IoMux
  SmnRegisterWrite (GnbHandle->Address.Address.Bus, 0x2D02568, &GpioHigh, 0);            //GPIO26

  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1b, &IoMuxSelect, 0);        //IoMux
  SmnRegisterWrite (GnbHandle->Address.Address.Bus, 0x2D0256c, &GpioHigh, 0);             //GPIO27

  IoMuxSelect  = 0;
  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1a, &IoMuxSelect, 0);
  SmnRegisterWrite8 (GnbHandle->Address.Address.Bus, 0x2d01d1c, &IoMuxSelect, 0);
}
  ...
  )
{
  ...
  AssertPciReset (GnbHandle);
  MicroSecondDelay (200000);
  DeAssertPciReset (GnbHandle);
  ...
  for (Index = 0; Index < HsioNodeCount; Index++) {
    if (HsioEnginePointer->EngineType == HsioPcieEngine) {
      ...
      while (EndOfList != TRUE) {
        ...
        if ((UserCfgPortPointer->EngineData.StartLane >= HsioEnginePointer->StartLane) &&
            (UserCfgPortPointer->EngineData.EndLane <= HsioEnginePointer->EndLane) &&
