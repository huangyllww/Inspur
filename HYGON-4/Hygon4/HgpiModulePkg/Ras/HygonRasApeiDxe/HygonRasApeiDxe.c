...
#include "HygonRasApeiDxe.h"

/*----------------------------------------------------------------------------------------
 *                   D E F I N I T I O N S    A N D    M A C R O S
 *----------------------------------------------------------------------------------------
 */
#define FILECODE  RAS_HYGONRASAPEIDXE_HYGONRASAPEIDXE_FILECODE
...
/*********************************************************************************
 * Name: HygonRasApeiDxeInit
 *
 * Description
 *   Entry point of the HYGON RAS APEI DXE driver
  ...
  )
{
  ...
  // Allocate Memory for the the HYGON_RAS_APEI_PROTOCOL protocol.
  Status = gBS->AllocatePool (
                  EfiBootServicesData,                          // IN EFI_MEMORY_TYPE PoolType
                  sizeof (HYGON_RAS_APEI_PROTOCOL),             // IN UINTN Size
                  &HygonRasApeiProtocol                         // OUT VOID **Buffer
                  );
  if (EFI_ERROR (Status)) {
  ...
  DEBUG ((EFI_D_ERROR, "[RAS] NumberOfLogicProcessors = 0x%08x, TotalNumberOfProcessor = 0x%08x\n", NumberOfLogicProcessors, mHygonRasPolicy->TotalNumberOfProcessors));

  // The logical processor number shoudl match to the avaliable processors reported from HGPI.
  ASSERT (NumberOfLogicProcessors == mHygonRasPolicy->TotalNumberOfProcessors);

  // Initialize function pointers to protocol interfaces
  HygonRasApeiProtocol->McaErrorAddrTranslate   = &HygonMcaErrorAddrTranslate;
  HygonRasApeiProtocol->TranslateSysAddrToCS    = &HygonTranslateSysAddrToCS;
  HygonRasApeiProtocol->AddBootErrorRecordEntry = &AddBertErrorRecord;
  HygonRasApeiProtocol->AddHestErrorSourceEntry = &AddHestErrorRecord;
  HygonRasApeiProtocol->SearchMcaError = &HygonSearchMcaError;
  HygonRasApeiProtocol->AddMultipleHestErrorSourceEntry = &AddMultipleHestErrorRecord;
  Status = gBS->InstallProtocolInterface (
                  &Handle,                                          // IN OUT EFI_HANDLE
                  &gHygonRasApeiProtocolGuid,                       // IN EFI_GUID
                  EFI_NATIVE_INTERFACE,                             // IN EFI_INITERFACE_TYPE
                  HygonRasApeiProtocol                              // IN VOID* Interface
                  );
  if (EFI_ERROR (Status)) {
    ASSERT (!EFI_ERROR (Status));
  ...
  switch (ErrorType) {
    ...
    case ERROR_TYPE_GENERIC:   // Generic Error (Note: **According to WHEA, RAW data must follow any Generic error data entries**)
      ...
      if (OrgBertErrRgnTable->RawDataLength != 0) {
        gBS->CopyMem (
               (UINT8 *)(NewBertErrRgnTable + OrgBertErrRgnTable->RawDataOffset + RecordLen), \
               (UINT8 *)(OrgBertErrRgnTable + OrgBertErrRgnTable->RawDataOffset),
               OrgBertErrRgnTable->RawDataLength
               );
      }
